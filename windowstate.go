...
func applyMainWindowState() {
	...
	if MainWindowGeom().Maximised {
		// SW_MAXIMIZE both reveals and ACTIVATES the still-hidden window, so a
		// maximised user's bridge would take the foreground at startup before
		// the main.go reveal even runs. Same give-back as showMainWindowNoSteal.
