...
func (a *App) SetAudioMuted(m bool) {
	...
	s.AudioMuted = m
	UpdateSettings(s)
	// Only the speaker button calls this (App.svelte toggleMute) — never
	// startup — so it's a real user action.
	auditTimerHourly(m, "app_updatedtimer_audiomuted", "app_updatedtimer_audiounmuted")
}
