...
// playOverlayCue plays an overlay's audio cue (the CH chain "you're next").
// Honors the master mute like playMedia, but de-duplicates in its own
// namespace: the cue fires once per chain rotation and is never a burst, and
// sharing the "media:" key meant a trigger that happened to play the same
// file inside the 3s window swallowed it.
func playOverlayCue(path string) {
	if audioIsMuted() {
		return
	}
	if !audioAllow("cue:" + path) {
		return
	}
	playMediaAt(path)
}
