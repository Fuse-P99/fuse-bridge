...
// auto-assignment a chance in case they're brand new. name is the display-cased
// bare name (what the server has always been sent); key is the character's
// per-(character, world) storage key, which all LOCAL state uses. On Blue
// key == lower(name), so the server's rows merge into the same bucket.
func questTrackCharSwap(name, key string) {
	if strings.TrimSpace(key) == "" {
		return
	}
	go func() {
		if serverForwardOK() && !tqToonSynced(key) {
			syncToonQuests([]string{name})
		}
		questScanInventory(key)
		autoAssignEpics()
	}()
}
