...
func alarmTargetsFrom(d WorldTimersData, nowMs int64) map[string]alarmTarget {
	...
	for _, e := range d.Events {
		...
		out["event:"+e.Key] = alarmTarget{atMs: e.SpawnAtMs, label: e.Name, kind: alarmKindLead}
	}
...
// startWorldAlarmLoop polls the board and fires due alarms. The board fetch is
// cached (5s), so this cadence costs at most one request per interval and keeps
// the Timers tab's own view warm.
