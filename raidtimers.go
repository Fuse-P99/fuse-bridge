...
//
// Debuff bar lengths come from the Fuse Triggers package: the "Debuffs Macros"
// group (05 - Raiding > 01 - General Raid Triggers > Debuffs Macros). Each
// tracked debuff keyword uses the countdown duration configured on that
// group's matching trigger. CH bars are the fixed 10s Complete Heal cast:
...
// scanDebuffGroupLocked walks the trigger tree; once inside a group named
// "Debuffs Macros" it records each timered trigger's duration under the debuff
// keyword found in its name/pattern. Caller holds trigStoreMu.
func scanDebuffGroupLocked(g *GinaGroup, inDebuffGroup bool, out map[string]int64) {
	if g == nil {
		return
	}
	in := inDebuffGroup || strings.EqualFold(strings.TrimSpace(g.Name), "Debuffs Macros")
	if in {
