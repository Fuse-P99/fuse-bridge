...
type LocalCHTimer struct {
	...
	// Tank is the call's target half ("444 - CH - Dooce" → "Dooce") — what
	// lets the overlay swap a slot's shown target the instant the call is
	// heard instead of after the relay round trip.
	Tank            string `json:"tank"`
	AtMs            int64  `json:"at_ms"`
	InterruptedAtMs int64  `json:"interrupted_at_ms"` // own interrupted cast only
}

// LocalDiscTimer is one locally-seen fighting-style discipline. Everyone in
// range sees the line, so this fires on every client watching the tank — not
// just the tank's own — and starts the bar without waiting for the round trip.
type LocalDiscTimer struct {
	Name  string `json:"name"` // as it appeared in the log
...
type LocalRaidTimers struct {
	// Toon is the tailed character — the clerics view highlights their own
	// chain slot with it.
	Toon    string             `json:"toon"`
	Debuffs []LocalDebuffTimer `json:"debuffs"`
	CH      []LocalCHTimer     `json:"ch"`
	Discs   []LocalDiscTimer   `json:"discs"`
...
// discDurMs is the fighting-style discipline window. Kept in sync with
// tankDiscDuration on the server (eventTracker.go).
const discDurMs int64 = 180 * 1000
...
var (
	...
	localDiscs    = map[string]*LocalDiscTimer{}   // lower player name → timer
...
var (
	...
	// Fighting-style disciplines, matched locally so the bar starts the instant
	// the line is seen. Same patterns the forwarding filter uses (filter.go).
	rtDiscRE     = regexp.MustCompile(`^(\w+) assumes an? (defensive|evasive) fighting style\.$`)
	rtDiscSelfRE = regexp.MustCompile(`^You assume an? (defensive|evasive) fighting style\.$`)
)
...
// sanitizeMacroContent strips the invisible junk EQ macro lines pick up (the
// classic "clear the social line and retype it" bug): zero-width and other
// format runes, stray control characters, and bytes that don't decode are
// dropped, and exotic Unicode spaces (NBSP and friends) become plain spaces so
// the \s in the macro regexes still matches. Mirrors the server's helper of
// the same name so local bars and the card agree on what counts as a call.
// Pure printable-ASCII lines (nearly every line) return without allocating.
func sanitizeMacroContent(s string) string {
	plain := true
	for i := 0; i < len(s); i++ {
...
// emitRaidTimersChanged pushes a "raidtimers-changed" event so the raid-card
// sections drawing local bars (Clerics CH casts, Debuffs, Assignments discs)
// repaint the moment the line is seen instead of on their next 1s poll — the
// same reasoning as emitTriggersChanged: a cleric cues off the bar, and a
// second of lag turns a 1-second wait into a 2-second gap. No coalescing:
// these lines arrive at human cadence, one guild call at a time.
func emitRaidTimersChanged() {
	// Off the tail loop so a slow dispatch can never stall log reading.
	go v3App.Event.Emit("raidtimers-changed")
}
...
func RecordRaidTimerLine(line string) {
	// These bars draw on the BLUE raid card (visible to linked members from any
	// world) — so only the Blue log may feed them. Another world's guild using
	// the same "001 - CH - Tank" macro shapes must not paint bars on it.
	if !onHomeServer() {
		return
	}
	msg := sanitizeMacroContent(logMessageContent(line))
	...
	if msg == "Your spell is interrupted." {
		...
		raidTimersMu.Unlock()
		if hit {
			emitRaidTimersChanged()
		}
		return
	}

	// Fighting-style discipline (not guild chat — a combat line).
	if m := rtDiscRE.FindStringSubmatch(msg); len(m) > 2 {
		recordLocalDisc(m[1], m[2])
		return
	}
	if m := rtDiscSelfRE.FindStringSubmatch(msg); len(m) > 1 {
		recordLocalDisc(currentCharName, m[1])
		return
	}
	...
	if m := rtCHRE.FindStringSubmatch(content); len(m) > 2 {
		recordLocalCH(m[1], sender, m[2])
		return
	}
	if m := rtCHRampRE.FindStringSubmatch(content); len(m) > 2 {
		recordLocalCH("RR"+m[1], sender, m[2])
	}
}
...
func recordLocalCH(label, cleric, tank string) {
	...
	localCH[label+"|"+strings.ToLower(cleric)] = &LocalCHTimer{
		Label: label, Cleric: cleric, Tank: strings.TrimSpace(tank),
...
// recordLocalDisc starts a warrior's discipline bar. A repeat of the same style
// inside the window is the same pop seen again (the log can echo, and the tank's
// own client sees both its "You assume" line and nothing else) — it must not
// restart the clock. Switching style is a real new pop.
func recordLocalDisc(name, kind string) {
	...
	now := time.Now().UnixMilli()
	raidTimersMu.Lock()
	if d := localDiscs[key]; d != nil && d.Kind == kind && now-d.AtMs < discDurMs {
		raidTimersMu.Unlock()
		return
	}
	localDiscs[key] = &LocalDiscTimer{Name: name, Kind: kind, AtMs: now, DurMs: discDurMs}
	raidTimersMu.Unlock()
	emitRaidTimersChanged()
}
...
func GetLocalRaidTimersData() LocalRaidTimers {
	out := LocalRaidTimers{Toon: currentCharName, DebuffDurations: debuffDurations()}
	...
	// Disc entries expire on their own window, not the shared TTL — nothing can
	// draw them past it.
	nowMs := time.Now().UnixMilli()
	for k, d := range localDiscs {
		if nowMs-d.AtMs >= d.DurMs {
			delete(localDiscs, k)
			continue
		}
