...
type ViewAsUI struct {
	...
	// RealAdmin is the TRUE admin status (server-verified Discord roles,
	// cached), unaffected by the preview — the one gate the View-as control
...
func (a *App) GetViewAs() ViewAsUI {
	lvl := ViewAsActive()
	return ViewAsUI{Level: lvl, Label: viewAsLabels[lvl], RealAdmin: isAdminCachedRaw()}
}
