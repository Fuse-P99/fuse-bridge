...
// ── stuck move-loop / capture watchdog ───────────────────────────────────────
// Insurance, kept after the WS_EX_NOACTIVATE unwind (v2.5.3469): a modal move
// loop or an orphaned mouse capture on our UI thread swallows every click on
// the machine until an alt-tab delivers WM_CANCELMODE. With overlays back to
// normal activatable windows this state should be unreachable — during the
// NOACTIVATE era the lockouts users hit turned out NOT to trip this watchdog
// (the stuck capture likely lived in the WebView2 runtime process, outside
// our thread's GetGUIThreadInfo view), so treat any log line from here as a
// significant new signal, not a repeat of that saga.
//
...
func startFocusWatcher() {
	...
	go func() {
		...
		for range t.C {
			...
			// Re-assert always-on-top where the OS has stripped it (display
			// sleep, mode/DPI changes). Pure z-order repair with
			// SWP_NOACTIVATE — never activates, shows, or focuses anything.
			reconcilePopoutTopmost()
