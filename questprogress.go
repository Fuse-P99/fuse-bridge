...
// adoptQuestToonBucket copies the bare-name quest state to an off-home
// character's qualified key, once (see adoptOffHomeCharState). Deep copy via
// the same JSON shape the store persists in; the bare bucket stays.
func adoptQuestToonBucket(bare, key string) bool {
	tqMu.Lock()
	defer tqMu.Unlock()
	tqLoadLocked()
	if tqToons[key] != nil {
	...
	if cp.Assignments == nil {
		cp.Assignments = map[int]*tqAssign{}
	}
	if cp.Completions == nil {
		cp.Completions = map[int]*tqCompletion{}
	}
	tqToons[key] = &cp
	tqSaveLocked()
	return true
...
func tqEnqueueLocked(op tqOp) {
	// Off the home world, quest changes stay LOCAL only — never queue a sync op
	// for a Green/Red/test character, or it would flush on the next return to
	// Blue and collide with a same-named Blue toon's server-side progress. Local
	// state (tqSaveLocked) is written by the caller regardless.
	if !onHomeServer() {
		return
	}
...
// tqNoteLootedHeld records a just-looted quest item ahead of any inventory
// dump confirming it — a stand-in entry the next real scan replaces
// wholesale. Two callers, two dedupe rules: a medallion piece carries its
// game id (one entry per id — a piece is either held or not), while an
// ordinary hand-in component arrives id-less and COUNTS instead, because a
// step wanting 3× the same gem needs the second and third loot to register.
func tqNoteLootedHeld(toon, itemName string, itemID int) {
	...
	if itemID != 0 {
		...
		for i := range st.Held {
			if normalizeItemName(st.Held[i].Name) == key {
				st.Held[i].Count++
				tqSaveLocked()
				return
			}
...
// tqHeldCounts returns the toon's held quest items as normalized name → total
// count across bag slots — the evidence hand-in feasibility is judged on.
func tqHeldCounts(toon string) map[string]int {
	tqMu.Lock()
	defer tqMu.Unlock()
	tqLoadLocked()
	out := map[string]int{}
	for _, h := range tqStateLocked(toon).Held {
		...
		out[normalizeItemName(h.Name)] += n
	}
	return out
}
...
func (a *App) GetToonQuests(name string) ToonQuestView {
	...
	if serverForwardOK() && time.Since(tqLastSync) > tqSyncStaleness {
