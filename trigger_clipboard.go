...
// groupClipEffLocked reports whether a group is clipboard-blocked, directly
// or by any ancestor (the mute helper's twin). Caller holds trigStoreMu.
func groupClipEffLocked(groupID int) bool {
	for cur := groupByID[groupID]; cur != nil; cur = groupParentOf[cur.GroupID] {
		if trigClipGroups[cur.GroupID] {
			return true
		}
	}
	return false
...
func (a *App) SetTriggerGroupClipboardBlocked(groupID int, blocked bool) error {
	...
	if err == nil {
		auditTimerHourly(blocked, "app_updatedtimer_clipboardblocked", "app_updatedtimer_clipboardallowed")
	}
	return err
}
