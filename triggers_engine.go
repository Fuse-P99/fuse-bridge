...
func RebuildTriggerActivation() {
	...
	var defs []trigDef
	if trigCfg != nil {
		var walk func(g *GinaGroup, path []string, mutedInherit bool)
		walk = func(g *GinaGroup, path []string, mutedInherit bool) {
			p := append(append([]string{}, path...), g.Name)
			// Audio mutes nest: a muted group silences everything beneath it.
			...
			if effectiveGroupEnabledLocked(g, ctx) {
				for _, t := range g.Triggers {
					...
					if d, ok := defFromTrigger(t, p); ok {
						d.muted = gMuted || trigMuteTriggers[trigToggleKey(g, t)]
						defs = append(defs, d)
					}
				}
			}
