...
// left (the dbg marker near-instantly, the eqlog silence fallbacks 8s–5min
// late), and buffs stop ticking the moment they're out, so charging the
// detection gap burned real duration and could expire a bar outright. If they
// idled quietly before a real exit the bar comes back a little long, which is
// the cheap direction to be wrong in. Caller holds trigStateMu.
...
func tickTriggerTimers(now time.Time) []string {
	...
	for _, lt := range liveTimers {
		...
		// exit, the pause below stashes it with the time it had left as of the
		// last line; if the character is just quiet, their next line lets it
		// expire normally, end-actions and all. Held bars cost nothing visually
		// — the overlays already drop bars past their end. Preserved categories
		// only: a spawn window or raid call runs out in real time whether
