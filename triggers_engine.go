...
// QuitTriggerTimers freezes the active character's preserved timers on a
// deliberate /q or /exit, seen in dbg.txt (see tailDbgLog). Unlike camping,
// quitting writes no countdown to the eqlog, so without this the timers would
// keep burning until the 5-minute idle fallback — which stays in place for the
// cases that produce no marker at all (crash, link-dead, alt-F4).
//
// dbg.txt is shared by every EQ instance, so while boxing a quit on one client
// also freezes the tailed character's timers. That self-corrects: their next log
// line resumes the timers once quitPauseGrace has passed, at the cost of a brief
// gap on the board and a few seconds of overstated duration.
func QuitTriggerTimers() {
	...
	if kept > 0 || dropped > 0 {
		addStatus("Timers: %s — quit/exit; paused %d (Buffs (Self)/Disciplines), cleared %d",
