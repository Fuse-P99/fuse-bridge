...
// trigger's personal tint ("" = the overlay's color). Caller holds
// trigStateMu.
func pushAlertLocked(text, category, color string, now time.Time) {
...
func tickTriggerTimers(now time.Time) []string {
	...
	for _, lt := range liveTimers {
		if lt.endsAt.After(now) {
			...
			if lt.endingOffset > 0 && !lt.endingSpoken && !lt.ending.empty() &&
				...
				if lt.ending.text != "" {
					...
					pushAlertLocked(lt.ending.text, lt.category, lt.color, now)
					changed = true
				}
		...
		if !lt.ended.empty() {
			if lt.ended.text != "" {
				pushAlertLocked(lt.ended.text, lt.category, lt.color, now)
			}
			if !timerAudioMutedLocked(lt) {
...
func fireTrigger(c *compiledTrig, caps []string, named map[string]string, charName string, now time.Time) {
	...
	if c.useText {
		if txt := strings.TrimSpace(sub(c.displayText)); txt != "" {
			pushAlertLocked(txt, c.category, c.color, now)
		}
	}
