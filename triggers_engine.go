...
func tickTriggerTimers(now time.Time) []string {
	...
	for _, lt := range liveTimers {
		...
		if lt.endedAlert != "" {
			// A timer-ended alert inherits its timer's category, so it lands in
			// the same overlay as the bar that produced it.
			pushAlertLocked(lt.endedAlert, lt.category, now)
		}
	}
	liveTimers = keep
