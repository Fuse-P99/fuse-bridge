...
// triggersOfficerOnly gates the trigger engine to officers (and admin mode)
// until the Timers launch. Set to false at release together with the Timers
// tab gate in App.svelte — everyone then gets the engine: linked members run
// the shared Fuse set + Personal, unlinked users Personal only (assembleLocked
// never includes fuseRoot unlinked).
//
// The gate check below deliberately reads GetSettings().AdminMode RAW rather
// than the IsAdminMode binding: a View-as preview (viewas.go) suppresses the
// binding and isOfficerCached, but the raw setting stays true — so an admin
// previewing the member experience keeps the engine running, which is the
