...
// alertClearFlashKeep is how long an ender-cleared alert survives after its
// clear, so the overlay's triple-flash can play before the entry vanishes.
const alertClearFlashKeep = 4 * time.Second
...
func cullAlertsLocked(now time.Time) {
	...
	for _, a := range trigAlerts {
		...
		case !a.clearedAt.IsZero():
			if now.Sub(a.clearedAt) <= alertClearFlashKeep {
				kept = append(kept, a)
			}
		default:
			keep := trigAlertTTL
