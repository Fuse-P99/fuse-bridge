...
func snapshotTimersJSON(now time.Time) []byte {
	...
	for key, list := range pausedTimers {
		for _, lt := range list {
			pt := persistedTimer{Name: lt.name, Category: lt.category, TriggerID: lt.triggerID, EndedAlert: lt.endedAlert}
			if lt.wasLive {
				pt.WasLive = true
	...
	if trigActiveChar != "" {
		...
		for _, lt := range liveTimers {
			...
			out.Chars[key] = append(out.Chars[key], persistedTimer{
				Name: lt.name, Category: lt.category, TriggerID: lt.triggerID, EndedAlert: lt.endedAlert,
...
func loadPersistedTimers() {
	...
	for key, list := range f.Chars {
		for _, pt := range list {
			lt := &liveTimer{name: pt.Name, category: pt.Category, triggerID: pt.TriggerID, endedAlert: pt.EndedAlert}
			if pt.WasLive {
...
func RebuildTriggerActivation() {
	...
	trigStoreMu.Lock()
	// Enabled/disabled triggers are per character — swap in this toon's
	// overrides before reading enablement.
	useTrigTogglesForLocked(charName)
	var defs []trigDef
	if trigCfg != nil {
		var walk func(g *GinaGroup, path []string)
		walk = func(g *GinaGroup, path []string) {
			p := append(append([]string{}, path...), g.Name)
			if effectiveGroupEnabledLocked(g) {
