...
type trigDef struct {
	...
	// category is the EFFECTIVE overlay assignment: the package Category with
	// any personal Overlay override applied (trigger_overrides.go).
	category string
	// color is a personal per-trigger tint for the bars and alert text this
	// trigger produces ("" = the overlay's own color). From the override layer.
	color  string
...
type trigAlert struct {
	...
	text     string
	category string
	// color is the firing trigger's personal tint ("" = the overlay's color).
	color string
	at    time.Time
	// enders are this alert's early-end conditions, compiled at fire time with
	// that fire's own captures substituted — a package "^${1} looks focused\.$"
	// ender watches for THIS requester by name. Nil for most alerts.
	enders []*compiledEnder
	// clearedAt is set when an ender matched a later line: the overlay
	// triple-flashes the alert out and the cull ticker drops it shortly after.
	// Zero = live.
	clearedAt time.Time
}
...
type liveTimer struct {
	...
	name     string
	category string
	// color is the owning trigger's personal tint ("" = the overlay's color).
	color     string
	startedAt time.Time
	endsAt    time.Time
	triggerID int
	...
	visibleDurMs int64
	// clearedAt is set when an early-end condition ended this bar (and the same
	// line did NOT re-fire its trigger): the bar stays briefly, frozen, so the
	// overlays can triple-flash it out. Zero = live. A cleared bar fires no
	// ending/ended actions, is never paused/persisted/adopted, and is culled
	// once the flash window passes.
	clearedAt time.Time
...
const (
	...
	// showing only the latest. The TTL is a FLOOR: each alert is retained for
	// its overlay's configured Time Shown when that's longer (see
	// cullAlertsLocked), so the cap must hold a long-window overlay's backlog
	// alongside ordinary alert churn.
	trigAlertsMax = 150
...
var (
	...
	// trigActiveKey is trigActiveChar's per-(character, world) STORAGE key
	// (storeKeyFor): what pausedTimers and the toggle store key on, so a Green
	// namesake never touches the Blue character's stash. trigActiveChar stays
	// the bare name for {C} matching, display, and server payloads.
	trigActiveKey string
	...
	// trigIdleAway latches the plain-silence "away" episode so its one-shot
	// actions (map clear, status line) fire once per lull, not once per tick.
	// Cleared by the next log line.
	trigIdleAway bool
	...
	// trigLastSwapAt is when the tailer last switched the ACTIVE character. It
	// lets the shared-dbg.txt exit path (LeftWorldTriggerTimers) tell a genuine
	// same-character camp/quit from an unattributed marker that belongs to the
	// toon a just-completed swap already stashed.
	trigLastSwapAt time.Time
	pausedTimers   = make(map[string][]*liveTimer) // char store key → preserved timers
...
// clearTimersMentioningLocked removes live timers whose name mentions the
// slain mob's name (case-insensitive) — debuffs on it, kill windows — which
// are dead weight once it dies.
//
// Bars owned by a trigger in spareIDs/spareKeys are kept. Those belong to
// triggers THIS SAME slain line is about to fire — respawn timers named after
// the mob itself ("A mortiferous golem", started by its own death line). The
// unconditional sweep deleted the previous kill's running respawn bar a moment
// before the new fire appended a fresh one, so camps with several same-named
// mobs up only ever saw ONE bar, re-based to the most recent kill — the
// trigger's "start another timer" setting appeared to do nothing, and no
// amount of auditing the fire path could find it because the deletion happened
// here, before the fire. Caller holds trigStateMu.
func clearTimersMentioningLocked(mob string, spareIDs map[int]bool, spareKeys map[string]bool) int {
	...
	for _, lt := range liveTimers {
		spared := spareIDs[lt.triggerID] || (lt.trigKey != "" && spareKeys[lt.trigKey])
		if !spared && strings.Contains(strings.ToLower(lt.name), needle) {
			removed++
			continue
		}
		keep = append(keep, lt)
...
const (
	...
	// idleAwayAfter: silence alone marks the character "away" (map clear, and
	// the overlays hide IF the EQ process is gone too). It no longer pauses
	// timers — silence can't tell a crash from a character parked quietly at a
	// raid camp, and stash-and-refund handed quiet campers the whole gap back
	// (Aegolism observed ~58 minutes long across one poopsock evening). Timers
	// now tick through silence; a real exit is repaired by the login refund in
	// ProcessTriggerLine instead. See tickTriggerTimers.
	idleAwayAfter = 5 * time.Minute
	...
	// swapExitGrace: for this long after the tailer switches characters, ignore an
	// unattributed dbg.txt camp/quit marker. dbg.txt is shared by every EQ
	// instance and names no character for those exits, so one written by the
	// OUTGOING toon can be read a moment after the swap and would otherwise re-arm
	// the left-world latch against the freshly-swapped-in toon — suppressing that
	// toon's timer resume. The swap already stashed the outgoing toon under its
	// own key, so there is nothing left for the marker to do.
	swapExitGrace = 8 * time.Second
)
...
// trigFreezeInstantLocked is the moment pauses and checkpoints freeze a bar's
// clock at: the last log line — the latest instant the character was provably
// in the world — clamped to now. Every pause runs AFTER the character actually
// left (the dbg marker near-instantly, the eqlog camp fallback ~8s late), and
// buffs stop ticking the moment they're out, so charging the detection gap
// burned real duration and could expire a bar outright. If they idled quietly
// before a real exit the bar comes back a little long, which is the cheap
// direction to be wrong in. (Plain silence with no exit marker no longer
// pauses at all — see tickTriggerTimers.) Caller holds trigStateMu.
func trigFreezeInstantLocked(now time.Time) time.Time {
	if trigLastLine.IsZero() || trigLastLine.After(now) {
		return now
	}
	return trigLastLine
}
...
func pauseTimersLocked(charName string, now time.Time) int {
	asOf := trigFreezeInstantLocked(now)
	var kept []*liveTimer    // preserve (auto-pause) → frozen + stashed
	var carried []*liveTimer // carry (keep running) → retained live
	for _, lt := range liveTimers {
		// A cleared bar is mid-flash for whoever watched it end — it neither
		// carries nor freezes; the swap just drops it.
		if !lt.clearedAt.IsZero() {
			continue
		}
		// Carry FIRST: a "keep running" bar counts on real-world time, so the
		// swap doesn't touch it — it stays live and follows us onto the next
		// character (it lives in the one global liveTimers slice). Checked ahead
		// of preserve so a stale both-flags category can never freeze a carry
		// timer.
		if categoryCarryTimers(lt.category) {
			carried = append(carried, lt)
			continue
		}
		if !trigPreserveCategory(lt.category) {
			continue
		}
		rem := lt.endsAt.Sub(asOf)
		if rem <= 0 {
			continue
		}
...
// dropPausedNamedLocked removes the ACTIVE character's paused timers with the
// given name, so a bar they dismissed doesn't resurface on their next
// login/swap. Scoped to the character being played on purpose: the same buff
// name on an alt is that alt's own frozen bar with its own remaining time, and
// dismissing yours must not reach into their stash — that silently deleted
// alts' buff timers. Caller holds trigStateMu.
...
// the rest — called on a character swap (log file switch). charKey is the
// character's STORAGE key (storeKeyFor), so a swap between same-named
// characters on different worlds stashes into the right bucket.
func PauseTriggerTimers(charKey, reason string) {
	now := time.Now()
	trigStateMu.Lock()
	n := pauseTimersLocked(charKey, now)
	// The swap stashed the outgoing toon under its own key (above) and the
	// incoming toon is a fresh login. Record the swap so a straggler dbg.txt
	// exit marker for the OUTGOING toon is ignored (see LeftWorldTriggerTimers),
	// and clear any left-world latch outright so it can't suppress the incoming
	// toon's resume — the latch belonged to the character we just left.
	trigLastSwapAt = now
	trigLeftWorldAt = time.Time{}
	trigStateMu.Unlock()
	if n > 0 {
		addStatus("Timers: paused %d for %s (%s)", n, charKeyDisplay(charKey), reason)
	}
}
...
func LeftWorldTriggerTimers(reason string) {
	now := time.Now()
	trigStateMu.Lock()
	// A character swap moments ago already froze the outgoing toon under its own
	// key. This marker is unattributed (dbg.txt names no one for a camp/quit), so
	// acting now would only re-arm the left-world latch against the toon we just
	// swapped IN, holding off its resume for no reason. The incoming toon's own
	// exit, if it really leaves, still lands after this grace passes.
	if !trigLastSwapAt.IsZero() && now.Sub(trigLastSwapAt) < swapExitGrace {
		trigStateMu.Unlock()
		return
	}
	...
	if trigActiveKey != "" {
		...
		kept = pauseTimersLocked(trigActiveKey, now)
		dropped -= kept
	}
	...
	if kept > 0 || dropped > 0 {
		addStatus("Timers: %s — %s; paused %d (auto-pause overlays), cleared %d",
...
type persistedTimer struct {
	...
	// Color is the owning trigger's personal tint (trigger_overrides.go), so a
	// restored bar doesn't silently fall back to the overlay's color.
...
type persistedTimersFile struct {
	...
	Chars     map[string][]persistedTimer `json:"chars"` // lower(char) → frozen timers
	// Global holds "keep running across characters" (carry) timers, written with
	// an ABSOLUTE end (EndsAtMs/StartedAtMs) rather than frozen remaining. They
	// are not keyed per character — a carry timer is one machine-wide bar shown
	// on every toon — and restore LIVE (still counting) at startup, dropping any
	// whose instant already passed while the app was closed.
	Global []persistedTimer `json:"global,omitempty"`
}
...
// the given remaining/total — used for the preserve (auto-pause) categories.
func persistFrozen(lt *liveTimer, remaining, total time.Duration) persistedTimer {
	pt := persistedTimer{
		Name: lt.name, Category: lt.category, Color: lt.color,
		TriggerID: lt.triggerID, TrigKey: lt.trigKey,
...
// persistLive builds the on-disk form of a carry (keep-running) timer with its
// ABSOLUTE end preserved, so it keeps counting real-world time across a restart
// — the mob respawns whether or not the app was open. WasLive marks the
// absolute-end shape; loadPersistedTimers restores these live (not frozen).
func persistLive(lt *liveTimer) persistedTimer {
	pt := persistedTimer{
		Name: lt.name, Category: lt.category, Color: lt.color,
		TriggerID: lt.triggerID, TrigKey: lt.trigKey,
		WasLive:     true,
		EndsAtMs:    lt.endsAt.UnixMilli(),
		StartedAtMs: lt.startedAt.UnixMilli(),
	}
	fillEndPersist(&pt, lt)
	return pt
}

// snapshotTimersJSON serializes the timers that survive an app restart: the
// paused map (preserve categories, already frozen), the current character's
// running preserve-category bars (frozen as of this checkpoint), and every
// carry (keep-running) bar written with an absolute end into Global.
func snapshotTimersJSON(now time.Time) []byte {
	...
	// Same retroactive rule as pauseTimersLocked: freeze preserve bars as of the
	// last log line. Also what lets a bar being HELD past its zero (log quiet
	// since before it ran out — see tickTriggerTimers) checkpoint with the time
	// it had left when the character was last provably in the world, instead of
	// being dropped as expired.
	asOf := trigFreezeInstantLocked(now)
	activeKey := trigActiveKey
	for _, lt := range liveTimers {
		// A cleared (mid-flash) bar is already answered — never checkpointed.
		if !lt.clearedAt.IsZero() {
			continue
		}
		// Carry bars persist with their absolute end regardless of which
		// character is active — they're one machine-wide bar, not a per-toon
		// stash. Still running only matters at load time (expired ones drop).
		if categoryCarryTimers(lt.category) {
			out.Global = append(out.Global, persistLive(lt))
			continue
		}
		if activeKey == "" {
			continue
		...
		rem := lt.endsAt.Sub(asOf)
		if !trigPreserveCategory(lt.category) || rem <= 0 {
			continue
		}
		out.Chars[activeKey] = append(out.Chars[activeKey],
			persistFrozen(lt, rem, lt.endsAt.Sub(lt.startedAt)))
	}
	trigStateMu.Unlock()
...
func persistTriggerTimers(now time.Time, force bool) {
	...
	// atomicWrite (tmp + rename), not a bare WriteFile: an unclean exit mid-write
	// would leave a truncated file, and loadPersistedTimers silently discards a
	// file it can't parse — losing every saved timer, not just the last write.
	_ = atomicWrite(trigTimersStatePath(), data)
}
...
func loadPersistedTimers() {
	...
	for key, list := range f.Chars {
		for _, pt := range list {
			lt := &liveTimer{
				name: pt.Name, category: pt.Category, color: pt.Color,
				triggerID: pt.TriggerID, trigKey: pt.TrigKey,
	...
	// Carry (keep-running) timers restore LIVE with their absolute end intact,
	// straight into the global live set — so they show in Current Timers the
	// moment the app opens, before any character is tailed, and keep counting
	// real-world time. Anything whose instant already passed while the app was
	// closed is dropped (the mob is already back).
	for _, pt := range f.Global {
		if pt.EndsAtMs <= now.UnixMilli() {
			continue
		}
		startedAt := time.UnixMilli(pt.StartedAtMs)
		if pt.StartedAtMs <= 0 || startedAt.After(time.UnixMilli(pt.EndsAtMs)) {
			startedAt = time.UnixMilli(pt.EndsAtMs) // degenerate save: full bar
		}
		lt := &liveTimer{
			name: pt.Name, category: pt.Category, color: pt.Color,
			triggerID: pt.TriggerID, trigKey: pt.TrigKey,
			startedAt: startedAt, endsAt: time.UnixMilli(pt.EndsAtMs),
			visibleDurMs: pt.VisibleDurMs, ended: pt.Ended.toEndActions(),
		}
		if pt.EndingOffsetMs > 0 {
			lt.endingOffset = time.Duration(pt.EndingOffsetMs) * time.Millisecond
...
// pushAlertLocked appends a fired alert to the history. color is the firing
// trigger's personal tint ("" = the overlay's color). enders are the fire's
// capture-bound early-end conditions (nil for most alerts). Caller holds
// trigStateMu.
func pushAlertLocked(text, category, color string, now time.Time, enders []*compiledEnder) {
	if category = strings.TrimSpace(category); category == "" {
		category = "Default"
	}
	// A re-fire of the identical line on the same overlay refreshes the
	// existing alert in place — clock (and watch conditions) restart instead of
	// stacking a duplicate. That's what a repeat buff request or a re-sent call
	// means. A cleared entry is mid-flash and never revives: a fresh fire after
	// it is a new alert.
	for _, a := range trigAlerts {
		if a.clearedAt.IsZero() && a.category == category && a.text == text {
			a.at = now
			a.color = color
			a.enders = enders
			return
	...
	trigAlerts = append(trigAlerts, &trigAlert{
		id: alertNextID, text: text, category: category, color: color, at: now,
		enders: enders,
	})
...
func PushWorldAlarmAlert(text string) {
	...
	trigStateMu.Lock()
	pushAlertLocked(text, worldAlarmCategory, "", time.Now(), nil)
	trigStateMu.Unlock()
	emitTriggersChanged()
}

// clearFlashKeep is how long an ender-cleared alert or timer bar survives
// after its clear — long enough for the overlay's exit sequence (three
// flashes, then a readable fade-out) to finish before the entry vanishes.
// Overlays with "Flash if Ended Early" off simply hide cleared entries at
// once; the engine keeps them the same either way.
const clearFlashKeep = 5 * time.Second

// cullAlertsLocked drops alerts past their retention. A live alert is kept for
// its overlay's configured display window (plus polling slack) or the legacy
// TTL floor, whichever is longer — a 10-minute "Time Shown" overlay keeps its
// alerts 10 minutes. A cleared alert stays just long enough for the overlay's
// clear-flash to play. Caller holds trigStateMu.
func cullAlertsLocked(now time.Time) {
	kept := trigAlerts[:0]
	for _, a := range trigAlerts {
		switch {
		case !a.clearedAt.IsZero():
			if now.Sub(a.clearedAt) <= clearFlashKeep {
				kept = append(kept, a)
			}
		default:
			keep := trigAlertTTL
			if s := time.Duration(categoryAlertSeconds(a.category))*time.Second + 5*time.Second; s > keep {
				keep = s
			}
			if now.Sub(a.at) <= keep {
				kept = append(kept, a)
			}
		}
	}
...
// sweepAlertEndersLocked checks the live alerts that carry early-end
// conditions against one log line, marking any whose condition matched as
// cleared (the overlay triple-flashes them out). Returns whether anything
// changed. Zero cost per line while no live alert has conditions. Caller
// holds trigStateMu.
func sweepAlertEndersLocked(content string, now time.Time) bool {
	var lower string
	cleared := false
	for _, a := range trigAlerts {
		if len(a.enders) == 0 || !a.clearedAt.IsZero() {
			continue
		}
		if lower == "" {
			lower = strings.ToLower(content)
		}
		for _, ce := range a.enders {
			if ce.matches(content, lower) {
				a.clearedAt = now
				cleared = true
				break
			}
...
// tickTriggerTimers culls expired timers and detects "away": a camp countdown
// followed by log silence (a definitive exit — timers stash), or 5 minutes
// with no log activity at all (ambiguous — overlays hide only if the EQ
// process is gone, and timers keep ticking; a real exit is repaired by the
// login refund in ProcessTriggerLine). Returns status messages.
func tickTriggerTimers(now time.Time) []string {
	var msgs []string
	awayExit := false // camp/quit inferred from the eqlog — definitive, always hides
	awayIdle := false // plain silence — hides only if the EQ process is gone
	changed := false
	...
	keep := liveTimers[:0]
	for _, lt := range liveTimers {
		// A cleared bar (early-end condition answered it) is mid-flash: hold it
		// for the flash window and fire NO ending/ended actions — the condition
		// already closed the story.
		if !lt.clearedAt.IsZero() {
			if now.Sub(lt.clearedAt) <= clearFlashKeep {
				keep = append(keep, lt)
			} else {
				changed = true // flash over — the bar leaves the board
			}
			continue
		}
		if lt.endsAt.After(now) {
			...
			if lt.endingOffset > 0 && !lt.endingSpoken && !lt.ending.empty() &&
				...
				if lt.ending.text != "" {
					...
					pushAlertLocked(lt.ending.text, lt.category, lt.color, now, nil)
					changed = true
				}
		...
		// The bar has run out — but expiry needs proof the character outlived
		// it: a log line AFTER endsAt shows they were still in the world when
		// it lapsed. Without one (the log quiet since before the bar zeroed),
		// hold the bar instead of culling it: if the silence turns out to be an
		// exit, either the camp fallback below stashes it (frozen as of the
		// last line) or the login refund in ProcessTriggerLine shifts its end
		// back into the future when the character returns; if the character is
		// just quiet, their next line lets it expire normally, end-actions and
		// all. Held bars cost nothing visually — the overlays already drop bars
		// past their end. Preserved categories only: a spawn window or raid
		// call runs out in real time whether anyone is there to see it.
		if trigPreserveCategory(lt.category) && trigLastLine.Before(lt.endsAt) {
			keep = append(keep, lt)
			continue
		}
		...
		if !lt.ended.empty() {
			if lt.ended.text != "" {
				pushAlertLocked(lt.ended.text, lt.category, lt.color, now, nil)
			}
			if !timerAudioMutedLocked(lt) {
	...
	if trigActiveChar != "" && !trigLastLine.IsZero() {
		...
		if campRecent && idle >= campIdleAfter {
			// Camp countdown followed by silence: the eqlog-only fallback for a
			// completed camp where dbg.txt can't be read (which normally reports
			// the exit first, and stashes the timers itself). A definitive exit —
			// stash the preserved categories, frozen as of the last line.
			awayExit = true
			...
			if len(liveTimers) > 0 {
				...
				msgs = append(msgs, fmt.Sprintf(
					"Timers: %s — camped out; paused %d (auto-pause overlays), cleared %d",
					trigActiveChar, kept, dropped))
			}
			campLastMsg = time.Time{} // fire the pause/message only once per episode
		} else if idle >= idleAwayAfter {
			// Plain silence, no camp countdown. This used to stash the preserved
			// timers exactly like a camp — but silence can't distinguish a crash
			// or link-death from a character parked quietly at a raid camp, and
			// for the quiet-but-present case the stash-and-refund handed every
			// >5-minute lull in guild chat back to the buff timers (they came
			// back an hour long over a poopsock evening). So the timers KEEP
			// TICKING — correct for the in-world case — and a real exit repairs
			// itself instead:
			//  - a bar that runs out during the silence is already held rather
			//    than fired (the proof-of-life hold in the expiry pass above);
			//  - every way back into the world re-enters through "Welcome to
			//    EverQuest!", where ProcessTriggerLine refunds the whole offline
			//    gap to the preserved categories (buffs don't tick while out).
			// Overlay hiding moves to the post-lock check below, gated on the EQ
			// process actually being gone.
			awayIdle = true
			if !trigIdleAway {
				trigIdleAway = true
				// The position TTL on the shared map has already expired by now,
				// so this costs one dropped request when the character was
				// merely quiet.
				SendMapLocClear(trigActiveChar, "no log activity")
				if len(liveTimers) > 0 {
					msgs = append(msgs, fmt.Sprintf(
						"Timers: %s — no log activity for 5 minutes; %d timers keep ticking (offline time is refunded at the next login)",
						trigActiveChar, len(liveTimers)))
				}
			}
		}
	}
	...
	// and the idle branch does a process scan. An inferred camp always hides; the
	// plain-silence branch hides only when the EQ process is actually gone
	// (crash, task-manager kill): a quiet character with the game still running
	// — parked at a raid camp, or the user tabbed out with "Hide when EQ is
	// inactive" deliberately off — keeps their overlays. Re-checked every tick
	// while the silence lasts, so a client that dies mid-lull still hides within
	// the scan cache window.
	if awayExit || (awayIdle && !eqClientRunningCached()) {
...
// the admin cache RAW (isAdminCachedRaw) so a View-as preview (viewas.go)
// keeps the engine running for the previewing admin.
...
func RebuildTriggerActivation() {
	...
	if triggersOfficerOnly && !isOfficerCached() && !isAdminCachedRaw() {
		trigStateMu.Lock()
		trigActive = nil
		trigActiveChar = charName
		trigActiveKey = charKey
		trigStateMu.Unlock()
		return
	}
	...
	// Off-home the class is unknowable (no /who off Blue) and must NOT be
	// inherited from a Blue namesake — class-specific sections stay off.
	if _, token := splitStoreKey(charKey); token == "" {
	...
	// overrides and class, stored under their per-world key.
	ctx := trigCtxForLocked(charKey, false)
	var defs []trigDef
	if trigCfg != nil {
		...
		walk = func(g *GinaGroup, path []string, mutedInherit, clipInherit bool) {
			...
			for _, t := range g.Triggers {
				...
				if d, ok := defFromTrigger(t, p, trigOverrides[key]); ok {
					d.muteKey = key
...
// defFromTrigger snapshots the engine-relevant fields, applying the personal
// override layer (ov, from trigger_overrides.json; nil = none) so the def only
// ever carries effective values. ok=false when the trigger can never do
// anything we support (no pattern, or no action).
func defFromTrigger(t *GinaTrigger, groupPath []string, ov *TriggerOverride) (trigDef, bool) {
	...
	// The "does nothing" gate runs on the PACKAGE values, before the personal
	// customization below: a trigger whose every action a member suppressed
	// still compiles — its activity entry, timer, and clipboard behave exactly
	// like a fully muted trigger's.
	if !isTimer && !hasAlert && !hasTTS && !hasMedia && !copyClipboard {
		return trigDef{}, false // nothing we render, speak, play, or copy would happen
	}

	// Personal customization (trigger_overrides.go), applied here so the
	// snapshot below only ever sees effective values. Alert-text overrides
	// reword or suppress text the package already shows — they never create an
	// alert the officers didn't ship. TTS and sound may add as well as replace
	// or silence (audio is purely local). The ending/ended overrides apply
	// only while the package has that slot armed — members can't change
	// timing, so a slot the officers never enabled has nothing to fire from.
	color := ""
	if ov != nil {
		if ov.Color != nil {
			color = strings.TrimSpace(*ov.Color)
		}
		if ov.Text != nil && hasAlert {
			displayText = *ov.Text
			hasAlert = strings.TrimSpace(displayText) != ""
		}
		// Renames what the bar displays. The custom name is a template like
		// the package one — ${1}/{S}/{C}/{counter} still substitute at fire
		// time — and empty falls back to the package (there is no "no name").
		if isTimer && ov.TimerName != nil && strings.TrimSpace(*ov.TimerName) != "" {
			timerName = *ov.TimerName
		}
		if ov.TTS != nil {
			ttsText = *ov.TTS
			hasTTS = strings.TrimSpace(ttsText) != ""
		}
		if ov.Interrupt != nil {
			ttsInterrupt = *ov.Interrupt
		}
		if ov.Sound != nil {
			mediaFile = ""
			if strings.TrimSpace(*ov.Sound) != "" {
				mediaFile = mediaBasename(*ov.Sound)
			}
		}
		if isTimer && bool(t.UseTimerEnding) && t.TimerEndingTime > 0 {
			applyEndOverride(&ending, ov.EndingText, ov.EndingTTS, ov.EndingSound)
			if ov.EndingInterrupt != nil {
				ending.ttsInterrupt = *ov.EndingInterrupt
			}
			// The warning time is adjustable within the armed slot (a member
			// can re-time the warning, never conjure one on a disarmed slot).
			endingSecs := t.TimerEndingTime
			if ov.EndingSeconds != nil && *ov.EndingSeconds > 0 {
				endingSecs = *ov.EndingSeconds
			}
			// A member-added sound can arm a slot whose package actions were
			// all empty; a full suppression disarms it.
			endingOffset = 0
			if !ending.empty() {
				endingOffset = time.Duration(endingSecs) * time.Second
			}
		}
		if isTimer && bool(t.UseTimerEnded) {
			applyEndOverride(&ended, ov.EndedText, ov.EndedTTS, ov.EndedSound)
			if ov.EndedInterrupt != nil {
				ended.ttsInterrupt = *ov.EndedInterrupt
			}
		}
	}

	// Counter: active when GINA's flag is set or any text uses {counter}.
	// Scans the EFFECTIVE texts — a reworded alert can carry {counter} the
	// package text didn't. The reset window only applies when
	// UseCounterResetTimer is on.
	useCounter := bool(t.UseCounterResetTimer) ||
		textHasCounter(displayText, ttsText, t.ClipboardText,
			ending.text, ending.ttsText, ended.text, ended.ttsText)
	var counterResetMs int64
	if bool(t.UseCounterResetTimer) {
		counterResetMs = int64(t.CounterResetDuration) * 1000
	}

	// The effective overlay assignment: the local Overlay override wins over
	// the package Category; either way "" falls into "Default".
	cat := strings.TrimSpace(t.Category)
	if ov != nil && ov.Overlay != nil {
		cat = strings.TrimSpace(*ov.Overlay)
	}
	if cat == "" {
		cat = "Default"
	}
	...
	// "If already running" is personal preference — the override wins
	// (normalizeOverride admits only the engine's three known values).
	if ov != nil && ov.StartBehavior != nil && *ov.StartBehavior != "" {
		startBehavior = *ov.StartBehavior
	}
	// Early-end conditions end a timer's bar AND clear a fired alert (per-fire,
	// with the fire's captures bound — see fireTrigger). Collected for any
	// trigger that runs a timer or shows text. A blank condition would match
	// every line, so it's dropped here.
	var enders []enderDef
	if (isTimer || hasAlert) && t.TimerEarlyEnders != nil {
...
// applyEndOverride layers the personal customization onto one end-trigger
// slot: text rewords or suppresses only (it applies while the package shows
// text there), TTS and sound may also add. nil = keep the package value;
// "" = explicitly none.
func applyEndOverride(a *endActions, text, tts, sound *string) {
	if text != nil && a.text != "" {
		a.text = strings.TrimSpace(*text)
	}
	if tts != nil {
		a.ttsText = strings.TrimSpace(*tts)
	}
	if sound != nil {
		a.media = ""
		if strings.TrimSpace(*sound) != "" {
			a.media = mediaBasename(*sound)
		}
	}
}
...
// also resumes the character's paused timers and feeds camp-out detection; a
// login line additionally refunds the offline gap to preserved-category timers
// that ticked through the silence (see the refund block below).
func ProcessTriggerLine(line string) {
	...
	// Session-lock bookkeeping (tailer.go): a line proves the tailed log is
	// still alive — disarming any stale left-world signal — and a camp
	// countdown arms one, scoped to this character (it's THIS log's line).
	noteTailLineActivity()
	...
	trigStateMu.Lock()
	prevLine := trigLastLine
	trigLastLine = now
	trigIdleAway = false // any line ends a plain-silence away episode
	if campoutRE.MatchString(content) {
		campLastMsg = now
		noteTailLeftWorld(true)
	}
	// The slain-mob timer sweep runs AFTER the match pass below, so it can
	// spare the bars owned by triggers this same line fires.
	...
	resumed := 0
	refunded := 0
	var refundGap time.Duration
	if trigActiveChar != "" && trigLeftWorldAt.IsZero() {
		// The login refund. A "Welcome to EverQuest!" is the one moment the
		// character was provably OUT of the world since the previous line —
		// every way back in (camp, /q, crash, link-dead drop) re-enters through
		// it — and buffs don't tick while out. Preserved-category timers that
		// kept ticking through the silence (tickTriggerTimers no longer stashes
		// on silence alone) get the whole offline gap back here, which also
		// pulls any bar the expiry pass was holding past its end back into the
		// future. Exact match on the content: a guild-chat line quoting the
		// phrase must not trigger a refund. Timers stashed by a real exit
		// marker are not in liveTimers, so they can't be credited twice —
		// resumeTimersLocked below re-anchors those from their frozen
		// remaining instead.
		if content == "Welcome to EverQuest!" && !prevLine.IsZero() && now.After(prevLine) {
			refundGap = now.Sub(prevLine)
			for _, lt := range liveTimers {
				if !trigPreserveCategory(lt.category) {
					continue
				}
				lt.startedAt = lt.startedAt.Add(refundGap)
				lt.endsAt = lt.endsAt.Add(refundGap)
				refunded++
			}
		}
		resumed = resumeTimersLocked(trigActiveKey, now)
	}
	// Alert early-enders: an alert carrying watch conditions (compiled at its
	// fire with that fire's own captures) clears when a later line matches one
	// — the overlay triple-flashes it out and the cull ticker drops it.
	alertsCleared := sweepAlertEndersLocked(content, now)
	active := trigActive
	charName := trigActiveChar
	...
	for _, lt := range liveTimers {
		if !lt.clearedAt.IsZero() {
			continue // mid-flash — nothing left for an ender to end
		}
		if timerTrigIDs == nil {
	...
	if refunded > 0 {
		addStatus("Timers: refunded %s of offline time to %d timers for %s (login)",
			refundGap.Round(time.Second), refunded, charName)
	}
	changed := resumed > 0 || refunded > 0 || alertsCleared
	// Match pass. Enders still end their own trigger's bars inline (before that
	// trigger's own fire — see the comment below), but the FIRES are collected
	// and run after the slain-mob sweep, so the sweep can tell which bars belong
	// to triggers this line re-arms and spare them.
	type firedMatch struct {
		c     *compiledTrig
		caps  []string
		named map[string]string
	}
	var fires []firedMatch
	if len(active) > 0 {
		lower := strings.ToLower(content)
		for _, c := range active {
			// End-early BEFORE fire, and only for a timer that was live before
			// this line arrived. Order matters: the tank-list triggers (FMT/FRT/
			// FBL/FTT) use their own start pattern as the ender so a re-sent
			// list replaces the old bar — end the old, then start the new. The
			// old fire-then-end order killed the bar this same line had just
			// started whenever an older bar was also still running (the end
			// sweep removes every bar the trigger owns), which read as "the new
			// list cleared the timer and then nothing showed". With no older
			// bar live the ender is skipped entirely, which is why that failure
			// was intermittent.
			caps, named, ok := c.match(content, lower)
			if len(c.enders) > 0 && timerTrigIDs[c.id] && enderMatches(c, content, lower) {
				// Flash only a PURE early-end (the condition answered the bar).
				// When this same line also re-fires the trigger — the tank-list
				// replace idiom — the old bar is removed instantly so the fresh
				// one stands alone.
				if endTriggerTimersByID(c.id, !ok) > 0 {
					changed = true
				}
			}
			if ok {
				fires = append(fires, firedMatch{c: c, caps: caps, named: named})
			}
		}
	}
	// Slain-mob sweep: a dead mob's debuff bars and kill windows are dead
	// weight, but a respawn timer named after the mob — owned by a trigger this
	// very line fires — must survive, or a camp with several same-named mobs up
	// only ever shows one bar re-based to the latest kill (see
	// clearTimersMentioningLocked). Running the sweep after the resume above
	// also means a just-restored bar for the slain mob is swept like a live one.
	if mob != "" {
		spareIDs := make(map[int]bool, len(fires))
		spareKeys := make(map[string]bool, len(fires))
		for _, f := range fires {
			spareIDs[f.c.id] = true
			if f.c.muteKey != "" {
				spareKeys[f.c.muteKey] = true
			}
		}
		trigStateMu.Lock()
		cleared := clearTimersMentioningLocked(mob, spareIDs, spareKeys)
		trigStateMu.Unlock()
		if cleared > 0 {
	...
	for _, f := range fires {
		fireTrigger(f.c, f.caps, f.named, charName, now)
		changed = true
	}
...
func fireTrigger(c *compiledTrig, caps []string, named map[string]string, charName string, now time.Time) {
	...
	if c.useText {
		if txt := strings.TrimSpace(sub(c.displayText)); txt != "" {
			// Bind this fire's captures into the trigger's early-end conditions
			// so the alert can clear itself: the package's "^${1} looks
			// focused\.$" becomes "^Floptician looks focused\.$" for THIS fire.
			// Captured values come from \w+-style groups (player names), so
			// they're safe to splice into a pattern unescaped.
			var enders []*compiledEnder
			for _, ed := range c.trigDef.enders {
				resolved := enderDef{
					text:        substCaptures(ed.text, caps, named),
					enableRegex: ed.enableRegex,
				}
				if ce := compileEnder(resolved, charName); ce != nil {
					enders = append(enders, ce)
				}
			}
			pushAlertLocked(txt, c.category, c.color, now, enders)
		}
	}
	...
	case "RestartTimer":
		for _, lt := range liveTimers {
			...
			// bar instead of refreshing the restored one. A cleared bar is a
			// flashing corpse — never adopted; a fresh cast starts a new bar.
			if !lt.clearedAt.IsZero() || lt.name != name || !sameTriggerLocked(lt, c) {
				continue
			}
			lt.triggerID = c.id
			...
			// Same rule for the personal tint: the fresh cast owns the look.
	...
	case "IgnoreIfRunning":
		for _, lt := range liveTimers {
			if lt.clearedAt.IsZero() && lt.name == name {
				return
			}
		}
	}
	...
	liveTimers = append(liveTimers, &liveTimer{
		id: timerNextID, name: name, category: c.category, color: c.color,
...
func endTriggerTimersByID(id int, flash bool) int {
	trigStateMu.Lock()
	defer trigStateMu.Unlock()
	keep := liveTimers[:0]
	var names []string
	...
	now := time.Now()
	for _, lt := range liveTimers {
		if lt.triggerID == id && lt.clearedAt.IsZero() {
			removed++
			names = append(names, lt.name)
			// flash: the condition ANSWERED the bar — keep it briefly, marked,
			// so the overlays can triple-flash it out. The replacement path
			// (the ender's own line re-fires the trigger — the tank-list
			// pattern) removes instantly so the fresh bar stands alone.
			if flash {
				lt.clearedAt = now
				keep = append(keep, lt)
			}
