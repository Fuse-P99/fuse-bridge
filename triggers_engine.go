...
func snapshotTimersJSON(now time.Time) []byte {
	...
	if trigActiveChar != "" {
		key := strings.ToLower(trigActiveChar)
		// Same retroactive rule as pauseTimersLocked: freeze as of the last log
		// line. Also what lets a bar being HELD past its zero (log quiet since
		// before it ran out — see tickTriggerTimers) checkpoint with the time it
		// had left when the character was last provably in the world, instead of
