...
type QuestNudgeBanner struct {
	...
	// Char (bare name) and QuestID let the bar be clicked straight through to
	// that character's quest on the Characters tab — the quest whose step got
	// the map flag.
...
func setQuestNudgeBanner(text, char string, questID int) {
	nudgeBannerMu.Lock()
	nudgeBanner = QuestNudgeBanner{
		Text: text, SentAt: time.Now().UnixMilli(), Char: char, QuestID: questID,
	}
	nudgeBannerMu.Unlock()
}

// OpenCharacterQuest is the deep link behind the quest nudge bar and the map's
// quest flags: bring the main window forward and tell it to open this
// character's quest on the Characters tab. It goes through Go rather than a
// frontend store because the map also lives in an overlay window — a separate
// webview that shares no JS state with the main one; an app event reaches
// every window.
type openCharacterQuestEvent struct {
	Char    string `json:"char"`
	QuestID int    `json:"quest_id"`
}

func (a *App) OpenCharacterQuest(char string, questID int) {
	if v3App == nil {
		return
	}
	a.Show()
	v3App.Event.Emit("open-character-quest", openCharacterQuestEvent{
		Char: strings.TrimSpace(char), QuestID: questID,
	})
}
...
func questTrackZoneChange(zone string) {
	toon := currentCharKey  // quest state is keyed per (character, world)
	char := currentCharName // bare name, for the deep link's character pick
...
// questStepNeed is what a step yields — the thing worth grabbing — falling
// back to the step kind when it has no output item. Shared by the nudge (its
// marker label is quest name + ": " + need) and the tick-time marker
// retirement in questNoteItem, so the label built and the label retired can
// never drift apart.
func questStepNeed(q *Quest, idx int) string {
	if idx < 0 || idx >= len(q.Steps) {
		return ""
	}
	s := &q.Steps[idx]
	for ii := range s.Items {
		if s.Items[ii].Role == "out" {
			return s.Items[ii].Name
		}
	}
	return s.Kind
}

// questShortName is the quest prefix for marker labels and reminders. Epics
// carry the weapon list in their catalog name ("Ranger Epic: Swiftwind and
// Earthcaller"), which buries what the marker is actually pointing at —
// "Ranger Epic" says it all.
func questShortName(q *Quest) string {
	if questIsEpic(q) {
		return q.Class + " Epic"
	}
	return q.Name
...
// questNudgeLabel is the marker label the nudge drops for a step.
func questNudgeLabel(q *Quest, idx int) string {
	need := questStepNeed(q, idx)
	if need == "" {
		return ""
	}
	return questShortName(q) + ": " + need
}

// questNudge collects what this zone still owes the character's quests. char
// is the bare character name, carried into the banner and the marker so a
// click can open the right character's quest.
func questNudge(toon, char, zone string) {
	live := map[int]bool{}
	...
	// Zoning is a moment stale flags get looked at — prune markers whose step
	// has since been done before deciding what this zone still owes.
	questMarkersSweep(toon)
	...
	type nudgeHit struct {
		...
		y, x    float64
		center  bool
		questID int // the quest the flag opens when clicked
	}
	var hits []nudgeHit
	...
	for _, ref := range qdZoneRefs(zone) {
		...
		h := nudgeHit{label: questNudgeLabel(&q, ref.Step), need: need, center: true, questID: ref.QuestID}
	...
	next := hits[0]
	addQuestMarkerAuto(zone, next.y, next.x, next.label, next.center, next.questID, char)
	...
	addStatus("%s", msg)
	setQuestNudgeBanner(msg, char, next.questID)
}
