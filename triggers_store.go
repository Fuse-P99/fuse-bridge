...
// effectiveGroupEnabledLocked returns whether a group is enabled for ctx's
// character: their slider override wins; otherwise the rule-based default (see
// defaultGroupEnabledLocked). Caller holds trigStoreMu.
func effectiveGroupEnabledLocked(g *GinaGroup, ctx trigEnableCtx) bool {
	if v, ok := ctx.set.Groups[g.GroupID]; ok {
		return v
	}
