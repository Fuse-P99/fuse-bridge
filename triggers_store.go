...
var (
	...
	//
	// Enablement is per character: these two point at the active character's
	// maps inside trigTogglesAll, so every existing mutation site keeps working
	// and lands in the right bucket. Swapped by useTrigTogglesForLocked.
	...
	trigToggleChar string                        // lower(char) the maps above belong to
...
func loadTrigTogglesLocked() {
	...
	useTrigTogglesForLocked(currentCharName)
}

// useTrigTogglesForLocked points trigGroupToggle/trigTrigToggle at charName's
// overrides, creating the bucket on first sight.
//
...
// were made before any log was attached. The first real character to activate
// adopts them; after that the seed is gone and later toons start clean.
func useTrigTogglesForLocked(charName string) {
	key := strings.ToLower(charName)
	if trigGroupToggle != nil && trigTrigToggle != nil && key == trigToggleChar {
		return
	}
	trigToggleChar = key

	// No log attached yet: edits go straight to the seed so they carry to the
	// first character that logs in rather than vanishing.
	if key == "" {
		if trigToggleSeed == nil {
			trigToggleSeed = newTrigToggleSet()
		}
		trigGroupToggle, trigTrigToggle = trigToggleSeed.Groups, trigToggleSeed.Triggers
		return
	}

	set := trigTogglesAll[key]
	if set == nil {
		...
		trigGroupToggle, trigTrigToggle = set.Groups, set.Triggers
...
func saveTrigTogglesLocked() error {
	f := trigTogglesFile{Chars: trigTogglesAll, Seed: trigToggleSeed}
