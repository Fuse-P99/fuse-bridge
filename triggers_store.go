...
// trigClassFor resolves a character's class from their name or storage key.
// The active character prefers the class the sync layer resolved (it fetches
// on demand); everyone else reads the local character cache.
func trigClassFor(charName string) string {
	...
	key := strings.ToLower(strings.TrimSpace(charName))
	if key != "" && key == currentCharKey {
		if c := classForCurrentChar(); c != "" {
			return c
		}
	}
	bare, token := splitStoreKey(key)
	if token != "" {
		// Off-home namesake: class is unknowable (the char cache is Blue /who
		// data) and must not be borrowed from the Blue character of this name.
		return ""
	}
	return classForCharacter(bare)
}

// adoptTrigTogglesBucket copies the bare-name toggle set to an off-home
// character's qualified key, once (see adoptOffHomeCharState). A copy, not a
// move: leaving the bare bucket alone is harmless and safe if a Blue character
// with this name appears later.
func adoptTrigTogglesBucket(bare, key string) bool {
	trigStoreMu.Lock()
	defer trigStoreMu.Unlock()
	if trigTogglesAll[key] != nil {
		return false
	}
	src := trigTogglesAll[bare]
	if src == nil || src.empty() {
		return false
	}
	trigTogglesAll[key] = src.clone()
	_ = saveTrigTogglesLocked()
	return true
}
...
func effectiveGroupEnabledLocked(g *GinaGroup, ctx trigEnableCtx) bool {
	...
	for cur := g; cur != nil; cur = groupParentOf[cur.GroupID] {
		if cur != g && groupIsClassSpecificSectionLocked(cur) {
			crossedClass = true
		}
...
// groupIsClassSpecificSectionLocked reports whether g is THE class-specific
// section — the one whose ON means "auto: the detected class only" rather than
// "every class on". That section is always a direct child of the Fuse root
// (defaultGroupEnabledLocked/fuseSectionOfLocked model it that way). Position
// matters because sectionIsClassSpecific's "01 -" prefix also matches ordinary
// numbered subsections like "08 - Zone Specific > 01 - Old World"; treating one
// of those as the class boundary made enabling it clear its subgroups instead
// of turning them on (they defaulted back off), while a "02 -" sibling worked.
// Caller holds trigStoreMu.
func groupIsClassSpecificSectionLocked(g *GinaGroup) bool {
	if g == nil || !sectionIsClassSpecific(g.Name) {
		return false
	}
	p := groupParentOf[g.GroupID]
	return p != nil && p.GroupID == fuseRootGroupID
}
