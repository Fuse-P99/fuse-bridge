...
// showMainWindowNoSteal shows the main window WITHOUT taking focus. The
// automatic startup reveal uses this: by the time the UI is ready (shell-
// settle sleep + WebView2 spin-up) the user may already be back in the game,
// and an uninvited launch must not pull them out of it. Wails shows with
// SW_SHOW — the ACTIVATING variant — so capture the current foreground owner
// and hand it straight back, the same give-back the overlay restore path uses
// (popouts.go). Show() dispatches with InvokeSync, so the activation has
// already landed when it returns and the give-back is well-ordered. Explicit
// opens (tray, second launch, upgrade screen) keep Show()'s bring-to-front.
func showMainWindowNoSteal() {
	if mainWindow == nil {
		return
	}
	ensureMainWindowOnScreen()
...
func (a *App) SetAutoStart(enabled bool) error {
	...
	auditToggle("auto_start", enabled, "")
	return nil
}
...
type CharEntry struct {
	...
	// Current marks the character whose log is being tailed (i.e. online now);
	// the list pins them to the top.
...
func (a *App) GetCharNames(query string, excludeBots, excludeFiltered bool) []CharEntry {
	...
	// The tailed (online) character pins to the top: the tab almost always
	// opens to look at the toon being played. Stable, so everyone else keeps
	// the existing order.
	cur := strings.ToLower(strings.TrimSpace(currentCharName))
	if cur != "" {
		for i := range out {
			out[i].Current = strings.ToLower(out[i].Name) == cur
		}
...
func (a *App) GetSpellsForClass(class string) []SpellEntry {
	return fetchSpellsForClass(class)
}

// fetchSpellsForClass is the bare HTTP fetch, shared with the Characters tab
// quick search (which caches it per class rather than per request).
func fetchSpellsForClass(class string) []SpellEntry {
	base := strings.TrimSuffix(serverURL, "/submit")
...
// PlayOverlayCue plays a trigger media file as an overlay's audio cue (the
// CH chain count pulse). Unlike PlayTriggerMediaSample this respects the
// master mute — it fires on its own schedule, not from a user click.
func (a *App) PlayOverlayCue(name string) {
	playOverlayCue(resolveMediaPath(name))
}
...
// GetCurrentServer reports which EQ world the client is tailing and whether it
// is the home (P99 Blue) world. Off-home the UI shows a reduced-mode note:
// nothing is sent to the server, but a linked member still READS Blue state —
// the Fuse package, the raid board, batphones — from any world.
func (a *App) GetCurrentServer() ServerInfo {
	return ServerInfo{
		Token: getCurrentServerToken(),
		Name:  currentServerLabel(),
		Home:  onHomeServer(),
	}
}
...
func (a *App) SetAutomations(addTracking, swapBot, addMissed bool, mainToon string) error {
	// The previous state is fetched first so only the flags that actually
	// flipped are audited (the UI sends all four on every change).
	prev, prevErr := fetchAutomations()
	if err := saveAutomations(addTracking, swapBot, addMissed, mainToon); err != nil {
		return err
	}
	if prevErr == nil {
		auditAutomationsDiff(prev, AutomationSettings{
...
type wailsClientEntry struct {
	...
	Linked   bool   `json:"linked"` // attributed to a member — the lock icon
}

// GetSettingsSummary returns the officer settings-dashboard aggregate (the
// Clients tab's one-line dashboard opens it). Non-officers get an error the
// dashboard shows as its empty state.
func (a *App) GetSettingsSummary() (map[string]any, error) {
	return fetchSettingsSummary()
}
...
// GetOtherGuildChat returns the non-Fuse guild chat feeds for the Clients tab.
// Server-gated to admins (config list); non-admins get an empty list.
func (a *App) GetOtherGuildChat() ([]GuildChatFeed, error) {
	return fetchOtherGuildChat()
}

// GetCrossGuildToons returns the cross-guild character report for the Clients
// tab — same admin gating as the chat feeds.
func (a *App) GetCrossGuildToons() ([]CrossGuildToon, error) {
	return fetchCrossGuildToons()
}
...
// IsAdminMode reports whether the linked member holds BOTH admin Discord
// roles (Officer + Bot Manager) — asked of the server, like IsOfficer. Admin
// is not a local setting: a plaintext settings.json flag could be flipped by
// any member, and the server re-checks every admin endpoint anyway.
func (a *App) IsAdminMode() bool {
	// Any active View-as preview drops admin — the personas being imitated
	// aren't admins. The View-as control itself renders off GetViewAs()'s
	// RealAdmin, so this can't lock the admin out of exiting.
	if ViewAsActive() != "" {
		return false
	}
...
func (a *App) IsOfficer() bool {
	// View-as personas are never officers (viewas.go).
	if ViewAsActive() != "" {
		return false
	}
	return fetchIsOfficer()
}
...
// ClientsList carries the client rows plus the server's authoritative current
// build version, which the dashboard's version gradations derive from.
type ClientsList struct {
	Clients       []wailsClientEntry `json:"clients"`
	LatestVersion string             `json:"latest_version"`
}

func (a *App) GetClients() (ClientsList, error) {
	clients, latest, err := fetchClients()
	if err != nil {
	...
	return ClientsList{Clients: out, LatestVersion: latest}, nil
}
