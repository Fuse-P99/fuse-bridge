...
// OpenMap asks the frontend to switch to the Map tab. Fired when the player spams
// /loc (a deliberate "show me the map" gesture — see automap.go). Deliberately
// does NOT show/raise/focus the window — if the app is hidden in the tray it stays
// there; this only flips the active tab so the map is showing next time it's
// opened. Safe to call from any goroutine.
func (a *App) OpenMap() {
	if v3App == nil {
		return
	}
	v3App.Event.Emit("open-map")
}
...
func runWails() {
	...
	v3App.Event.OnApplicationEvent(events.Common.ApplicationStarted, func(*application.ApplicationEvent) {
		...
		// Restore the app-wide map overlay, then this character's timer overlays.
		// The tailer often attaches to the log before the UI exists, so that
		// earlier ApplyPopoutsForCharacter call was a no-op (no v3App yet) — apply
		// it here so a character already known at launch gets its layout.
		ReopenSavedPopouts()
		if currentCharName != "" {
...
func (a *App) GetCharTable(excludeBots, excludeFiltered bool) []CharTableRow {
	...
	for _, n := range names {
		...
		for _, it := range readInventoryItems(n, eqDir) {
			itemSet[normalizeItemName(it.Name)] = true
		}
		for _, kc := range keyColumns {
			row.Keys[kc.ID] = itemSet[normalizeItemName(kc.Item)]
		}
...
// SetAutomations stores the automation settings. Both toggles off clears the
// main toon (also enforced server-side).
func (a *App) SetAutomations(addTracking, swapBot bool, mainToon string) error {
	return saveAutomations(addTracking, swapBot, mainToon)
}
