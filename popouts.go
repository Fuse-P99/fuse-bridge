...
// OpenPopout opens (or focuses) an overlay window. kind is "map", "timers", or
// "alerts"; category is the trigger category name (ignored for the map).
...
// ReopenSavedPopouts reopens the app-wide map overlay if it was open. Timer
// overlays are per character, so they're opened by ApplyPopoutsForCharacter once
// the tailed character is known. Called once the application is ready.
...
func ApplyPopoutsForCharacter(charName string) {
	...
	// leave the state untouched so the ApplicationStarted handler's call still
	// applies this character's layout once the UI is up.
	if !popoutsCanOpen() || charName == "" {
		return
	}
	...
	// their overlays must come back when they log in again.
