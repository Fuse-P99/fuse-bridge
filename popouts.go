...
func (a *App) SetAllPopoutsLocked(locked bool) {
	...
	for name, w := range popouts {
		// The map stays interactive (pan/zoom/buttons); lock only timer
		// overlays. Racing Mode is PERMANENTLY click-through (it sits over the
		// game viewport and is auto-placed — there is nothing to interact
		// with), so a global unlock must not make it start eating the mouse.
		if name != mapName && name != "popout-racing" {
			wins = append(wins, w)
		}
	}
	popoutsMu.Unlock()
...
func openPopoutWindow(kind, category string, focus bool) {
	...
	// (the map is never locked). Racing Mode is click-through from birth,
	// lock state or not: it covers the game viewport, is auto-placed, and
	// must never intercept a click meant for the game. Its options live in
	// the Manage Overlays right-click editor.
	popoutsMu.Lock()
	lk := popoutsLocked
	popoutsMu.Unlock()
	if kind == "racing" {
