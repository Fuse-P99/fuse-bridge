...
//     logging in for the first time inherits the layout of a configured
//     same-class character, else starts with nothing (plus the migrated legacy
//     special-overlay records — see migrateSpecialsForLocked).
//
...
func ApplyPopoutsForCharacter(charName string) {
	...
	// Close every live overlay of the outgoing character — timers, alerts, and
	// special overlays alike. Flagged as a switch-close so the close hook leaves
	// that character's saved layout intact — their overlays must come back when
	// they log in again. Only the map is app-wide and stays open across swaps.
