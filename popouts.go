...
func openPopoutWindow(kind, category string, focus bool) {
	...
	// must not steal the game's keyboard. The primary defense is
	// WS_EX_NOACTIVATE, applied right after creation below, which makes every
	// Show()/SetWindowPos non-activating outright. This give-back stays as the
	// backstop for the paths the style doesn't cover (a title-bar drag's
	// SetFocus, a WebView2-internal grab); its failure mode is a no-op.
	...
	// Non-activating from birth. Window creation is synchronous (NewWithOptions
	// InvokeSyncs the native create once the app is running), so this lands
	// before the first Show() below AND before Wails' async NavigationCompleted
	// re-Show — the one that used to fire after the give-back had already run.
	...
	// Diagnostic: overlays are non-activating, so any activation is either
	// deliberate (Pop out button, title-bar drag, click-to-type) or a
	// focus-steal bug — log both flavors so "the camera spun" reports become a
	// one-line grep of the client log. WA_ACTIVE = programmatic, WA_CLICKACTIVE
	// = via mouse click; they arrive as different events.
	...
	// Title-bar drags and frameless edge-resizes run the OS-modal move loop,
	// which must not run on a NOACTIVATE window whose process isn't foreground
	// — that's the machine-wide input wedge (see popoutDragBegin). Activate
	// for the loop's duration, restore on exit. StartMove/StartResize are both
	// WM_ENTERSIZEMOVE; Wails splits them on the mouse-button state.
	w.OnWindowEvent(events.Windows.WindowStartMove, func(*application.WindowEvent) {
		popoutDragBegin(w, name)
	})
	w.OnWindowEvent(events.Windows.WindowStartResize, func(*application.WindowEvent) {
		popoutDragBegin(w, name)
	})
	w.OnWindowEvent(events.Windows.WindowEndMove, func(*application.WindowEvent) {
		popoutDragEnd(w, name)
	})
	w.OnWindowEvent(events.Windows.WindowEndResize, func(*application.WindowEvent) {
		popoutDragEnd(w, name)
	})
