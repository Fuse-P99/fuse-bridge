...
var (
	...
	// popoutGrace: per window name, until when a freshly popped-out overlay
	// stays movable while the global lock is on (see openPopoutWindow and the
	// PopoutGrace* methods). Guarded by popoutsMu.
	popoutGrace = map[string]time.Time{}
...
func isSpecialPopoutKind(kind string) bool {
	switch kind {
	case "raidassign", "raiddebuffs", "raidclerics", "othertimers", "voicespeakers", "randoms", "threat", "raiddps", "guildchat":
		return true
	}
	return false
}
...
func reconcilePopoutVisibility(onlyMismatched bool) {
	...
	// The hover card belongs to whichever overlay the cursor is over, so it
	// goes away with them. Hide only, and only on the way down: nothing but an
	// actual hover may ever show it again (hovercard.go).
	if manual || auto || racingFocus {
		hideHoverCard()
	}
}
...
// guildChatLocksWithAllLocked reports whether the Guild Chat overlay has opted
// into the global overlay lock (its "Stay unlocked" toggle is off) — the same
// opt-out the map has, on the same field. Guild Chat is a special overlay, so
// its record belongs to the ACTIVE CHARACTER; with no character in play, or
// none stored for this one yet, the default stands and it stays unlocked.
// Caller holds popoutsMu.
func guildChatLocksWithAllLocked() bool {
	name, _, _, _, _, _, _ := popoutIdent("guildchat", "")
	st := popoutStateForLocked("guildchat", "", name, false)
	return st != nil && st.LockWithAll
}

// GetGuildChatStayUnlocked reports the Guild Chat overlay's lock opt-out for
// its settings panel: true (the default) means Lock Overlays leaves the chat
// interactive, so it can still be scrolled back through.
func (a *App) GetGuildChatStayUnlocked() bool {
	popoutsMu.Lock()
	defer popoutsMu.Unlock()
	return !guildChatLocksWithAllLocked()
}

// SetGuildChatStayUnlocked flips that opt-out and applies it immediately, the
// map's rule exactly: with the overlays already locked, unchecking locks the
// chat on the spot and re-checking frees it. A chat locked this way is
// click-through, so it shows only the most recent lines — the wheel goes with
// the mouse — and re-checking from the overlay itself isn't possible; Unlock
// Overlays on the Timers tab stays the escape hatch, same as every other
// overlay. (The record is created through the same path OpenPopout uses, so
// nothing is stored until a character is known.)
func (a *App) SetGuildChatStayUnlocked(stay bool) {
	name, _, _, _, _, _, _ := popoutIdent("guildchat", "")
	popoutsMu.Lock()
	if st := popoutStateForLocked("guildchat", "", name, true); st != nil {
		st.LockWithAll = !stay
		savePopoutStoreLocked()
	}
	w := popouts[name]
	...
	auditEvent("app_updatedguildchatsettings_stayunlocked_"+onOff(stay), "")
}
...
func (a *App) SetMapStayUnlocked(stay bool) {
	...
	auditEvent("app_updatedmapsettings_stayunlocked_"+onOff(stay), "")
}

// onOff is the audit vocabulary for a boolean setting value.
...
func (a *App) SetAllPopoutsLocked(locked bool) {
	...
	// An explicit lock or unlock overrides every placement grace in progress:
	// the user just said what they want. The overlays drop their countdowns on
	// the event below.
	popoutGrace = map[string]time.Time{}
	mapLocks := mapLocksWithAllLocked()
	gcName, _, _, _, _, _, _ := popoutIdent("guildchat", "")
	gcLocks := guildChatLocksWithAllLocked()
	var wins []*application.WebviewWindow
	for name, w := range popouts {
		// The map and Guild Chat each carry their own "Stay unlocked" toggle
		// (popoutState.LockWithAll; the default, off, means stay unlocked). With
		// it on they keep the mouse under the lock — the map so pan/zoom/buttons
		// still work, the chat because a log is read by scrolling and
		// click-through kills the wheel. Turn it off and they lock with
		// everything else. Racing Mode is PERMANENTLY click-through (it sits over
		// the game viewport and is auto-placed — there is nothing to interact
		// with), so a global unlock must not make it start eating the mouse.
		if (name != mapName || mapLocks) && (name != gcName || gcLocks) && name != "popout-racing" {
			wins = append(wins, w)
		}
	}
	popoutsMu.Unlock()
	...
	emitPopoutPositioning() // graces were cleared above
}

// How long a freshly popped-out overlay stays movable under the global lock
// before locking itself where it sits; every move or resize restarts it.
// Keep in step with GRACE_MS in Popout.svelte.
const popoutGraceDur = 30 * time.Second

// popoutGraceLockable reports whether the global lock applies to this kind of
// overlay at all (the same rule SetAllPopoutsLocked and openPopoutWindow use):
// never for Racing Mode, and for the map and Guild Chat only when their "Stay
// unlocked" toggle is off. Caller holds popoutsMu.
func popoutGraceLockableLocked(kind string) bool {
	return kind != "racing" && (kind != "map" || mapLocksWithAllLocked()) &&
		(kind != "guildchat" || guildChatLocksWithAllLocked())
}

// Positioning mode: at least one overlay is in its placement grace under the
// lock. EVERY overlay then shows its title bar and a red border, so the new
// one can be placed against the others — most of which are otherwise
// collapsed to nothing, or title-less, while locked. Caller holds popoutsMu.
func popoutPositioningLocked() bool {
	return popoutsLocked && len(popoutGrace) > 0
}

var popoutPositioningLast bool // last state announced; guarded by popoutsMu

// emitPopoutPositioning announces a change in positioning mode to the
// overlays. Call with popoutsMu released; it takes the lock itself.
func emitPopoutPositioning() {
	popoutsMu.Lock()
	now := popoutPositioningLocked()
	changed := now != popoutPositioningLast
	popoutPositioningLast = now
	popoutsMu.Unlock()
	if changed && v3App != nil {
		v3App.Event.Emit("popouts-positioning", now)
	}
}

// ArePopoutsPositioning reports positioning mode, for an overlay that mounts
// while it is already on. Bound.
func (a *App) ArePopoutsPositioning() bool {
	popoutsMu.Lock()
	defer popoutsMu.Unlock()
	return popoutPositioningLocked()
}

// PopoutGraceMs reports how long (ms) an overlay's placement grace has left, 0
// when it has none. The overlay asks at mount to know whether to run its
// countdown — Go is the truth for "opened under the lock". A grace found
// already expired (the overlay never reported back, e.g. its page reloaded)
// is closed out here, so no window stays movable by accident. Bound.
func (a *App) PopoutGraceMs(kind, category string) int64 {
	name, _, _, _, _, _, _ := popoutIdent(kind, category)
	popoutsMu.Lock()
	until, ok := popoutGrace[name]
	if !ok || !popoutsLocked {
		popoutsMu.Unlock()
		return 0
	}
	...
	popoutsMu.Unlock()
	a.PopoutGraceEnd(kind, category)
	return 0
}

// PopoutGraceExtend restarts an overlay's placement grace: it was moved or
// resized, so the user is still arranging it. No-op without a grace in
// progress. Bound.
func (a *App) PopoutGraceExtend(kind, category string) {
	name, _, _, _, _, _, _ := popoutIdent(kind, category)
	popoutsMu.Lock()
	defer popoutsMu.Unlock()
	if _, ok := popoutGrace[name]; ok && popoutsLocked {
		popoutGrace[name] = time.Now().Add(popoutGraceDur)
	}
}

// PopoutGraceEnd ends an overlay's placement grace and locks it where it sits
// (click-through), provided the global lock is still on. Bound.
func (a *App) PopoutGraceEnd(kind, category string) {
	name, _, _, _, _, _, _ := popoutIdent(kind, category)
	popoutsMu.Lock()
	_, had := popoutGrace[name]
	delete(popoutGrace, name)
	w := popouts[name]
	// Guild Chat with "Stay unlocked" on ends its grace like the map does — the
	// placement freezes, but it never goes click-through, because a chat log is
	// read by scrolling and the wheel dies with the mouse. That call belongs to
	// popoutGraceLockableLocked now, which answers for both opt-outs.
	lock := had && popoutsLocked && popoutGraceLockableLocked(kind)
	popoutsMu.Unlock()
	// Window call only after popoutsMu is released (popouts_noactivate.go
	// contract: window ops SendMessage the main thread — deadlock risk).
	if lock && w != nil {
...
type popoutState struct {
	...
	// LockWithAll opts an overlay INTO the global overlay lock. Two kinds have
	// that opt-out and so use this field: the map and Guild Chat. The default
	// (false) is "Stay unlocked": Lock Overlays freezes and click-throughs every
	// other overlay but leaves these two interactive — the map for pan/zoom/
	// search, Guild Chat because a log is read by scrolling and click-through
	// kills the wheel. Unchecking that overlay's "Stay unlocked" toggle sets
	// this, making it lock exactly like a timer overlay.
...
// adoptPopoutCharBucket copies the bare-name overlay layout to an off-home
// character's qualified key, once (see adoptOffHomeCharState). Class is NOT
// copied: it may describe a Blue namesake from /who data, and the adopted
// character already has a layout, so inheritance never needs it.
func adoptPopoutCharBucket(bare, key string) bool {
	popoutsMu.Lock()
	defer popoutsMu.Unlock()
	if len(popoutChars[key]) > 0 {
		return false
	}
	src := popoutChars[bare]
	if len(src) == 0 {
		return false
	}
	popoutChars[key] = clonePopoutSet(src)
	savePopoutStoreLocked()
	return true
}
...
func popoutIdent(kind, category string) (name, title, hash string, defW, defH, minW, minH int) {
	...
	case "guildchat":
		// Wider than the other specials: a chat line is long.
		return "popout-guildchat", "Guild Chat", "#popout=guildchat", 420, 220, 260, 100
	case "racing":
...
func (a *App) OpenPopout(kind, category string) {
	...
	// Every Pop out button (map tab, Manage Timers, Manage Overlays) lands
	// here, so this is the one place the pop-out is audited. Named by the
	// overlay's display title: "Map", the timer/alert category, or the special
	// overlay's name.
	_, title, _, _, _, _, _ := popoutIdent(kind, category)
	auditEvent("app_poppedoverlay_"+title, "kind: "+kind)
}
...
type OverlayDefaultInfo struct {
	...
	Char       string `json:"char"`       // the character in play (display text)
	Key        string `json:"key"`        // that character's storage key — what Save/Reset and the look-settings suffix use
...
func (a *App) GetOverlayDefaultInfo() OverlayDefaultInfo {
	popoutsMu.Lock()
	defer popoutsMu.Unlock()
	char := charDisplayName(currentCharName, currentCharKey)
	key := currentCharKey
	if popoutDefaultEditing && popoutPendingChar != "" {
		key = popoutPendingChar
		char = charKeyDisplay(popoutPendingChar)
	}
	...
	return OverlayDefaultInfo{
		...
		Char:       char,
		Key:        key,
		CharSet:    key != "" && hasTimerLayoutLocked(key),
...
func (a *App) SetOverlayDefaultEditing(on bool) {
	...
	if !on {
		...
		popoutPendingChar = currentCharKey
	}
	...
	if on {
		...
		addStatus("Overlays: back to %s's layout", charKeyDisplay(target))
	}
	emitOverlayDefaults()
}
...
// rebuilt by hand. charName is the character's storage key (OverlayDefaultInfo
// hands the frontend the exact value to pass back). Bound.
func (a *App) SaveOverlayDefaultFromCharacter(charName string) error {
	...
	if len(set) == 0 {
		popoutsMu.Unlock()
		return fmt.Errorf("%s has no overlays set up yet", charKeyDisplay(key))
	}
	...
	addStatus("Overlays: saved %s's layout (%d overlays) as the default", charKeyDisplay(key), n)
	emitOverlayDefaults()
	return nil
}
...
// only owns geometry and the open set. charName is the character's storage key
// (from OverlayDefaultInfo). Bound.
func (a *App) ResetCharacterToDefault(charName string) error {
	...
	addStatus("Overlays: reset %s to the default layout", charKeyDisplay(key))
	emitOverlayDefaults()
	return nil
}
...
func maybeApplyDeferredPopouts() {
	...
	if currentCharKey != "" {
		ApplyPopoutsForCharacter(currentCharKey)
	}
}

// ApplyPopoutsForCharacter switches the timer overlays to a character's saved
...
// left untouched. Called on login and on every character swap. charKey is the
// per-(character, world) STORAGE key (storeKeyFor) — for off-home characters
// classForCharacter misses on it by design, so a Green namesake never inherits
// a Blue character's class or layout.
func ApplyPopoutsForCharacter(charKey string) {
	...
	if !popoutsCanOpen() || startupPopoutsPending.Load() || charKey == "" {
		return
	}
	key := strings.ToLower(charKey)
	...
	if popoutDefaultEditing {
		...
		if cls := classForCharacter(key); cls != "" {
...
func (a *App) ClosePopout(kind, category string) {
	...
	// Guild Chat is the one overlay that raises the hover card, and a card
	// showing for a line in a window that just went away would hang there
	// until its safety timer fired (hovercard.go).
	if name == "popout-guildchat" {
		hideHoverCard()
	}
}
...
func openPopoutWindow(kind, category string, focus bool) {
	...
	// Focus protection, take two — "no-steal shows" (v2.5.3569). Register the
	// window so the module patch shows it with SW_SHOWNOACTIVATE and
	// re-orders it with SWP_NOACTIVATE: no programmatic show during a login
	// burst — including Wails' async navigationCompleted re-show, which no
	// app-side bracketing can reach — can take the game's foreground.
	// Crucially the window itself stays completely stock for clicks and
	// drags. The previous approach, WS_EX_NOACTIVATE on the window
	// (v2.5.2469–3369), suppressed CLICK activation too, which broke the OS
	// move loop and WebView2 mouse capture — machine-wide input lockouts —
	// and was unwound in 3469 (see popouts_noactivate.go for the saga). A
	// deliberate user click activates normally, exactly like pre-2469.
	// Unregistered in the closing hook below: HWND values are recycled, and
	// a stale entry would silence an unrelated future window's shows.
	noStealHwnd := uintptr(w.NativeWindow())
	application.FuseBridgeSetNoStealShow(noStealHwnd, true)

	popoutsMu.Lock()
	popouts[name] = w
	lk := popoutsLocked
	mapLk := mapLocksWithAllLocked()
	gcLk := guildChatLocksWithAllLocked()
	// A user-initiated pop-out under the global lock gets a placement grace:
	// it comes up movable and locks itself once it has sat still for
	// popoutGraceDur (the overlay counts it down on screen and calls
	// PopoutGraceEnd; a move or resize restarts it via PopoutGraceExtend).
	// Without it the fresh overlay was click-through from birth, and placing
	// it meant unlocking everything, dragging, and locking again. Startup and
	// character-swap restores (focus false) are already placed and lock at
	// once, as before.
	grace := focus && lk && kind != "racing" && (kind != "map" || mapLk) && (kind != "guildchat" || gcLk)
	if grace {
		popoutGrace[name] = time.Now().Add(popoutGraceDur)
	} else {
		delete(popoutGrace, name)
	...
	popoutsMu.Unlock()
	if grace {
		emitPopoutPositioning() // the OTHER overlays outline themselves now
	}
	...
	//
	// Guild Chat has the same opt-out as the map, and the same default: with its
	// "Stay unlocked" toggle on it keeps the mouse even when it opens under the
	// lock — a chat log is read by scrolling — and only its placement freezes.
	if kind == "racing" || (lk && !grace && (kind != "guildchat" || gcLk) && (kind != "map" || mapLk)) {
		w.SetIgnoreMouseEvents(true)
	}
	...
	w.RegisterHook(events.Common.WindowClosing, func(*application.WindowEvent) {
		// The no-steal-show registration dies with the window (see creation
		// above — recycled HWNDs must not inherit it).
		application.FuseBridgeSetNoStealShow(noStealHwnd, false)
		...
		popoutsMu.Lock()
		delete(popouts, name)
		if _, inGrace := popoutGrace[name]; inGrace {
			delete(popoutGrace, name)
			// Goroutine: it takes popoutsMu, which this hook holds.
			go emitPopoutPositioning()
		}
		// The topmost-intent record dies with the window (recycled names must
		// not inherit an old opt-out; the frontend re-applies stored opt-outs
		// on boot).
