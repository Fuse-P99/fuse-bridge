...
var raidAEForwardDefs = []raidAEForwardDef{
	...
	// ── Great Divide ──
	// Ring War: Garadain's "TROOPS! FALL IN!" is an NPC /say that only players
	// in say range of the ring log, and the repop message is zone-wide. Both
	// start Ring War timers the whole war wants. (Narandi's ocean shouts have
	// their own unconditional forward — ringWarShoutPattern in filter.go.)
	{[]string{"greatdivide"}, regexp.MustCompile(`^Garadain Glacierbane says, 'TROOPS! FALL IN!'$`)},
	{[]string{"greatdivide"}, regexp.MustCompile(`^<ZONE MESSAGE>: This zone is repopping now!$`)},
