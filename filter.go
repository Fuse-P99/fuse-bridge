...
// /who output is NOT forwarded line by line here: whoGateFeed (whogate.go)
// buffers each block and forwards it only once its footer proves it describes
// the local zone — /who all output never leaves the client. The server still
// deduplicates identical /who lines (5-minute TTL), so repeated snapshots of
// the same zone cost nothing.
...
func ShouldForward(line string) bool {
	...
	// /who output is deliberately absent here — whoGateFeed owns it (main.go
	// consults the gate only for lines nothing above claimed).
...
// guildSelfLineRE matches ONLY when "You say to your guild, " is the line's
// own verb — immediately after the leading "[timestamp] " (or at the very
// start of a timestamp-less line).
var guildSelfLineRE = regexp.MustCompile(`^(\[[^\]]*\] )?You say to your guild, `)
...
//
// Anchored on purpose: a "You say to your guild, " INSIDE the quoted message
// must never be rewritten. When a player pastes their own earlier guild line
// back into guild chat, every listener's log carries that self-say phrase in
// the quotes — the old unanchored Replace made each client substitute its OWN
// character name there, turning ~30 byte-identical reports of one message
// into ~30 unique ones, which sailed past the server's duplicate collapse
// and flooded the guild stream (2026-08-30).
func rewriteSelfGuildSay(line string) string {
	m := guildSelfLineRE.FindStringSubmatch(line)
	if m == nil {
		return line
	}
	...
	return m[1] + name + " tells the guild, " + line[len(m[0]):]
}
