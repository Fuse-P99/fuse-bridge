...
// CharUIINI returns the parsed UI_<Char>_<Server>.ini for a character. The
// server token matches whichever world is being tailed (Blue's is project1999),
// falling back to Blue when no log is attached yet.
func CharUIINI(char string) iniFile {
	...
	return ReadINIFile(eqRootFilePath(GetSettings().EQDirectory, "UI_"+char+"_"+tok+".ini"))
}
