...
func resolveClassFor(charName string) {
	...
	go func() {
		...
		if ci, ok := cachedCharInfos([]string{charName})[key]; ok && ci.Class != "" {
			...
			// ...and their class epic can finally be matched and auto-assigned.
			questTrackClassResolved()
		}
	}()
}

// ── officer / admin status cache (gates Fuse editing + admin UI) ─────────────
...
func isOfficerCached() bool {
	// View-as personas are never officers. The cached flag keeps the real
	// value underneath, so exiting the preview restores officer features
	// without waiting for a re-fetch (viewas.go).
	if ViewAsActive() != "" {
		return false
	}
	officerMu.RLock()
...
// isAdminCachedRaw reports the last-known admin status (the server's
// Officer + Bot Manager role pair, /isadmin), UNMASKED by View-as.
// IsAdminMode() layers the persona mask on top; the trigger engine and the
// View-as control itself need the real value (viewas.go — otherwise entering
// a preview would hide the way back out).
func isAdminCachedRaw() bool {
	officerMu.RLock()
	defer officerMu.RUnlock()
	return adminFlag
}

// refreshAdminStatus fetches the server's admin verdict, updates the cache,
// and returns it. IsAdminMode and the View-as entry gate call this live so
// admin UI decisions are always server-fresh; hot paths (the trigger engine,
// GetViewAs) read the cache via isAdminCachedRaw instead.
func refreshAdminStatus() bool {
	adm := IsLinked() && fetchIsAdmin()
	officerMu.Lock()
	changed := adminFlag != adm
	adminFlag = adm
	officerMu.Unlock()
	// Admin bypasses the triggersOfficerOnly gate like officer does, so a
	// status flip re-evaluates the engine's active set too.
	if changed {
...
func refreshOfficerStatus() {
	// Admin rides along in parallel so callers that await this (the Timers
	// window open) pay one round-trip's latency, not two.
	admDone := make(chan struct{})
	go func() { refreshAdminStatus(); close(admDone) }()
	v := false
	if IsLinked() {
		v = fetchIsOfficer()
	}
	officerMu.Lock()
	changed := officerFlag != v
	officerFlag = v
	officerMu.Unlock()
	// Officer status gates the whole engine while triggersOfficerOnly is set:
	// a newly confirmed officer gets their triggers compiled (the startup
	// rebuild ran before this status arrived), and a demotion silences them.
	if changed {
		RebuildTriggerActivation()
	}
	<-admDone
