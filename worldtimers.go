...
// RecordBoatLine watches for a boat dock announcement and reports the
// sighting to the server. Call it for every raw log line.
func RecordBoatLine(line string) {
	...
	for _, a := range boatAnnouncements {
		if strings.HasPrefix(msg, a.prefix) {
			go postBoatSighting(a.boat, a.zone)
			return
		}
	}
}

func postBoatSighting(boat, zone string) {
	if !GetSettings().WorldTimers || !IsLinked() {
		return
	}
	base := strings.TrimSuffix(serverURL, "/submit")
	body, _ := json.Marshal(map[string]string{"boat": boat, "zone": zone})
