...
func postBoatSighting(boat, zone, event string) {
	if !GetSettings().WorldTimers || !serverForwardOK() {
		return
	}
...
func fetchWorldTimers() WorldTimersData {
	...
	// Board reads are linked-members-anywhere; only postBoatSighting (a
	// gameplay-derived WRITE) stays home-world-gated.
