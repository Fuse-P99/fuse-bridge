...
var boatManualReports = []boatManualReport{
	{
		...
		hint: "No arrival shout on this route — as it docks, type  /say Boat arrived at OT  or  /say Boat arrived at TD  to update this timer for everyone. Whichever end you report is pinned exactly.",
	},
	// Reporting the Timorous end is what makes it predictable at all: the
	// schedule carried no data for that side, so it read "—" no matter how
	// often the Overthere end was reported.
...
func fetchWorldTimers() WorldTimersData {
	...
	out.Enabled = true
	// A boat can have several report phrases (one per end) but one hint, so take
	// the first entry that carries text rather than the first entry that matches.
	for i := range out.Boats {
		for _, m := range boatManualReports {
			if out.Boats[i].Key == m.boat && m.hint != "" {
