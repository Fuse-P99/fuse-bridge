...
func RecordRaidDPSLine(line string) {
	...
	if m := rdpsNonMeleeRE.FindStringSubmatch(content); m != nil {
		// Anonymous — the line names no source, so it is ours BY POSITION:
		// our damage shield or our proc, landing on something we are fighting.
		// The identical wording also carries damage TAKEN, and that is the
		// whole difficulty. A damage-shielded mob burns everything that swings
		// at it, so the log fills with "<our pet> was hit by non-melee for N"
		// — the mob's shield hurting US. Reading those as damage dealt is how
		// a group meter reaches four figures: with several pets on a shielded
		// mob, every pet swing pays us back a tick we never dealt.
		//
		...
		// And never OPEN a board from this line. A mob we are fighting already
		// has one (something has swung at it); a pet we have not identified
		// yet is only ever a damage TARGET, so its ticks land nowhere instead
		// of inventing a fight named after it. The cost is the opening tick of
		// a damage-shield pull, which the first swing re-opens a moment later.
		f := rdpsFights[normThreatMob(m[1])]
		if f == nil {
			return
		}
