...
// one lets us freeze the preserved timers and hide the overlays right away.
		...
		case <-pollTick.C:
			...
			for _, line := range lines {
				if isDbgQuitLine(line) {
					// Freeze Buffs (Self)/Disciplines now instead of letting them
					// burn until the 5-minute idle fallback (which still covers
					// crashes, where EQ writes no quit marker).
					QuitTriggerTimers()
