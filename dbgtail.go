	...
	for {
		...
		case <-pollTick.C:
			...
			for _, line := range lines {
				if isDbgQuitLine(line) {
					// Any quit or exit counts, whichever client wrote it: dbg.txt
					// is shared, and a boxer's /q is indistinguishable from the
					// tailed character's. Freeze every category configured to
					// auto-pause now, instead of letting those bars burn until the
					// 5-minute idle fallback (which still covers crashes, where EQ
					// writes no quit marker at all).
					QuitTriggerTimers()
					addStatus("Detected quit/exit — pausing timers and hiding overlays.")
					setPopoutsAutoHidden(true)
					// A quit is a camp for quest-waypoint purposes.
					go questMarkersCamp()
				}
			}
		}
