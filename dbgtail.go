...
// dbgCrashRE matches EQ's crash marker:
//
//	2026-08-08 14:38:41	Crash (char = Judass, zone = overthere)
//
// Unlike the quit/exit lines, a crash NAMES the character — which is what
// lets it be scoped: dbg.txt is shared across boxed clients, and a boxed alt
// going down must not freeze timers or hide overlays for the tailed character
// still fighting. Only the tailed character's crash acts; anyone else's is
// ignored (their timers aren't running in this app anyway).
var dbgCrashRE = regexp.MustCompile(`Crash \(char = ([^,)]+)`)
		...
		case <-pollTick.C:
			...
			for _, line := range lines {
				...
				case isDbgQuitLine(line):
					QuitTriggerTimers()
					// Unattributed marker (dbg.txt is shared): arms the
					// session-lock release with the longer shared fuse.
					noteTailLeftWorld(false)
				...
				case dbgCrashRE.MatchString(line):
					// A crash is leaving the world without a countdown: freeze
					// the buff/discipline timers now instead of letting them
					// burn until the idle fallback notices the silence.
					// currentCharName is written on the tailer goroutine, so
					// this cross-goroutine read goes through the atomic mirror.
					name := strings.TrimSpace(dbgCrashRE.FindStringSubmatch(line)[1])
					cur, _ := currentCharNameShared.Load().(string)
					if !strings.EqualFold(name, strings.TrimSpace(cur)) {
						continue // a boxed client's crash, not the tailed character
					}
					QuitTriggerTimers()
					// Names the tailed character — the scoped (fast) fuse.
					noteTailLeftWorld(true)
					addStatus("Detected crash (%s) — pausing timers and hiding overlays.", name)
				default:
					continue
				}
				...
				go questMarkersCamp()
				// ...and takes the character off the shared map: a camped player
				// is not standing where their last /loc says they are. Shares the
				// caveat above — a boxed client's exit clears the tailed
				// character's marker too, which their next /loc puts back.
				// (Atomic mirror: same cross-goroutine read as the crash case.)
				if cur, _ := currentCharNameShared.Load().(string); cur != "" {
					SendMapLocClear(cur, "left the world")
				}
			}
		}
	}
