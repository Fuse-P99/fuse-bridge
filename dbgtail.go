...
// line, so the trigger engine only notices via the 5-minute idle timeout. These
// debug lines are a definitive, immediate "leaving the world" signal — seeing
// one lets us hide the timer overlays right away.
		...
		case <-pollTick.C:
			...
			for _, line := range lines {
				if isDbgQuitLine(line) {
					addStatus("Detected quit/exit — hiding timer overlays.")
