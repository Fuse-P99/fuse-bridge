...
var (
	...
	qdInputIdx   map[string]bool        // normalizeItemName(step input, alts included)
	...
	for qi := range qdQuests {
		...
		for si := range q.Steps {
			...
			for ii := range s.Items {
				...
				// Inputs (with their alternatives) feed hand-in feasibility:
				// the inventory scan retains them so the nudge can tell a
				// doable hand-in from one whose components aren't in the bags.
				if k := normalizeItemName(s.Items[ii].Name); k != "" {
					qdInputIdx[k] = true
				}
				for _, alt := range s.Items[ii].Alts {
					if k := normalizeItemName(alt); k != "" {
						qdInputIdx[k] = true
					}
				}
			}
...
// qdIsInputItem reports whether any quest step consumes an item (key already
// normalized) — the retention filter for hand-in feasibility.
func qdIsInputItem(key string) bool {
	qdMu.Lock()
	defer qdMu.Unlock()
	return qdInputIdx[key]
}
...
// vpPieceIDsForRune returns the three piece ids that assemble a rune
// medallion, or nil when the name isn't one of the nine-piece runes. This is
// what lets a medallion hand-in be judged by piece IDENTITY: the pieces share
// one item name across nine drops, so holding "3 pieces" proves nothing
// unless they are this rune's three.
func vpPieceIDsForRune(name string) []int {
	...
	for i := range vpMedallions {
		if normalizeItemName(vpMedallions[i].Rune) == key {
			ids = append(ids, vpMedallions[i].ID)
		}
	}
	return ids
}
