...
// Marks where the card should draw a divider. A unique object rather than a
// magic string, so it is compared by identity and can never collide with a
// real line — item names and effects are arbitrary text.
export const TIP_RULE = { rule: true };
...
export function tipStats(it) {
  ...
  // Everything below is money, not stats — two different kinds of fact about
  // the same item, so they get a divider rather than running together.
  const statLines = lines.length;
  const hasMoney =
    it.dkp_count ||
    (it.quest_routes && it.quest_priced && (it.quest_items || []).some((c) => c.value > 0));
  if (statLines && hasMoney) lines.push(TIP_RULE);
