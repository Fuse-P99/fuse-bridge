...
// The quest prefix for map-marker labels: epics carry the weapon list in
// their catalog name ("Ranger Epic: Swiftwind and Earthcaller"), which buries
// what the marker points at — "Ranger Epic" says it all. Mirrors Go's
// questShortName (questnudge.go); the two must agree so manually-dropped and
// nudge-dropped markers read alike.
export function questShortName(q) {
  if (q && q.class && /\bepic\b/i.test(q.name || "")) return `${q.class} Epic`;
  return (q && q.name) || "";
}
