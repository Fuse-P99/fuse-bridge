<script>
  ...
  function toSeed(q) {
    return {
      ...
      direct_rewards: q.direct_rewards || undefined,
  ...
  async function check() {
    // A RE-check (after stub creation) keeps the report on screen with a
    // progress line, instead of flipping back to the file picker — that made
    // it look like the import was lost and the file had to be re-selected.
    phase = report ? "rechecking" : "checking";
    err = "";
    log = [];
    const r = {
      quests: seed.length,
    ...
    try {
      ...
      err = String(e);
      // A failed re-check keeps the previous report rather than dumping the
      // whole session back to the picker.
      phase = report ? "checked" : "pick";
    }
    progress = "";
  }
