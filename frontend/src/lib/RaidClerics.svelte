<script>
  ...
  function splitBlocks(rows) {
    const votes = rows.map((r) =>
      r.s.dead || r.s.stale ? "" : tankVote(r.tank),
    );
  ...
  function blockFor(rows, blocks, meName) {
    ...
    const mineIn = blocks.find((b) =>
      b.some(
        (r) =>
          !r.s.dead &&
          !r.s.stale &&
          (r.s.cleric || "").toLowerCase() === meName,
      ),
    );
    return mineIn || rows;
  }
  ...
  function ownCasting(rows, meName) {
    const r = rows.find(
      (x) =>
        !x.s.dead && !x.s.stale && (x.s.cleric || "").toLowerCase() === meName,
    );
    return !!(r && r.t);
  }
  $: pulseAnchor =
    (mainNextAt && !mainSkipped && !ownCasting(myMainRows, me) && mainNextAt) ||
    (rampNextAt && !rampSkipped && !ownCasting(rampRows, me) && rampNextAt) ||
    0;
  let beepAnchor = 0;
