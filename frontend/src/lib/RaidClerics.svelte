<script>
  ...
  // Overlay "Count Audio": pulse a sound in time with the pre-cast ring —
  // once at the predecessor's cast start, then each second until the user's
  // own cast starts. The raid card (tab) never passes it, so the tab is
  // always silent.
  ...
  // and wins outright). The timing pass MUST use this merged clock: raw
  // server times carry relay latency and batch skew, and pairing on them
  // produced visibly wrong gaps.
  ...
  function chTankOf(timers, slot) {
    ...
    if (loc && (loc.tank || "").trim()) {
      ...
      if (!at || Math.abs(loc.at_ms - at) < 8000 || loc.at_ms > at) {
  ...
  // ── audible count pulse ──────────────────────────────────────────────────
  // Mirrors the visual ring's live condition exactly (predecessor mid-cast,
  // not skipped, own cast not started): one cue at the predecessor's cast
  // start, then one per whole second, ending with the ring. Anchor changes
  // (a re-call, the next rotation) reset the count; mounting mid-window
  // picks up at the current second rather than replaying missed ones.
  ...
  $: if (pulseAudio && pulseAnchor) {
    ...
    // nudge is the SAME cast: keep the running count, never re-fire off it.
    // Only a genuinely new cast (the next rotation, seconds away) resets.
    if (Math.abs(pulseAnchor - beepAnchor) > 2000) {
      beepAnchor = pulseAnchor;
      beepSec = -1;
    } else if (pulseAnchor !== beepAnchor) {
      beepAnchor = pulseAnchor; // track the correction, keep the count
    }
    // One cue at the predecessor's cast start (sec 0), then one per whole
    // second — the audible twin of the ring's 1s pulse. Nothing fires on the
    // cleric's own cast: the anchor gates off the moment they begin (and the
    // CH window itself caps the count at 10s regardless).
    const sec = Math.floor((now - beepAnchor) / 1000);
    if (sec >= 0 && sec > beepSec && sec <= 9) {
      beepSec = sec;
