<script>
  ...
  function updateDeltas(rows) {
    for (let i = 0; i < rows.length; i++) {
      ...
      // this chain that still precedes mine, whoever's slot that is. The
      // positional walk (slot before mine, wrapping) broke on real chains:
      // 111's slot-order predecessor is the TAIL, and chains rarely run that
      // deep — the caller restarts at 111 early — so 111 showed nothing
      // until the tail happened to cast late in the fight. This pairing also
      // survives skipped slots and out-of-order casts.
      let best = 0;
      for (let j = 0; j < rows.length; j++) {
        if (j === i) continue;
        const p = rows[j].s;
        if (p.dead || p.stale) continue;
        const pat = p.called_at_ms || 0;
        if (pat > 0 && pat < at && pat > best) best = pat;
      }
