<script>
  ...
  // under the reader when it lands. So the identity bracket is NOT between the
  // time and the toon any more: it has a reserved column at the right-hand end
  // of the row, held open from the first frame, and slides in from the right
  // once the server answers for that line.
  //
  ...
  // Is there a bracket to draw for this row yet? A pending row is one the
  // server hasn't answered for, so nothing is known about its speaker unless
  // they have already spoken this session (Go labels those from its identity
  // cache) — draw no bracket at all rather than an invented one, and let the
  // real one slide into the reserved column when the answer lands. A row the
  // server HAS answered for always gets one, "[unrostered]" included: that is
  // the server's actual verdict on the speaker, and it has always shown here.
  function hasIdent(e) {
    return !e.pending || !!(e.display || e.handle || e.officer || e.bot);
  }
    ...
    <div class="gc-lines" class:bottom={!newestTop}>
      {#each rows as e (e.id)}
        ...
        >
          ...
          <!-- The identity column. Always present, even with nothing in it, so
               the body's width is settled from the row's first frame and the
               message never shifts when the server's answer fills this in —
               it slides in from the right instead. Intro only, never an outro:
               a list rebuilt on every change strands outro-ing elements. -->
          <span class="gc-who"
            >{#if hasIdent(e)}<!-- svelte-ignore a11y-no-static-element-interactions --><span
                class="gc-who-in"
                in:fly|local={{ x: 24, duration: 220 }}
                on:mouseenter={(ev) => showMember(ev, e)}
                on:mouseleave={hideCard}
                >[{#if e.officer}<Icon name="badge" title="Officer" />{" "}{:else if e.bot}<Icon name="robot" title="Guild bot" />{" "}{/if}{ident(e)}]</span
              >{/if}</span
          >
        </div>
      {/each}
...
<style>
  ...
  /* Two columns: the line itself, and the identity's reserved space at the
     right-hand end. align-items: flex-start keeps the bracket on the FIRST
     line of a message that wraps to several. */
  .gc-row {
    display: flex;
  ...
  /* The line itself: inline spans, so a long message wraps under itself rather
     than squeezing the time and name columns. min-width: 0 lets it actually
     shrink — a flex item's default min-width is its content, which would push
     the identity column off the row. */
  .gc-body {
    flex: 1 1 auto;
    min-width: 0;
    white-space: normal;
  ...
  /* The identity column, held open whether or not there is a name in it yet.
     Its width is in em so it tracks the Text size setting: 9.5em is the
     10-character identity plus a mark and the two brackets at the row's own
     font size. Nothing wraps and nothing overflows — an identity longer than
     the column is clipped rather than allowed to steal the message's width. */
  .gc-who {
    flex: 0 0 9.5em;
    text-align: right;
    white-space: nowrap;
