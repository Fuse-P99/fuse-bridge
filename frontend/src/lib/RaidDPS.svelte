<script>
  ...
  // The fight the server is currently naming (the mob absorbing the majority
  // of the raid's DPS), pushed up so the overlay can show it in its title bar.
  ...
  <div class="rc-col">
    ...
    <div class="rd">
      ...
      <div class="rd-third rd-gauge">
        ...
        {#if data.mob_dps > 0}
          <div
            class="rd-total rd-mob"
            title="The mob's own melee output over the fight, all victims combined"
          >
            Mob DPS - <span class="rd-big rd-mob-big">{num(data.mob_dps)}</span>
          </div>
        {/if}
      </div>
      ...
      <div class="rd-third">
        ...
        {#each data.top as p (p.name)}
          <div class="rd-row" class:dead={p.dead}>
            <span class="rd-name">
              {#if p.dead}<span class="rd-tomb" title="Dead"
                  ><Icon name="headstone" title="Dead" /></span
                >{/if}{p.name}
            </span>
...
<style>
  ...
  /* The mob's line sits under the raid's and must read as the adversary's
     number, not a second raid stat. */
  .rd-mob {
    color: #a89090;
  }
  .rd-mob-big {
