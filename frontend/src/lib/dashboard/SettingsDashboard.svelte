<script>
  ...
  const OVERLAY_TIPS = {
    ...
    engages:
      "Include raid-mob engage lines in the Guild Chat panel on the Raids tab.",
    newesttop:
      "Guild Chat: newest message at the TOP of the panel instead of the bottom.",
    msgbold: "Guild Chat panel: message text in bold.",
    msgfont_set:
      "Guild Chat panel: a custom font is set for the chat text (which font is never reported).",
    msgcolor_set:
      "Guild Chat panel: a custom color is set for message text, replacing the default white (which color is never reported).",
    msgsize_set:
      "Guild Chat panel: a custom size is set for the chat lines, instead of the default 12.5px.",
  ...
  $: ovGroups = (ov.settings || []).map((g) => ({
    ...
    rows: (g.items || []).map((it) => ({
      ...
      // No "overlay" in the wording: one of these groups is the Guild Chat
      // panel on the Raids tab, which is not an overlay at all.
      note: `Of the ${g.n} installs that have configured ${g.label}.`,
    })),
  }));
  $: cats = (ov.categories && {
...
<style>
  ...
  /* Chart hovers use the app tooltip (ApexChart.svelte → lib/tooltip.js), so
     there is no chart-side tooltip styling to do. */
