<script>
  ...
  import { SURFACE, TEXT, TEXT_PRIMARY, tipText, fmtPct } from "./chartTheme.js";
  ...
  // A pie reports the slice as seriesIndex.
  function tip(si) {
    const s = slices[si];
    return s
      ? tipText(s.label, [
...
{#if sum > 0}
  <ApexChart type="donut" {series} {options} {height} {tip} />
{:else}
