<script>
  ...
  // Overlay "Smart Hide": show nothing at all while THIS user is not in the
  // raid voice channel — the speakers list is noise for someone not on comms.
  export let smartHide = false;

  let voice = { speakers: [], in_channel: 0, you_in_channel: false };
  let off;
  ...
  function norm(v) {
    return {
      ...
      you_in_channel: !!(v && v.you_in_channel),
    };
  }
  $: hidden = smartHide && !voice.you_in_channel;
  $: hasContent = !hidden && voice.speakers.length > 0;
...
{#if !hidden}
  <div class="vs">
    <div class="vs-head">
      <span class="vs-ico"><Icon name="volume" /></span>
