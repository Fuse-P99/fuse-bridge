<script>
  ...
  // Always list these debuffs, in this order; attribute the most recent
  // caster + a check when seen. Names match the server's debuff keys exactly
  // (substring matching would let ESlow light the Slow row and vice versa).
  ...
  function debuffCaster(key) {
    ...
    for (const d of card.debuffs) {
      if (d.name && d.name.toLowerCase() === k) return d.value
    }
    return ''
  }
</script>
