  ...
  function clusterOthers(list) {
    ...
    for (const g of list || []) {
      ...
      for (const c of g.classes || []) {
        e.classes.set(c.class, (e.classes.get(c.class) || 0) + (c.count || 0));
      }
    }
    return [...byName.values()]
      .map((e) => {
        ...
        // /roleplay hides class and level but not the guild tag, so those
        // characters are in the guild's count with no class to file under.
        // Shown as their own row (the Zones tab's ROLE) rather than opening
        // to nothing — a guild of one roleplayer did exactly that.
        const known = classes.reduce((s, c) => s + c.count, 0);
        if (e.total > known) {
          classes.push({ class: "ROLE", count: e.total - known });
        }
        return {
          name: e.name,
          alliance: e.alliance,
