<script>
  ...
  $: castDebuffs = DEBUFF_ORDER
    .map(name => {
      const d = (card.debuffs || []).find(x => x.name && x.name.toLowerCase() === name.toLowerCase())
    ...
    <div class="rc-col">
      ...
      {#each castDebuffs as { name, caster }}
        <div class="rc-line">
          ...
          <span class="rc-check">✓</span> <span class="rc-caster">{caster}</span>
        </div>
      {:else}
