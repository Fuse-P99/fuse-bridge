<script>
  ...
  // Bar height is DRIVEN by the configured font size — the text plus a snug
  // ~2-3px of track above and below — never by the window: a bar in a tall
  // window must not inflate to fill it. (Bars used to be flex-grow with a
  // max-height cap; that stretched in some layouts, which is how a fresh
  // category's bars could render huge while configured ones looked right.)
  $: barHeight = Math.max(16, (style?.font_size || 12) + 0);
...
<style>
  /* Each bar sits at its font-derived height (inline). Bars never GROW to
     fill a tall window; they still SHRINK together when the window is too
     short, so every running timer stays visible. */
  .ptimers {
    flex: 1;
    min-height: 0;
    display: flex;
