<script>
  ...
  // alive; the soonest-ending are the stale dregs — plus one line saying how
  // many are hidden. This caps pixels, not data: the engine keeps every
  // timer, hidden bars still fire their ending sounds, and shrinking the
  // window never discards anything — widen it and they're back.
  let listH = 0;
  const BAR_GAP = 3; // matches .ptimers gap
  const MORE_H = 16; // the hidden-count line, incl. its share of gap
  ...
  $: shownN =
    active.length > fitN
      ? Math.max(
          1,
          Math.floor((listH - PAD_V - MORE_H + BAR_GAP) / (barHeight + BAR_GAP)),
        )
      : active.length;
...
<div class="ptimers" bind:clientHeight={listH}>
  ...
  {:else}
    {#if hiddenN > 0}
      <!-- The hidden bars are the soonest-ending, which sort ABOVE the shown
           ones — so the count sits at the top, where they would have been. -->
      <div class="tmore">{hiddenN} more hidden…</div>
    {/if}
      ...
      >
        <div
          ...
          style="width:{barFrac(t) *
            100}%; background:{color}; opacity:{fillOpacity}"
        ></div>
