<script>
  ...
  // Group layout: six rows. The total moved up beside the headline number.
  ...
  // Right-hand total: the whole fight, not just the rows shown. Named for
...
<div class="th">
  {#if data.engaged}
    ...
    <!-- Headline row: YOUR number on the left, the whole fight's on the right.
         The two used to be stacked with the breakdown chart between them,
         which read as though the chart described the group — it never has.
         Everything in the left column is the viewer's own damage, the right
         column is everyone's, and the gap between them says so.

         A pure debuffer generates hate without ever dealing damage; showing
         them a permanent 0.0 DPS is noise, so the left block only appears
         once they've actually done some. -->
    {#if data.own_damage > 0 || totalDps > 0}
      <div class="th-dps-row">
        <div class="th-dps-me">
          {#if data.own_damage > 0}
            ...
            <!-- Optional damage-composition bar (overlay settings): the
                 viewer's OWN outgoing damage by type, biggest slice first
                 (server-sorted). Sits under the left number because that is
                 whose damage it splits. -->
                  ...
                  {#each bd as c (c.label)}
                    <div
                      ...
                      style="width:{(c.dmg / bdTotal) *
                        100}%;background:{CAT_COLORS[c.label] ||
                        CAT_COLORS.Other}"
                ...
                <div class="th-bd-legend">
                  {#each bd as c (c.label)}
                    <span class="th-bd-chip"
                      ><span
                        class="th-bd-dot"
                        style="background:{CAT_COLORS[c.label] ||
                          CAT_COLORS.Other}"
                      ></span>{c.label}
        ...
        {#if totalDps > 0}
          <div class="th-dps-all" title="Every attacker on this fight">
            <span class="th-dps-num alt">{fmtDps(totalDps)}</span>
            <span class="th-dps-lbl">{totalLabel}</span>
          </div>
        {/if}
      </div>
    {/if}
    ...
    {:else if groupRows.length}
      <div class="th-top">
        ...
        {/each}
        <!-- The group total used to sit here as a final row. It now leads the
             panel beside the viewer's own number, where the comparison it
             exists for actually reads. -->
      </div>
    {/if}
