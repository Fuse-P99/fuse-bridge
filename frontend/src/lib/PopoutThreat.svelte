<script>
  // DPS & Threat special overlay. Four stacked sections:
  ...
  // reads, computed and ranked identically: ordered by damage over the whole
  // fight, showing that same SDPS figure. In a raid the two panels agree row
  // for row.
  //
  // GetFightDPS rather than GetRaidDPS is the one difference, and it's about
  // WHICH fight, never about the maths: it also answers outside a raid, falling
  // back to the mob we've personally damaged, because this overlay is the one
  // worth having in a group. Cached client-side, so the 1s poll here costs one
