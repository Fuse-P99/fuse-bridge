<script>
  ...
  // Group layout: six rows. The fight total closes the list as its own row.
  $: groupRows = top.slice(0, 6);

  // The viewer's own row on the board, which is where the PET lives: the
  // parse folds an owned pet's damage onto its owner and suffixes the label,
  // so this row is "me + pet" whenever there is a pet. Preferred over
  // data.dps for the headline, because data.dps is the first-person ledger
  // and a pet's swings were never in it.
  $: ownBoardRow = ownIdx >= 0 ? top[ownIdx] : null;
  $: myDps = ownBoardRow ? ownBoardRow.sdps : data.dps;
  // "You + Pet" reads better than the character's own name, but the " + Pet"
  // half has to survive — it is the difference between a number that looks
  // wrong and one that explains itself.
  $: myLabel =
    ownBoardRow && ownBoardRow.name.endsWith(" + Pet") ? "You + Pet" : "You";
  // The fight total: the whole fight, not just the rows shown. Named for
  // what the board actually covers in each mode.
  $: totalLabel = data.raid_mode ? "Raid Total" : "Group Total";
  $: totalDps = board.raid_dps || 0;
...
<div class="th">
  {#if data.engaged}
    ...
    <!-- Headline: the viewer's OWN number, with their damage-composition bar
         under it — both spanning the overlay's full width. The fight total
         used to sit to the right of this block, but the two columns squeezed
         each other: labels collided and the breakdown bar couldn't span the
         overlay. The total now closes the damage list below instead, where
         it reads as the sum of the rows above it.
    ...
    {#if data.own_damage > 0}
      <div class="th-dps">
        <span class="th-dps-num">{fmtDps(myDps)}</span>
        <span class="th-dps-lbl">{myLabel}</span>
      </div>

      <!-- Optional damage-composition bar (overlay settings): the viewer's
           OWN outgoing damage by type, biggest slice first (server-sorted).
           Sits under the headline number because that is whose damage it
           splits. -->
        ...
        <div class="th-bd">
          <div class="th-bd-bar">
            {#each bd as c (c.label)}
              <div
                class="th-bd-seg"
                style="width:{(c.dmg / bdTotal) *
                  100}%;background:{CAT_COLORS[c.label] || CAT_COLORS.Other}"
    ...
    {#if data.raid_mode}
      {#if raidTop.length || totalDps > 0}
        <div class="th-top">
          ...
          {/if}
          {#if totalDps > 0}
            <div class="th-top-row total" title="Every attacker on this fight">
              <span class="th-top-name">{totalLabel}</span>
              <span class="th-top-dps">{fmtDps(totalDps)}</span>
            </div>
          {/if}
        </div>
      {/if}
    {:else if groupRows.length || totalDps > 0}
      <div class="th-top">
        ...
        {/each}
        <!-- The fight total, closing the list it sums. (It once led the panel
             beside the viewer's own number, but that side-by-side squeezed
             the breakdown chart out of its width.) -->
        {#if totalDps > 0}
          <div class="th-top-row total" title="Every attacker on this fight">
            <span class="th-top-name">{totalLabel}</span>
            <span class="th-top-dps">{fmtDps(totalDps)}</span>
    ...
    {#if data.raid_mode}
      ...
      <!-- The per-hand swing model (TPS / est. DMG / est. delay / haste) used to
         render here while it was being calibrated. It was always a developer
         readout, not something a raider acts on mid-fight, so it's gone from
         the overlay. The model itself still drives the threat estimate, and
         Admin Settings → Copy Threat Debug still dumps every one of those
         numbers when a reading needs diagnosing. -->
      ...
      {#if data.raid_active}
        <div class="th-gauge">
          ...
          {#if data.have_ref}
            ...
            <div
              class="th-ref"
              title={data.tank_source === "estimated"
                ? data.ref_name +
                  " isn't running FuseBridge — threat is an estimate."
                : data.ref_name}
            >
              {#if data.is_tank}
                vs closest rival: {data.ref_name}
              {:else}
                vs {data.ref_name}{data.tank_source === "maintank"
                  ? " (MT)"
                  : data.tank_source === "estimated"
                    ? " (MT, estimated)"
                    : ""}
              {/if}
            </div>
          {:else if data.tank_name}
            <div class="th-ref muted">
              {data.tank_name} (MT) isn't running FuseBridge — no threat to compare
              against
            </div>
          {:else}
            <div class="th-ref muted">
...
<style>
  ...
  /* The pinned own row (raid mode) sits under a thin rule so it reads as a
     footer to the list, not a sixth competitor. The group-total row that
     shared this styling now lives in the headline row instead. */
  .th-top-row.gap {
    margin-top: 3px;
    padding-top: 3px;
    border-top: 1px solid rgba(255, 255, 255, 0.14);
  }
  /* The fight total, ruled off under the rows it sums. */
  .th-top-row.total {
    margin-top: 3px;
    padding-top: 3px;
    border-top: 1px solid rgba(255, 255, 255, 0.14);
  ...
  /* Headline row: own damage left (with its breakdown beneath), whole-fight
     total right. baseline alignment keeps the two big numerals on one optical
     line even when the left column grows a chart under it. */
  /* .th-haste is the one survivor of the old swing-model block — the spell
     hate line still uses it. */
  .th-haste {
    font-size: 11px;
    color: var(--text-muted);
  }
