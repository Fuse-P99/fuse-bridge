<script>
  ...
  const FOLDER_FILTERS = {
    ...
    // No Dragon Roar trigger exists for Wuoshi in the package (his folder has
    // only Ceticious Cloud), so this local filter still matches nothing — his
    // 24s bar comes entirely from the shared anchor in raidAE.go. Left in
    // place so a package trigger, if one is ever added, is picked up.
    wuoshi: [
      { chain: ["raid spells", "wuoshi"], name: /^dragon (roar|fear) cooldown$/i },
    ],
    zlandicar: [
      { chain: ["raid spells", "zlandicar"], name: /^stun breath( cooldown)?$/i },
      { chain: ["raid spells", "zlandicar"], name: /^putrefy flesh( cooldown)?$/i },
    ],
    aaryonar: [
      {
    ...
    // local bar; the shared anchor (raidAE.go) supplies the 12s countdown.
    "lady nevederia": [
    ...
    jorlleag: [
      { chain: ["raid spells", "jorlleag"], name: /^frost breath cooldown$/i },
    ],
    // Gozzrem's Frost Breath is the shared Jorlleag trigger ("See Jorlleag"),
    // so his card admits the same timer from Jorlleag's folder.
    gozzrem: [
      { chain: ["raid spells", "jorlleag"], name: /^frost breath cooldown$/i },
    ],
    "lord koi`doken": [
      { chain: ["raid spells", "koi"], name: /^tsunami cooldown$/i },
    ],
    // Mob DB spells her "Mirenilla"; the package folder says "Mirenella" —
    // the KEY must match the raid card (DB), the CHAIN the trigger's folder.
    "lady mirenilla": [
      { chain: ["raid spells", "mirenella"], name: /^cloud of fear cooldown$/i },
    ],
    "lord kreizenn": [
      { chain: ["raid spells", "kreizenn"], name: /^wave of flame cooldown$/i },
    ],
    // Sevalak's Chaos Breath is Lord Vyemm's shared Scream of Chaos trigger.
    sevalak: [
      { chain: ["raid spells", "lord vyemm"], name: /^scream of chaos cooldown$/i },
    ],
  ...
  let liveZone = ""; // popout mode: the resolved raid's home zone
  ...
  // Shared anchors (raidAE.go) come from the server: one client who saw the
  // line anchors the bar for the whole raid. Card mode used to skip the fetch
  // unless the target was in a hardcoded key list, which went stale the moment
  // anchors were added for more mobs — those cards silently showed no shared
  // bar for months. The list is gone: whether an anchor exists is the server's
  // question to answer, and it answers it by returning one (or not).
  ...
  // A complete card means the event's boss is dead — the raid ends ON the
  // kill server-side, so the review/completed card is the only complete one
  // an event key ever rides. Flips the Narandi row from "UP" to a tombstone.
  $: bossDead = !!card && card.status === "complete";
  ...
  $: hasAny =
    inRaidZone &&
    (active.length > 0 || derived.length > 0 || aeRows.length > 0);
  ...
  // reports — and raw lowercase comparison has bitten before (timers.go once
  // had "Great Divide" never matching "great divide"). Compare on a stripped
  // key: no leading article, no punctuation, no case.
  const zoneKey = (z) =>
    (z || "")
      .toLowerCase()
      .replace(/^the\s+/, "")
  ...
  async function pollContext() {
    try {
      ...
      // FIRST because the context pick below is zone-aware.
    ...
    try {
      ...
      if (card) return;
      // Base precedence mirrors the raid overlays (PopoutRaidSection): a live
      // mob raid first — an interrupt's AE timers belong to that fight — then
      // the event raid. ZONE-AWARE override, same as there: the card for the
      // zone the viewer is STANDING IN wins, because the fixed order had a
      // blind spot — a mob raid live anywhere pointed this overlay at that
      // fight, and the in-zone gate then hid the Ring War waves from the
      // people at the ring while the (ungated) Raids-tab card showed them
      // fine (2026-08-25).
      const sources = [];
      ...
      if (d && d.event_raid) sources.push(d.event_raid);
      const zn = zoneKey(myZone);
      const c =
        (zn && sources.find((x) => x.zone && zoneKey(x.zone) === zn)) ||
        sources[0] ||
        null;
  ...
  // As an overlay this only renders for players standing in the raid's zone —
  // the same rule as the other raid overlays (PopoutRaidSection): unknown raid
  // zone fails open, unknown player zone hides. Ring War's wave shouts travel
  // in guild chat, so without this everyone's local triggers drew the wave
  // bars wherever they were. Card mode is the Raids TAB, which informs from
  // anywhere and is deliberately ungated.
  $: inRaidZone =
    !!card || !liveZone || (!!myZone && zoneKey(myZone) === zoneKey(liveZone));
  ...
  onMount(async () => {
    ...
    // Card mode ticks too — that is where the shared AE anchors come from.
...
<div class="rc-col">
  ...
  <!-- The whole body sits behind the zone gate (not just hasAny): a sticky
       overlay renders even when "empty", and bars must not leak through it
       from another zone. -->
  {#if inRaidZone}
  ...
  {#each derived as d (d.key)}
    <div class="obar" class:sched={true} class:done={d.done} class:dead={d.dead}>
      <div
        class="obar-fill"
      ...
      <span class="obar-time"
        >{#if d.dead}<Icon name="headstone" title="Dead" /> DEAD{:else if d.done}UP{:else}{fmtRemain(
            d.at - now,
          )}{/if}</span
      >
    </div>
...
<style>
  ...
  /* Narandi dead (the card is complete): the gold "UP" urgency drains out to
     a grey tombstone state. Declared after .sched/.done so the same
     specificity resolves in the dead state's favor. */
  .obar.dead .obar-fill {
    background: rgba(125, 130, 140, 0.22);
  }
  .obar.dead .obar-name,
  .obar.dead .obar-time {
    color: #9aa0ab;
