<script>
  ...
  async function poll() {
    ...
    // Stamp the clock here too. animLoop drives it from animation frames,
    // which WebView2 stops delivering while the overlay window is hidden or
    // minimized — and with the clock frozen, nothing expires and a cleared
    // alert's faded-out row never leaves the list. The poll keeps ticking
    // through that, so expiry rides on it as well.
    now = Date.now();
    polling = false;
    if (pollAgain) {
      pollAgain = false;
...
<div class="palerts">
  {#each shown as a (a.id)}
    <!-- a.color: a per-trigger tint from the customization layer wins over
         ...
         Intro only, never an outro. An outro keeps the element in the page
         until its animation finishes on an animation frame, and this list is
         rebuilt every frame from a clock — an alert re-firing right at its
         expiry, or a hide/unhide landing mid-transition, could leave Svelte
         with an element it no longer tracked: invisible, full height, and
         pinned above every later alert until the overlay was reopened (the
         "new alerts start two blank lines down" report). Removal is now
         immediate; a cleared alert's flash-and-fade is CSS and unaffected. -->
    <div
      class="alert"
      ...
      in:fly|local={{ y: -6, duration: 160 }}
    >
      <span class="alert-txt">{a.text}</span
      >{#if stopwatch && !a.cleared_at_ms}<span class="sw"
...
<style>
  ...
  /* Color, size, family, and backdrop come from the category style, inline.
     Flex row so the stopwatch pins to the RIGHT edge while the text keeps its
     centered look in the remaining width. */
  .alert {
    display: flex;
    align-items: center;
    gap: 8px;
  ...
  /* The "Show age" readout, pinned right of the alert. Monospace with
     tabular digits so it ticks without jitter; near-full size and full
     opacity so it reads at a glance over the game. */
  .sw {
    flex: 0 0 auto;
    font-size: 0.9em;
    font-weight: 700;
