<script>
  ...
  // vanishes: three 0.5s pulses (see .alert.cleared).
  const CLEAR_FLASH_MS = 1500;
  ...
  // cleared ones only while their flash plays.
  $: shown = alerts
    .filter(
      (a) =>
        (a.category || "Default") === category &&
        (a.cleared_at_ms
          ? now - a.cleared_at_ms < CLEAR_FLASH_MS
          : now - a.at_ms < showMs),
    )
...
<style>
  ...
  /* An early-end condition answered this alert: three quick pulses, then the
     entry vanishes. Same treatment on every overlay — a clear is a clear,
     whatever the trigger was. */
  .alert.cleared {
    animation: clearflash 0.5s ease-in-out 3;
  }
