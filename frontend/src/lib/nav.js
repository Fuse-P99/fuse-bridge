...
// A request to land on one SECTION of the General tab — the Guild Chat panel's
// note points at Forwarded Messages when the toggles it depends on are off.
// Same shape and contract as raidDeepLink: `seq` bumps per request and the tab
// acts once per seq (scrolls the section into view). Set through
// openGeneralTab, never directly.
export const generalDeepLink = writable({ section: '', seq: 0 })

// openGeneralTab switches to the General tab, scrolled to `section` (empty
// simply opens the tab).
export function openGeneralTab({ section = '' } = {}) {
  generalDeepLink.update((d) => ({ section, seq: d.seq + 1 }))
