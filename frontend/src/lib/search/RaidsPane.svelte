...
<script>
  ...
  // ── loot item card ──────────────────────────────────────────────────────
  // The shared ItemTipCard on hover, same as the live raid cards. Items are
  // fetched once per name and cached; a name the item DB doesn't have says
  // so rather than showing a blank card.
  let itemCache = {};
  let tip = null; // { name, item, x, y }

  // "Held by" — the user's own characters holding the hovered loot (local
  ...
  function tipPos(e) {
    // Positioned inside the zoomed shell, so cursor coordinates divide by
    // the UI scale or the card drifts at Medium/Large.
    const z = $scale || 1;
    const pad = 14;
    return {
  ...
  async function showItemTip(e, name) {
    tip = { name, item: itemCache[name] || null, ...tipPos(e) };
                    ...
                    {#if r.loot.length}
                      ...
                      <ul class="lootlist">
                        {#each r.loot as l}
                          ...
                          <li
                            on:mouseenter={(e) => showItemTip(e, lp.name)}
