...
<script>
  ...
  // over Discord handles and display names (the same prefix query /who's
  // autocomplete uses); the lookup resolves through the same fallback chain
  // (handle → display name → toon owner), so a toon name typed in full works
  // here too.
    ...
    <div class="controls">
      <div class="acwrap">
        ...
        {#if sugOpen}
          <div class="sugs">
            {#each sugs as s (s)}
              <button class="sug" on:mousedown={() => pick(s)}>{s}</button>
            {/each}
          </div>
        {/if}
      </div>
