...
<script>
  ...
  // Credentials modal state (from Details or a successful Claim); rendered
  // by the shared BotCredsModal.
  let detail = null;
  let detailTitle = "";
  ...
  async function claim(b) {
    const ok = await confirmDialog({
      title: "Claim bot",
      message: `Claim ${b.name} for 5 minutes?`,
      detail:
        "This is announced in the guild's bots-data Discord channel under your name.",
      confirmLabel: "Claim",
    });
    if (!ok) return;
    inflight = b.name;
