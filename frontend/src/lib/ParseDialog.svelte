  ...
  <div class="modal">
    ...
    {:else}
      <div class="head">
        ...
        <!-- The pinned row below carries the RAID's numbers against the mob;
             the mob's own output is a different quantity, so it gets its own
             label here rather than a column that would read as its rank. -->
        <span class="meta">
          {num(mobRow.total)} damage · {mmss(mobRow.secs)} · {num(
            mobRow.dps,
          )} raid DPS{#if data.mob_dps > 0}
            · {num(data.mob_dps)} mob DPS{/if}
        </span>
      </div>
          ...
          <tbody>
            ...
            {#each rows as p (p.name)}
              <tr class:dead={p.dead}>
                ...
                <td class="c-name"
                  >{p.name}{#if p.dead}<span class="tomb" title="Died"
