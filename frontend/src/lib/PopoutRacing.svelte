...
<style>
  ...
  /* Line geometry, retuned 2026-08-30 from the previous spans (green 22–70%,
     red 27.6–66%): each line raised by 30% of its own height, then lengthened
     35% from the shifted top (growth extends downward). */
  /* Green: strafe-run direction. */
  .line.green {
    top: 7.6%;
    height: 64.8%;
    background: #28d75a;
  ...
  /* Red: strafe-jump direction — shorter, nested inside the green span. */
  .line.red {
    top: 16.1%;
    height: 51.8%;
    background: #ff4040;
  ...
  .speedo {
    position: absolute;
    /* Just above the RED line's top (16.1%). It used to clear the green top,
       but the raised lines leave too little headroom there — above red keeps
       it on-screen, still inside the green line and over the red one. */
    bottom: calc(83.9% + 10px);
    transform: translateX(-50%);
    display: flex;
