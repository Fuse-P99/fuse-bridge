<script>
  ...
  // (see the aspect correction below) — starting values, expected to be tuned
  // against live strafe testing. The red (strafe-jump) inset is a first guess
  // pending that same testing.
  const GREEN_X = { left: 23.3, right: 76.7 }; // percent
  const RED_X = { left: 26.3, right: 73.7 }; // percent

  // Bounds cover the speed-mode base offset plus full manual calibration:
  // Run +14 with +20 manual, Bard −10 with −20 manual.
  $: off = Math.max(-30, Math.min(34, +offsetPct || 0));
