<script>
  ...
  // Answer the server's request for game item ids. The inventory dump is the
  // only place they appear — the wiki almost never publishes one — and they're
  // the only way to tell apart items that share a name, of which there are real
  // cases (nine "Piece of a Medallion" in the Veeshan's Peak key quest).
  //
  // Only names the server asked about, only from the live dump, and only once
  // per name per session. Fire-and-forget: nothing here is worth an error in
  // front of someone looking at their gear, and the next inventory retries.
  const reportedIDs = new Set();
  function reportItemIDs(need) {
    if (!need || !need.length) return;
    const want = new Set(need.map((n) => n.toLowerCase()));
    const ids = {};
    for (const inv of inventory) {
      const k = (inv.name || "").toLowerCase();
      if (!inv.item_id || !want.has(k) || reportedIDs.has(k)) continue;
      ids[inv.name] = inv.item_id;
      reportedIDs.add(k);
    }
    if (Object.keys(ids).length) ReportItemIDs(ids).catch(() => {});
  }
  ...
  // LORE items exist one-per-character, so an item already on the character
  // can't legally go in this slot. Only the EQUIPPED items need their lore
  // flag, and ensureLookup has already fetched those into itemsBy — the
  // candidates are matched by name alone, so no extra lookups are needed.
  //
  // Every slot counts, INCLUDING the one being edited: re-picking the item
  // that's already there is a no-op, and the "Now: …" line above the list
  // already says what's equipped. General1-8 count too — a lore item sitting in
  // your inventory blocks wearing a second one just as much as a worn one does.
  $: loreTaken = (() => {
    const taken = new Set();
    if (!edSlot) return taken;
    for (const e of Object.values(editSlots)) {
      if (!e || !e.name) continue;
      const it = itemsBy[e.name.toLowerCase()];
      if (it && it.lore) taken.add(e.name.toLowerCase());
    }
    return taken;
  })();
  // loreTaken is referenced directly in each filter rather than through a shared
  // helper: Svelte derives dependencies from the identifiers in the statement,
  // so hiding it behind a const would stop these updating when a pending item
  // lookup resolves and reveals that an equipped item is LORE.
  $: edSugsShown = edSugs.filter(
    (n) => !loreTaken.has((n || "").toLowerCase()),
  );
  $: edTopShown = edTop.filter(
    (t) => !loreTaken.has(((t && t.name) || "").toLowerCase()),
  );
  // How many candidates the rule removed, so an item going missing from the
  // list reads as a rule rather than a bug.
  $: loreHidden =
    edSugs.length - edSugsShown.length + (edTop.length - edTopShown.length);
  ...
  // Quest turn-in editor (admin mode; its endpoints are officer-gated too).
  ...
  // (their local inventory dumps only; never guildmates). The viewed
  // character is excluded — their copy is the one being hovered.
  let tipHolders = [];
  let tipHoldersFor = "";
  $: tipItemName = tip ? tip.inv.name : "";
  ...
  async function loadTipHolders(name) {
    ...
    try {
      ...
      if (tipHoldersFor === name) {
        tipHolders = hits.filter(
          (h) => h.char.toLowerCase() !== (charName || "").toLowerCase(),
        );
      }
    } catch {
  ...
  let libErr = "";
  let libMsg = "";
  let libSelClass = ""; // class filter ("" = all classes)
  let libClassMenu = false; // class-picker dropdown open
  let libSelDkp = ""; // DKP bucket filter ("" = any)
  let libDkpMenu = false; // DKP dropdown open
  ...
  // A status code is not something a user can act on. mageloPost only falls
  // back to "server returned NNN" when the server sent no message of its own,
  // so that string is the one case where a number would reach the screen.
  function humanErr(e) {
    const s = String(e)
      .replace(/^Error:\s*/i, "")
      .trim();
    return /^server returned \d+$/i.test(s)
  ...
  // Any click that a dropdown didn't swallow (stopPropagation) closes both.
  function libCloseMenus() {
    libClassMenu = false;
    libDkpMenu = false;
  }
  // The caller's "Share 52+ Magelos" setting (General tab) rides on the list
  // response — it decides whether the Current tab offers "name & tag your
  // live listing" or the enable-it hint.
  let libAutoShare = true;
  // Server-side search: toon, display name, member name/handle, class, tags,
  // and worn item names (items live server-side, hence not a client filter).
  let libSearch = "";
  let libSearchTimer;
  function libSearchChanged() {
    clearTimeout(libSearchTimer);
    libSearchTimer = setTimeout(libRefresh, 300);
  }
  function libRefresh() {
    // Background refreshes (globe markers on character switch) are always
    // unfiltered; the query only narrows the open dialog's list.
    ListLibrary(showLib ? libSearch.trim() : "")
      .then((r) => {
        libRows = r.entries || [];
        libAutoShare = r.auto_share !== false;
      })
      .catch((e) => (libErr = humanErr(e)));
  }
  // The library list is loaded for every character, not just when the panel is
  // open, because the magelo picker marks which profiles are published. Without
  // this the globe would only appear after someone happened to open the library.
  let sharesFor = null;
  $: if ($linked && charName && charName !== sharesFor) {
    sharesFor = charName;
    libRefresh();
  }
  // This character's own published entries, keyed by magelo name ("" = Current).
  $: myShares = (() => {
    const m = {};
    const me = (charName || "").toLowerCase();
    for (const r of libRows) {
      if (!r.mine || (r.toon || "").toLowerCase() !== me) continue;
      m[(r.magelo || "").toLowerCase()] = r;
    }
    return m;
  })();
  // myShares is named in both so Svelte re-runs these when the list reloads.
  const shareKey = (magelo) => (magelo || "current").toLowerCase();
  $: shareOf = (magelo) => myShares[shareKey(magelo)] || null;
  $: curShare = myShares && shareOf("");
  ...
  // Filters: one class picker (single-select dropdown), one DKP-bracket
  // dropdown, and the tag chips (tags AND together — each narrows the list).
  $: libClasses = [
  ...
  // DKP brackets over the server's worn-set estimate (entry.dkp_total).
  const DKP_BUCKETS = [
    { v: "low", label: "Low DKP", hint: "0–10k DKP" },
    { v: "mid", label: "Mid DKP", hint: "10–20k DKP" },
    { v: "high", label: "High DKP", hint: "20–26k DKP" },
    { v: "sweaty", label: "Sweaty DKP", hint: "26k+ DKP" },
  ];
  function dkpBucketOf(total) {
    if (total >= 26000) return "sweaty";
    if (total >= 20000) return "high";
    if (total >= 10000) return "mid";
    return "low";
  }
  function fmtDkp(n) {
  ...
  // Curated builds (frozen custom magelos) first, then the live listings, each
  // group by name. Mirrors the server's order.
  $: libShown = libRows
    .filter(
      (r) =>
        (!libSelClass || r.class === libSelClass) &&
        (!libSelDkp || dkpBucketOf(r.dkp_total || 0) === libSelDkp) &&
        libSelTags.every((t) => libRowTags(r).includes(t)),
    )
    .slice()
    .sort(
      (a, b) =>
        (libIsLive(a) ? 1 : 0) - (libIsLive(b) ? 1 : 0) ||
        libName(a).localeCompare(libName(b)),
    );

  // A listing that follows its character's real gear: the automatic 52+
  // listing, or a manually shared "current" magelo (row-backed, so it keeps
  // its own name/tags). Both present the same way — LIVE badge, "current
  // gear" byline — so a share made by hand looks no different from one the
  // app published.
  const libIsLive = (r) => !!(r.auto || r.dynamic);
  ...
  function libDelete(r) {
    DeleteLibraryEntry(r.id)
      .then(() => {
        ...
        if (r.auto) {
          // Deleting a live listing's row only strips its name/tags/votes —
          // the listing itself stays (auto), so re-fetch its reverted form.
          libRefresh();
        } else {
  ...
  // Shown in the share panel for things that are not failures — a listing that
  // turned out to be already up to date.
  let libShareWarn = "";

  // Tags compare as SETS, not as joined strings: the server stores them in its
  // own fixed order, which is not the order LIB_TAGS lists them in here. A
  // string compare would call an unchanged entry changed.
  const tagSet = (s) =>
    new Set(
      (s || "")
        .split(",")
  ...
  // The entry this share would replace, if there is one.
  $: libShareExisting = myShares && shareOf(curMagelo);
  // Nothing left to publish: it's already up, the gear matches (a dynamic entry
  // matches by definition, and a static one is told by `diverged`), and the name
  // and tags are the ones already stored. The server rejects exactly this, so
  // the button is disabled rather than left to fail — but editing the name or a
  // tag re-enables it, because that IS a change worth publishing.
  $: libShareNothingToDo =
    !!libShareExisting &&
    !libShareExisting.diverged &&
    libShareExisting.name === libShareName.trim() &&
    sameTags(libShareExisting.tags, LIB_TAGS.filter((t) => libShareTags[t]).join(", "));
  function libShareStart() {
    libShareOpen = true;
    libErr = "";
    libMsg = "";
    libShareWarn = "";
    // Pre-fill from the existing entry. Re-sharing to fix a tag must not
    // silently rename the listing.
    const sh = shareOf(curMagelo);
    libShareName = (sh ? sh.name : curMagelo || charName).slice(0, 50);
    libShareTags = {};
    if (sh) {
      for (const t of (sh.tags || "").split(",")) {
        const k = t.trim();
        if (k) libShareTags[k] = true;
      }
    }
  }
  async function libShareGo() {
    ...
    try {
      ...
      await ShareMageloToLibrary(
        ...
        (info && info.race) || "",
        (info && info.level) || 0,
        false, // confirm: a changed build just replaces the old one now
      );
      libMsg = `Shared "${name}" to the library.`;
      libShareOpen = false;
      // Drop the cached snapshot for this entry so the sharer's own expanded
      // view shows what they just published rather than the previous gear.
      const sh = shareOf(curMagelo);
      if (sh && libSlots[sh.id]) {
        const next = { ...libSlots };
        delete next[sh.id];
        libSlots = next;
      }
      ...
      const msg = humanErr(e);
      if (/already shared/i.test(msg)) {
        // Another client got there first, or this one's list was stale. Reload
        // and let the disabled state explain it — nothing actually went wrong.
        libShareWarn = msg;
        libRefresh();
      } else {
        libErr = msg;
  ...
  // Gear sums for an arbitrary worn map, so the same maths can run over the
  // alternate magelo being viewed AND over Current for the comparison column.
  function gearSumOf(map, field) {
    let n = 0;
    for (const slot of WORN_ORDER) {
      const it = itemFor(map[slot]);
      if (it) n += it[field] || 0;
    }
    return n;
  }
  function gearStatsOf(map) {
    const g = (f) => gearSumOf(map, f);
    return {
      str: g("str"),
      sta: g("sta"),
      agi: g("agi"),
      ...
      fire: g("sv_fire"),
      cold: g("sv_cold"),
      disease: g("sv_disease"),
      poison: g("sv_poison"),
      magic: g("sv_magic"),
    };
  }
  $: gearStats = itemsBy && worn && gearStatsOf(worn);
  ...
  function wornHasteOf(wornMap, map) {
    let h = 0;
    for (const slot of WORN_ORDER) {
      const it = itemFor(wornMap[slot], map);
      const m = it && it.effect && /haste\s*\+?(\d+)\s*%/i.exec(it.effect);
      if (m) h = Math.max(h, +m[1]);
    }
    return h;
  }
  const hasteCapped = (worn, buff, lvl) =>
    Math.min(worn + buff, lvl ? hasteCap(lvl) : 100);
  $: wornHaste = ((map) => wornHasteOf(worn, map))(itemsBy);
  $: hasteTotal = hasteCapped(wornHaste, buffStats.haste, level);
  $: atkTotal = (gearStats ? gearStats.atk : 0) + buffStats.atk;

  // ── comparison against Current ───────────────────────────────────────────
  // An alternate magelo is a "what if I wore this instead" sheet, so the number
  // that actually matters is the difference from what the character has on now.
  // Shown only while viewing an alternate AND once the real inventory has
  // loaded — with no Current to compare against, every delta would read as a
  // full gain, which is worse than showing nothing.
  //
  // Deltas are taken on the FINAL values, not the gear contributions: gear STA
  // feeds HP through characterStats, so a gear-only delta would understate the
  // HP a stamina swap actually costs. Creation points and buffs are identical
  // on both sides, so they cancel and only the gear difference survives.
  $: cmpOn = !!curMagelo && !!inventory && inventory.length > 0;
  $: cmpGear = cmpOn && itemsBy ? gearStatsOf(assignSlots(inventory)) : null;
  $: cmpStats = cmpGear
    ? characterStats({
        cls,
        race,
  ...
  $: cmpHasteTotal = cmpOn
    ? hasteCapped(
        wornHasteOf(assignSlots(inventory), itemsBy),
        buffStats.haste,
        level,
      )
    : 0;
  // Blank rather than "+0": an unchanged row should stay quiet.
  const dBadge = (n, suffix = "") =>
    n ? `${n > 0 ? "+" : ""}${n}${suffix}` : "";
  // A stat is only comparable when both sides resolved (mana is -1 for classes
  // that never have any, and subtracting that would invent a delta).
  const dNum = (a, b) => (a >= 0 && b >= 0 ? a - b : 0);
  ...
  $: regen = ((map) => regenOf(worn, map))(itemsBy);
  $: cmpRegen = cmpOn ? regenOf(assignSlots(inventory), itemsBy) : null;

  // ── the worn set valued in DKP ─────────────────────────────────────────────
  // Median sale price, not last: one inflated auction shouldn't move the whole
  // set. A quest reward has no sale history of its own, so it falls back to
  // quest_value — the server's total for its turn-ins, already resolved through
  // any chain of earlier quests. An item with neither contributes nothing AND
  // isn't counted, so the ratio reads as coverage: how much of the set we can
  // actually price.
  //
  // A quest item that costs nothing also scores zero here and so reads as
  // unpriced, which is imprecise but unreachable: a free step hands out notes
  // and torches, not worn gear. Worth knowing if that ever stops being true.
  function itemDkpValue(it) {
    if (!it) return 0;
    return it.dkp_count ? it.dkp_median : it.quest_value || 0;
  }
  function dkpTotalOf(wornMap, map) {
    let total = 0;
    let priced = 0;
    for (const slot of WORN_ORDER) {
      const v = itemDkpValue(itemFor(wornMap[slot], map));
      if (v > 0) {
        total += v;
        priced++;
      }
    }
    return { total, priced, slots: WORN_ORDER.length };
  }
  // worn and itemsBy are both named in the statement so the total recomputes
  // when a pending lookup lands (see itemFor's note on hidden dependencies).
  // Linked, not admin: the prices come from the guild's own loot records, and
  // any linked member can already see them per item on hover. Gated on the
  // link rather than shown always because an unlinked client gets no item data
  // at all, which would render as a confident "0" across every slot.
  $: dkpWorn =
    $linked && worn && itemsBy ? dkpTotalOf(worn, itemsBy) : null;
  // The same total for the live Current set, so an alternate can show what it
  // would cost relative to what's worn now. Only the DELTA is displayed, but
  // both sides have to be priced to have one.
  $: dkpCmp =
    $linked && cmpOn && cmpWorn && itemsBy ? dkpTotalOf(cmpWorn, itemsBy) : null;

  // Which slots this alternate changes from Current. Compared by item name
  // (case-insensitive) rather than object identity — the two sides come from
  // different sources (editSlots vs the outputfile inventory) and never share
  // objects. An empty slot on either side counts as a change, so removing a
  // piece is flagged as loudly as swapping one.
  $: cmpWorn = cmpOn ? assignSlots(inventory) : null;
  $: cmpGeneralNames = cmpOn
    ? Array.from({ length: 8 }, (_, i) => {
        const e = inventory.find((x) => x.location === "General" + (i + 1));
        return e ? e.name.toLowerCase() : "";
      })
    : null;
  const slotName = (e) => (e && e.name ? e.name.toLowerCase() : "");
  $: wornChanged = (slot) =>
    cmpOn && slotName(worn[slot]) !== slotName(cmpWorn[slot]);
  $: generalChanged = (i) =>
    cmpOn && slotName(generalSlots[i]) !== cmpGeneralNames[i];
...
<svelte:window on:click={libCloseMenus} />
...
<div class="mg">
  ...
  <div class="mg-tabs">
    <button
      ...
      on:click={() => selectMagelo("")}
      >Current{#if curShare}<span
          class="mg-pub"
          title="Shared to Fuse Shared Magelos as “{curShare.name}”. This one is
          live — it follows your gear, so there is nothing to re-share."
        >
          <svg
            viewBox="0 0 24 24"
            fill="none"
    ...
    {#each mageloList as m}
      ...
      <button
        ...
        on:click={() => selectMagelo(m)}
        >{m}{#if sh}<span
            class="mg-pub"
            title="Shared to Fuse Shared Magelos as “{sh.name}”."
          >
            <svg
              viewBox="0 0 24 24"
              fill="none"
          ...
          </span>{/if}{#if sh && sh.diverged}<span
            class="mg-pub mg-pub-chg"
            title="This profile has changes since it was shared — the library
            still shows the version you published. Share it again to publish
            them."
          >
            <svg
              viewBox="0 0 24 24"
  ...
  <div class="mg-row">
    ...
    <div class="mg-stats" class:mg-float={floating}>
      ...
      {/if}
      <!-- TOTAL DKP — the worn set priced from the guild's own loot records,
           with a coverage ratio (how many slots we could price at all). Up
           here rather than in the stats grid below because it describes the
           whole character, not another derived stat, and it carries no
           comparison column. -->
      {#if dkpWorn}
        {@const dDkp = dkpCmp ? dkpWorn.total - dkpCmp.total : 0}
        <div
          class="mg-dkp"
          title="Estimate only — the median of recent guild sale prices for each worn item, falling back to a quest's component cost where an item has never been auctioned. Prices move, and {dkpWorn.slots -
            dkpWorn.priced} of {dkpWorn.slots} worn slots have no price at all.{dkpCmp
            ? ` This set prices ${Math.abs(dDkp)} DKP ${dDkp >= 0 ? 'above' : 'below'} what you're wearing now.`
            : ''}"
        >
          <span class="mg-dkp-k">TOTAL DKP</span>
          <span class="mg-dkp-n">{dkpWorn.priced}/{dkpWorn.slots}</span>
          {#if cmpOn}
            <span class="mg-dkp-d" class:up={dDkp > 0} class:down={dDkp < 0}
              >{dBadge(dDkp)}</span
            >
          {/if}
          <span class="mg-dkp-v">{dkpWorn.total}</span>
        </div>
      {/if}
      <div
        class="mg-est"
        title="Race base + class bonus + assigned creation points + worn gear. The gold middle column is what worn gear alone contributes; the right column is the final value. Creation points are yours to assign below."
      >
        Estimates
      </div>

      <!-- Each row is label / worn-gear contribution (gold) / [change vs
           Current — green up, red down, alternates only] / final value. -->
      <div class="mg-core">
        <div>
          <span>HP</span>
          <span class="mg-core-g">{gearBadge(gs.hp)}</span>
          {#if cmpOn}
            {@const d = dNum(hpEst, cmpStats.hp)}
            <span class="mg-core-d" class:up={d > 0} class:down={d < 0}
              >{dBadge(d)}</span
            >
          {/if}
        ...
        <div>
          ...
          {#if cmpOn}
            {@const d = dNum(manaEst, cmpStats.mana)}
            <span class="mg-core-d" class:up={d > 0} class:down={d < 0}
              >{dBadge(d)}</span
            >
          {/if}
        ...
        <div>
          ...
          {#if cmpOn}
            {@const d = hasteTotal - cmpHasteTotal}
            <span class="mg-core-d" class:up={d > 0} class:down={d < 0}
              >{dBadge(d, "%")}</span
            >
          {/if}
        ...
        <div>
          ...
          {#if cmpOn}
            {@const d = (gs.atk || 0) - (cmpGear.atk || 0)}
            <span class="mg-core-d" class:up={d > 0} class:down={d < 0}
              >{dBadge(d)}</span
            >
          {/if}
        ...
        <div>
          ...
          <span class="mg-core-g"></span>
          {#if cmpOn}
            {@const d = regen.mana - cmpRegen.mana}
            <span class="mg-core-d" class:up={d > 0} class:down={d < 0}
              >{dBadge(d)}</span
            >
          {/if}
        ...
        <!-- WEIGHT/STRENGTH — red when over STR (encumbered). No comparison:
             an editor magelo carries no bag contents while Current does, so the
             two weights aren't measuring the same thing. -->
        <div>
          <span>WT/STR</span>
          <span class="mg-core-g"></span>
          {#if cmpOn}<span class="mg-core-d"></span>{/if}
      ...
      <div class="mg-attrs">
        {#each STAT_NAMES as n, i}
          <div class="mg-attr">
            ...
            {#if cmpOn}
              {@const d = base ? totals[i] - cmpStats.totals[i] : 0}
              <span class="mg-attr-d" class:up={d > 0} class:down={d < 0}
                >{dBadge(d)}</span
              >
            {/if}
      ...
      <div class="mg-resists">
        {#each resists as [n, v], i}
          <div class="mg-res">
            ...
            {#if cmpOn && cmpStats.resists[i]}
              {@const d = v - cmpStats.resists[i][1]}
              <span class="mg-res-d" class:up={d > 0} class:down={d < 0}
                >{dBadge(d)}</span
              >
            {/if}
        ...
        >
          {#each WORN_ORDER as slot}
            ...
            >
              ...
              {/if}
              {#if wornChanged(slot)}
                <span class="mg-swap" title="Changed from Current">
                  <svg
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
  ...
  <div class="mg-addrow">
    {#if isAdmin}
      ...
      >
      <button class="mg-btn" on:click={() => (showQuests = true)}>Quests…</button
      >
    {/if}
  </div>
</div>
...
{#if showQuests}
  <QuestEditor onClose={() => (showQuests = false)} />
{/if}
...
{#if showLib}
  ...
  <div class="mg-ov" on:click|self={() => (showLib = false)}>
    <div class="mg-dlg mg-dlg-lib">
      <div class="mg-dlg-title"><Icon name="globe" /> Fuse Shared Magelos</div>

      <input
        class="mg-in mg-lib-searchbox"
        placeholder="Search by toon, member, item, or class…"
        bind:value={libSearch}
        on:input={libSearchChanged}
      />
      ...
      {#if libShareOpen}
        <div class="mg-lib-share">
          ...
          {#if libShareWarn}
            <div class="mg-lib-warn">{libShareWarn}</div>
          {/if}

          {#if libShareNothingToDo}
            <!-- Already published and nothing to add. Show the entry itself
                 rather than only saying so — the useful question at this point
                 is "what did I share?", and the answer is one row away. -->
            <div class="mg-lib-already">
              {#if libShareExisting.dynamic}
                Already shared. This one is <strong>live</strong> — it follows
                {charName}'s gear on its own, so there is never anything to
                re-share. Change the name or tags above to update the listing.
              {:else}
                Already shared, and <strong>{curMagelo}</strong> hasn't changed
                since. Edit the profile and share again to publish the changes,
                or change the name or tags above to update the listing.
              {/if}
            </div>
            <div class="mg-lib-row">
              <div class="mg-lib-head mg-lib-head-flat">
                {#if libIsLive(libShareExisting)}
                  <span class="mg-lib-lvl"
                    >{libShareExisting.level
                      ? libShareExisting.level + " "
                      : ""}{libShareExisting.class}</span
                  >
                  <span class="mg-lib-name"
                    >{libShareExisting.toon || libName(libShareExisting)}</span
                  >
                  <span class="mg-lib-tag mg-lib-live">LIVE</span>
                  {#if libShareExisting.name && libShareExisting.name !== libShareExisting.toon}
                    <span class="mg-lib-tag">{libShareExisting.name}</span>
                  {/if}
                {:else}
                  <span class="mg-lib-cls" title={libShareExisting.class}
                    >{classAbbr(libShareExisting.class) ||
                      libShareExisting.class ||
                      "?"}</span
                  >
                  <span class="mg-lib-name">{libName(libShareExisting)}</span>
                {/if}
                {#each (libShareExisting.tags || "")
                  .split(",")
                  .map((t) => t.trim())
                  .filter(Boolean) as t}
                ...
                <span class="mg-lib-by">
                  {#if libIsLive(libShareExisting)}
                    {libShareExisting.dkp_total
                      ? `${fmtDkp(libShareExisting.dkp_total)} DKP`
                      : ""}
                  {:else}
                    {libShareExisting.level ? libShareExisting.level + " " : ""}{libShareExisting.class}
                    {libShareExisting.toon ? `· ${libShareExisting.toon}` : ""} —
                    shared by {libShareExisting.shared_by || "?"}
                  {/if}
                </span>
                <button
                  class="mg-buff-x"
          ...
          {:else}
            <!-- Sharing Current publishes a live view; a named magelo publishes
                 a snapshot. Said plainly here because it decides whether anyone
                 ever has to press this button a second time. -->
            <div class="mg-lib-sharenote">
              {#if curMagelo}
                This saves a snapshot of <strong>{curMagelo}</strong>. Edit it
                later and you'll need to share again to publish the changes.
              {:else}
                {charName}'s live listing follows their gear automatically —
                this just names and tags it.
              {/if}
            </div>
          {/if}
        </div>
        ...
        <div class="mg-dlg-row">
          {#if curMagelo}
            <button class="mg-btn" on:click={libShareStart}
              >Share "{curMagelo}" ({charName})…</button
            >
          {:else if libAutoShare}
            <!-- Current is never "shared" any more — it's listed automatically
                 for 52+ characters. This only names and tags that listing. -->
            <button class="mg-btn" on:click={libShareStart}
              >{curShare ? "Edit" : "Add"} name & tags for {charName}'s live
              listing…</button
            >
          {:else}
            <div class="mg-lib-hint">
              Current gear is listed here automatically for level 52+
              characters, but you've turned "Share 52+ Magelos" off — enable it
              on the General tab to list your characters.
            </div>
          {/if}
        </div>
      {/if}
      ...
      <div class="mg-lib-filters">
        <button
          class="mg-tab"
          class:active={!libSelClass && !libSelDkp && !libSelTags.length}
          on:click={() => {
            libSelClass = "";
            libSelDkp = "";
            libSelTags = [];
        ...
        <div class="mg-lib-dd">
          <button
            class="mg-tab"
            class:active={!!libSelClass}
            title={libSelClass || "Filter by class"}
            on:click|stopPropagation={() => {
              libClassMenu = !libClassMenu;
              libDkpMenu = false;
            }}
            >{libSelClass ? classAbbr(libSelClass) || libSelClass : "Class"}
            ▾</button
          >
          {#if libClassMenu}
            <!-- svelte-ignore a11y-click-events-have-key-events a11y-no-static-element-interactions a11y-no-noninteractive-element-interactions -->
            <div class="mg-lib-menu" on:click|stopPropagation>
              <button
                class="mg-lib-mi"
                class:sel={!libSelClass}
                on:click={() => {
                  libSelClass = "";
                  libClassMenu = false;
                }}>Any class</button
        ...
        <div class="mg-lib-dd">
          <button
            class="mg-tab"
            class:active={!!libSelDkp}
            title="Filter by the build's TOTAL DKP estimate ({DKP_BUCKETS.map(
              (b) => `${b.label}: ${b.hint}`,
            ).join(', ')})"
            on:click|stopPropagation={() => {
              libDkpMenu = !libDkpMenu;
              libClassMenu = false;
            }}
            >{libSelDkp
              ? DKP_BUCKETS.find((b) => b.v === libSelDkp)?.label
              : "DKP"} ▾</button
          >
          {#if libDkpMenu}
            <!-- svelte-ignore a11y-click-events-have-key-events a11y-no-static-element-interactions a11y-no-noninteractive-element-interactions -->
            <div class="mg-lib-menu" on:click|stopPropagation>
              ...
              >
              {#each DKP_BUCKETS as b}
                <button
                  class="mg-lib-mi"
                  class:sel={libSelDkp === b.v}
                  title={b.hint}
                  on:click={() => {
                    libSelDkp = b.v;
                    libDkpMenu = false;
        ...
        {#each libShown as r (r.id)}
          <div class="mg-lib-row">
            ...
            <div class="mg-lib-head" on:click={() => libToggle(r)}>
              {#if libIsLive(r)}
                <!-- Live listing: "60 Cleric Dooce LIVE … 1,234 DKP". The
                     character IS the listing, so it reads as a character
                     line; a custom name the owner gave it rides as a tag. -->
                <span class="mg-lib-lvl"
                  >{r.level ? r.level + " " : ""}{r.class}</span
                >
                <span class="mg-lib-name">{r.toon || libName(r)}</span>
                <span
                  class="mg-lib-tag mg-lib-live"
                  title="Live gear, read straight from {r.toon}'s latest outputfile — it follows the character on its own"
                  >LIVE</span
                >
                {#if r.name && r.name !== r.toon}
                  <span class="mg-lib-tag">{r.name}</span>
                {/if}
              {:else}
              ...
              <span class="mg-lib-by">
                {#if libIsLive(r)}
                  {r.dkp_total ? `${fmtDkp(r.dkp_total)} DKP` : ""}
                {:else}
              ...
              {#if (r.mine || isAdmin) && r.id > 0}
                <button
                  class="mg-buff-x"
                  title={r.auto
                    ? "Remove the custom name/tags — the live listing stays, reverting to the character's name"
                    : "Remove from library"}
            ...
            {#if libOpen === r.id}
              <div class="mg-lib-body">
                {#if libSlots[r.id]}
                  ...
                  {@const ld = dkpTotalOf(lw, itemsBy)}
                  <div class="mg-lib-sheet">
                    <div class="mg-lib-stats">
                      <!-- What this build costs, priced the same way the main
                           panel prices your own. No comparison column: a shared
                           build is often another character entirely, so there's
                           nothing of yours it would be meaningful to subtract. -->
                      {#if $linked}
                        <div
                          class="mg-dkp"
                          title="Estimate only — the median of recent guild sale prices for each worn item, falling back to a quest's component cost where an item has never been auctioned. Prices move, and {ld.slots -
                            ld.priced} of {ld.slots} worn slots have no price at all."
                        >
                          <span class="mg-dkp-k">TOTAL DKP</span>
                          <span class="mg-dkp-n">{ld.priced}/{ld.slots}</span>
                          <span class="mg-dkp-v">{ld.total}</span>
                        </div>
                      {/if}
                      <div
                        class="mg-est"
                        title="Race base + class bonus + worn gear (gold middle column) — creation points and buffs don't travel with a share, so the sharer's real numbers may sit a bit higher."
                      >
                        Estimates
                      </div>
                      <div class="mg-core">
...
{#if edSlot}
  ...
  <div class="mg-ov" on:click|self={closeSlotEd}>
    <div class="mg-dlg">
      ...
      {#if loreHidden}
        <div class="mg-dlg-note mg-lore-note">
          {loreHidden} LORE item{loreHidden === 1 ? "" : "s"} hidden — already on
          this character.
        </div>
      {/if}
      {#if edTopShown.length}
        <div class="mg-dlg-note">
          Top {edTopShown.length} for this slot, ranked by {edTopSort === "mana"
            ? "mana"
            : "HP"} — click to equip.
        </div>
...
{#if tip}
  <ItemTipCard
    ...
    x={tip.x}
    y={tip.y}
    fallback="Not in the item DB yet — queued for scraping."
  />
{/if}
...
<style>
  ...
  /* Published markers on a picker tab. Inline-flex with its own left margin so
     it sits with the label rather than on the text baseline, where the round
     globe reads as sunk. Pointer-events stay on: the title IS the explanation,
     and the tab's own click still works through it. */
  .mg-pub {
    display: inline-flex;
    align-items: center;
    margin-left: 5px;
  ...
  /* Edited since sharing — gold, matching the "changed from Current" marker on
     an alternate's slots, because it means the same thing here. */
  .mg-pub-chg {
    margin-left: 3px;
    color: var(--accent);
  }
  ...
  /* Row layout: label | gear contribution (gold) | final value. The gear cell
  ...
  /* Gear contribution: gold. It can only ever add, so it needs no sign colour —
     which frees green/red to mean something in the comparison column. */
  ...
  /* Change vs the Current magelo: green for a gain, red for a loss. Fixed width
     and always rendered (blank when unchanged) so the value column stays
     aligned down the panel. */
  .mg-core span.mg-core-d {
    color: var(--text-muted);
    font-size: 10.5px;
    letter-spacing: normal;
    font-variant-numeric: tabular-nums;
  ...
  .mg-core span.mg-core-d.up,
  .mg-attr-d.up,
  .mg-res span.mg-res-d.up {
    color: var(--success);
  }
  .mg-core span.mg-core-d.down,
  .mg-attr-d.down,
  .mg-res span.mg-res-d.down {
    color: #ff5555;
  }
  ...
  /* Coverage ratio trailing the DKP total — dimmed so the total reads first.
     The gap is a margin, not a whitespace text node, so reformatting the
     markup can't silently close it up. */
  /* TOTAL DKP, directly under the level/class line. Its own row rather than a
     stats-grid entry: it values the whole character, and the grid's gold
     "from gear" and delta columns mean nothing here. */
  /* Columns mirror the stats grid below — label, then coverage where the gold
     "from gear" figure sits, then the comparison delta, then the value in the
     same right-aligned style as every stat. */
  .mg-dkp {
    display: flex;
    align-items: baseline;
    gap: 6px;
  ...
  /* Same three columns as .mg-core: name | gear (gold) | final value. */
  .mg-attr-v {
    color: var(--text-primary);
    font-weight: 600;
    font-variant-numeric: tabular-nums;
  ...
  /* Same three columns as .mg-core: label | gear (gold) | final value. */
  .mg-res {
    display: flex;
    align-items: baseline;
    gap: 6px;
  ...
  /* "Changed from Current" marker on an alternate magelo's slots. Top-right so
     it never collides with the stack count, which lives bottom-right. The dark
     disc keeps the gold readable over a bright item icon, and pointer-events
     stay off so the slot's own click/hover behaviour is untouched. */
  .mg-swap {
    position: absolute;
    top: 1px;
    right: 1px;
  ...
  .mg-lib-sharenote strong {
    color: var(--text-secondary);
    font-weight: 600;
  }
  /* Not an error: a question the server is asking, or a listing that was
     already current. Amber rather than red — nothing has gone wrong. */
  .mg-lib-warn {
    font-size: 11px;
    line-height: 1.5;
    color: var(--accent);
  ...
  /* "Already shared" explanation above the entry's own row. */
  .mg-lib-already {
    font-size: 11px;
    line-height: 1.5;
    color: var(--text-secondary);
  }
  .mg-lib-already strong {
    color: var(--accent);
    font-weight: 600;
  }
  /* The same row the library list shows, minus the expand-on-click: there is no
     list to expand into from here, so the pointer would promise nothing. */
  .mg-lib-head-flat {
    cursor: default;
  }
  .mg-lib-head-flat:hover {
    background: none;
  }
  .mg-lib-tag-pick {
    display: flex;
  ...
  /* Library search — full-width, above the share row and filters. */
  .mg-lib-searchbox {
    width: 100%;
    margin-bottom: 8px;
  }
  /* Opt-out hint shown in place of the Current listing's name/tags button. */
  .mg-lib-hint {
    color: var(--text-muted);
    font-size: 12px;
    line-height: 1.5;
  }
  /* Class / DKP dropdown chips. */
  .mg-lib-dd {
    position: relative;
  }
  .mg-lib-menu {
  ...
  /* Automatic listings: live outputfile gear rather than a curated share. */
  .mg-lib-live {
  ...
  /* "60 Cleric" ahead of a live listing's character name. */
  .mg-lib-lvl {
    color: var(--text-secondary);
    font-size: 11px;
    white-space: nowrap;
  ...
  /* Tinted so a shorter-than-expected list reads as a rule, not a missing item. */
  .mg-lore-note {
    color: var(--accent);
  }
  .mg-form {
  ...
  /* The hover tooltip itself is the shared ItemTipCard component. */
</style>
