<script>
  ...
  function tipStats(it) {
    ...
    // Keyed off quest_routes, not the shopping list: a quest can legitimately
    // need nothing from outside (every step a ground spawn or a dialogue
    // grant) and would otherwise render as though it weren't a quest at all.
    if (it.quest_routes) {
      // quest_items is the SHOPPING LIST — what the quest needs from outside,
      // with everything its own steps produce already netted out server-side.
      // It is not the step list; a twenty-step epic usually needs four things.
      const comps = it.quest_items || [];
      lines.push(
        `Quest${it.quest_name ? ": " + it.quest_name : ""}` +
          (it.quest_class ? ` [${it.quest_class}]` : "") +
          (it.quest_steps
            ? ` — ${it.quest_steps} step${it.quest_steps === 1 ? "" : "s"}`
            : ""),
      );
      // A faction gate is why an otherwise-affordable quest isn't open to
      // everyone, so it belongs next to the price rather than only in the
      // editor.
      if (it.quest_faction) {
        lines.push(`  Requires ${it.quest_faction}`);
      }
      // Coin, kept separate from the DKP figure below on purpose: platinum and
      // DKP are different currencies and summing them would be a lie.
      if (it.quest_plat) {
        lines.push(`  Plus ${it.quest_plat}pp`);
      }
      // The same work also yields these — you can hold one of them, not all.
      if (it.quest_cycle && it.quest_cycle.length) {
        lines.push(`  Or trade up for ${it.quest_cycle.join(" → ")}`);
      }
      // More than one quest produces this item. What follows is the cheapest
      // route, and saying so beats implying it's the only one.
      if (it.quest_routes > 1) {
        lines.push(`  Cheapest of ${it.quest_routes} routes — others exist`);
      }
      // The item has its own sale price, so the server never walked the chain.
      // Name what it needs and say why there are no numbers, rather than
      // printing "no DKP data" against entries nobody looked up.
      if (!it.quest_priced) {
        for (const c of comps) lines.push(`  ${c.name}`);
        if (comps.length) {
          lines.push(`  Not priced — the sale history above is the price.`);
        }
        return lines;
      }
      if (!comps.length) {
        lines.push(`  Needs nothing from outside — only time.`);
        return lines;
      }
      let priced = 0;
      for (const c of comps) {
        if (c.dkp_count) {
          priced++;
          // "(3 sales)" spelled out — a bare count next to an item name would
          // read as how many of it the quest wants.
          lines.push(
            `  ${c.name} — ${c.value} DKP (${c.dkp_count} sale${c.dkp_count === 1 ? "" : "s"})`,
          );
        } else if (c.from_quest) {
          priced++;
          lines.push(`  ${c.name} — ${c.value} DKP (from quest)`);
        } else if (c.free) {
          // Known to cost nothing, which is a different fact from having no
          // price — and it counts as priced, because it is.
          priced++;
          lines.push(`  ${c.name} — free`);
        } else {
          lines.push(`  ${c.name} — no DKP data`);
        }
      }
      // Only worth a total when more than one entry is priced; with one it
      // would just repeat the line above it. A repeated item legitimately
      // appears twice — two trade slots, two prices, both counted.
      if (priced > 1) {
        lines.push(
          `  Needs: ${it.quest_value} DKP` +
            (priced === comps.length
              ? ""
              : ` (${priced} of ${comps.length} priced)`),
        );
      }
    }
    return lines;
  ...
  // ── admin: the worn set valued in DKP ──────────────────────────────────────
  ...
  $: dkpWorn =
    isAdmin && worn && itemsBy
      ? (() => {
          let total = 0;
          let priced = 0;
          for (const slot of WORN_ORDER) {
            const v = itemDkpValue(itemFor(worn[slot], itemsBy));
            if (v > 0) {
              total += v;
              priced++;
            }
  ...
  <div class="mg-row">
    ...
    <div class="mg-stats" class:mg-float={floating}>
      ...
      <div class="mg-core">
        ...
        <!-- TOTAL DKP (admin only) — the worn set priced from loot_records,
             with a coverage ratio: how many of the slots we could price at all.
             No comparison column: the delta cell stays empty like WT/STR so
        ...
        {#if dkpWorn}
          <div>
            ...
            <span class="mg-core-v"
              >{dkpWorn.total}<span
                class="mg-dkp-n"
                title="{dkpWorn.priced} of {dkpWorn.slots} worn slots have a DKP price"
                >{dkpWorn.priced}/{dkpWorn.slots}</span
              ></span
            >
          </div>
        {/if}
...
<style>
  ...
  .mg-core span.mg-core-v .mg-dkp-n {
    color: var(--text-muted);
    font-size: 10.5px;
    font-weight: 400;
    margin-left: 5px;
