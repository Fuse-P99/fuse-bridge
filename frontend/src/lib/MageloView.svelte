<script>
  ...
  function tipStats(it) {
    ...
    if (it.quest_items && it.quest_items.length) {
      lines.push(`Quest turn-in${it.quest_name ? ": " + it.quest_name : ""}`);
      ...
      let priced = 0;
      for (const c of it.quest_items) {
        if (c.dkp_count) {
          priced++;
          // "(3 sales)" spelled out — a bare count next to an item name reads
          // as a quantity, and these are one-each turn-ins.
          lines.push(
            `  ${c.name} — ${c.dkp_median} DKP (${c.dkp_count} sale${c.dkp_count === 1 ? "" : "s"})`,
          );
      ...
      if (priced > 1) {
        const all = priced === it.quest_items.length;
        lines.push(
          `  Components: ${it.quest_value} DKP` +
            (all ? "" : ` (${priced} of ${it.quest_items.length} priced)`),
        );
      }
    }
    return lines;
