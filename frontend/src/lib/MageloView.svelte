<script>
  ...
  // At equal votes a curated share outranks an automatic listing.
  $: libShown = libRows
    .filter(
      (r) =>
  ...
  // Set when the server refuses a re-share that would cost votes; the next
  // press sends the acknowledgement.
  let libShareConfirm = false;
  // Shown in the share panel for things that are not failures: the vote-reset
  // check, and a listing that turned out to be already up to date.
  let libShareWarn = "";
  ...
  function libShareStart() {
    ...
    libErr = "";
    libMsg = "";
    libShareConfirm = false;
    libShareWarn = "";
    // Pre-fill from the existing entry. Re-sharing to fix a tag must not
    // silently rename the build people already voted on.
  ...
  async function libShareGo() {
    ...
    try {
      ...
      if (/confirm to continue/i.test(msg)) {
        // Not a failure — the server is refusing to spend someone's votes
        // without being asked twice. Say so in our own words and arm the
        // button, rather than showing a refusal the user didn't cause.
        const votes = libShareExisting ? libShareExisting.votes : 0;
        libShareConfirm = true;
        libShareWarn =
          `${curMagelo || "Current"} has changed since it was shared. ` +
          `Publishing the new version replaces the build people voted on` +
          (votes > 0
            ? `, and resets its ${votes} vote${votes === 1 ? "" : "s"}.`
            : ".");
      } else if (/already shared/i.test(msg)) {
  ...
  <div class="mg-tabs">
    <button
      ...
      >Current{#if curShare}<span
          class="mg-pub"
          title="Shared to Fuse Shared Magelos as “{curShare.name}” ({curShare.votes}
          vote{curShare.votes === 1 ? '' : 's'}). This one is live — it follows
          your gear, so there is nothing to re-share."
        >
          <svg
            viewBox="0 0 24 24"
            fill="none"
    ...
    {#each mageloList as m}
      ...
      <button
        ...
        >{m}{#if sh}<span
            class="mg-pub"
            title="Shared to Fuse Shared Magelos as “{sh.name}” ({sh.votes}
            vote{sh.votes === 1 ? '' : 's'})."
          >
            <svg
              viewBox="0 0 24 24"
      ...
      {#if libShareOpen}
        <div class="mg-lib-share">
          ...
          <div class="mg-lib-tagrow">
            ...
            <button
              class="mg-btn mg-btn-go"
              class:mg-btn-danger={libShareConfirm}
          ...
          {#if libShareNothingToDo}
            ...
            <div class="mg-lib-row">
              <div class="mg-lib-head mg-lib-head-flat">
                ...
                <span class="mg-lib-votes-flat"
                  >👍 {libShareExisting.votes}</span
                >
                <button
                  class="mg-buff-x"
            ...
            <div class="mg-lib-head" on:click={() => libToggle(r)}>
              ...
              {#if r.auto}
                <span
                  ...
                  title="Live gear, read straight from {r.toon}'s latest outputfile — listed automatically for level 52+ characters"
                  >LIVE</span
                >
              {/if}
              {#each (r.tags || "")
              ...
              <span class="mg-lib-by">
                {#if r.auto}
                  {r.level ? r.level + " " : ""}{r.class} · {r.shared_by || "?"}'s
                  current gear{r.dkp_total
                    ? ` · ${fmtDkp(r.dkp_total)} DKP`
                    : ""}
                {:else}
              ...
              {#if r.id > 0}
                <button
                  ...
                  on:click|stopPropagation={() => libVote(r)}
                  >👍 {r.votes}</button
                >
              {/if}
