<script>
  ...
  function tipStats(it) {
    ...
    // Keyed off quest_routes, not the component list: a step can legitimately
    // consume nothing (a ground spawn, a dialogue grant) and would otherwise
    // render as though it weren't a quest item at all.
    if (it.quest_routes) {
      const comps = it.quest_items || [];
      const kind =
        it.quest_step_kind && it.quest_step_kind !== "handin"
          ? { combine: "Combine", kill: "Drop", ground: "Ground spawn", dialogue: "Dialogue" }[
              it.quest_step_kind
            ] || "Quest turn-in"
          : "Quest turn-in";
      lines.push(
        `${kind}${it.quest_name ? ": " + it.quest_name : ""}` +
          (it.quest_class ? ` [${it.quest_class}]` : ""),
      );
      // A faction gate is why an otherwise-affordable turn-in isn't open to
      // everyone, so it belongs next to the price rather than only in the
      // editor.
      if (it.quest_faction) {
        lines.push(`  Requires ${it.quest_faction} faction`);
      }
      // Who to hand the components to. Worth a line of its own — knowing what a
      // reward costs is only half of knowing how to get it.
      if (it.quest_mob) {
        lines.push(`  Hand in to ${it.quest_mob}`);
      }
      ...
      if (it.quest_routes > 1) {
        lines.push(
          `  Cheapest of ${it.quest_routes} routes — others exist`,
        );
      }
      // The item has its own sale price, so the server never walked the chain.
      // List what to hand over and say why there are no numbers, rather than
      // printing "no DKP data" against components nobody looked up.
      if (!it.quest_priced) {
        for (const c of comps) {
          lines.push(`  ${c.name}${c.consumed === false ? " (returned)" : ""}`);
        }
        if (comps.length) {
      ...
      let costed = 0;
      for (const c of comps) {
        // A returned item is part of the step but not part of its cost, so it
        // is listed and then excluded from both counts.
        if (!c.consumed) {
          lines.push(`  ${c.name} — returned`);
          continue;
        }
        costed++;
        if (c.dkp_count) {
          ...
          // read as how many of it the step wants.
          lines.push(
      ...
      // Only worth a total when more than one component is priced; with one it
      // would just repeat the line above it. A repeated item legitimately
      // appears twice here — two slots, two prices, both counted.
      if (priced > 1) {
        lines.push(
          `  Components: ${it.quest_value} DKP` +
            (priced === costed ? "" : ` (${priced} of ${costed} priced)`),
        );
      }
    }
    return lines;
