<script>
  ...
  // ── Fuse Shared Library: browse / vote / copy shared magelos ──────────────
  let showLib = false;
  let libRows = [];
  let libErr = "";
  let libMsg = "";
  let libClass = ""; // quick class filter ("" = all)
  ...
  $: libClasses = [...new Set(libRows.map((r) => r.class).filter(Boolean))].sort();
  // Score-sorted (server pre-sorts too, but votes change live in this list).
  $: libShown = (libClass ? libRows.filter((r) => r.class === libClass) : libRows)
    .slice()
  ...
  const LIB_TAGS = ["BIS", "Value", "Twink", "Solo", "Starter", "Bot"];
  ...
  function libGeneral(slots) {
    ...
    for (let i = 1; i <= 8; i++) {
      ...
      out.push(s ? { location: s.slot, name: s.name, count: s.count || 1 } : null);
    }
    return out;
  }
  ...
  function libStats(slots, map, clsName, raceName, lvl) {
    ...
    const rc = (raceName || "").toLowerCase();
    const b = RACE_BASE[rc] || null;
    const bo = CLASS_BONUS[c] || null;
    ...
    const g = {
      str: sum("str"), sta: sum("sta"), agi: sum("agi"), dex: sum("dex"),
      wis: sum("wis"), int: sum("int"), cha: sum("cha"),
      hp: sum("hp"), mana: sum("mana"), ac: sum("ac"), atk: sum("atk"),
      fire: sum("sv_fire"), cold: sum("sv_cold"), disease: sum("sv_disease"),
      poison: sum("sv_poison"), magic: sum("sv_magic"),
    };
    const totals = STAT_NAMES.map((_, i) =>
      cap255((b ? b[i] : 0) + (bo ? bo.s[i] : 0) + g[GKEY[i]]),
    );
    const hp =
      lvl && c
        ? Math.floor(
            5 +
              hpMultiplier(c, lvl) * lvl +
              ((totals[1] || 0) * lvl * hpMultiplier(c, lvl)) / 300,
          ) + g.hp
        : 0;
    const mIdx = CASTER_WIS.includes(c) ? 4 : CASTER_INT.includes(c) ? 5 : -1;
    let mana = -1; // -1 = not a caster (renders "—")
    if (mIdx >= 0 && lvl) {
      const st = (totals[mIdx] || 0) + MANA_STAT_OFFSET;
      mana =
        Math.round(
          ((80 * lvl) / 425) * Math.min(st, 200) +
            ((40 * lvl) / 425) * Math.max(0, st - 200),
        ) + g.mana;
    }
    let haste = 0;
    let wt = 0;
    ...
    const rr2 = RACE_RESIST[rc] || [0, 0, 0, 0, 0];
    return {
      hp, mana, ac: g.ac, atk: g.atk, haste,
      weight: Math.round(wt),
      str: b ? totals[0] : 0,
      totals, hasBase: !!b, gear: g,
      resists: [
        ["FIRE", cap255(rr2[0] + g.fire)],
        ["COLD", cap255(rr2[1] + g.cold)],
        ["DISEASE", cap255(rr2[2] + g.disease)],
        ["POISON", cap255(rr2[3] + g.poison)],
        ["MAGIC", cap255(rr2[4] + g.magic)],
      ],
    };
  }
  ...
  function libTargets(r) {
    ...
    return cls ? allChars.filter((c) => c.class && c.class.toLowerCase() === cls) : [];
  }
  async function libSaveCopy(r) {
    ...
    try {
      ...
      for (
        let i = 2;
        existing.includes(name.toLowerCase()) || name.toLowerCase() === "current";
        i++
      )
        name = `${base} ${i}`;
  ...
  $: totals = STAT_NAMES.map((_, i) => {
    ...
    return cap255(
      b +
        c +
        assigned[i] +
        (gearStats ? gearStats[GKEY[i]] : 0) +
        (buffStats[GKEY[i]] || 0),
    );
  });

  // HP estimate: HP = 5 + mult×level + STA×level×mult/300 with the classic
  // class/level-banded multipliers, plus gear +HP. The bands reproduce the
  // wiki's STA→HP tables exactly (Game_Mechanics "Health calculations":
  // per-STA at 60 = mult/5 — WAR 6.0, SHD/PAL 5.2, RNG 4.2, BRD/MNK/ROG 4.0,
  // priests 3.0, INT casters 2.4; at 50 = mult/6). Buffs aren't modeled.
  ...
  // Mana estimate: the wiki Statistics "Mana (MANA)" formula — each point of
  // the casting stat gives (80×level)/425 mana up to 200, (40×level)/425
  // above 200 — plus gear +mana. WIS classes are the priests plus paladin and
  // ranger; INT classes are the four INT casters plus shadow knight and bard.
  $: manaStatIdx = CASTER_WIS.includes(cls)
    ? 4
    : CASTER_INT.includes(cls)
      ? 5
      : -1;
  // Empirical p99 calibration from naked test characters:
  //   L1 DE cleric  WIS 93  → 21 mana
  //   L1 Iksar shm  WIS 90  → 21 mana
  //   L13 gnome wiz INT 118 → 336 mana
  // All three are exact ONLY with round() (not floor — the Iksar and the
  // wizard bracket every floor offset out) and a stat offset in [19.1, 19.5];
  // 19.3 sits mid-band and also retro-predicts the L49 ranger calibration
  // (WIS 103, +125 gear → 1253) to the point. The wiki page's bare rate
  // (offset 0, floor) reads consistently short.
  const MANA_STAT_OFFSET = 19.3;
  $: manaEst = (() => {
    if (manaStatIdx < 0 || !level) return 0;
    const st = (totals[manaStatIdx] || 0) + MANA_STAT_OFFSET;
    const lo = Math.min(st, 200);
    const hi = Math.max(0, st - 200);
    return (
      Math.round(((80 * level) / 425) * lo + ((40 * level) / 425) * hi) +
  ...
  $: resists = gearStats
    ? [
        ["FIRE", cap255(rr[0] + gearStats.fire + buffStats.svf)],
        ["COLD", cap255(rr[1] + gearStats.cold + buffStats.svc)],
        ["DISEASE", cap255(rr[2] + gearStats.disease + buffStats.svd)],
        ["POISON", cap255(rr[3] + gearStats.poison + buffStats.svp)],
        ["MAGIC", cap255(rr[4] + gearStats.magic + buffStats.svm)],
      ]
    : [];
...
<div class="mg">
  ...
  <div class="mg-tabs">
    ...
    <button
      class="mg-btn mg-lib-open"
      title="Browse magelos shared by the guild"
      on:click={openLib}>🌐 Fuse Shared Library</button
    >
  </div>
  ...
  <div class="mg-row">
    ...
    <div class="mg-stats">
      ...
      <div
        class="mg-est"
        title="Race base + class bonus + assigned creation points + worn gear. HP/mana use wiki formulas; creation points are yours to assign below."
      >
        Estimates
      </div>
      ...
      <div class="mg-core">
        ...
        <div>
          <span>AC</span>{(gearStats ? gearStats.ac : 0) + buffStats.ac}
        </div>
        <div><span>HASTE</span>{hasteTotal}%</div>
        <div><span>ATK</span>{atkTotal}</div>
        <div><span>HP REGEN</span>+{regen.hp}</div>
        <div><span>MANA REGEN</span>+{regen.mana}</div>
...
{#if showLib}
  ...
  <div class="mg-ov" on:click|self={() => (showLib = false)}>
    <div class="mg-dlg mg-dlg-lib">
      <div class="mg-dlg-title">🌐 Fuse Shared Library</div>
        ...
        <div class="mg-lib-share">
          ...
          <div class="mg-lib-tagrow">
            ...
            <button
              ...
              on:click={libShareGo}>{libShareBusy ? "Sharing…" : "Share"}</button
            >
          </div>
        </div>
      {:else}
      ...
      <div class="mg-lib-filters">
        <button
          class="mg-tab"
          class:active={libClass === ""}
          on:click={() => (libClass = "")}>All</button
        >
        {#each libClasses as c}
          <button
            class="mg-tab"
            class:active={libClass === c}
            title={c}
            on:click={() => (libClass = c)}>{classAbbr(c) || c}</button
          >
        {/each}
      </div>
        ...
        {#each libShown as r (r.id)}
          <div class="mg-lib-row">
            ...
            <div class="mg-lib-head" on:click={() => libToggle(r)}>
              ...
              {#each (r.tags || "").split(",").map((t) => t.trim()).filter(Boolean) as t}
                ...
                {#if libSlots[r.id]}
                  ...
                  <div class="mg-lib-sheet">
                    <div class="mg-lib-stats">
                      <div
                        class="mg-est"
                        title="Race base + class bonus + worn gear — creation points and buffs don't travel with a share, so the sharer's real numbers may sit a bit higher."
                      >
                        Estimates
                      </div>
                      <div class="mg-core">
                        <div><span>HP</span>{ls.hp || "—"}</div>
                        <div><span>MANA</span>{ls.mana >= 0 ? ls.mana : "—"}</div>
                        <div><span>AC</span>{ls.ac}</div>
                        <div><span>HASTE</span>{ls.haste}%</div>
                        <div><span>ATK</span>{ls.atk}</div>
                        <div>
                          <span>WT/STR</span>
                          <span
                            class="mg-wt"
                  ...
                  <div class="mg-lib-copy">
                    {#if libTargets(r).length}
                      ...
                      <select class="mg-in mg-lib-sel" bind:value={libCopySel[r.id]}>
                      ...
                      <button
                        ...
                        >{libCopyBusy === r.id ? "Saving…" : "Save Copy"}</button
                      >
                    {:else}
                      <span class="mg-lib-copy-lbl">
                        No {r.class || "matching-class"} characters of yours to
                        copy this to.
                      </span>
                    {/if}
                  </div>
...
<style>
  ...
  /* WT/STR value: double class outranks the .mg-core span label styling. */
  .mg-core span.mg-wt {
    color: var(--text-primary);
    font-size: 12.5px;
