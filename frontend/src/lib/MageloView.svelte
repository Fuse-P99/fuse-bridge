<script>
  ...
  function loadBuffSel() {
    try {
      ...
      activeBuffNames = Array.isArray(a) ? a.filter((x) => typeof x === "string") : [];
    } catch {
      activeBuffNames = [];
    }
  }
  ...
  $: buffStats = ((list) => {
    const t = {
      hp: 0, mana: 0, ac: 0, atk: 0, haste: 0, ds: 0, hpregen: 0,
      str: 0, sta: 0, agi: 0, dex: 0, wis: 0, int: 0, cha: 0,
      svf: 0, svc: 0, svd: 0, svp: 0, svm: 0,
    };
    for (const b of list) {
  ...
  const MOD_LABELS = {
    hp: "HP", mana: "MANA", ac: "AC", atk: "ATK", haste: "Haste", ds: "DS",
    hpregen: "HPrgn", str: "STR", sta: "STA", agi: "AGI", dex: "DEX",
    wis: "WIS", int: "INT", cha: "CHA", svf: "SvF", svc: "SvC", svd: "SvD",
    svp: "SvP", svm: "SvM",
  };
  function modText(mods) {
    return Object.entries(mods || {})
      .filter(([, v]) => v)
      .map(([k, v]) => `${MOD_LABELS[k] || k.toUpperCase()} +${v}${k === "haste" ? "%" : ""}`)
      .join(" · ");
  }
  ...
  const BUFF_MOD_KEYS = [
    ...
    ["haste", "Haste %"], ["ds", "Dmg Shield"], ["hpregen", "HP Regen"],
    ["str", "STR"], ["sta", "STA"], ["agi", "AGI"], ["dex", "DEX"],
    ["wis", "WIS"], ["int", "INT"], ["cha", "CHA"],
    ["svf", "Sv Fire"], ["svc", "Sv Cold"], ["svd", "Sv Disease"],
    ["svp", "Sv Poison"], ["svm", "Sv Magic"],
  ];
  function buffEdLoad(name) {
    ...
    const mods = {};
    for (const [k] of BUFF_MOD_KEYS) mods[k] = (src && src.mods && src.mods[k]) || 0;
    be = src
      ? { name: src.name, icon: src.icon, conflicts: src.conflicts, note: src.note, mods }
  ...
  function buffFill(p, base) {
    ...
    for (const [k] of BUFF_MOD_KEYS)
      mods[k] = found ? (p.mods && p.mods[k]) || 0 : (base && base.mods[k]) || 0;
    be = {
  ...
  function buffEdSave() {
    ...
    SaveBuff({ ...be, mods })
      ...
      .then((b) => {
        allBuffs = b;
        beExisting = true;
        buffAdmMsg = `"${be.name}" saved.` + (be.icon ? "" : " Icon will auto-scrape from the wiki.");
      })
  ...
  $: totals = STAT_NAMES.map((_, i) => {
    ...
    return cap255(
      b + c + assigned[i] + (gearStats ? gearStats[GKEY[i]] : 0) + (buffStats[GKEY[i]] || 0),
    );
  });
  ...
  function hpMultiplier(c, l) {
    if (c === "warrior")
      return l < 20 ? 22 : l < 30 ? 23 : l < 40 ? 25 : l < 53 ? 27 : l < 57 ? 28 : l < 60 ? 29 : 30;
  ...
  $: hasteTotal = Math.min(wornHaste + buffStats.haste, level ? hasteCap(level) : 100);
...
<div class="mg">
  ...
  <div class="mg-stats">
    ...
    <div class="mg-core">
      ...
      <div><span>AC</span>{(gearStats ? gearStats.ac : 0) + buffStats.ac}</div>
...
{#if showBuffPick}
  ...
  <div class="mg-ov" on:click|self={() => (showBuffPick = false)}>
    <div class="mg-dlg mg-dlg-sm">
      ...
      <div class="mg-sugs">
        {#each allBuffs.filter((b) => !activeBuffNames.includes(b.name)) as b (b.name)}
          <button class="mg-sug mg-sug-buff" title={b.note || ""} on:click={() => addBuff(b)}>
            {#if b.icon && iconBase}
              <img class="mg-buff-ico" src={iconBase + b.icon} alt="" />
            {/if}
            <span class="mg-buff-name">{b.name}</span>
        ...
        {:else}
          <div class="mg-dlg-note">Every buff in the catalog is already active.</div>
        {/each}
      </div>
      <div class="mg-dlg-btns">
        <button class="mg-btn" on:click={() => (showBuffPick = false)}>Close</button>
      </div>
    </div>
  </div>
{/if}
    ...
    <div class="mg-dlg">
      ...
      <div class="mg-dlg-row">
        ...
        <button
          ...
          on:click={buffUrlScrape}>{beBusy && !be ? "Scraping…" : "Scrape"}</button
        >
      ...
      {#if be}
        <div class="mg-form">
          <label class="mg-f mg-f-wide">
            <span>Name (exact wiki spell name — looked up when you tab out)</span>
            <input class="mg-in" bind:value={be.name} on:blur={buffNameBlur} />
          </label>
          <label class="mg-f mg-f-wide">
            <span>Icon (blank = auto-scrape from the wiki)</span>
            <input class="mg-in" bind:value={be.icon} placeholder="Spellicon_X.png" />
          </label>
          <label class="mg-f mg-f-wide">
      ...
      <div class="mg-dlg-btns">
        ...
        {/if}
        <button class="mg-btn" on:click={() => (showBuffAdmin = false)}>Close</button>
        <button
