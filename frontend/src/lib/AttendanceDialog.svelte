          ...
          <div class="auto-row">
            <select class="auto-sel" bind:value={pickedChannel} disabled={sending}>
              ...
              {#each channels as c (c.id)}
                <!-- The count is how many characters that channel already
                     credits, so a log someone else has entered is obvious
                     before you post a second copy of it. -->
                <option value={c.id}
                  >#{c.name}{c.raiders > 0 ? ` (${c.raiders})` : ""}</option
                >
              {/each}
            </select>
            <button
