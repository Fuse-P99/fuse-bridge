<script>
  ...
  // "load" is the zone transition instant (LOADING, PLEASE WAIT...) and is the
  // lap marker to trust: "zone" is the same crossing plus however long the
  // client took to load, which varies with disk and foreground state.
  // "announce" is the dock shout — the instant the schedule's offsets are
  // measured from.
  const btKind = (k) =>
    k === "load"
      ? "LOAD"
  ...
  const btKindTip = (k) =>
    k === "load"
      ? "Zone transition instant — the precise lap marker. Use this series."
      : k === "zone"
        ? "Zone load finished. Carries the client's load time, so it scatters more than LOAD."
        : k === "announce"
          ? "Dock announcement — what the schedule's offsets are measured from."
          : "Your marker phrase.";
  ...
  function btSeries(events) {
    ...
    // LOAD series first: it's the one to read off, and burying it under the
    // noisier ZONE series would invite using the wrong number.
    const rank = (k) => (k === "load" ? 0 : k === "mark" ? 1 : 2);
    return out.sort((a, b) => rank(a.kind) - rank(b.kind) || b.laps - a.laps);
  }
...
<div class="bt">
  ...
  {#if btStats.length}
    <div class="bt-stats">
      {#each btStats as s}
        <div class="bt-stat">
          <span class="bt-kind bt-{s.kind}" title={btKindTip(s.kind)}>{btKind(s.kind)}</span>
  ...
  <div class="bt-log">
    ...
    {:else}
      {#each [...bt.events].reverse() as e, ri}
        ...
        <div class="bt-row">
          <span class="bt-kind bt-{e.kind}" title={btKindTip(e.kind)}>{btKind(e.kind)}</span>
...
<style>
  ...
  /* The precise lap marker gets the accent; ZONE is dimmed to the same family
     so it reads as the lesser sibling of the pair rather than a rival. */
  .bt-load {
    background: #2122af;
    color: #dfe3ff;
  }
