...
// Resist field names on gear and on buffs, parallel to RESIST_NAMES.
const RESIST_GEAR_KEY = ["fire", "cold", "disease", "poison", "magic"];
...
// Every character starts with these resists regardless of race, class or
// level: MR/FR/CR 25, PR/DR 15. Confirmed from the calculator, which reports
// them for a naked High Elf (a race with no racial modifiers) both at level 60
// and with the level field blank — so they're flat, not level-scaled.
...
export function characterStats({
  ...
  if (mIdx >= 0 && level) {
    if (HYBRIDS.includes(c) && level < HYBRID_MANA_LEVEL) {
      ...
      mana =
        Math.round(
          ((80 * level) / 425) * Math.min(st, 200) +
            ((40 * level) / 425) * Math.max(0, st - 200),
        ) +
        g("mana") +
        b("mana");
    }
  ...
  const resists = RESIST_NAMES.map((n, i) => [
    n,
    cap255(
      BASE_RESIST[i] + rr[i] + g(RESIST_GEAR_KEY[i]) + b(RESIST_BUFF_KEY[i]),
    ),
  ]);
