...
// A police badge is gold or it is not a badge. Like the globe, this one keeps
// its color wherever it is drawn — an officer mark that took the surrounding
// text's color would be just one more shape in the line.
const BADGE_GOLD = "#ffc94a";
...
export const ICONS = {
  ...
  // Police badge: the shield with a star on it. Marks a toon whose owner holds
  // the officer role, matching the flair the Discord guild stream gives them.
  badge: {
    color: BADGE_GOLD,
    paths: [
      "M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z",
      "M12 7.6l1 2.83 2.99.07-2.37 1.83.85 2.87L12 13.5l-2.47 1.7.85-2.87-2.37-1.83 2.99-.07z",
    ],
  },
  // A known guild bot — the robot icon the Discord stream uses. The eyes are
  // `dots` rather than strokes for the reason noted above: the guild-chat
  // overlay draws this at ~12px, where a hairline dot is nothing at all.
  robot: {
    paths: [
      "M12 8V4H8",
  ...
  // Disclosure arrows for a collapsible panel: the chevron points the way the
  // panel will move — up to open one that slides up out of the bottom edge,
