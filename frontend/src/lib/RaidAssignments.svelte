<script>
  ...
  // Overlay "Flashing" setting: off suppresses the proc-count flash. The raid
  // card (tab) never passes it, so the tab always flashes.
  ...
  {#if currentTanks.length}<div class="rc-line">
      ...
      <span class="rc-assignedname"
        >{#each currentTanks as tk, i}{i > 0 ? " / " : ""}{tk}{#if procFor(card.tank_procs, tk)}{#key procFor(card.tank_procs, tk)}<span
              class="rc-proc"
              title="Weapon procs"
              ><Icon name="bolt" />x{procFor(card.tank_procs, tk)}</span
            >{/key}{/if}{/each}</span
      >
    </div>{/if}
  ...
  {#each currentTanks as tk}
    ...
    {#if d}
      <div class="rc-disc" class:warn={d.warn}>
        ...
        <span class="rc-discico"><Icon name="shield" /></span>
...
<style>
  ...
  /* Overlay "Flashing" off: the counter still updates, it just doesn't pulse. */
  .noflash .rc-proc {
    animation: none;
  }
  ...
  /* The shield draws in currentColor and takes the label's teal, so the pair
     reads as one mark. */
  .rc-discico,
  .rc-disclabel {
    color: #7fd8cf;
