<script>
  ...
  // Every slot any reward occupies, for the dropdown. Built from the data
  // rather than a fixed list of EQ slots, so it only ever offers something
  // that would actually match — and a slot spelling we haven't seen still
  // shows up instead of being silently unfilterable.
  $: slotOptions = [
    ...new Set(
      (quests || []).flatMap((q) =>
        (q.rewards || []).map((r) => (r.slot || "").trim()).filter(Boolean),
      ),
    ),
  ...
  function matches(q, t, cls, slot, epic, faction) {
    ...
    if (slot && !(q.rewards || []).some((r) => (r.slot || "") === slot))
      return false;
                      ...
                      <div class="q-checktext">
                        ...
                        {#if stepIns(s).length || s.plat_cost}
                          <div class="q-fact">
                            ...
                            >
                            <span>
                              {#each rollIns(s) as row, k}
                                {#if k > 0}<span class="q-sep">+</span>{/if}
                                {#each slotNames(row.it) as nm, ai}
                                  ...
                                  >
                                {/each}
                                {#if row.n > 1}<span class="q-mult"
                                    >×{row.n}</span
                                  >{/if}
                                {#if row.it.consumed_ok === false}<span
                                    class="q-back">(returned)</span
                                  >{/if}
                              {/each}
                              {#if s.plat_cost}
