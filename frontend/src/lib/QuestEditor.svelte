<script>
  ...
  function stepErr(st, n) {
    ...
    if ([...st.into, ...st.out].some((i) => !i.name.trim()))
  ...
  function stepSummary(st) {
    const bits = [];
    if (st.mob_name) bits.push(st.mob_name);
            ...
            <div class="q-row">
              <div class="q-row-main">
                ...
                <div class="q-qname">
                  ...
                  step{q.steps.length === 1 ? "" : "s"}{q.prereqs.length
    ...
    {:else}
      ...
      {#each form.steps as st, i}
        ...
        <div class="q-card">
          ...
          {#if openStep === i}
            {#if f.mob}
              <div class="q-label">{MOB_LABEL[st.kind] || "NPC"}</div>
              <div class="q-slot-wrap">
                <button
                  class="q-slot"
                  class:q-empty={!st.mob_name}
                  on:click={() =>
                    openMobPick(st.mob_name, (m) => {
                      st.mob_id = m.id;
                      st.mob_name = m.name;
                      // The faction a step gates on is usually the faction of
                      // the NPC involved, so fill it — but only when empty. An
                      // officer who typed something else meant it.
                      if (m.faction && f.faction && !st.faction_group.trim()) {
                        st.faction_group = m.faction;
                      }
                    })}>{st.mob_name || "Pick an NPC…"}</button
                >
                {#if st.mob_name}
                  <button
                    class="q-x"
                    title="Clear"
                    aria-label="Clear NPC"
                    on:click={() => {
                      st.mob_id = 0;
                      st.mob_name = "";
                      form = form;
              ...
              <div class="q-head">
                ...
                <button
                  ...
                  on:click={() => {
                    st.into = [
                      ...st.into,
                      { name: "", consumed_ok: true, consumed_fail: true },
                    ];
                    form = form;
                    openItemPick(
                      "Item handed over",
                      "",
                      (n) => (st.into[st.into.length - 1].name = n),
                    );
                  }}>+ Item</button
                >
              </div>
              {#each st.into as it, j}
                <div class="q-itemrow">
                  <button
                    ...
                    on:click={() =>
                      openItemPick("Item handed over", it.name, (n) => (it.name = n))}
                    >{it.name || "Pick an item…"}</button
                  >
                  <label
                    class="q-chk q-tight"
                  ...
                  {#if f.fail}
                    ...
                    >
                      <input type="checkbox" bind:checked={it.consumed_fail} />
                      Lost on failure
                    </label>
                  {/if}
                  <button
