<script>
  ...
  async function dropMarker(q, i, tk) {
    try {
      ...
      const label =
        tk.pt.what !== "here"
          ? `${questShortName(q)}: ${tk.pt.what}`
          : questShortName(q);
      // No character in scope in the editor: the flag falls back to the
      // current character when clicked.
      await AddQuestMarker(tk.name, tk.pt.y, tk.pt.x, label, q.id, "");
  ...
  // ── create a missing item from the picker ──────────────────────────────────
  // quest_step_items has a foreign key into eqitems, so a component with no
  // row can't be entered at all. The picker offers to create the typed name as
  // a stub (name + wiki link — StubItems refuses to touch an existing row),
  // which satisfies the FK immediately; the server's stub sweep re-scrapes
  // placeholders on its own, so if the wiki has a page the stats and icon fill
  // in later with no further action.
  let pickAddBusy = false;
  let pickAddErr = "";
  // Offered only when nothing listed matches the query exactly — if the exact
  // name is already in the DB, adding it again is a no-op waiting to confuse.
  $: pickExact =
    pickRows &&
    pickRows.some(
      (s) => (s || "").toLowerCase() === pickQ.trim().toLowerCase(),
    );
  async function addPickItem() {
    const name = pickQ.trim();
    if (!name || pickAddBusy) return;
    pickAddBusy = true;
    pickAddErr = "";
    try {
      const res = await StubItems([name]);
      const fails = res && res.failed ? Object.values(res.failed) : [];
      if (fails.length) {
        pickAddErr = String(fails[0]);
        return;
      }
      // The server stores the apostrophe form (backtick is normalized away);
      // pick the row that actually exists.
      chooseItem(name.replace(/`/g, "'"));
    } catch (e) {
      pickAddErr = String(e);
    } finally {
      pickAddBusy = false;
...
{#if pick}
  ...
  <div class="q-ov q-ov-top" on:click|self={() => (pick = null)}>
    <div class="q-dlg q-dlg-sm">
      ...
      {#if pickQ.trim().length >= 2 && !pickExact}
        <button
          class="q-btn q-addstub"
          disabled={pickAddBusy}
          on:click={addPickItem}
          >{pickAddBusy
            ? "Adding…"
            : `＋ Add “${pickQ.trim()}” to the item DB`}</button
        >
        {#if pickAddErr}<div class="q-err">{pickAddErr}</div>{/if}
        <div class="q-note">
          Creates a name + wiki-link entry (stats fill in automatically if the
          wiki has a page). Double-check the exact in-game spelling first.
        </div>
      {/if}
      <div class="q-btns">
...
<!-- Item card — the shared ItemTipCard every surface shows. -->
{#if tip}
  <ItemTipCard
    name={tip.name}
    item={tip.item}
    ...
    x={tip.x}
    y={tip.y}
    fallback="Not in the item DB — it can't be used in a quest until it is."
  />
{/if}
...
<style>
  ...
  /* The picker's create-missing-item action: full width and gold-accented so
     it reads as "do something new", not another search result. */
  .q-addstub {
    width: 100%;
