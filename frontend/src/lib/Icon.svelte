<script>
  ...
  import { ICONS, ICON_DOT_R, iconColor } from "./icons.js";
...
>
  ...
  <!-- Filled features (a die's pips): fill via style for the same var() reason. -->
  {#each def.dots || [] as [cx, cy]}<circle {cx} {cy} r={ICON_DOT_R} style="fill:{stroke};stroke:none" />{/each}
</svg>
