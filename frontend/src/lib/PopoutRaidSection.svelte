<script>
  ...
  async function poll() {
    try {
      ...
      // The ghost is assembled from the whole guild's chat, so it carries the
      // tank lists and debuffs that a locally-revived card never had.
      card = pickActive(data) || (data && data.ghost_raid) || null;
    } catch {
      /* keep last card */
    }
    try {
