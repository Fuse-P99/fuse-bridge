<script>
  // The Guild Chat panel: the live #guild-stream — every guild message, plus
  // the raid-mob engage lines, as the server already relays them to Discord.
  // It sits in a collapsible box at the bottom of the Raids tab, where the
  // conversation belongs beside the board it is usually about, and it opens to
  // every linked member (the guild's own chat is not officer business).
  //
  ...
  // The two pill toggles in the panel's divider row (RaidsTab.svelte owns them
  // and their storage).
  export let showEngages = true;
  export let newestTop = false;
  // The gear popover's text style, owned and persisted by RaidsTab. Font, bold
  ...
  // "New lines arrived while the reader was scrolled away" — RaidsTab pulses
  ...
  // install never starts the server poller at all. It runs once per mount,
  // and the panel is mounted by each expand — the Raids tab is rebuilt on
  // every visit — so a toggle flipped on the General tab is picked up on the
  // next open without anything having to watch for it.
  ...
  // A generous estimate of a card's height, used only to decide which side of
  // the cursor it goes on. Over-estimating flips a little early, which is the
  // harmless direction: under-estimating would let a tall card run off-screen.
  const CARD_H = 320;
  const CARD_PAD = 14;

  // cardPos puts the card beside the cursor, flipping to the other side when
  // it would leave the window. The shell is CSS-zoomed (lib/scale.js), so the
  // cursor's screen coordinates divide by the UI scale or the card drifts at
  // Medium/Large.
  function cardPos(ev) {
    const z = $scale || 1;
    ...
    let x = cx + CARD_PAD;
    if (x + CARD_W > vw) x = cx - CARD_PAD - CARD_W;
    let y = cy + CARD_PAD;
    if (y + CARD_H > vh) y = cy - CARD_PAD - CARD_H;
  ...
  async function showItem(ev, name) {
    ...
    tip = { name, item: itemCache[key] ?? null, x: pos.x, y: pos.y };
  ...
  let mtip = null; // { q, info, loading, x, y }
  ...
  async function showMember(ev, e) {
    ...
    if (hit && Date.now() - hit.at < MEMBER_TTL) {
      mtip = { q, info: hit.info, loading: false, x: pos.x, y: pos.y };
      return;
    }
    // "Looking up…" rather than a blank card while the request is in flight.
    mtip = { q, info: null, loading: true, x: pos.x, y: pos.y };
    let info = null;
    try {
...
<div class="gc" class:gc-gated={gated} bind:this={el}>
  {#if gated}
...
<!-- The shared cards every surface shows, positioned at the cursor. -->
{#if tip}
  <ItemTipCard
    name={tip.name}
    item={tip.item}
