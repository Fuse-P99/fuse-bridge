<script>
  ...
  // memberQuery is what the toon name's hover card looks up, or "" when the
  // row has no card — and the dotted underline follows the same answer, so a
  // name is underlined exactly when hovering it will show something.
  //
  // A pending row is one the server hasn't answered for: nothing is known
  // about its speaker yet. The card starts working on its own the moment the
  // answer merges onto the row.
  //
  // A bot's row is the one case the toon itself is no answer: its owner in
  // the roster is the bot account, whose card would show the guild's own DKP.
  // So for a bot the server names the PLAYER instead — the linked member whose
  // FuseBridge is tailing the bot's log — in the handle, and leaves it empty
  // when nobody it knows is at the keyboard. An empty handle on a bot row
  // therefore means no card at all, never a fallback to the bot account. Every
  // other row resolves the way Discord's /who does: the handle is unique, and
  // a toon with no handle resolves to whoever owns it.
  function memberQuery(e) {
    if (e.pending) return "";
    if (e.bot) return (e.handle || "").trim();
    return (e.handle || e.toon || "").trim();
  }

  async function showMember(ev, e) {
    const q = memberQuery(e);
    if (!q) return;
    hideItem();
    const s = ++mseq;
        ...
        >
          <div class="gc-body">
            ...
            <!-- Who the toon belongs to is the hover card's business, not the
                 ...
                 dotted underline appears exactly when the card has something
                 to say (memberQuery): once the row is confirmed, and for a
                 bot, only when the server could name its player.
...
<style>
  ...
  /* A row with a card behind its name (memberQuery): the server has said who
     this is, so the name is worth hovering. The dotted underline is the same
     quiet affordance the item names carry, and its appearing is how the row
     announces that its card is ready. A bot nobody known is playing never
     gets it. */
  .gc-hover {
    text-decoration: underline dotted;
    text-underline-offset: 2px;
