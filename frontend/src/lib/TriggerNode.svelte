...
<div class="node" style="margin-left:{depth ? 14 : 0}px">
  ...
  >
    ...
    {#if node.total_triggers}
      ...
      >
    {/if}
    <!-- Audio mute (Fuse subtree only): silences sounds/TTS for everything in
         this group while alerts and timer bars keep working. Sits left of the
         enable switch. Inherited mutes show dimmed — unmute at the ancestor. -->
...
<style>
  ...
  /* Audio mute toggle, immediately left of the enable switch. The switch
     normally pushes itself right with margin-left:auto; when a mute button is
     present IT takes the push instead (and the sibling rule below zeroes the
     switch's), so the pair sits together at the row's right edge. Quiet until
