<script>
  ...
  // ✅/❌ for whether that overlay is popped out right now — an alert routed
  ...
  function quickView(t, poppedNow) {
    ...
    if (t.copy_clipboard) {
      lines.push(`|Clipboard|${t.clip_blocked_eff ? "❌" : "✅"}`);
    }
    lines.push(`|Audio|${t.muted_eff ? "❌" : "✅"}`);
    let alertText =
    ...
    lines.push(`|Customizations|${t.has_override ? "✅" : "❌"}`);
    return lines.join("\n");
  }
    ...
    <span class="acts">
      ...
      {#if onToggleClip && !node.personal}
        <button
          ...
          on:click|stopPropagation={() => onToggleClip("group", node)}>📋</button
        >
      {/if}
      {#if node.class_auto}
        <span
