<script>
  ...
  // Clipboard block for the Fuse subtree — same shape and gating as the mute.
  export let onToggleClip = null; // ("group"|"trigger", node|trigger) => void
  ...
  // Hover quick view for a trigger leaf: what fires and where it lands, with
  // {icon:check:ok}/{icon:x:bad} tokens — tooltip.js swaps those for the check and x
  // icons — for whether that overlay is popped out right now: an alert routed
  // to an overlay that isn't up is one the player will never see. Effective
  // values: personal overrides win over the package fields. poppedNow is
  // passed explicitly so the title recomputes when the popped set refreshes.
  function quickView(t, poppedNow) {
    const overlay = t.category || "Default";
    const ovRow = (kind) =>
      `|Overlay|${overlay} ${
        poppedNow && poppedNow.has("popout-" + kind + "-" + overlay)
          ? "{icon:check:ok} overlay enabled"
          : "{icon:x:bad} overlay disabled"
      }`;
    const lines = [`**${t.name}**`]; // gold headline (tooltip.js ** markup)
    lines.push(`|Enabled|${t.enabled ? "{icon:check:ok}" : "{icon:x:bad}"}`);
    // Clipboard state only when the trigger HAS a clipboard action — for the
    // rest a red mark would read as "broken" instead of "not applicable".
    if (t.copy_clipboard) {
      lines.push(
        `|Clipboard|${t.clip_blocked_eff ? "{icon:x:bad}" : "{icon:check:ok}"}`,
      );
    }
    lines.push(`|Audio|${t.muted_eff ? "{icon:x:bad}" : "{icon:check:ok}"}`);
    let alertText =
      t.override && t.override.text != null
        ? t.override.text
        : t.on_match && t.on_match.use_text
          ? t.on_match.display_text
          : "";
    alertText = (alertText || "").trim();
    if (t.shows_alert && alertText) {
      lines.push(`|Alert|“${alertText}”`);
      lines.push(ovRow("alerts"));
    }
    if (t.timer_enabled && t.timer_seconds > 0) {
      const tn = (t.override && t.override.timer_name) || t.timer_name || t.name;
      lines.push(`|Timer|“${tn}” for ${t.timer_seconds} seconds`);
      lines.push(ovRow("timers"));
    }
    lines.push(
      `|Customizations|${t.has_override ? "{icon:check:ok}" : "{icon:x:bad}"}`,
    );
    return lines.join("\n");
  }

  // Mixed subtree: some triggers under this group fire and some don't (a
  // disabled group with one timer reached in and enabled, or an enabled
  // section with pieces switched off). The slider shows it as a half state —
  // dot centered, track yellow — instead of lying with a clean on or off.
  // enabled_triggers counts what actually FIRES (explicit overrides
  // included), so this can't disagree with the engine.
  $: partial =
    (node.total_triggers || 0) > 0 &&
    (node.enabled_triggers || 0) > 0 &&
    node.enabled_triggers < node.total_triggers;

  // Whether a trigger produces visible ALERT text (match, ending, or ended)
  // comes from the server as t.shows_alert — computed there so the personal
  // customization layer (reworded or hidden text) is folded in exactly the
  // way the engine applies it, instead of re-deriving from package fields.
  ...
  $: isRoot = depth === 0;
  // The Fuse root is the one top-level group that isn't Personal. The globe
  // icon marks it as guild-shared, matching the Fuse Shared Magelos button.
...
<div class="node" style="margin-left:{depth ? 14 : 0}px">
  ...
  >
    ...
    {#if isShared}
      <span class="shared" title="Shared with the guild"
        ><Icon name="globe" /></span
      >
    {/if}
    ...
    <span class="acts">
      ...
      {#if onToggleMute && !node.personal}
        <button
          ...
          >{#if node.muted_eff}<Icon name="volume-off" />{:else}<Icon
              name="volume"
            />{/if}</button
        >
      {/if}
      <!-- Clipboard block: stops everything in this group from taking the
           system clipboard on match. Everything else the triggers do — alerts,
           bars, sounds — is untouched. -->
      {#if onToggleClip && !node.personal}
        <button
          class="mute clip"
          class:on={node.clip_blocked_eff}
          class:inherited={node.clip_blocked_eff && !node.clip_blocked}
          title={node.clip_blocked_eff && !node.clip_blocked
            ? "Clipboard blocked by a parent group — allow it there (click to also block this group)"
            : node.clip_blocked
              ? "Allow this group's triggers to copy to your clipboard"
              : "Stop this group's triggers taking your clipboard (everything else still works)"}
          on:click|stopPropagation={() => onToggleClip("group", node)}
      ...
      {:else}
        ...
        <label
          ...
          class:partial
          title={partial
            ? `Mixed — ${node.enabled_triggers} of ${node.total_triggers} enabled. Click to ${node.enabled ? "disable" : "enable"} all.`
            : node.enabled
              ? "Disable group"
              : "Enable group"}
          on:click|stopPropagation
  ...
  {#if expanded[node.id]}
    ...
    {#each node.triggers || [] as t (t.id)}
      ...
      >
        ...
        {#if t.has_override}
          <!-- Same gear glyph as the overlay cards' edit button, gold: this
               trigger carries personal customizations (Customize tab). -->
          <span class="ovr-gear" title="Customizations applied">⚙</span>
        {/if}
        {#if t.unsupported}
        ...
        <span class="acts">
          ...
          {#if onToggleMute && !node.personal}
            <button
              ...
              >{#if t.muted_eff}<Icon name="volume-off" />{:else}<Icon
                  name="volume"
                />{/if}</button
            >
          {/if}
          {#if onToggleClip && !node.personal}
            <button
              class="mute clip"
              class:on={t.clip_blocked_eff}
              class:inherited={t.clip_blocked_eff && !t.clip_blocked}
              title={t.clip_blocked_eff && !t.clip_blocked
                ? "Clipboard blocked by a parent group — allow it there (click to also block this trigger)"
                : t.clip_blocked
                  ? "Allow this trigger to copy to your clipboard"
                  : "Stop this trigger taking your clipboard (its alert, sound and timer still work)"}
              on:click|stopPropagation={() => onToggleClip("trigger", t)}
              ><Icon name="clipboard" /></button
            >
          {/if}
          {#if t.class_auto}
...
<style>
  ...
  /* Audio mute toggle, immediately left of the enable switch. Quiet until
     relevant: unmuted speakers only show on hover; a mute always shows. The
     icons draw in currentColor, so the color here IS the icon color — a
     button's default text color is near-black on this background. */
  .mute {
    background: none;
    border: none;
    cursor: pointer;
  ...
  /* Blocked clipboard keeps the SAME clipboard glyph and gets a red slash drawn
     over it, so the row reads as one icon in two states rather than two icons
     to tell apart. Drawn in CSS rather than with a combining-slash character,
     which renders unpredictably across emoji fonts on Windows. */
  .clip {
    position: relative;
  }
  .clip.on::after {
    ...
    border-radius: 1px;
    background: #ef5350;
    /* Dark edge so the slash stays legible over the light parts of the icon. */
  ...
  /* Personal customization marker — a gold gear beside the name. Accent, not
     grey: it's a deliberate choice, not a problem to fix. */
  .ovr-gear {
    color: var(--accent);
    font-size: 11px;
    line-height: 1;
  ...
  /* Mixed subtree: dot parked halfway on a yellow track, grey so it reads as
     "neither". Declared AFTER the :checked rules at equal specificity — the
     half state must win whether the group's own switch is on or off. */
  .sw.partial input + .knob {
    background: rgba(227, 195, 8, 0.45);
  }
  .sw.partial input + .knob::after {
    left: 8px;
    background: var(--text-secondary);
  }
  .sw.disabled {
