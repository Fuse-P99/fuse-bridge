<script>
  ...
  //
  // Each cast debuff row carries a countdown bar. Local-first: the client's own
  // guild-chat parse (GetLocalRaidTimers) starts the bar instantly; the
  // server's at_ms on the card is the fallback for out-of-game viewers. Bar
  // length is the matching Fuse "Debuffs Macros" trigger's configured timer.
  ...
  // Remaining fraction (0..1, 0 = no bar) for one debuff row on one mob.
  ...
  function debuffFrac(nowMs, timers, name, mobName, serverAtMs) {
    if (!card || card.status === "complete") return 0;
    ...
    return remain > 0 ? Math.min(1, remain / dur) : 0;
  }
</script>
...
<div class="rc-col">
  ...
  {#each castDebuffs as { name, caster, atMs }}
    {@const frac = debuffFrac(now, lt, name, card.target, atMs)}
    <div class="rc-line">
      {#if frac > 0}<div class="rc-tfill" style="width:{frac * 100}%"></div>{/if}
    ...
    <div class="rc-curtargets" class:two={card.current_targets.length === 2}>
      {#each card.current_targets as ct}
        <div class="rc-curtarget">
          ...
          {#each debuffList(ct.debuffs) as { name, caster, atMs }}
            {@const frac = debuffFrac(now, lt, name, ct.name, atMs)}
            <div class="rc-line">
              {#if frac > 0}<div
                  class="rc-tfill"
