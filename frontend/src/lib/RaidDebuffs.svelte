<script>
  ...
  // Overlay "Flashing" setting: off suppresses the sieve-count flash. The
  // raid card (tab) never passes it, so the tab always flashes.
  ...
  // Event raids (Sky / HoT / Ring War) have no boss: the section is entirely
  // Current Target blocks — up to 4, in a fixed 2-column grid (2 top, 2
  // bottom) instead of the boss-on-top + 2-adds-below layout.
  ...
  // Countdown state for one debuff row on one mob. Three outcomes, and the
  // difference between the last two matters:
  //   {frac, remain}    — running
  //   {expired: true}   — we HAD a duration and it ran out; the debuff is off
  //                       the mob, so the row drops its ✓ and caster
  //   null              — no countdown is knowable (no cast time, or no
  //                       configured trigger duration). Renders exactly as it
  //                       always has; marking these expired would be a lie,
  //                       since nothing was ever counting down.
...
<div class="rc-col" class:noflash={!flash}>
  ...
  <!-- Boss rows — an event raid has no boss, so its section is adds only. -->
  {#if !isEvent}
    {#each castDebuffs as { name, caster, atMs }}
      {@const t = debuffTimer(now, lt, name, card.target, atMs)}
      {@const gone = !!(t && t.expired)}
      <!-- Expired: the ✓ and caster are hidden rather than removed, so the
           debuff name stays in its column and the list doesn't jump when one
           row's timer runs out. -->
      <div class="rc-line">
        {#if t && !gone}<div
            class="rc-tfill"
            style="width:{t.frac * 100}%"
          ></div>{/if}
        <span class="rc-check" class:gone>✓</span>
        <span class="rc-dname done">{name}</span>
        {#if t && !gone}<span class="rc-time">{fmtRemain(t.remain)}</span>{/if}
        <span class="rc-caster" class:gone>{caster}</span>
      </div>
    {:else}
    ...
    {/if}
  {:else if !(card.current_targets && card.current_targets.length)}
    <div class="rc-line rc-none">None called yet</div>
  {/if}

  <!-- Current Target(s): non-boss adds we're fighting en route. One block, or
       two side-by-side when two adds are up; an event raid always uses the
       2-column grid (up to 4 equal targets, 2 top / 2 bottom). -->
  {#if card.current_targets && card.current_targets.length}
    <div
      class="rc-curtargets"
      class:two={!isEvent && card.current_targets.length === 2}
      class:grid4={isEvent}
    >
...
<style>
  ...
  /* A run-out countdown means the debuff is off the mob: the ✓ and the caster
     both go, leaving just the name as the record that it was called. Hidden,
     not removed — visibility keeps the layout box so names stay aligned with
     the rows whose timers are still running. */
  .rc-check.gone,
  .rc-caster.gone {
    visibility: hidden;
  }
  ...
  /* Overlay "Flashing" off: counts still update, they just don't pulse. */
  .noflash .rc-countbox {
    animation: none;
  }
