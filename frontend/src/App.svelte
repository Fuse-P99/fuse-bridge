<script>
  ...
  import { activeTab, questDeepLink } from "./lib/nav.js";
  ...
  $: bpMob = bpBots && bpBots.mob ? bpBots.mob : "";
  $: if (bpMob !== prevBpMob) {
    prevBpMob = bpMob;
    showBpBots = false;
  }
  ...
  <div class="shell" style="zoom:{$scale}; height:calc(100vh / {$scale})">
    ...
    {#if batphones.length || bpMob}
      <div class="batphone-bar" transition:slide|local>
        ...
        {/each}
        {#if bpMob}
          <!-- Mob batphone — same line as the freeform ones, plus the alert
               bots King Ak'Anon advertises for this fight. -->
          <div class="bp-line">
            <span class="bp-body">
              <span class="bp-tag">BATPHONE</span>
              {bpMob}
            </span>
            {#if bpBots.bots && bpBots.bots.length}
              <button
                class="bp-showbots"
              ...
              >
            {/if}
            {#if bpBots.sent_at_ms}
              <span class="bp-age">{bpAge(bpBots.sent_at_ms, clockNow)}</span>
            {/if}
          </div>
        {/if}
      </div>
