<script>
  ...
  import { IsAdminMode, IsLinked, IsUpgrading, GetBatphones } from '../wailsjs/go/main/App'
  ...
  async function pollBatphones() {
    if (!$linked) { batphones = []; return }
    try { batphones = await GetBatphones() || [] } catch { batphones = [] }
  }
  ...
  onDestroy(() => clearInterval(bpTimer))
...
<div class="shell" style="zoom:{$scale}; height:calc(100vh / {$scale})">
  ...
  {#if batphones.length}
    <div class="batphone-bar" transition:slide|local>
      {#each batphones as b}
        <div class="bp-line"><span class="bp-tag">BATPHONE</span> {b.text}</div>
      {/each}
    </div>
  {/if}
...
<style>
  ...
  .bp-line { font-size: 13px; color: var(--text-primary); }
  .bp-tag {
