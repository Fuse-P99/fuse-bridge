<script>
  ...
  // Clients tab is visible to admins OR officers. Timers is gated the same
  // way FOR NOW — it's rolling out to a small officer test group first.
  $: tabs =
    isAdmin || isOfficer
      ? [
          ...baseTabs,
  ...
  <div class="shell" style="zoom:{$scale}; height:calc(100vh / {$scale})">
    ...
    {#if !$linked}
      <div class="unlinked-bar" transition:slide|local>
        ...
        Link your Discord account to enable zones, timers, raids and guildmate
        maps.
