<script>
    ...
    kind === "map"
      ...
      : isSpecial
        ? {
            ...
            // line held and there would be nothing left to scroll.
            ...(kind === "guildchat" ? { engages: true, fit: false } : {}),
  ...
  // Go on every save and once on mount. Go whitelists the keys it keeps
  // (no colors; a chosen cue sound becomes a yes/no). SETTINGS TELEMETRY
  // (CLAUDE.md): a new per-kind key here also needs its dashboard row.
...
<div class="popout" class:positioning={posMode}>
  ...
  >
    ...
    {:else if kind === "guildchat"}
      ...
      <PopoutGuildChat showEngages={settings.engages !== false} {locked} />
    {:else if kind === "racing"}
