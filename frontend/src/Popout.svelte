<script>
  ...
  // Title-bar drag, manual: the OS move loop the old --wails-draggable region
  // triggered is unsafe on never-activating windows (see popouts_drag.go), so
  // pointerdown asks Go to follow the physical cursor instead. Go also
  // decides when the drag ends (hardware button state), so there are no
  // move/up handlers here and nothing to clean up. Buttons and inputs are
  // excluded the same way their no-drag opt-outs used to exclude them.
  function barDragDown(e) {
    if (locked || e.button !== 0) return;
    if (e.target.closest("button, input, select, [contenteditable='true']"))
      return;
    BeginPopoutDrag(kind, category || "").catch(() => {});
  }
...
<div class="popout">
  <!-- title bar: the drag handle (frozen when locked); buttons opt out via
       the closest() guard in barDragDown. Deliberately NOT --wails-draggable:
       that hands the drag to the OS's modal move loop, which is unsafe on a
       never-activating window (machine-wide input wedge — popouts_drag.go).
       pointerdown hands the drag to Go instead, which follows the physical
  ...
  <div
    ...
    style="--wails-draggable: no-drag; background:{titleBg}"
    on:pointerdown={barDragDown}
  >
    {#if flashing}
