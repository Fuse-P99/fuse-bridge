<script>
  ...
  onMount(async () => {
    ...
    if (!settings.aot) SetPopoutAlwaysOnTop(kind, category || "", false).catch(() => {});
...
<div class="popout">
  ...
  >
    {#if kind === "map"}
      <MapTab popout={true} bgFill={bgRgba} locked={locked} />
    ...
    <div class="set-panel" bind:this={panelEl}>
      ...
      {#if cat}
        ...
        {#if kind === "timers"}
          ...
          <div class="set-hint">
            {#if cat.carry_timers}
              Mob respawns: bars count down in real time on every character, even
        ...
        {#if kind === "alerts"}
          ...
          >
            ...
            />
            <span>Show alert age (m:ss)</span>
          </label>
          <div class="set-hint">
            Alerts stay this many seconds, or until one of their trigger's End
            Early conditions matches — then they flash three times and clear.
          </div>
        {/if}
      {/if}
