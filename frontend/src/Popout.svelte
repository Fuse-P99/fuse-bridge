<script>
  ...
  // Everything except the map participates in the global overlay lock. The map
  // joins too when its "Stay unlocked" toggle (map settings panel) is off.
  let mapStayUnlocked = true; // map only; loaded at mount, persisted via Go
  $: lockable = kind !== "map" || !mapStayUnlocked;
  $: title =
      ...
      {#if kind === "map"}
        ...
        >
          <span class="psw">
            <input
              type="checkbox"
              checked={mapStayUnlocked}
              on:change={(e) => setMapStayUnlocked(e.target.checked)}
            />
            <span class="psw-track"><span class="psw-thumb"></span></span>
          </span>
