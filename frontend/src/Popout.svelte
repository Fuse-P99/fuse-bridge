    ...
    kind === "map"
      ...
      : isSpecial
        ? {
            ...
            // newesttop off. msgfont/msgcolor dress the message text alone,
            // msgsize the whole line; empty and 0 mean "whatever the overlay
            // itself uses".
            ...(kind === "guildchat"
              ? {
                  engages: true,
  ...
  {#if showSettings}
    <div class="set-panel" bind:this={panelEl}>
      ...
      {#if kind === "guildchat"}
        ...
        <!-- The message text's own font and color, plus the line's text size.
             Deliberately three separate settings rather than one style: font
             and color dress the message alone, so the columns to its left keep
             their look and the ledger keeps its shape, while size applies to
             the whole line — bigger chat means bigger chat, clock included. -->
        <label
          class="set-row"
          title="Font for the message text only — the time, name and identity keep the overlay's font."
        >
        ...
        <label
          class="set-row"
          title="Size in px for the chat lines: the message text and, scaled with it, the time, name and identity. Blank or 0 keeps the overlay's size."
        >
          <span>Text size</span>
