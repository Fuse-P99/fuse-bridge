<script>
  ...
  $: titleHidden =
    ...
    // overlay landed.
    (kind === "racing" && !setupMode && !flashing) ||
    (kind === "map"
  ...
  function pushOverlayPrefs() {
    ...
    SetOverlayPrefs(kind, category || "", JSON.stringify(rest)).catch(
      () => {},
    );
  }
  ...
  onMount(async () => {
    ...
    Events.On("popouts-unlocked", () => (lockedGlobal = false));
    if (kind === "map") {
      GetMapStayUnlocked()
