<script>
  ...
  export let kind = "map"; // "map" | "timers" | "alerts"
  export let category = "";

  // Everything except the map is a per-category trigger overlay and behaves the
  // same way (per-character settings, transparent by default).
  $: isTrigger = kind !== "map";
  $: title = kind === "map" ? "Map" : category || "Timers";
  // Trigger overlay look settings are per character; the map's are app-wide. The
  // character suffix is resolved at mount — trigger overlays are closed and
  // reopened on a swap, so a fresh mount always reflects the incoming character.
  const BASEKEY = `fuse.popout.${kind === "map" ? "map" : kind + ":" + category}`;
  let KEY = BASEKEY;
  ...
  // the game); the map defaults to a mostly-opaque dark backdrop so its lines
  // read well. Both default always-on-top.
  let settings =
    kind === "map"
  ...
  onMount(async () => {
    ...
    // Lock (click-through + frozen drag/resize) applies to trigger overlays only
    // — the map must stay interactive (pan/zoom/buttons). Reflect the global
    // lock state and pick it up if this overlay opened while already locked.
    if (isTrigger) {
      ArePopoutsLocked()
        .then((v) => (locked = v))
        .catch(() => {});
