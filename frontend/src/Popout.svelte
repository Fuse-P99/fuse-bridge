<script>
  ...
  // One overlay has a "Stay unlocked" toggle of its own — the map, which is
  // panned, zoomed and searched, so click-through would make it useless.
  // Everything else always participates in the global overlay lock; the map
  // joins it only when its toggle is off. Go owns the value
  // (popoutState.LockWithAll).
  const stayUnlockedKind = kind === "map";
  let stayUnlocked = true; // map only; loaded at mount, persisted via Go
  ...
  // for a map with "Stay unlocked" on, which tracks the state but ignores it
  // (and picks it up mid-lock if the toggle is flipped off).
  ...
  function setStayUnlocked(v) {
    ...
    // Overlays on the Timers tab is the road back to an interactive map.
  ...
  onMount(async () => {
    ...
    // global state, including the map — whether the map OBEYS it is the
    // "Stay unlocked" toggle's call (the `locked` derivation applies it, so
    // flipping the toggle while locked takes effect immediately). Reflect the
    // current state too, for an overlay that opened while already locked.
    ArePopoutsLocked()
