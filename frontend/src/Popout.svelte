<script>
  ...
  $: fitActive, hasContent, setupMode, scheduleFit();
  ...
  async function applyFit() {
    if (!fitActive || !contentEl || resizing || collapseAnim) return;
    const tb = titleBarHeight();
    const want = Math.max(
      tb,
      Math.min(tb + Math.ceil(contentEl.offsetHeight), fitMaxHeight()),
    );
    if (Math.abs(want - lastFitH) < 2) return;
    try {
      const s = await Window.Size();
      lastFitH = want;
      if (Math.abs(s.height - want) < 2) return;
      await Window.SetSize(s.width, want);
    } catch {
      /* window may be mid-close */
    }
  }
  ...
  onMount(async () => {
    ...
    if (kind !== "map") {
      try {
        ...
        if (p && p.char) {
          ...
          // and for the special overlays, from the legacy shared
          // (pre-per-character) settings so the migration keeps your look.
    ...
    // the watcher only runs until the first zone change (or a lock).
    if (setupMode) {
      setupWatch();
...
<div class="popout">
  ...
  <div
    ...
    class:collapsed
    class:fit={fitActive}
    class:empty={isSpecial && !hasContent && !setupMode}
