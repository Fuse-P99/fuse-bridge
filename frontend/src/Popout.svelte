<script>
  ...
  import PopoutRaidDPS from "./lib/PopoutRaidDPS.svelte";
  import PopoutRacing from "./lib/PopoutRacing.svelte";
  import PopoutGuildChat from "./lib/PopoutGuildChat.svelte";
  ...
  // Two overlays have a "Stay unlocked" toggle of their own — the map and Guild
  // Chat (a log is read by scrolling, and a click-through window can't be
  // scrolled). Everything else always participates in the global overlay lock;
  // these two join it only when their toggle is off. Go owns the value
  // (popoutState.LockWithAll) for both.
  const stayUnlockedKind = kind === "map" || kind === "guildchat";
  let stayUnlocked = true; // those kinds only; loaded at mount, persisted via Go
  $: lockable = !stayUnlockedKind || !stayUnlocked;
  $: title =
    kind === "map" ? "Map" : SPECIAL_TITLES[kind] || category || "Timers";
  // Raid DPS names its fight in the title bar ("Raid DPS - Vulak`Aerr") —
  // the target the server picked as the one absorbing the raid's damage.
  let raidDpsTarget = "";
  $: dispTitle =
    kind === "raiddps" && raidDpsTarget ? `${title} - ${raidDpsTarget}` : title;
    ...
    kind === "map"
      ...
      : isSpecial
        ? {
            ...
            aot: true,
            fit: true,
            // Per-kind extras, all default ON: the raid sections' flash
            // pulses, the clerics chain timing, voice Smart Hide, and the
            // DPS-breakdown bar.
            ...(kind === "threat" ? { breakdown: true } : {}),
            ...(kind === "raidassign" ||
            kind === "raiddebuffs" ||
            kind === "raidclerics"
              ? { flash: true }
            ...
            ...(kind === "raidclerics"
              ? {
                  timing: true,
                  pulseaudio: false,
                  pulsesound: "intuition-561.mp3",
                }
              : {}),
            ...(kind === "voicespeakers" ? { smarthide: true } : {}),
            // Guild Chat scrolls its own log, so shrink-to-content is off by
            // default: with it on the window would grow to the height of every
            // line held and there would be nothing left to scroll. It also
            // reads like the game's chat window unless asked otherwise, hence
            // newesttop off. msgfont/msgbold/msgcolor dress the message text
            // alone, msgsize the whole line; empty, false and 0 mean "whatever
            // the overlay itself uses".
            ...(kind === "guildchat"
              ? {
                  engages: true,
                  newesttop: false,
              ...
              : {}),
            // Racing draws at viewport percentages inside an auto-placed
            // window: fully transparent, and never shrunk to content.
            ...(kind === "racing"
              ? {
                  sides: "both",
                  avg: 3,
  ...
  // (not a per-overlay setting, not persisted). lockedGlobal mirrors the raw
  // global state; `locked` is what THIS overlay obeys — the same thing except
  // for a map or Guild Chat with "Stay unlocked" on, which tracks the state but
  // ignores it (and picks it up mid-lock if the toggle is flipped off).
  // A placement grace (graceActive) holds it off for this overlay alone.
  let lockedGlobal = false;
  $: locked = lockedGlobal && lockable && !graceActive;

  // Racing Focus Mode: while this racing overlay is up with the option on,
  // the Go side hides every other overlay. Reactive so a live option change
  // (gear or Manage Overlays editor) applies immediately; Go clears the latch
  // itself when the racing window closes.
  $: if (kind === "racing") {
    SetRacingFocusMode(settings.focusmode !== false).catch(() => {});
  }

  // Racing speed-mode base offsets, in percent of viewport width in the 4:3
  // calibration frame (PopoutRacing scales them to the real viewport). Strafe
  // angle is side speed over forward speed, so a faster forward speed pulls
  // the line IN toward centre: Run (slowest) is widest, Bard (fastest) is
  // narrowest. Zero is SoW, which is where the anchors themselves are set.
  //
  // Run and SoW were measured live 2026-08-19; Bard is derived from them —
  // the two measurements pinned the aspect scale, and it carries Bard's old
  // value across with it. Cross-checked against the implied speed ratios
  // (SoW ~1.54x base run, Bard ~2.5x), which are sane for P99. Worth
  // confirming with an actual bard. The user's manual Line offset is fine
  // calibration added on top (see the PopoutRacing prop).
  const RACING_SPEED_OFF = { run: 17.5, sow: 0, bard: -12 };

  // Overlay title-bar visibility mode (app-wide, from Manage Overlays):
  // "always" | "locked" (hide while locked) | "zero" (hide when idle). Trigger
  // overlays honor it in full. The map joins in only while it is actually
  // locked (Stay-unlocked off + Lock Overlays): its title bar is the drag
  // handle, so it never hides on an unlocked map — see titleHidden.
  ...
  // Racing's lines ARE its content and it has no child bind to say so — pin
  // it truthy or the special-overlay empty collapse hides the whole thing
  // once setup mode ends.
  // Racing and Guild Chat never bind it: their window IS the content. A guild
  // chat box is sized and made opaque to sit over the game's own chat window,
  // and an "empty" collapse to the title bar the moment nobody has spoken
  // yet is exactly the wrong thing for it to do.
  let hasContent = kind === "racing" || kind === "guildchat";
  ...
  // Sticky Mode: this overlay opts out of every automatic hide. Held in the Go
  // popout store rather than localStorage, because the hides it overrides are
  // Go-side window Hide() calls — the frontend can't argue with a hidden
  // window. Read once on mount and whenever it's toggled from anywhere.
  let sticky = false;
  async function loadSticky() {
    try {
      sticky = await GetPopoutSticky(kind, category || "");
    } catch {
      /* leave it off — the safe default */
    }
  }
  async function toggleSticky(on) {
    sticky = on; // optimistic: the switch must not lag the finger
    try {
      await SetPopoutSticky(kind, category || "", on);
    } catch {
      loadSticky(); // put it back to whatever Go actually holds
    }
    scheduleFit();
  }
  ...
  // ── placement grace: a fresh pop-out under Lock Overlays ────────────────
  // Go opens it movable (PopoutGrace* in popouts.go) and the overlay counts
  // down GRACE_MS, saying so in the middle of itself; any move or resize
  // restarts the count, and at zero it locks itself where it sits. An
  // explicit global lock or unlock ends it early (events below). Keep
  // GRACE_MS in step with popoutGraceDur in popouts.go.
  const GRACE_MS = 30000;
  let graceLeft = 0; // seconds shown; 0 = no grace running
  let graceUntil = 0;
  let graceTimer;
  let graceGeom = ""; // last sampled position/size, to notice a move
  let graceEl; // the notice card, so fit-to-content can make room for it
  let graceTb = 26; // title bar height at grace start: the notice centres below it
  $: graceActive = graceLeft > 0;
  // Positioning mode: SOME overlay is in its grace (Go announces it), so this
  // one — placed or not — shows its title bar and a red border, and the new
  // overlay can be set against it. The one in grace is in the mode by
  // definition, mount-order aside.
  let positioning = false;
  $: posMode = positioning || graceActive;
  function startGrace(ms) {
    graceUntil = Date.now() + ms;
    graceLeft = Math.max(1, Math.ceil(ms / 1000));
    graceGeom = "";
    graceTb = titleBarHeight();
    clearInterval(graceTimer);
    graceTimer = setInterval(graceTick, 250);
    if (isSpecial) beginSetup(); // keeps an empty special visible to place
  }
  function stopGrace() {
    clearInterval(graceTimer);
    graceTimer = null;
  ...
  async function graceTick() {
    ...
    // Moved or resized → the user is still arranging: 30 more seconds. The
    // height only counts when fit isn't driving it (content growth would
    // otherwise restart the count on its own).
    try {
      const p = await Window.Position();
      const s = await Window.Size();
      const g = `${p.x},${p.y},${s.width},${fitActive ? "" : s.height}`;
      if (graceGeom && g !== graceGeom) {
        graceUntil = Date.now() + GRACE_MS;
        PopoutGraceExtend(kind, category || "").catch(() => {});
      }
      graceGeom = g;
    } catch {
      /* window mid-close */
    ...
    endSetup();
    saveGeometry();
    PopoutGraceEnd(kind, category || "").catch(() => {});
  }
  ...
  // Sticky exempts the overlay from the hide-when-empty mode but NOT from
  // hide-while-locked: the latter is about keeping a placed overlay tidy, not
  // about making it disappear when idle, and someone who locks their layout
  // still wants the title bars out of the way.
  $: titleHidden =
    // Racing Mode hides its title bar once placement is settled: the window
    // is permanently click-through and auto-placed over the game viewport, so
    // the bar would just paint over the game. It stays visible through setup
    // mode (fresh pop-out until lock/zone) so the user can see where the
    // overlay landed.
    // Positioning mode (posMode) overrides every hide: an overlay being placed
    // needs the others' title bars to line up against.
    (kind === "racing" && !setupMode && !flashing && !posMode) ||
    (kind === "map"
      ? // The map hides its title only while LOCKED — a hidden title is the
        // drag handle gone, acceptable solely when the window is click-through
        // anyway. "locked" mode hides it outright; "zero" counts a
        // /loc-timed-out, auto-collapsed map as idle and hides the leftover
        // title strip too — nothing on screen until the next /loc slides the
        // map back down. (An unlocked map never falls in here: `locked`
        // derives from lockable, and hasContent-based hiding doesn't apply —
        // the map has no child pushing content state.)
        locked &&
        !flashing &&
        !posMode &&
        !editingDefaults &&
        (titleMode === "locked" ||
          (titleMode === "zero" && settings.autohide && collapsed))
      : lockable &&
        !flashing &&
        !setupMode &&
        !posMode &&
        ...
        ((titleMode === "locked" && locked) ||
          (titleMode === "zero" && !hasContent && !sticky)));
  ...
  // These settings live only in this window's localStorage, so the daily
  // settings snapshot (settings_snapshot.go) can't read them — push them into
  // Go on every save and once on mount. Go whitelists the keys it keeps and
  // reduces the rest to yes/no there (a chosen cue sound becomes
  // has_pulsesound, a chosen message font msgfont_set), which is why the raw
  // values may be pushed as they are: only `bg` is dropped here, because no
  // overlay's background color has ever been part of the dataset. SETTINGS
  // TELEMETRY (CLAUDE.md): a new per-kind key here also needs its dashboard
  // row.
  function pushOverlayPrefs() {
    const { bg, ...rest } = settings;
    // KEY carries "@<char>" once the profile has resolved; Go keys the entry
    // by it so another character's overlay can't overwrite this one's.
    const char =
      KEY.length > BASEKEY.length ? KEY.slice(BASEKEY.length + 1) : "";
    SetOverlayPrefs(kind, category || "", JSON.stringify(rest), char).catch(
      () => {},
    );
  }
  ...
  // Go is the truth for "Stay unlocked" (it applies the click-through change to
  // the live window itself), so it is read back rather than kept in the
  // localStorage store — at mount, and again whenever Manage Overlays says a
  // setting changed.
  function loadStayUnlocked() {
    if (!stayUnlockedKind) return;
    (kind === "map" ? GetMapStayUnlocked() : GetGuildChatStayUnlocked())
      .then((v) => (stayUnlocked = v))
      .catch(() => {});
  }

  function setStayUnlocked(v) {
    stayUnlocked = v;
    // Go persists it and, if the overlays are locked right now, applies the
    // click-through change to this window on the spot.
    (kind === "map"
      ? SetMapStayUnlocked(v)
      : SetGuildChatStayUnlocked(v)
    ).catch(() => {});
    // That immediate case strands the options panel: the window just went
    // click-through with the panel still open, so its Done button can never
    // be clicked. Close it ourselves (undoing any panel growth too) — Unlock
    // Overlays on the Timers tab is the road back to an interactive overlay.
    if (!v && lockedGlobal) closeSettings();
  }

  function setAOT(v) {
    settings.aot = v;
    saveSettings();
    // Through Go rather than the raw window API: the topmost self-heal
    // (popouts_topmost.go) re-asserts always-on-top when the OS strips it,
    // and must know a deliberate opt-out so it never fights this toggle.
    SetPopoutAlwaysOnTop(kind, category || "", v).catch(() => {});
  }
  ...
  async function saveGeometry() {
    ...
    // Nor the size the gear panel temporarily grew the window to: that is
    // ours, not the user's, and persisting it would make the growth permanent
    // on the next reopen.
    if (settingsGrew) return;
    try {
      const s = await Window.Size();
  ...
  async function gripDown(e) {
    ...
    // A grip drag while the gear panel is open is the user taking the size
    // over: forget the panel's temporary growth so Done leaves their size be.
    if (showSettings) settingsGrew = null;
    try {
      const s = await Window.Size();
  ...
  async function snapCheck() {
    // showSettings: the panel grew the window to an off-grid size on purpose;
    // grid-rounding it here changed the size out from under closeSettings,
    // whose restore then saw a size it didn't set and left the growth in place.
    if (!snapEnabled || resizing || collapsed || collapseAnim || showSettings) {
      lastSnapKey = "";
      return;
    }
    try {
  ...
  // The same families offered for the Guild Chat message text, with the empty
  // entry renamed: there "default" means this overlay's own font, not the
  // app's. Kept in step with FONTS by deriving from it.
  const GC_FONTS = FONTS.map((f) =>
    f.v === "" ? { v: "", label: "Overlay default" } : f,
  );
  // What the Message color swatch shows while nothing is set. A color input
  // cannot render "unset" — it would show black and read as a choice — so it
  // previews the color the messages actually inherit (--text-primary).
  const GC_MSG_COLOR = "#e0e2ea";
  // Message size is 0 (the overlay's own 12.5px) or a legible 8-40px; the
  // in-between sizes are nobody's intent, so a typed 3 becomes 8 rather than
  // three-pixel guild chat. Mirrored by `snap` in the Manage Overlays editor.
  function clampMsgSize(v) {
  ...
  // Sound library for the clerics Cue Sound picker, refreshed each time the
  // gear opens so files added in the main window show up without a restart.
  let mediaFiles = [];
  ...
  async function toggleSettings() {
    ...
    try {
      ...
      // Measured in CSS px; the window is sized in window units, which differ
      // above 100% display scaling (see windowUnitsPerCssPx) — without the
      // conversion the grown window was too short and the panel still clipped.
      const k = windowUnitsPerCssPx(s.height);
      const needW = Math.ceil(
        (Math.max(panelEl.offsetWidth, panelEl.scrollWidth) + 14) * k,
      );
      const needH = Math.ceil(
        (26 + Math.max(panelEl.offsetHeight, panelEl.scrollHeight) + 8) * k,
      );
  ...
  async function closeSettings() {
    ...
    if (!g) return;
    try {
      // Put the size back. A grip drag while the panel was open already
      // cleared settingsGrew (gripDown), so reaching here means nobody but us
      // changed the size. The old "only if it still equals what we set" check
      // was fooled by anything that nudged the size by a few pixels — the
      // snap-to-grid pass did exactly that — and then left the growth in place
      // for good.
  ...
  async function checkAutohide() {
    ...
    // While globally hidden ("Hide overlays" or camp-out/idle), Go fully
  ...
  $: fitActive, hasContent, setupMode, editingDefaults, graceActive, scheduleFit();
  ...
  async function applyFit() {
    ...
    // A placement notice needs room to be read: at least the title bar plus
    // the card and a margin, whatever the (usually empty) content wants —
    // an empty special otherwise fits down to its title bar and the notice
    // is cut off.
    if (graceActive && graceEl) {
      want = Math.max(want, tb + graceEl.offsetHeight + 20);
    }
    try {
      const s = await Window.Size();
  ...
  // Click-to-type: overlay windows are WS_EX_NOACTIVATE, so the OS never
  // gives them keyboard focus on its own — but Chromium still delivers mouse
  // events and runs its internal focus model, so focusin fires when the user
  // clicks a form control. Escalate that one deliberate click to a real
  // Go-side Focus() so typing lands here. document.hasFocus() mirrors OS
  // focus — skip when already active; the timestamp gate stops a focus storm
  // from spamming the bridge (no timer, nothing to clean up). Locked overlays
  // are click-through, so this is unreachable while locked — same as today.
  let lastFocusEsc = 0;
  function onFocusIn(e) {
    ...
    const now = Date.now();
    if (now - lastFocusEsc < 400) return;
    lastFocusEsc = now;
    FocusPopout(kind, category || "").catch(() => {});
  }

  onMount(async () => {
    // App-styled tooltips for the gear panel's titles (lib/tooltip.js).
    installTooltips();
    ...
    // Capture phase so it sees focus entering any nested component (map
    // search, settings inputs) regardless of local handlers.
    window.addEventListener("focusin", onFocusIn, true);
    document.body.style.background = "transparent";
    loadSticky();
    // Sticky can be flipped from Manage Overlays too; Go announces it so an
    // open overlay doesn't keep rendering by its old setting.
    Events.On("popout-sticky", (ev) => {
      const name = ev && ev.data != null ? ev.data : ev;
      if (name === WINDOW_NAME || name === "*") loadSticky();
    });
    // Stored look settings can be edited from Manage Overlays (main window —
    // shared localStorage) too; its NotifyOverlaySettings nudge makes an open
    // overlay re-read them so the change applies live. Only re-merges the
    // store — settings needing side effects (always-on-top) are not edited
    // over there.
    Events.On("popout-settings", (ev) => {
      const name = ev && ev.data != null ? ev.data : ev;
      if (name === WINDOW_NAME || name === "*") {
        const stored = readStore();
        if (stored.settings) settings = { ...settings, ...stored.settings };
        pushOverlayPrefs(); // keep Go's mirror in step with the store
        // "Stay unlocked" isn't in the store — it lives in Go — but it IS
        // editable from Manage Overlays, so re-read it on the same nudge or
        // this window's `lockable` would keep using the old value.
        loadStayUnlocked();
      }
    });
    if (kind !== "map") {
    ...
    if (stored.settings) settings = { ...settings, ...stored.settings };
    pushOverlayPrefs();
    // Window is created always-on-top; apply a stored opt-out (through Go so
    // the topmost self-heal records the intent — see setAOT).
    if (!settings.aot)
      SetPopoutAlwaysOnTop(kind, category || "", false).catch(() => {});
    // Lock (click-through + frozen drag/resize): every overlay tracks the
    // global state, the map and Guild Chat included — whether either OBEYS it
    // is its "Stay unlocked" toggle's call (the `locked` derivation applies it,
    // so flipping the toggle while locked takes effect immediately). Reflect
    // the current state too, for an overlay that opened while already locked.
    ArePopoutsLocked()
      .then((v) => (lockedGlobal = v))
      .catch(() => {});
    // Opened under the lock? Then Go left this window movable for a grace
    // period — count it down.
    PopoutGraceMs(kind, category || "")
      .then((ms) => {
        if (ms > 0) startGrace(ms);
      })
      .catch(() => {});
    // Another overlay may already be in its grace when this one mounts.
    ArePopoutsPositioning()
      .then((v) => (positioning = !!v))
      .catch(() => {});
    Events.On("popouts-positioning", (ev) => {
      positioning = !!(ev && ev.data != null ? ev.data : ev);
    });
    Events.On("popouts-locked", () => {
      lockedGlobal = true;
      endSetup(); // locking means "I'm done arranging these"
      stopGrace(); // Go locked every window, this one included
    });
    Events.On("popouts-unlocked", () => {
      lockedGlobal = false;
      stopGrace(); // Go cancelled the grace along with the lock
    });
    loadStayUnlocked();
    // Title-bar visibility mode (live-updated from Manage Overlays). The map
    // loads it too, but only consults it while locked — see titleHidden.
    const loadTitleMode = () =>
      GetOverlayTitleMode()
        .then((m) => (titleMode = m))
        .catch(() => {});
...
<div class="popout" class:positioning={posMode}>
  {#if graceActive}
    <!-- Placement grace notice: centred in the area below the title bar,
         never in the way of a drag. Flashes through the last five seconds
         so the lock doesn't land unannounced. -->
    <div
      class="grace"
      class:urgent={graceLeft <= 5}
      bind:this={graceEl}
      style="top: calc(50% + {graceTb / 2}px); --wails-draggable: no-drag"
    >
      <div class="grace-t">Locking overlay in {graceLeft}s</div>
      <div class="grace-s">Position and resize now.</div>
    </div>
  {/if}
  <!-- title bar: the draggable region (frozen when locked); buttons opt out.
       Native OS dragging again since the NOACTIVATE unwind (v2.5.3469) —
       overlays are normal activatable windows, which is what the OS move
       loop requires (see popouts_noactivate.go for the saga).
  ...
  >
    ...
    <span class="tb-title" class:defaults={editingDefaults}>{dispTitle}</span>
    {#if editingDefaults}
      <span
        class="tb-defaults"
  ...
  >
    {#if kind === "map"}
      <MapTab popout={true} bgFill={bgRgba} {locked} />
    {:else if kind === "raidassign"}
      <PopoutRaidSection
        section="assign"
        flash={settings.flash !== false}
        bind:hasContent
      />
    ...
    {:else if kind === "raidclerics"}
      <PopoutRaidSection
        section="clerics"
        flash={settings.flash !== false}
        showTiming={settings.timing !== false}
        pulseAudio={settings.pulseaudio === true}
        pulseSound={typeof settings.pulsesound === "string"
          ? settings.pulsesound
          : "intuition-561.mp3"}
        bind:hasContent
      />
    ...
    {:else if kind === "voicespeakers"}
      <PopoutVoiceSpeakers
        smartHide={settings.smarthide !== false}
        bind:hasContent
      />
    {:else if kind === "randoms"}
      <PopoutRandoms bind:hasContent />
    {:else if kind === "threat"}
      <PopoutThreat bind:hasContent showBreakdown={!!settings.breakdown} />
    {:else if kind === "raiddps"}
      <PopoutRaidDPS bind:hasContent bind:targetName={raidDpsTarget} />
    {:else if kind === "guildchat"}
      <!-- No hasContent bind: the box is the content (see hasContent above),
           so a quiet guild never collapses it to the title bar. -->
      <PopoutGuildChat
        showEngages={settings.engages !== false}
        newestTop={!!settings.newesttop}
        msgFont={settings.msgfont || ""}
        msgBold={!!settings.msgbold}
        msgSize={+settings.msgsize || 0}
        msgColor={settings.msgcolor || ""}
        windowName={WINDOW_NAME}
        {locked}
      />
    {:else if kind === "racing"}
      <!-- No hasContent bind: the lines ARE the content, and racing's
           fit/title behaviors are forced off regardless. -->
      <PopoutRacing
        sides={settings.sides || "both"}
        avgSec={+settings.avg || 3}
        offsetPct={(RACING_SPEED_OFF[settings.speed] ?? RACING_SPEED_OFF.sow) +
          (+settings.offset || 0)}
        showSpeedo={settings.speedo !== false}
      />
    {:else if kind === "alerts"}
  ...
  {#if showSettings}
    <div class="set-panel" bind:this={panelEl}>
      ...
      <!-- Color + opacity on one line: color box first, slider filling the
           rest. Same pattern as the category style rows below. -->
      <label class="set-row">
      ...
      <!-- Every on/off here is the same gold slider the right-click editor in
           Manage Overlays uses. -->
      ...
      {#if kind === "map"}
        <label
          class="set-row chk"
          title="Keep the map interactive (pan/zoom/search) when Lock Overlays is pressed on the Timers tab. Uncheck to have the map lock and turn click-through like every other overlay — Unlock Overlays over there frees it again."
        >
          <span class="psw">
            <input
              type="checkbox"
              checked={stayUnlocked}
              on:change={(e) => setStayUnlocked(e.target.checked)}
            />
            <span class="psw-track"><span class="psw-thumb"></span></span>
          </span>
          ...
          <span class="psw">
            <input
              ...
              on:change={(e) => {
                ...
                checkAutohide();
                AuditEvent(
                  "app_updatedmapsettings_autohide_" +
      ...
      {/if}
      {#if kind === "raidassign" || kind === "raiddebuffs" || kind === "raidclerics"}
        <label
          class="set-row chk"
          title="Pulse animations: tank proc counters, sieve counts, and the CH cast-start flash. Off = every number and bar still updates, nothing blinks."
        >
          <span class="psw">
            <input
              type="checkbox"
      ...
      {#if kind === "raidclerics"}
        <label
          class="set-row chk"
          title="Show “+3.4s” after each cleric in the CH chain — how long after the previous caster they started their cast. Shown for every cleric, refreshed on each new cast."
        >
          <span class="psw">
            <input
              type="checkbox"
              checked={settings.timing !== false}
              on:change={(e) => {
                settings.timing = e.target.checked;
                saveSettings();
              }}
            />
            <span class="psw-track"><span class="psw-thumb"></span></span>
        ...
        <label
          class="set-row chk"
          title="Play one audio cue the moment the cleric before you starts casting — the audible version of your gold pre-cast ring appearing."
        >
          <span class="psw">
            <input
              type="checkbox"
              checked={settings.pulseaudio === true}
              on:change={(e) => {
                settings.pulseaudio = e.target.checked;
                saveSettings();
              }}
            />
            <span class="psw-track"><span class="psw-thumb"></span></span>
        ...
        {#if settings.pulseaudio === true}
          {@const curSound =
            typeof settings.pulsesound === "string"
              ? settings.pulsesound
              : "intuition-561.mp3"}
          <label
            class="set-row"
            title="Sound played when the cleric before you starts casting — any of your trigger sounds."
          >
            <span>Cue Sound</span>
            <select
              class="set-sel"
              value={curSound}
              on:change={(e) => {
                settings.pulsesound = e.target.value;
                saveSettings();
              }}
            >
              <option value="">— none —</option>
              {#each curSound && !mediaFiles.includes(curSound) ? [curSound, ...mediaFiles] : mediaFiles as m (m)}
            ...
            <button
              ...
              disabled={!curSound}
              on:click={() => PlayTriggerMediaSample(curSound)}
            >
        ...
        {/if}
      {/if}
      {#if kind === "voicespeakers"}
        <label
          class="set-row chk"
          title="Only show this overlay while YOU are in the raid voice channel. When you're not on comms it hides itself completely, and it reappears on its own when you join."
        >
          <span class="psw">
            <input
              type="checkbox"
              checked={settings.smarthide !== false}
              on:change={(e) => {
                settings.smarthide = e.target.checked;
                saveSettings();
              }}
            />
            <span class="psw-track"><span class="psw-thumb"></span></span>
          </span>
          <span>Smart Hide</span>
        </label>
      {/if}
      {#if kind === "guildchat"}
        <label
          class="set-row chk"
          title="Include the raid-mob engage lines the bot posts to #guild-stream (who pulled what, ours and other guilds')."
        >
          <span class="psw">
            <input
              type="checkbox"
              checked={settings.engages !== false}
              on:change={(e) => {
                settings.engages = e.target.checked;
                saveSettings();
              }}
            />
            <span class="psw-track"><span class="psw-thumb"></span></span>
          </span>
          <span>Show Engages</span>
        </label>
        <label
          class="set-row chk"
          title="Show the newest messages at the top instead of the bottom. Off, it reads like the game's chat window: newest at the bottom, scroll up for older lines."
        >
          <span class="psw">
            <input
              type="checkbox"
              checked={!!settings.newesttop}
              on:change={(e) => {
                settings.newesttop = e.target.checked;
                saveSettings();
              }}
            />
            <span class="psw-track"><span class="psw-thumb"></span></span>
          </span>
          <span>Reverse Messages</span>
        </label>
        <!-- The message text's own font, weight and color, plus the line's
             text size. Deliberately separate settings rather than one style:
             font, bold and color dress the message alone, so the columns to
             its left keep their look and the ledger keeps its shape, while
             size applies to the whole line — bigger chat means bigger chat,
             clock included.

             Bold rides on the font row rather than taking one of its own: it
             is the same "how is the message dressed" decision, and this panel
             is read inside a small overlay window where every row costs
             height. A div, not a label, because the row now holds two controls
             and a label would hand every click on its text to the first of
             them; the select carries its own aria-label instead. -->
        <div
          class="set-row"
          title="Font for the message text only — the time, the officer/bot mark and the name keep the overlay's font."
        >
          <span>Message font</span>
          <!-- A plain checkbox, not the .psw slider the switch rows use: the
               slider is 30px wide and as tall as the row, and there is no room
               for one beside a font dropdown. -->
          <label class="set-bold" title="Bold message text.">
            <input
              type="checkbox"
              checked={!!settings.msgbold}
              on:change={(e) => {
                settings.msgbold = e.target.checked;
                saveSettings();
              }}
            />
            <span>Bold</span>
          ...
          <select
            ...
            value={settings.msgfont || ""}
            on:change={(e) => {
              settings.msgfont = e.target.value;
              saveSettings();
            }}
          >
            {#each GC_FONTS as f (f.v)}
        ...
        <label
          class="set-row"
          title="Size in px for the chat lines: the message text and, scaled with it, the time, the officer/bot mark and the name. Blank or 0 keeps the overlay's size."
        >
          <span>Text size</span>
          ...
          <input
            ...
            min="0"
            max="40"
            value={+settings.msgsize || 0}
            on:change={(e) => {
              settings.msgsize = clampMsgSize(e.target.value);
              e.target.value = settings.msgsize;
              saveSettings();
            }}
          />
          <span class="set-val">px</span>
        </label>
        <label
          class="set-row"
          title="Color for the message text only. Engage lines stay gold. Default follows the overlay's text color."
        >
          <span>Message color</span>
          <input
            type="color"
            value={settings.msgcolor || GC_MSG_COLOR}
            on:input={(e) => {
              settings.msgcolor = e.target.value;
              saveSettings();
            }}
          />
          <button
            type="button"
            class="set-clear"
            title="Clear the custom color — messages follow the overlay's text color again."
            disabled={!settings.msgcolor}
            on:click={() => {
              settings.msgcolor = "";
              saveSettings();
            }}>Default</button
        ...
        <label
          class="set-row chk"
          title="Keep this overlay movable and scrollable when Lock Overlays is pressed on the Timers tab. Uncheck to have it lock and turn click-through like every other overlay: it then shows only the most recent messages and can't be scrolled."
        >
          <span class="psw">
            <input
              type="checkbox"
      ...
      {#if kind === "threat"}
        <label
          class="set-row chk"
          title="Add a bar under the DPS number splitting this fight's damage by type — melee verbs, kick, backstab, bow/throw, damage shield, spell — biggest first."
        >
          <span class="psw">
            <input
              type="checkbox"
              checked={settings.breakdown}
              on:change={(e) => {
                settings.breakdown = e.target.checked;
                saveSettings();
              }}
            />
            <span class="psw-track"><span class="psw-thumb"></span></span>
      ...
      {#if cat}
        ...
        <!-- A color and its opacity share one line — color box first, the
             slider filling the rest — matching the Manage Overlays form. -->
        {#if kind === "timers"}
          <label class="set-row">
            <span>Bar color</span>
            <input
              type="color"
              bind:value={cat.bar_color}
              on:input={catChanged}
            />
            <input
              type="range"
        ...
        {#if kind === "timers"}
          <label class="set-row">
            <span>On leave / swap</span>
            <select
              class="set-sel"
              value={cat.carry_timers
                ? "keep"
                : cat.auto_pause
                  ? "pause"
                  : "end"}
              on:change={(e) => {
                ...
                cat.auto_pause = m === "pause";
                cat.carry_timers = m === "keep";
                catChanged();
              }}
            >
              <option value="end">End timers</option>
          ...
          <div class="set-hint">
            {#if cat.carry_timers}
              Mob respawns: bars count down in real time on every character,
              even while the app is closed.
            {:else if cat.auto_pause}
              Buffs: bars freeze when you leave and resume on that character's
              next login.
            {:else}
              Timers end when the character leaves the world or you swap toons.
            {/if}
          </div>
          <label
            class="set-row chk"
          ...
          >
            <span class="psw">
              <input
                type="checkbox"
                bind:checked={cat.flash_ended_early}
                on:change={catChanged}
              />
              <span class="psw-track"><span class="psw-thumb"></span></span>
            </span>
            <span>Flash if ended early</span>
          </label>
        {/if}
        {#if kind === "alerts"}
          <label class="set-row">
            <span>Time shown (s)</span>
            <input
              class="set-num"
              type="number"
              min="1"
              max="3600"
              bind:value={cat.alert_seconds}
              on:input={catChanged}
            />
          </label>
          <label
          ...
          >
            <span class="psw">
              <input
                type="checkbox"
                bind:checked={cat.alert_stopwatch}
                on:change={catChanged}
              />
              <span class="psw-track"><span class="psw-thumb"></span></span>
            </span>
            <span>Show age (m:ss)</span>
          </label>
          <label
            class="set-row chk"
      ...
      <!-- Sticky Mode, last. Everything above adjusts how this overlay looks;
           this one changes when it exists, so it sits at the bottom in its own
           red field rather than in the run of preferences. -->
      <div class="sticky-box">
        <label class="sw-row">
          ...
          <span class="sw">
            <input
              type="checkbox"
              checked={sticky}
              on:change={(e) => toggleSticky(e.target.checked)}
            />
        ...
        <div class="sticky-warn">
          ...
          button on the Timers page still puts it away.
        </div>
      </div>
...
<style>
  ...
  /* The DPS-breakdown switch: gold when on, matching the Manage Timers
     enable switches (the panel's other rows keep native checkboxes). The
     thumb is absolutely positioned — a margin-based dot collapses its top
     margin through the track and rides high instead of centering. */
  .psw {
    position: relative;
    flex: none;
    line-height: 0;
  ...
  .psw input:checked + .psw-track {
    background: rgba(200, 169, 81, 0.45);
  }
  .psw input:checked + .psw-track .psw-thumb {
    left: 16px;
  ...
  /* Sticky Mode: its own faint red field so it reads as a deliberate override
     rather than another preference in the list. */
  .sticky-box {
    margin: 6px 0 2px;
    padding: 6px 8px;
  ...
  /* "Bold" beside the Message font dropdown: box and word as one small target,
     sized down so the pair still fits on one row of a narrow gear panel. */
  .set-bold {
    display: inline-flex;
    align-items: center;
    gap: 4px;
  ...
  /* One-line explainer under the timer-behavior selector (mirrors the app-side
     dialog's note, trimmed for the compact gear panel). */
  .set-hint {
    font-size: 10px;
    line-height: 1.4;
    color: var(--text-muted);
  ...
  /* Cue Sound sample button — same footprint as a psw switch row control. */
  .set-play {
    background: none;
    border: 1px solid var(--border);
    border-radius: 4px;
  ...
  /* "Default" next to a color swatch — the only way back to "not set", since
     a color input has no empty state. Same footprint as the sample button. */
  .set-clear {
    background: none;
    border: 1px solid var(--border);
    border-radius: 4px;
  ...
  /* Placement grace notice — see startGrace/graceTick in the script. */
  .grace {
    position: absolute;
    left: 50%;
    top: 50%;
  ...
  .grace-s {
    color: var(--text-secondary, #7880a0);
    font-size: 11px;
  }
  /* Positioning mode: a dark red frame on EVERY overlay while one is being
     placed. A pseudo-element rather than a border or inset shadow so it paints
     over the title bar and content — children would cover either of those. */
  .popout.positioning::after {
    content: "";
    position: absolute;
    inset: 0;
  ...
  /* Final five seconds: a steady 1Hz flash between the normal card and a
     red-bordered one — the same red the locate flash uses. */
  .grace.urgent {
    animation: grace-flash 1s steps(2, jump-none) infinite;
  }
  @keyframes grace-flash {
    from {
