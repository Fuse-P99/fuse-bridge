<script>
  ...
  // Forwarded-message toggles (moved here from the old Relay tab). Each tip
  // says which features depend on the box being checked — sourced from what
  // the filter actually gates (filter.go) and what the server does with it.
  const fwdOptions = [
    {
      key: "guild_chat",
      label: "Guild Chat",
