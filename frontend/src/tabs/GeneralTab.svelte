<script>
  ...
  import {
    GetStatus, GetSettings, SaveSettings, GetAutoStart,
    SetAutoStart, BrowseEQDirectory, GetAvailableUpdate, StartUpdate
  ...
  async function checkUpdate() {
    try { updateVersion = await GetAvailableUpdate() || '' } catch { updateVersion = '' }
  }
  ...
  onDestroy(() => { clearInterval(interval); clearInterval(updateInterval) })
  ...
  async function browseDir() {
    ...
    if (result === 'INVALID') {
      dirError = "That folder doesn't look like an EverQuest install (no eqgame.exe found). Pick the folder containing eqgame.exe."
      return
    }
    if (result) { eqDir = result; dirError = '' }
  }
...
<div class="general">
    ...
    <div class="panel-body">
      <div class="status-rows">
        ...
        <div class="status-row">
          ...
          {#if updateVersion}
            <button class="btn update-btn" disabled={updating} on:click={doUpdate}>
  ...
  <label class="check-label" title="Runs the built-in P99 login proxy (p99-login-middlemand) and points eqhost.txt at it. Fixes the 'server list fails to populate' login issue. Unchecking restores your original eqhost.txt.">
    <input type="checkbox" checked={useMiddlemand} on:change={toggleMiddlemand} />
...
<style>
  ...
  .update-btn:hover:not(:disabled) { background: #f0b429; }
