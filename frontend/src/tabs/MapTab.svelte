...
<script>
  ...
  async function loadZone(zone) {
    zoneName = zone;
    // Clear the trail and temporary waypoints only when the actual zone changes —
    // not when the component remounts (tab switch) for the same zone.
    if (zone !== mTrailZone) {
      mTrail = [];
      mTempPOIs = [];
      mTrailZone = zone;
  ...
  function draw() {
    ...
    ctx.font = "10px sans-serif";
    ctx.textAlign = "center";
    for (const p of permPOIs.concat(tempPOIs)) {
    ...
    <div class="poi-ui">
      {#if poiMode === "add"}
        <div class="poi-panel">
          ...
          <label class="poi-chk">
            <input type="checkbox" bind:checked={poiPerm} /> Save marker permanently
          </label>
        ...
        <div class="poi-panel">
          ...
          {#each allPOIs as p}
            <button class="poi-item" on:click={() => deletePOI(p)}>
              ...
              {p.name}
              <span class="poi-tag">{p.perm ? "saved" : "temp"}</span>
            </button>
          {:else}
