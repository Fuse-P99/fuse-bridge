<script context="module">
  ...
  // "Show Radius" selection: "" (off) | a track class | "Leash" | "Custom".
  let mTrackClass = "";
  let mCustomRadius = 500; // units drawn when the selection is "Custom"
...
<script>
  ...
  export let bgFill = "";
  // Popout only: the overlay lock has made this window click-through. The
  // bottom-left tool row (POIs, track range, copy loc, grid) is unreachable
  // then, so it's hidden rather than left painted over the game; any open
  // tool panel or marker placement mode is cancelled the moment the lock
  // lands. The main-window Map tab never sets this.
  ...
  let trackClass = mTrackClass; // radius-ring selection, "" = off
  let customRadius = mCustomRadius; // units for the "Custom" selection
  $: mCustomRadius = customRadius;
  ...
  // Feature-usage audit for the map settings (audit.go):
  // app_updatedmapsettings_<setting>_<value>. Fire-and-forget — a failed post
  // never touches the map.
  function auditMap(setting, value) {
    AuditEvent(`app_updatedmapsettings_${setting}_${value}`, "").catch(
      () => {},
    );
  }
  // The map settings live only in this webview (module state + localStorage),
  // so the daily settings snapshot (settings_snapshot.go) can't read them —
  // push them into Go whenever they change. Main-window instance only: the
  // overlay's copy of this component has its own defaults and must not
  // overwrite the real choices.
  function pushMapPrefs() {
    if (popout) return;
    SetOverlayPrefs(
      "maptab",
      "",
      JSON.stringify({
        ...
        radius: trackClass || "",
        custom_radius: Number(customRadius) || 0,
        grid: gridAlways ? "always" : gridSession ? "session" : "off",
      }),
    ).catch(() => {});
  }
  ...
  function toggleTrail(e) {
    ...
    auditMap("trail", showTrail ? "on" : "off");
    pushMapPrefs();
    requestDraw();
  }
  ...
  function toggleFocusLevel(e) {
    ...
    auditMap("focuslevel", focusLevel ? "on" : "off");
    pushMapPrefs();
    requestDraw();
  }
  function resetTrail() {
  ...
  // ── radius ring ──────────────────────────────────────────────────────────────
  // A ring centered on the local player, refreshed each /loc. Fixed options are
  // the three /who-track classes plus a mob leash range; "Custom" draws whatever
  // the user types. Radii are in map (loc) units.
  const TRACK_OPTIONS = [
    { name: "Bard", radius: 100 * 7 },
    { name: "Druid", radius: 125 * 10 },
    { name: "Ranger", radius: 200 * 12 },
    { name: "Leash", radius: 1400 },
  ];
  const TRACK_RADII = { Bard: 100 * 7, Druid: 125 * 10, Ranger: 200 * 12, Leash: 1400 };

  // Resolves the current selection to a unit count: a fixed option's radius, or
  // the typed Custom value (0 when blank/invalid, which draws nothing).
  function curTrackRadius() {
    if (trackClass === "Custom") {
      const v = Number(customRadius);
      return v > 0 ? v : 0;
    }
    return TRACK_RADII[trackClass] || 0;
  }
  ...
  function selectTrackClass(name) {
    ...
    mTrackClass = name;
    trackMenuOpen = false;
    auditMap("radius", name ? name.toLowerCase() : "off");
    pushMapPrefs();
    requestDraw();
  }
  // Custom keeps the menu open so the user can type/adjust the value.
  function selectCustom() {
    trackClass = "Custom";
    mTrackClass = "Custom";
    auditMap("radius", "custom");
    pushMapPrefs();
    requestDraw();
  }
  ...
  // The typed value is audited on change (focus leaves the box), not per
  // keystroke.
  function onCustomChange() {
    const n = Number(customRadius);
    if (n > 0) auditMap("radius", `custom_${Math.round(n)}`);
    pushMapPrefs();
  }
  ...
  async function loadZoneInner(zone) {
    ...
    fitView();
    if (popout && canvas) {
      // Overlay: never the whole-zone fit — a small zone comes up hugely
      // zoomed in, a big one unreadably out, and either way every zone looked
      // different. Every zone starts at the follow zoom (the widest window
      // dimension spans FOLLOW_SPAN locs), centered on the zone until the
      // first /loc fix pulls the view onto the player.
      applyFollowScale();
      const W = canvas.width,
        H = canvas.height;
  ...
  function onMouseDown(e) {
    // Recorded before the overlay early-out: a click on a quest flag is the
    // one thing an overlay map does answer to (see onMouseUp).
    downMX = e.clientX;
    downMY = e.clientY;
    if (popout) return; // overlay map isn't pannable
    dragging = true;
    lastMX = e.clientX;
    lastMY = e.clientY;
  }

  // The quest flag under a canvas point, if any — the same placement math as
  // the draw (a Center flag sits at the map's geometric center). The flag is a
  // 12px pole with a pennant off to the right, so the hit box is a generous
  // pad around it rather than the exact pixels. Only flags that know their
  // quest are clickable.
  function questMarkAt(pt) {
    if (!pt) return null;
    for (const p of questMarks) {
      if (!p.quest_id) continue;
      const x =
          p.center && bounds
            ? sx((bounds.minX + bounds.maxX) / 2)
            : sx(playerX(p.x)),
        y =
          p.center && bounds
            ? sy((bounds.minY + bounds.maxY) / 2)
            : sy(playerY(p.y));
      if (pt.x >= x - 8 && pt.x <= x + 14 && pt.y >= y - 18 && pt.y <= y + 4)
        return p;
    }
    return null;
  }
  ...
  function onMouseUp(e) {
    ...
    // A click on a quest flag opens that character's quest (Characters →
    // Quests). Works in the overlay too; checked ahead of marker placement so
    // a flag under the cursor wins.
    if (isClick && poiMode !== "add") {
      const hit = questMarkAt(canvasPoint(e));
      if (hit) {
        OpenCharacterQuest(hit.char || "", hit.quest_id);
        return;
      }
    }
    ...
    if (wasPress && poiMode === "add" && isClick) {
  ...
  // Follow: center on the player and zoom so the WIDEST canvas dimension spans
  // FOLLOW_SPAN loc units — ~500 in each direction from the player, across
  // whatever size the user gave their minimap. Keyed on the widest edge so a
  // tall or wide overlay still shows that much in its long direction.
  const FOLLOW_SPAN = 1000;
  ...
  function applyFollowScale() {
    if (!canvas) return;
    const widest = Math.max(canvas.width, canvas.height);
    if (widest > 0) scale = widest / FOLLOW_SPAN;
    followZoomAuto = true;
  }
  function recenter() {
  ...
  onMount(async () => {
    ...
    accentColor =
      getComputedStyle(document.documentElement)
        ...
        .trim() || accentColor;
    loadGridPref();
    pushMapPrefs(); // seed the snapshot store with whatever is in effect
    ...
    <div class="poi-ui">
      ...
      {#if trackMenuOpen}
        <div class="poi-panel track-panel">
          <div class="poi-title">Show Radius</div>
          {#each TRACK_OPTIONS as c}
            <button
              class="poi-item"
          ...
          <button
            class="poi-item"
            class:sel={trackClass === "Custom"}
            on:click={selectCustom}
          >
            Custom
            <span class="poi-tag">your own radius</span>
          </button>
          {#if trackClass === "Custom"}
            <div class="track-custom">
              <input
                class="track-custom-in"
                type="number"
                min="1"
                step="1"
      ...
      {/if}
      <div class="tool-row" class:toolhidden={locked}>
        <button
          class="btn poi-flag"
        ...
        {#if isOfficer}
          <button
            ...
            on:click={startStrobe}><Icon name="siren" /></button
          >
        {/if}
      </div>
    </div>
...
<style>
  ...
  /* Locked popout: the window is click-through, so the tools are dead weight
     painted over the game — gone entirely, not dimmed. */
  .tool-row.toolhidden {
    display: none;
  }
  .strobe-btn.on {
  ...
  /* The Custom radius entry row, shown under the Custom option. */
  .track-custom {
    display: flex;
    align-items: center;
    gap: 6px;
