...
<script>
  ...
  // the admin sample rows never count.
  $: yourMob = data && data.mobs ? (data.mobs.find(isYou) || {}).name : undefined;
          ...
          <div class="mob">
            ...
            {#if m.trackers && m.trackers.length}
              <div class="mob-trackers">
                {#each m.trackers as t, i}{i > 0 ? ", " : ""}<span
                    class="trk-name"
                    class:you={t.is_you}
                    role="button"
                    tabindex="-1"
        ...
        {#each inWindow as m}
          <div class="mob">
            <div class="mob-head">
              ...
              <span
                class="hov-wrap"
                class:show={isYou(m) || pendingStartOn(pending, m)}
              >
                <span
                  class="hov-trigger"
                  class:on={isYou(m) || pendingStartOn(pending, m)}
                >
                  {#if isYou(m)}{myRole(m)} ✓{:else if pendingStartOn(pending, m)}{pending.role}
                    …{:else}+ sign up{/if}
                </span>
                <div class="hov-menu">
                  <div class="hm-body">
                    {#if isYou(m) || pendingStartOn(pending, m)}
            ...
            {/if}
            {#if pending && pending.mob === m.name}
              <div class="pending-note" class:err={pending.state === "error"}>
                ...
                {:else}
                  {pending.msg}
                  <button class="pn-x" on:click={() => setPending(null)}
                    >×</button
                  >
                {/if}
              </div>
