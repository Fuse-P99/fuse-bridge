<script>
  ...
  // The six main slots call in rotation every 2.5s — a realistic chain cadence
  // that keeps several of the 10s casts overlapping — and the rampage pair
  // alternates every 10s. The dead slot never casts, exactly as at a raid.
  ...
  function fireSampleChain() {
    ...
    chainIdx++;
    if (!s.dead) s.called_at_ms = Date.now();
  ...
  $: inWindow =
    data && data.mobs ? data.mobs.filter((m) => m.status === "in_window") : [];
  $: upcoming =
          ...
          <div class="mob">
            ...
            {#if m.trackers && m.trackers.length}
              <div class="mob-trackers">
                {#each m.trackers as t, i}{i > 0 ? ", " : ""}{trackerLabel(
                    t,
                  )}{/each}
              </div>
            {/if}
