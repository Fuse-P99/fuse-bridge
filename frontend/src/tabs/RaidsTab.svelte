<script context="module">
  ...
  // The last raid deep-link request acted on (nav.js raidDeepLink.seq), so a
  // later remount doesn't replay it.
  let savedDeepSeq = 0;
</script>
...
<script>
  ...
  import { activeTab, raidDeepLink } from "../lib/nav.js";
  ...
  import GuildChatPanel from "../lib/GuildChatPanel.svelte";
  ...
  let loading = true;
  let admin = false;
  // Which EQ world we're on: raids are a Blue-only feature, so off-home we show
  // a paused note instead of a verification error (nothing is fetched there).
  let srv = { home: true, name: "" };
  let officer = false;
  ...
  const closeMenus = (e) => {
    trackerCtx = null;
    // The chat style popover sits OUTSIDE the header row (so its controls
    // don't trip the row's own click handler), which means its clicks reach
    // this window listener. Only a click landing somewhere else closes it —
    // and the gear itself stops propagating, so it can toggle in peace.
    const t = e && e.target;
    if (!(t && t.closest && t.closest(".gc-pop"))) chatStyleOpen = false;
  };
  ...
  // Deep link from a batphone's swords (by mob) or the Zones tab's (by zone):
  // expand the matching card, scroll it into view, flash it once. Acts once
  // per request seq, and only once the board has loaded — the request usually
  // arrives before the first poll returns. No match: the tab stays as it is.
  let boardEl;
  let deepSeq = savedDeepSeq;
  let hitKey = "";
  let hitTimer;
  function raidKeyFor(d, list, { mob, zone }) {
    const lm = (mob || "").toLowerCase();
    const lz = (zone || "").toLowerCase();
    if (!lm && !lz) return "";
    for (const m of list) {
      if (!m.is_raid) continue;
      const c = m.raid || {};
      const name = (m.name || "").toLowerCase();
      const target = (c.target || "").toLowerCase();
      const z = (c.zone || "").toLowerCase();
      if ((lm && (name === lm || target === lm)) || (lz && z === lz)) {
        return m.name;
      }
    }
    const ev = d && d.event_raid;
    if (ev && lz && (ev.zone || "").toLowerCase() === lz) {
      return "event:" + (ev.event_key || "event");
    }
    return "";
  }
  async function revealRaid(key) {
    if (!(openRaid[key] ?? true)) {
      openRaid = { ...openRaid, [key]: true };
      savedOpenRaid = openRaid;
    }
    await tick();
    const el =
      boardEl && boardEl.querySelector(`[data-raid="${CSS.escape(key)}"]`);
    if (!el) return;
    el.scrollIntoView({ block: "start", behavior: "smooth" });
    hitKey = key;
    clearTimeout(hitTimer);
    hitTimer = setTimeout(() => (hitKey = ""), 1800);
  }
  $: if (data && $raidDeepLink.seq && $raidDeepLink.seq !== deepSeq) {
    deepSeq = savedDeepSeq = $raidDeepLink.seq;
    const key = raidKeyFor(data, popped, $raidDeepLink);
    if (key) revealRaid(key);
  }
  ...
  async function load() {
    ...
    try {
      srv = await GetCurrentServer();
    } catch {
      /* older build without the binding — assume home */
    }
    loading = false;
  ...
  // ── Guild Chat panel ─────────────────────────────────────────────────────
  // The guild's live chat feed in a collapsible box under the board: the
  // conversation is usually ABOUT what is on the board, so the two belong on
  // one screen.
  //
  // Open/closed, the box's height and the two view toggles are all written to
  // the browser store. This component is destroyed on every tab switch, so
  // anything not written down is lost the moment the user looks at Triggers.
  const CHAT_OPEN_KEY = "fuse.raids.chat.open";
  const CHAT_H_KEY = "fuse.raids.chat.h";
  const CHAT_ENGAGES_KEY = "fuse.raids.chat.engages";
  const CHAT_NEWEST_KEY = "fuse.raids.chat.newest";
  const CHAT_STYLE_KEY = "fuse.raids.chat.style";
  const CHAT_MIN = 100;
  const CHAT_MAX = 600;
  // Roughly how tall the style popover is, in the same CSS px the panel height
  // is measured in. Only used to decide which side of the header it opens on,
  // so an estimate is enough — and erring high flips it upward early, which is
  // the harmless direction.
  const CHAT_POP_H = 150;
  // What a color input shows while nothing is set. It cannot render "unset" —
  // it would show black and read as a choice — so it previews the color the
  // messages actually get, which is white.
  const CHAT_MSG_COLOR = "#ffffff";
  const clampChatH = (v) =>
    Math.min(CHAT_MAX, Math.max(CHAT_MIN, Math.round(v)));
  // Ten lines of chat AT THE CURRENT TEXT SIZE: each row is line-height 1.35
  // of the font size plus the 2px row gap, and the box adds 6px of padding top
  // and bottom. All CSS px — the shell's UI scale zooms the whole window on
  // top (lib/scale.js), so the panel must never measure itself against the
  // screen (no vh) or a Large UI would get ten lines' worth of pixels showing
  // six lines of text.
  const tenLineH = (size) => Math.round(10 * ((size || 12.5) * 1.35 + 2) + 12);
  let chatOpen = false;
  let chatEngages = true;
  let chatNewest = false;
  let chatStyle = { font: "", bold: false, size: 0, color: "" };
  // A height the user has actually chosen. Until there is one the panel opens
  // to ten lines and tracks the text size; the moment they drag the splitter
  // their height wins and stops following it.
  let chatHSaved = false;
  let chatH = tenLineH(0);
  try {
    chatOpen = localStorage.getItem(CHAT_OPEN_KEY) === "1";
    const v = parseInt(localStorage.getItem(CHAT_H_KEY) || "", 10);
    if (!isNaN(v)) {
      chatH = clampChatH(v);
      chatHSaved = true;
    }
    chatEngages = localStorage.getItem(CHAT_ENGAGES_KEY) !== "0";
    chatNewest = localStorage.getItem(CHAT_NEWEST_KEY) === "1";
    const st = JSON.parse(localStorage.getItem(CHAT_STYLE_KEY) || "{}") || {};
    chatStyle = {
    ...
    /* storage blocked or a corrupt blob — the defaults stand for this session */
  }
  // The ten-line default follows the text size until a height is persisted.
  $: if (!chatHSaved) chatH = clampChatH(tenLineH(chatStyle.size));
  function saveChat(key, value) {
    try {
  ...
  function toggleChat() {
    chatOpen = !chatOpen;
    saveChat(CHAT_OPEN_KEY, chatOpen ? "1" : "0");
    // A closed panel has no text to style, and leaving the popover armed would
    // pop it open again with the panel.
    if (!chatOpen) chatStyleOpen = false;
  }
  function chatHeadKey(e) {
    // The row itself only: the pills inside it answer to their own Enter and
    // Space, and that keystroke must not also open or close the panel.
    if (e.target !== e.currentTarget) return;
    if (e.key !== "Enter" && e.key !== " ") return;
    e.preventDefault();
    toggleChat();
  }
  // SETTINGS TELEMETRY (CLAUDE.md): the view toggles and the text style join
  // the daily snapshot the way an overlay's gear settings do, under the kind
  // "raidchat" with no category and no character — the panel is one box on one
  // tab, not a per-character overlay. Pushed on mount and on every change,
  // since Go cannot read the browser store itself.
  //
  // The style keys travel as themselves only for msgbold; the font, the color
  // and the size are reduced to "is it customised at all" on the Go side
  // (sanitizeOverlayPrefs in settings_snapshot.go), so which font or which
  // color was picked never leaves the machine.
  function pushChatPrefs() {
    SetOverlayPrefs(
      "raidchat",
      "",
      JSON.stringify({
  ...
  function setChatEngages(v) {
    chatEngages = v;
    saveChat(CHAT_ENGAGES_KEY, v ? "1" : "0");
    pushChatPrefs();
  }
  function setChatNewest(v) {
    chatNewest = v;
    saveChat(CHAT_NEWEST_KEY, v ? "1" : "0");
    pushChatPrefs();
  }

  // ── chat text style (the gear popover) ───────────────────────────────────
  let chatStyleOpen = false;
  // Size is 0 (the panel's own 12.5px) or a legible 8-40px; the in-between
  // sizes are nobody's intent, so a typed 3 becomes 8 rather than three-pixel
  // guild chat. The same rule the overlay gear panels apply.
  ...
  function saveChatStyle() {
    chatStyle = { ...chatStyle }; // one object identity per change, for props
    saveChat(CHAT_STYLE_KEY, JSON.stringify(chatStyle));
    pushChatPrefs();
  }
  function setChatSize(e) {
    chatStyle.size = clampMsgSize(e.target.value);
    e.target.value = chatStyle.size; // show what was actually taken
    saveChatStyle();
  }
  ...
  // Bumping the counter swaps which of the two identical flash classes the
  // header wears, and a CHANGED animation-name is what makes the browser run
  // the animation again — removing and re-adding one class inside a single
  // frame does not. Back-to-back messages therefore each pulse.
  let chatFlashSeq = 0;
  function onNewLines() {
    chatFlashSeq++;
  }

  // Draggable split between the board and the chat panel: the drag sets the
  // panel's height and the board (flex: 1) soaks up the rest, so an open panel
  // takes its room from the board and never pushes the tab past the window.
  ...
  function chatSplitKey(e) {
    ...
    if (!step) return;
    e.preventDefault();
    chatH = clampChatH(chatH + step);
    saveChatH();
  }
  // A height the user set by hand outranks the ten-line default from here on.
  function saveChatH() {
    chatHSaved = true;
    saveChat(CHAT_H_KEY, String(chatH));
  }

  onMount(async () => {
    window.addEventListener("click", closeMenus);
    pushChatPrefs(); // mirror the chat toggles for the daily snapshot
...
<div class="timers">
  ...
  {:else}
    <div class="board" bind:this={boardEl}>
      {#if srv && !srv.home}
        <!-- Off-home: the board stays live (it's Blue state, read-only from
             here) — just make clear whose raids these are. -->
        <div class="offworld">
          P99 Blue raid board — you're playing on {srv.name}.
        </div>
      {/if}
      {#if data.porter}
      ...
      {#if popped.length || data.event_raid}
        ...
        {#if data.event_raid}
          ...
          <div class="mob" data-raid={evKey} class:hit={hitKey === evKey}>
            <div class="mob-head clickable" on:click={() => toggleRaid(evKey)}>
              <span class="swords" title="Event raid"
                ><Icon name="swords" /></span
              >
        ...
        {#each popped as m (m.name)}
          ...
          <div class="mob" data-raid={m.name} class:hit={hitKey === m.name}>
            <div
              class="mob-head"
            ...
            >
              {#if m.is_raid}
                <span class="swords" title="Current raid"
                  ><Icon name="swords" /></span
                >
              {:else}
    ...
    <!-- Guild Chat: the guild's live chat feed, collapsed by default. The whole
         divider row is the toggle, so there is nothing to aim at; the gear and
         the two view pills inside it stop their own clicks, since flipping a
         view must not also close the box the view is in. The popover is a
         SIBLING of the row rather than a child, so its controls never reach
         the row's handler at all — and the wrapper is what it anchors to. -->
    <div class="gc-headwrap">
      <div
        class="gc-head"
        class:flash-a={chatFlashSeq > 0 && chatFlashSeq % 2 === 1}
        class:flash-b={chatFlashSeq > 0 && chatFlashSeq % 2 === 0}
        role="button"
        tabindex="0"
        aria-expanded={chatOpen}
        on:click={toggleChat}
        on:keydown={chatHeadKey}
      >
        <span class="gc-title">Guild Chat</span>
        {#if chatOpen}
          <!-- The app's own gear glyph, as the Timers tab writes it: a text
               character, never an emoji (WebView2 would draw a color one from
               the OS font). -->
          <button
            class="gc-gear"
            class:on={chatStyleOpen}
            title="Chat text: font, bold, size and color"
            aria-label="Chat text style"
            aria-expanded={chatStyleOpen}
            on:click|stopPropagation={() => (chatStyleOpen = !chatStyleOpen)}
            >⚙</button
          >
        {/if}
        <div class="gc-toggles">
          <button
            class="gc-pill"
            class:on={chatEngages}
            aria-pressed={chatEngages}
            title="Include raid-mob engage lines in the feed"
            on:click|stopPropagation={() => setChatEngages(!chatEngages)}
            >Engages</button
          >
          <button
            class="gc-pill"
            class:on={chatNewest}
            aria-pressed={chatNewest}
            title="Put the newest message at the top instead of the bottom"
            on:click|stopPropagation={() => setChatNewest(!chatNewest)}
            >Newest first</button
          >
        </div>
        <span class="gc-chev"
          ><Icon
            ...
            title={chatOpen ? "Close guild chat" : "Open guild chat"}
          /></span
        >
      </div>
      {#if chatOpen && chatStyleOpen}
        <!-- Anchored under the header, over the chat — unless the chat is too
             short to hold it, in which case it opens upward over the board
             instead. The tab clips its own overflow, so a popover that hung
             past the bottom edge would simply be cut off. -->
        <div class="gc-pop" class:up={chatH < CHAT_POP_H}>
          <div class="gc-poprow">
            <span class="gc-poplabel">Font</span>
            <select
              class="gc-popsel"
              aria-label="Chat font"
              bind:value={chatStyle.font}
              on:change={saveChatStyle}
            >
              {#each FONTS as f (f.v)}
          ...
          <div class="gc-poprow">
            ...
            <!-- on:change, not on:input: clamping per keystroke would rewrite
                 a half-typed "1" of "12" into the 8px floor. -->
            <input
              class="gc-popnum"
              type="number"
              min="0"
          ...
          <div class="gc-poprow">
            ...
            <!-- on:input previews while the picker is open; on:change is what
                 writes it down, so dragging through a hundred shades is one
                 save and one telemetry push. -->
            <input
              class="gc-popcolor"
              type="color"
              aria-label="Chat message color"
              value={chatStyle.color || CHAT_MSG_COLOR}
              on:input={(e) => (chatStyle.color = e.target.value)}
              on:change={saveChatStyle}
            />
    ...
    {#if chatOpen}
      <div class="gc-open" transition:slide|local={{ duration: 160 }}>
        <!-- Drag handle: resizes the board above against the chat below (also
        ...
        <div class="gc-chat" style="height:{chatH}px">
          <GuildChatPanel
            showEngages={chatEngages}
            newestTop={chatNewest}
            msgFont={chatStyle.font}
            msgBold={chatStyle.bold}
            msgSize={chatStyle.size}
            msgColor={chatStyle.color}
            on:newlines={onNewLines}
          />
        </div>
      </div>
    {/if}
...
<style>
  ...
  /* min-height: 0 so the board — not the window — is what gives way when the
     Guild Chat panel opens: without it a flex item refuses to shrink below its
     content and the panel would push the footer off the bottom. */
  .board {
    flex: 1;
    min-height: 0;
    overflow-y: auto;
  ...
  /* Off-home world: same amber "reduced mode" tint the General tab uses. */
  .offworld {
  ...
  /* Deep-link landing: one gold pulse so the eye finds the card. */
  .mob.hit {
    animation: raid-hit 1.8s ease-out;
  }
  @keyframes raid-hit {
    0%,
    60% {
  ...
  /* Guild Chat, under the footer. The wrapper exists so the style popover can
     anchor to the header without being inside it (a child's clicks would
     reach the header's toggle handler). */
  .gc-headwrap {
    flex-shrink: 0;
    position: relative;
  }
  /* The divider row wears the .group-title treatment in the app's gold
     accent, and the whole row is the toggle. */
  .gc-head {
    flex-shrink: 0;
    display: flex;
    align-items: center;
  ...
  .gc-head:focus-visible {
    outline: 1px solid var(--accent-dim);
    outline-offset: -1px;
  }
  /* New lines while the reader is scrolled away: one short gold pulse of the
     bar. Two identical animations, alternated by the counter's parity — the
  ...
  /* Someone who has asked for less motion is not shown a pulsing bar; they
     still have the chat itself, which is where the new lines are. */
  ...
  /* The gear, in the Timers tab's plain-glyph style: a quiet mark that brightens
     on hover and stays gold while its popover is open. */
  .gc-gear {
    background: none;
    border: none;
    padding: 0 2px;
  ...
  /* Compact pill checkboxes: buttons rather than real checkboxes so the row
     around them can stay one big toggle, and so they read as chips beside the
     uppercase label rather than as a form. */
  .gc-pill {
    background: none;
    border: 1px solid var(--border);
    border-radius: 8px;
  ...
  /* The style popover: anchored under the header, flipped above it when the
     chat below is too short to show it (.up). */
  .gc-pop {
    position: absolute;
    left: 14px;
    top: 100%;
    ...
    /* The header's uppercase gold is a label style, not a form style. */
    color: var(--text-secondary);
    font-size: 11px;
    font-weight: 400;
    letter-spacing: 0;
  ...
  /* Drag handle between the board and the chat, the Clients tab's splitter:
     an 8px grab strip with a 2px bar in it that brightens on hover/drag/focus. */
  .gc-split {
    flex-shrink: 0;
    height: 8px;
    margin: 3px 0;
