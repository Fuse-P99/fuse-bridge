<script>
  ...
  // Magelo sub-tab kill switch: hidden from everyone while it's being
  // finished; set true to release it to officers/admin again.
  const MAGELO_ENABLED = false;
  ...
  const utilCols = [
    ...
    { key: "sof", label: "SOF", title: "SOF — Scepter (Scepter of the Forlorn)" },
  ];
  const mobCols = [
    { key: "wc", label: "WC", title: "WC — WC cap (Leatherfoot Raider Skullcap)" },
    { key: "ot", label: "OT", title: "OT — OT Hammer (Worker Sledgemallet)" },
    { key: "thg", label: "Thg", title: "Thg — Thurg Pot (Vial of Velium Vapors)" },
  ];
  const consCols = [
    { key: "sow", label: "SOW", title: "SOW — Sow Pot (10 Dose Blood of the Wolf)" },
    { key: "shr", label: "SHR", title: "SHR — Shrink Pot (10 Dose Ant's Potion)" },
    {
      key: "wrt",
      label: "WRT",
    ...
    <div class="detail-pane">
      {#if selected}
        ...
        <div class="sub-tabs">
          ...
          <!-- Magelo is hidden for everyone while under construction; flip
               MAGELO_ENABLED to bring it back (officers/admin only). -->
          {#if MAGELO_ENABLED && (isOfficer || isAdmin)}
            <button
              class="sub-tab"
              ...
              <div class="spell-summary">
                ...
                <span class="spell-count-info">
                  {#if spellbook === null}
                    ...
                    <span class="no-sb"
                      >No spellbook file — run <code>/outputfile spellbook</code
                      ></span
                    >
                  {:else}
