<script>
  ...
  import {
    ...
    IsFilteredToon, ToggleFilteredToon, IsAdminMode
...
<div class="chars">
  ...
  <div class="cmd-footer">
    ...
    {/if}
    {#if isAdmin}
      <button class="cmd-btn admin-add-spell" on:click={() => showAddSpell = true}>+ Add Missing Spell</button>
    {/if}
  </div>
</div>

{#if showAddSpell}
  <AddSpellModal on:close={() => showAddSpell = false} on:added={onSpellAdded} />
{/if}
...
<style>
  ...
  .char-name { overflow:hidden; text-overflow:ellipsis; white-space:nowrap; font-weight:600; color:var(--text-primary); }
  ...
  .admin-add-spell { margin-left:auto; border-color:var(--accent-dim); color:var(--accent); }
