<script context="module">
  // Which quest deep-link request (questDeepLink.seq) this tab last honoured.
  // Module-level so a remount doesn't replay a request that was already acted
  // on before the user switched tabs and came back.
  let appliedLinkSeq = 0;
</script>
...
<script>
  ...
  import { charactersView, questDeepLink } from "../lib/nav.js";
  import {
    stepLine,
    sayLines,
    kindLabel,
  ...
  // 'magelo' | 'quests' | 'inventory' | 'spells' | 'all' — Magelo is the
  // landing tab: it's the character-sheet view, and it's open to everyone.
  let detailTab = "magelo";
  ...
  // A fresh /outputfile inventory was scanned (Go pushes quest-inv-changed
  // from questScanInventory): reload the selected character's file-backed
  // panes in place — deliberately NOT clearState, so open quest cards, the
  // sub-tab, and scroll position stay put while the data underneath updates.
  async function refreshFromInventory(toon) {
    if (!selected || selected.toLowerCase() !== toon.toLowerCase()) return;
    const name = selected;
    try {
      const [content, inventory, sb] = await Promise.all([
        GetCharContent(name),
      ...
      if (selected !== name) return; // switched away mid-load
      rawContent = content;
      inventoryItems = inventory || [];
      spellbook = sb;
      rebuildHighlight();
    ...
    if (questsLoaded === selected) await loadQuests(true);
  }
  onMount(() => {
    const off = Events.On("quest-inv-changed", (ev) => {
      const toon = ev && ev.data != null ? ev.data : ev;
      refreshFromInventory(String(toon || ""));
    });
    return () => off && off();
  });
  ...
  let marked = "";
  let markedTimer;
  async function dropMarker(questId, questName, key, tk) {
    try {
      // Always carry the quest name: a marker labeled with a bare NPC name
      // ("Andad Filla") is unidentifiable on the map days later. The quest id
      // and character ride along so clicking the flag on the map opens this
      // quest here.
      const label =
        tk.pt.what !== "here" ? `${questName}: ${tk.pt.what}` : questName;
      await AddQuestMarker(tk.name, tk.pt.y, tk.pt.x, label, questId, selected);
  ...
  // ── selection persistence + quest deep links ──────────────────────────────
  // The tab is destroyed on every main-tab switch, so the selection lives in
  // the charactersView store: leaving and coming back lands on the same
  // character and sub-tab. A quest deep link (nudge bar / map flag, via
  // questDeepLink) selects the character, opens Quests, and expands the card.
  let pendingQuestOpen = 0; // quest id to expand once its character's quests load
  let restored = false; // initial selection applied (restore or top character)

  $: if (restored) charactersView.set({ char: selected, tab: detailTab, viewMode });

  // Select a character (when the roster has it), switch sub-tab and view, and
  // queue a quest card to expand.
  async function applyView(char, tab, questId, mode = "detail") {
    const target = chars.some((e) => e.name === char) ? char : "";
    if (questId) pendingQuestOpen = questId;
    detailTab = tab;
    viewMode = mode;
    if (target && target !== selected) await selectChar(target);
  }

  // A deep link arriving while the tab is already open (a mount consumes any
  // request that brought it up — see onMount). Once per seq.
  $: if (restored && $questDeepLink.seq && $questDeepLink.seq !== appliedLinkSeq) {
    appliedLinkSeq = $questDeepLink.seq;
    applyView($questDeepLink.char || selected, "quests", $questDeepLink.questId);
  }

  // Expand the linked quest once its character's quests are in. selectChar's
  // clearState wipes questsOpen, so this waits for the load instead of setting
  // it up front.
  $: if (
    pendingQuestOpen &&
    detailTab === "quests" &&
    ...
    const qid = pendingQuestOpen;
    pendingQuestOpen = 0;
    questsOpen = { ...questsOpen, [qid]: true };
    tick().then(() =>
      document
        .getElementById(`qcard-${qid}`)
  ...
  onMount(async () => {
    ...
    // Initial selection. A deep link that brought the tab up wins; otherwise
    // the last selection, restored; otherwise the top character (the tailed /
    // online toon pins to the top) on its Magelo sheet.
    const link = $questDeepLink;
    const isNewLink = link.seq !== 0 && link.seq !== appliedLinkSeq;
    if (isNewLink) appliedLinkSeq = link.seq;
    const v = $charactersView;
    restored = true;
    if (isNewLink) {
      await applyView(
        link.char || v.char || (chars[0] && chars[0].name) || "",
        "quests",
        link.questId,
      );
    } else if (v.char && chars.some((e) => e.name === v.char)) {
      await applyView(v.char, v.tab || "magelo", 0, v.viewMode || "detail");
    } else if (chars.length) {
      await applyView(chars[0].name, "magelo", 0);
    }
    ...
    <div class="filter-row">
      ...
      <button
        ...
        on:click={() => {
          ...
          detailTab = "magelo";
          libraryOpenReq++;
        }}><Icon name="globe" /> Fuse Shared Magelos</button
      >
    </div>
  </div>
      ...
      <div class="list" bind:this={listEl} tabindex="-1">
        {#each chars as entry}
          ...
          <div
            ...
            title={entry.current
              ? "Currently playing — pinned to the top of the list"
              : null}
            role="button"
            tabindex="0"
            ...
            <div class="detail quests-pane">
              ...
              {:else if questsData}
                ...
                {#each questsData.assignments as a (a.quest.id)}
                  <div class="q-card" id="qcard-{a.quest.id}">
                    <div class="q-headrow">
                      <button
                        class="q-head"
                        ...
                        {#each a.quest.steps as s, i}
                          ...
                          <div class="q-step" class:done={st && st.done}>
                            ...
                            <div class="q-steptext">
                              ...
                              <!-- Each branch opens on its own line with the
                                   element indented inside — exactly the
                                   QuestEditor layout. Elements flush against
                                   the block boundaries ({#if ...}<span) lose
                                   the whitespace text nodes between tokens and
                                   the line renders as KillMobinZone. -->
                              {#each stepLine(s) as tk}
                                {#if tk.t === "text" || tk.t === "sep"}
                                  <span class="q-dim">{tk.s}</span>
                                {:else if tk.t === "item"}
                                  <button
                                    class="q-item"
                                    on:mouseenter={(e) =>
                                ...
                                {:else if tk.t === "zone"}
                                  ...
                                  {/if}
                                {:else if tk.t === "mob"}
                                  <span class="q-mob">{tk.names.join(" / ")}</span>
                                {:else if tk.t === "say"}
                                  <span class="q-says">
                                ...
                                {:else if tk.t === "plat"}
                                  <span class="q-plat">{tk.n}pp</span>
                                {:else if tk.t === "skill"}
                                  <span class="q-skill">[{tk.s}]</span>
                                {:else if tk.t === "gate"}
                                  <span class="q-gate">({tk.s})</span>
                                {:else if tk.t === "mult"}
                                  <span class="q-dim">×{tk.n}</span>
                                {:else if tk.t === "ret"}
                                  <span class="q-dim">(returned)</span>
                                {/if}
                              {/each}
                              {#if held && !(st && st.done)}
                                <span
...
<!-- Item card on hover — the shared ItemTipCard every surface shows. -->
{#if qTip}
  <ItemTipCard
    name={qTip.name}
    item={qTip.item}
...
<style>
  ...
  /* The tailed (online) character is pinned to the top of the list; the gold
     left edge + glow marks WHY this row sits out of alphabetical order. */
  .char-item.current {
    border-left: 2px solid rgba(232, 205, 130, 0.9);
    padding-left: 10px; /* 12px minus the border, so the text doesn't shift */
    box-shadow: inset 10px 0 12px -8px rgba(200, 169, 81, 0.6);
  }
  .char-row {
    display: flex;
    align-items: center;
  ...
  /* The walkthrough tokens are trimmed words ("Talk to", "in", "to receive")
     with no spaces of their own — the flex column-gap is what spaces the
     sentence, exactly like the editor's .q-line. Without it the line renders
     as "Talk toNPCinZONE". */
  .q-steptext {
    flex: 1;
    min-width: 0;
  ...
  /* The item hover card is the shared ItemTipCard component. */
</style>
