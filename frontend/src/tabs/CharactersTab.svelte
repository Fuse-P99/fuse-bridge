<script>
  ...
  // Open a character's detail from a table row.
  function openFromTable(name) {
    viewMode = "detail";
          ...
          <tbody>
            {#each sortedRows as row (row.name)}
              <tr
                ...
                on:click={() => openFromTable(row.name)}
              >
                <td class="c-name">{@html hlCell(row.name, query)}</td>
                <td>{@html hlCell(row.class, query)}</td>
                <td>{@html hlCell(row.race, query)}</td>
                <td class="num"
                  >{@html hlCell(row.level ? String(row.level) : "", query)}</td
                >
