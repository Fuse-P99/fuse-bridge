<script>
  ...
  $: alertColor = catColor(state.alert?.category || "Default");
  ...
  <div class="main">
    ...
    {:else if view === "overlays"}
      <div class="ov-intro">
        ...
        Position, size, and look are saved per character — a new character
        inherits the layout of one you've already set up in the same class.
      </div>
      ...
      <div class="ov-sec-title">Timer Bars</div>
      <div class="ov-grid">
        {#each timerCats as c (c.name)}
          <div class="ov-card">
            ...
            <button
              class="btn ov-pop"
              title="Pop out “{c.name}” timer bars as an overlay"
              on:click={() => OpenPopout("timers", c.name)}>Pop out</button
            >
          </div>
        {:else}
          <div class="hint">No enabled triggers start countdown timers.</div>
        {/each}
      </div>

      <div class="ov-sec-title">Text Alerts</div>
      <div class="ov-grid">
        {#each alertCats as c (c.name)}
          <div class="ov-card">
            <span class="cat-dot" style="background:{catColor(c.name)}"></span>
            <span class="ov-name">{c.name}</span>
            <span class="ov-count">{c.count}</span>
            <button
              class="btn ov-pop"
              title="Pop out “{c.name}” text alerts as an overlay"
              on:click={() => OpenPopout("alerts", c.name)}>Pop out</button
            >
          </div>
        {:else}
          <div class="hint">No enabled triggers show text alerts.</div>
        {/each}
      </div>
    {:else}
      <div class="char-row">
