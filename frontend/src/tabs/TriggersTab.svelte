<script>
  ...
  async function editSpecial(so) {
    ...
    const opts = (SP_OPTS[so.kind] || []).map((o) => ({
      ...o,
      on: o.key in stored ? !!stored[o.key] : o.def,
    }));
  ...
  async function setSpecialOpt(optKey, on) {
    if (!spForm) return;
    spForm.opts = spForm.opts.map((o) => (o.key === optKey ? { ...o, on } : o));
    try {
      const key = await overlayStoreKey(spForm.kind);
      const raw = JSON.parse(localStorage.getItem(key) || "{}");
      raw.settings = { ...(raw.settings || {}), [optKey]: on };
