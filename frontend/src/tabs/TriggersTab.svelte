<script>
  ...
  $: if (
    !ctaDone &&
    (ovDef.configured || ovDef.count > 0 || ovDef.char_set)
  ) {
    ctaDone = true;
    try {
      ...
      {#if !ctaDone}
        <div class="ov-cta">
          <span class="ov-cta-msg">
            <strong>New to timers?</strong> Check that you have some timers
            enabled you want to use and then set up your default overlays —
            position each overlay once and every character starts from that
            layout.
          </span>
