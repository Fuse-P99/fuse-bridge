<script>
  ...
  const SP_OPTS = {
    ...
    guildchat: [
      ...
      // Font and color dress the message text only — the time, identity
      // bracket and toon keep the overlay's own look. Size is set on the whole
      // line instead, so those columns scale with the message. Empty/0 means
      // "inherit" for all three.
      {
        key: "msgfont",
        label: "Message font",
        type: "choice",
        ...
        title:
          "Font for the message text only — the time, name and identity keep the overlay's font.",
        def: "",
      },
      {
        key: "msgsize",
        ...
        unit: "px",
        title:
          "Size in px for the chat lines: the message text and, scaled with it, the time, name and identity. Blank or 0 keeps the overlay's size.",
        def: 0,
      },
      {
        key: "msgcolor",
    ...
    <div class="modal">
      ...
      {#if spForm.opts && spForm.opts.length}
        <div class="opt-box">
          ...
          {#each spForm.opts.filter((x) => !x.showIf || spForm.opts.some((p) => p.key === x.showIf && p.on)) as o (o.key)}
            {#if o.type === "choice" && o.choices.length > 4}
              <!-- Long list (the font families): a row of segmented buttons
                   ...
                   dropdown the sound picker uses. -->
