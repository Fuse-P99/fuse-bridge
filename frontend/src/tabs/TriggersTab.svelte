<script>
  ...
  const SPECIAL_OVERLAYS = [
    ...
    },
    {
      kind: "threat",
      name: "DPS & Threat",
      desc: "Group/solo: a DPS parser — up to 6 attackers plus the group total, shared with guildmates on the same fight. In the raid's zone: the raid's top 5 (matching the raid card) plus your row, threat meter and gauge.",
              ...
              {#if isPopped(sec.kind, c.name)}
                <button
                  ...
                  on:click|stopPropagation={() =>
                    ClosePopout(sec.kind, c.name)}>Remove</button
                >
              {:else}
                <button
                  class="btn ov-pop"
