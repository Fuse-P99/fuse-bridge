<script>
  ...
  const SPECIAL_OVERLAYS = [
    ...
    },
    {
      kind: "racing",
      name: "Racing Mode",
      desc: "Strafe-racing guide lines and speedometers, auto-placed over your game viewport (from your EQ ini files). Always click-through — pop it out to race, close it when done. Spam /loc on a hotkey to feed the speedo.",
  ...
  const SP_OPTS = {
    ...
    raidclerics: [
      ...
      },
      {
        ...
        label: "Count Audio",
        title:
          "Pulse an audio cue in time with your pre-cast ring: once when the cleric before you starts casting, then every second until your own cast starts. The audible version of counting off the chain.",
        def: false,
      },
      {
        key: "pulsesound",
        label: "Count Sound",
        type: "media",
        title: "Sound played for each count pulse — any of your trigger sounds.",
    ...
    racing: [
      ...
      },
      {
        ...
        title:
          "Movement speed the lines are tuned for — each applies its own base offset (Run +14, SoW +0.4, Bard −10). Line offset below remains personal fine calibration added on top.",
        def: "sow",
      },
      {
        key: "offset",
  ...
  // Read-only view of a Fuse trigger for non-officers: the same editor form
  // with every control disabled. When members get their customization layer
  // (category, sounds, TTS overrides), this is where those fields unlock.
              ...
              <label class="sw-row" title={o.title}>
                ...
                <input
                  ...
                  on:change={(e) => {
                    ...
                    v = Math.max(o.min, Math.min(o.max, Math.round(v * 100) / 100));
                    e.target.value = v;
...
{#if form}
  <div class="overlay" on:click|self={() => (form = null)}>
    <div class="modal form">
      <div class="modal-title">
        {formIsNew ? "New Trigger" : formReadOnly ? "View Trigger" : "Edit Trigger"}
      </div>
      {#if formReadOnly}
        <div class="f-ro">
          Read-only — Fuse Triggers are maintained by officers. Per-member
          customization (category, sounds, TTS) is planned.
        </div>
      {/if}
      ...
      <div class="modal-actions">
        ...
        {/if}
        <button class="btn" on:click={() => (form = null)}
          >{formReadOnly ? "Close" : "Cancel"}</button
        >
      </div>
    </div>
  </div>
