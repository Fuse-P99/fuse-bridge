<script>
  ...
  import SpecialPreview from "../lib/SpecialPreview.svelte";
  ...
  import {
    ...
    GetPopoutSticky,
    SetPopoutSticky,
    GetGuildChatStayUnlocked,
    SetGuildChatStayUnlocked,
    GetPopoutProfile,
    NotifyOverlaySettings,
    SetOverlayPrefs,
    DismissTimer,
  ...
  import TriggerOverrideSlot from "../lib/TriggerOverrideSlot.svelte";
  ...
  // A per-trigger tint from the customization layer wins over the overlay's.
  $: alertColor =
    state.alert?.color ||
    styleColor("alerts", state.alert?.category || "Default");

  // Group running timers by category; a category renders only while it has
  // active timers. Soonest-ending first inside each category. A cleared bar
  // (End Early condition matched) stays long enough to triple-flash and fade
  // — when its overlay's "Flash if Ended Early" is on; off = it just goes.
  const CLEAR_TOTAL_MS = 4000;
  function flashEnded(name) {
    const s = catStyles.get(`timers|${(name || "Default").toLowerCase()}`);
    return !s || s.flash_ended_early !== false;
  }
  $: activeTimers = (state.timers || []).filter((t) =>
    t.cleared_at_ms
      ? flashEnded(t.category) && now - t.cleared_at_ms < CLEAR_TOTAL_MS
      : t.ends_at_ms > now,
  );
  $: cats = (() => {
    const m = new Map();
  ...
  async function saveCharAsDefault() {
    if (ovBusy || !ovDef.key) return;
    ovBusy = true;
    try {
      await SaveOverlayDefaultFromCharacter(ovDef.key);
      // Mirror the look settings: geometry lives on the Go side, but each
      // overlay's colour/opacity/fit is localStorage keyed by the character's
      // storage KEY (ovDef.key — never recomputed from the display name, which
      // may carry a "(P99 Green)" world suffix).
      copyOverlayLook(ovDef.key, ovDef.default_key);
  ...
  async function resetCharToDefault() {
    ...
    ovConfirmReset = false;
    try {
      await ResetCharacterToDefault(ovDef.key);
      copyOverlayLook(ovDef.default_key, ovDef.key, true);
  ...
  // "View edits" — what publishing would change, diffed live against the
  // server's current copy (so it also reflects another officer's interim
  // release). Read-only; the report comes fully formatted from the backend.
  let fuseEdits = null; // { loading, err, data: FusePendingEdits }
  async function openFuseEdits() {
    fuseEdits = { loading: true, err: "", data: null };
    try {
      const d = await GetFusePendingEdits();
      fuseEdits = { loading: false, err: "", data: d };
    } catch (e) {
      fuseEdits = { loading: false, err: String(e), data: null };
    }
  }
  const FED_SECTIONS = [
    { kind: "modified", label: "Changed" },
    { kind: "moved", label: "Moved" },
  ...
  const fedSect = (items) =>
    FED_SECTIONS.map((s) => ({
      ...s,
      items: (items || []).filter((it) => it.kind === s.kind),
    })).filter((s) => s.items.length > 0);
  $: fedGroupSections = fuseEdits?.data ? fedSect(fuseEdits.data.groups) : [];
  $: fedTrigSections = fuseEdits?.data ? fedSect(fuseEdits.data.triggers) : [];
  $: fedTotal = fuseEdits?.data
    ? fuseEdits.data.groups.length + fuseEdits.data.triggers.length
    : 0;
  ...
  let editChar = "";
  let chars = [];
  // The select's option values are per-(character, world) storage KEYS
  // (c.key) — same name on two servers is two different characters. Display
  // names come from c.name ("Franko (P99 Green)").
  $: shownChar = editChar || state.character_key || "";
  const editCharName = () =>
    (chars.find((c) => c.key === editChar) || {}).name || editChar;
  ...
  const SPECIAL_OVERLAYS = [
    ...
    },
    {
      kind: "threat",
      name: "DPS & Threat",
      desc: "Group/raid mode switching. Group - simple parser view. Raids - adds a threat indicator that monitors your threat vs. tank.",
    ...
    },
    {
      kind: "guildchat",
      name: "Guild Chat",
      desc: "Live #guild-stream: every guild message with the sender's Discord name, plus raid-mob engages. Officers only.",
      officer: true,
    },
    {
      kind: "racing",
      name: "Racing Mode",
      desc: "Strafe-racing guide lines and speedometers, auto-placed over your game viewport.",
      // Reads the local log's /loc lines and local ini files only.
      local: true,
    },
  ];
  // The raid overlays render server raid data an unlinked client never
  // receives; local ones stand on their own. The `officer` flag hides an
  // overlay from everyone else — only Guild Chat sets it, because the server
  // refuses its feed to non-officers and the card would be a dead end. The
  // damage and threat overlays describe the fight you are standing in rather
  // than anything to manage, so they stay open to all.
  ...
  // Which overlay a trigger feeds. Alert membership comes from the server
  // (shows_alert), which folds the personal customization layer in — hidden
  // text removes a trigger from the Alerts overlay, reworded text keeps it.
  function feedsKind(t, kind) {
    if (kind === "timers") return t.timer_enabled;
    return !!t.shows_alert;
  }
  ...
  // The same families as a choice-typed overlay option (SP_OPTS wants {v,l}),
  // for Guild Chat's message font. Its empty entry is renamed: there "default"
  // means the overlay's own font, not the app's.
  const GC_MSG_FONTS = FONTS.map((f) => ({
    v: f.v,
    l: f.v === "" ? "Overlay default" : f.label,
  }));
  ...
  function newCatForm(kind) {
    ...
    catForm = {
      ...
      style: {
        ...
        auto_pause: false,
        // Off by default; opt-in for mob respawn overlays (see the timer-
        // behavior selector in the category form).
        carry_timers: false,
        // Alerts only: display window (seconds) and the ticking age readout.
        alert_seconds: 10,
        alert_stopwatch: false,
        // Both kinds: flash + fade anything an End Early condition answers.
        flash_ended_early: true,
      },
    };
  }
  ...
  // ── bulk re-assign: move a selection of an overlay's triggers to another
  // overlay. The Go side routes each trigger by authority: Personal edits
  // directly, Fuse edits the package for officers, and members' Fuse triggers
  // move via local overrides (the guild package is untouched).
  let catReassign = null; // {name, kind, rows:[{id,name,path,sel}], to, options, busy, err}
  $: raAllSelected =
    !!catReassign &&
    catReassign.rows.length > 0 &&
    catReassign.rows.every((r) => r.sel);
  $: raAnySelected = !!catReassign && catReassign.rows.some((r) => r.sel);

  function flattenCatRows(nodes, trail = []) {
    const out = [];
    for (const g of nodes) {
      const here = [...trail, g.name];
      for (const t of g.triggers || [])
        out.push({ id: t.id, name: t.name, path: here.join(" › "), sel: true });
      out.push(...flattenCatRows(g.groups || [], here));
    }
    return out;
  }
  async function openCatReassign(c) {
    if (!treeLoaded) await loadTree();
    let options = [];
    try {
      options = (await GetCategoryNames()) || [];
    } catch {
      /* fall back to no targets */
    }
    catReassign = {
      name: c.name,
      kind: c.kind,
      rows: flattenCatRows(filterByCategory(tree, c.name, c.kind)),
      to: "",
      options: options.filter((n) => n.toLowerCase() !== c.name.toLowerCase()),
      busy: false,
      err: "",
  ...
  function toggleAllReassign(e) {
    const v = e.target.checked;
    catReassign.rows = catReassign.rows.map((r) => ({ ...r, sel: v }));
  }
  async function confirmReassign() {
    const ids = catReassign.rows.filter((r) => r.sel).map((r) => r.id);
    if (!ids.length) return;
    catReassign.busy = true;
    catReassign.err = "";
    catReassign = catReassign;
    try {
      await ReassignTriggersOverlay(ids, catReassign.to);
      catReassign = null;
      await loadTree();
      await loadCategories();
    } catch (e) {
      catReassign.busy = false;
      catReassign.err = String(e);
      catReassign = catReassign;
    }
  }
  ...
  async function onToggleEnable(kind, obj, val) {
    // The Fuse ROOT slider cascades an explicit on/off onto every group and
    // trigger in the package — which also overwrites the user's per-trigger
    // tuning, the one change here with no way back to the previous mix. So
    // it's allowed, but confirmed. On cancel the tree is reloaded: the DOM
    // checkbox has already flipped (one-way checked=), and only a re-render
    // snaps it back to the real state.
    if (kind === "group" && fuseRootNode && obj.id === fuseRootNode.id) {
      const n = obj.total_triggers || 0;
      const ok = await confirmDialog({
        title: "Entire Fuse package",
        message: `${val ? "Enable" : "Disable"} ALL ${n || "~1500"} Fuse triggers for this character?`,
        detail:
          "This stamps every group and trigger in the package at once, overwriting your per-trigger on/off tuning. Getting your previous mix back means re-doing it by hand.",
        confirmLabel: val ? "Enable all" : "Disable all",
        danger: true,
      });
      if (!ok) {
        await loadTree();
  ...
  let spForm = null; // {kind, name, sticky, opts: [{key,label,title,on}]}
  // Per-kind overlay options editable from here — the same settings the
  // overlay's own gear panel holds, all defaulting ON.
  const SP_OPTS = {
    raidassign: [
      {
        key: "flash",
        label: "Flashing",
        title: "Pulse animations for tank proc counters.",
        def: true,
      },
    ],
    raiddebuffs: [
      {
        key: "flash",
        label: "Flashing",
        title: "Pulse animations for sieve counts.",
        def: true,
      },
    ],
    raidclerics: [
      {
        ...
        label: "Flashing",
        title:
          "Pulse animations the CH cast-start flash. Off = every number and bar still updates, nothing blinks.",
        def: true,
      },
      {
        key: "timing",
        label: "Show Timing",
        title:
          "Show “+3.4s” after each cleric in the CH chain — how long after the previous caster they started their cast.",
        def: true,
      },
      {
        key: "pulseaudio",
        label: "Audio Cue",
        title:
          "Play audio cue when the cleric before you casts — the audible version of your gold pre-cast ring appearing.",
        def: false,
      },
      {
        key: "pulsesound",
        label: "Cue Sound",
        type: "media",
        // Meaningless while the cue itself is off — hidden, not disabled.
        showIf: "pulseaudio",
        title: "Sound played for audio cue.",
        def: "intuition-561.mp3",
      },
    ],
    voicespeakers: [
      {
        ...
        label: "Smart Hide",
        title:
          "Only show this overlay while YOU are in the raid voice channel. When you're not on comms it hides itself completely, and it reappears on its own when you join.",
        def: true,
      },
    ],
    guildchat: [
      {
        ...
        label: "Show Engages",
        title:
          "Include the raid-mob engage lines the bot posts to #guild-stream (who pulled what, ours and other guilds').",
        def: true,
      },
      {
        key: "newesttop",
        label: "Reverse Messages",
        title:
          "Show the newest messages at the top instead of the bottom. Off, it reads like the game's chat window: newest at the bottom, scroll up for older lines.",
        def: false,
      },
      // Font, bold and color dress the message text only — the time, the
      // officer/bot mark and the toon keep the overlay's own look. Size is set
      // on the whole line instead, so those columns scale with the message.
      // Empty/false/0 means "inherit" for all of them.
      {
        key: "msgfont",
        label: "Message font",
        type: "choice",
        choices: GC_MSG_FONTS,
        // Bold is drawn on this row rather than one of its own — see the
        // dropdown branch of the editor's renderer, and the same pairing in
        // the overlay's gear panel.
        boldKey: "msgbold",
        title:
          "Font for the message text only — the time, the officer/bot mark and the name keep the overlay's font.",
        def: "",
      },
      {
        // Never a row of its own: Message font renders it (its boldKey). The
        // entry exists so this option seeds from the store and saves back
        // through the ordinary editSpecial/setSpecialOpt path like every other
        // boolean, and `hidden` keeps it out of the list below.
        key: "msgbold",
        label: "Bold",
        hidden: true,
      ...
      },
      {
        ...
        min: 0,
        max: 40,
        // 0 is "the overlay's size"; below 8px is nobody's intent, so the
        // slider steps over the gap (the gear panel's clampMsgSize rule).
        snap: 8,
        unit: "px",
        title:
          "Size in px for the chat lines: the message text and, scaled with it, the time, the officer/bot mark and the name. Blank or 0 keeps the overlay's size.",
        def: 0,
      },
      {
        key: "msgcolor",
        ...
        type: "color",
        title:
          "Color for the message text only. Engage lines stay gold. Default follows the overlay's text color.",
        def: "",
      },
      {
        // Not a localStorage setting like the rest of this table — Go owns it
        // (popoutState.LockWithAll, the map's field). editSpecial reads it from
        // there and setSpecialOpt writes it back.
        key: "stayunlocked",
        label: "Stay unlocked",
        title:
          "Keep this overlay movable and scrollable when Lock Overlays is pressed on the Timers tab. Uncheck to have it lock and turn click-through like every other overlay: it then shows only the most recent messages and can't be scrolled.",
        def: true,
      },
    ],
    threat: [
      {
        ...
        label: "DPS Breakdown",
        title:
          "Add a bar under the DPS number splitting this fight's damage by type — melee verbs, kick, backstab, bow/throw, damage shield, spell — biggest first.",
        def: true,
      },
    ],
    // Racing Mode is permanently click-through, so this editor is the ONLY
    // way to reach its options — the overlay's own gear can't be clicked.
    racing: [
      {
        key: "sides",
        label: "Show lines",
        ...
        title:
          "Which side's racing lines and speedometer to draw — strafing left uses the left set, strafing right the right set.",
        def: "both",
      },
      {
        key: "avg",
        ...
        unit: "s",
        title:
          "The speedometer shows a trailing average of your /loc-derived speed over this many seconds.",
        def: 3,
      },
      {
        key: "speed",
        ...
        title:
          "Movement speed the lines are tuned for — each applies its own base offset (Run +17.5, SoW 0, Bard −12). Line offset below remains personal fine calibration added on top.",
        def: "sow",
      },
      {
        key: "offset",
        ...
        unit: "%",
        title:
          "Calibration: shift each side's line pair as a group, in percent of viewport width (0.01 steps). Positive moves both pairs outward, negative moves them inward. Applied on top of the Speed setting's base offset.",
        def: 0,
      },
      {
        key: "speedo",
        label: "Speedometer",
        title: "Show the speed widgets above the racing lines.",
        def: true,
      },
      {
        key: "focusmode",
        label: "Focus Mode",
        title:
          "While Racing Mode is open, hide every other overlay so nothing distracts from the race. They all come back the moment the racing overlay closes.",
        def: true,
      },
    ],
  };
  // The special overlays' look settings live in localStorage under the
  // overlay's own per-character key (every window shares one origin store),
  // so this editor can read and patch them directly; NotifyOverlaySettings
  // makes an open overlay re-read so the change applies live.
  async function overlayProfileChar() {
    try {
      const p = await GetPopoutProfile();
      if (p && p.char) return p.char;
    } catch {
      /* no character yet — the shared key */
    }
    return "";
  }
  async function overlayStoreKey(kind) {
    const c = await overlayProfileChar();
    return c ? `fuse.popout.${kind}@${c}` : `fuse.popout.${kind}`;
  }
  async function editSpecial(so) {
    // Media-typed options pick from the trigger sound library — make sure the
    // list is fresh before the form renders its dropdown.
    if ((SP_OPTS[so.kind] || []).some((o) => o.type === "media")) {
      loadMediaFiles();
    }
    let on = false;
    try {
    ...
    try {
      const raw = JSON.parse(
        localStorage.getItem(await overlayStoreKey(so.kind)) || "{}",
      );
      stored = raw.settings || {};
    } catch {
      /* defaults stand */
    }
    const opts = (SP_OPTS[so.kind] || []).map((o) => ({
      ...o,
      // `on` drives the boolean sliders; `val` carries choice/number options.
      on: o.key in stored ? !!stored[o.key] : o.def === true,
      val: o.key in stored ? stored[o.key] : o.def,
    }));
    // Guild Chat's "Stay unlocked" is the one option here Go owns rather than
    // the overlay's localStorage store (it is the map's lock opt-out, applied
    // to the live window by Go itself), so its true value comes from there.
    if (so.kind === "guildchat") {
      try {
        const stay = await GetGuildChatStayUnlocked();
        const o = opts.find((x) => x.key === "stayunlocked");
        if (o) {
          o.on = !!stay;
          o.val = !!stay;
        }
      } catch {
        /* the default (on) stands */
      }
    }
    spForm = { kind: so.kind, name: so.name, sticky: on, opts };
  }
  ...
  // A numeric option may have a dead band just above zero — Guild Chat's
  // message size is 0 (the overlay's own) or a legible 8px and up, never 3px.
  // `snap` names that floor: dragging into the gap lands on it, and only the
  // very bottom of the track still means "default".
  function snapNum(o, v) {
    if (!o.snap || v <= 0) return v;
    return Math.max(o.snap, v);
  }
  // What a color-typed option's swatch shows while nothing is set: a color
  // input has no empty state, so it previews the color the text inherits
  // (--text-primary) rather than reading as a deliberate black.
  const OPT_COLOR_UNSET = "#e0e2ea";
  // value may be a boolean (sliders), a string (choice, color) or a number
  // (range).
  async function setSpecialOpt(optKey, value) {
    if (!spForm) return;
    spForm.opts = spForm.opts.map((o) =>
      o.key === optKey ? { ...o, on: value === true, val: value } : o,
    );
    // "Stay unlocked" (Guild Chat) is Go state, not a stored look setting: Go
    // persists it with the overlay's record and applies the click-through
    // change to the open window on the spot. Writing it to localStorage too
    // would leave a second copy that nothing reads and that goes stale.
    if (optKey === "stayunlocked") {
      try {
        await SetGuildChatStayUnlocked(value === true);
        // The open overlay's own gear panel re-reads it on this nudge.
        await NotifyOverlaySettings(spForm.kind, "");
      } catch {
        /* Go keeps the truth; reopening this form shows it */
      }
      return;
    }
    try {
      const char = await overlayProfileChar();
      const key = char
        ? `fuse.popout.${spForm.kind}@${char}`
        : `fuse.popout.${spForm.kind}`;
      const raw = JSON.parse(localStorage.getItem(key) || "{}");
      raw.settings = { ...(raw.settings || {}), [optKey]: value };
      localStorage.setItem(key, JSON.stringify(raw));
      // Mirror into Go for the settings snapshot (CLAUDE.md, settings
      // telemetry): this path bypasses the overlay's own gear panel, whose
      // save is what normally pushes — a closed overlay never would.
      SetOverlayPrefs(
        spForm.kind,
        "",
        JSON.stringify({ [optKey]: value }),
        char,
      ).catch(() => {});
      await NotifyOverlaySettings(spForm.kind, "");
    } catch {
      /* the overlay's own gear panel still works */
    }
  }
  // Left-click preview: which special overlay's sample is slid open.
  let spPreview = "";
  ...
  let formIsNew = false;
  let formReadOnly = false; // non-officer viewing a Fuse trigger
  let formGroupId = 0;
  let formErr = "";
  ...
  async function onToggleMute(kind, item) {
    // The Fuse ROOT row acts on the whole package in one click — allowed,
    // but confirmed: a stray click is cheap to make and tedious to unwind.
    if (kind === "group" && fuseRootNode && item.id === fuseRootNode.id) {
      const n = item.total_triggers || 0;
      const ok = await confirmDialog({
        title: "Entire Fuse package",
        message: item.muted
          ? "Unmute the sounds and speech of the WHOLE Fuse Triggers package?"
          : "Mute the sounds and speech of the WHOLE Fuse Triggers package?",
        detail: `This applies to all ${n || "~1500"} Fuse triggers at once. Alerts, timer bars, and clipboard keep working either way.`,
        confirmLabel: item.muted ? "Unmute all" : "Mute all",
      });
      if (!ok) return;
    }
    try {
  ...
  async function onToggleClip(kind, item) {
    // Same whole-package confirmation as the mute (see onToggleMute).
    if (kind === "group" && fuseRootNode && item.id === fuseRootNode.id) {
      const n = item.total_triggers || 0;
      const ok = await confirmDialog({
        title: "Entire Fuse package",
        message: item.clip_blocked
          ? "Allow the WHOLE Fuse Triggers package to copy to your clipboard again?"
          : "Stop the WHOLE Fuse Triggers package from taking your clipboard?",
        detail: `This applies to all ${n || "~1500"} Fuse triggers at once. Everything else the triggers do keeps working.`,
        confirmLabel: item.clip_blocked ? "Allow all" : "Block all",
      });
      if (!ok) return;
    }
    try {
  ...
  // ── "My Customization": the personal override layer (trigger_overrides.go).
  // Fuse triggers get the full panel (overlay, color, per-slot text/TTS/sound)
  // for everyone — members because the package fields are locked, officers
  // because editing the package changes it for the whole guild. Personal
  // triggers (and brand-new ones) get just the color row: their real fields
  // are directly editable. Each channel is tri-state: follow the package,
  // custom value, or off.
  let formCustomize = false; // full panel (existing Fuse trigger)
  let ovForm = null;

  // "If already running" values, labeled the way the package edit form
  // labels them (the customization select shows the package's inside its
  // "Package setting" entry).
  const START_BEHAVIOR_LABELS = {
    StartNewTimer: "Start another timer",
    RestartTimer: "Restart this trigger's timer",
    IgnoreIfRunning: "Ignore",
  };

  // Override *string → { mode, value }: absent = follow the package,
  // "" = explicitly off, anything else = custom.
  const ovChan = (p) => {
    if (p === undefined || p === null) return { mode: "pkg", value: "" };
    if ((p || "").trim() === "") return { mode: "off", value: "" };
    return { mode: "custom", value: p };
  };
  const blankOvSlot = () => ({
    text: ovChan(),
    tts: ovChan(),
    sound: ovChan(),
    intr: null, // null = the package's interrupt-speech flag
  });
  function blankOvForm() {
    return {
      overlay: null, // null = the package assignment
      colorOn: false,
      color: "#c8a951",
      timerName: "", // "" = the package timer name
      startBehavior: null, // null = the package "if already running"
      endingSeconds: "", // "" = the package warning time
      on_match: blankOvSlot(),
      ending: blankOvSlot(),
      ended: blankOvSlot(),
    };
  }
  function buildOvForm(src) {
    const o = src.override || {};
    return {
      overlay: o.overlay === undefined || o.overlay === null ? null : o.overlay,
      colorOn: !!o.color,
      color: o.color || "#c8a951",
      timerName: o.timer_name || "",
      startBehavior: o.start_behavior ?? null,
      endingSeconds: o.ending_seconds ?? "",
      on_match: {
        text: ovChan(o.text),
        tts: ovChan(o.tts),
        sound: ovChan(o.sound),
        intr: o.interrupt ?? null,
      },
      ending: {
        text: ovChan(o.ending_text),
        tts: ovChan(o.ending_tts),
        sound: ovChan(o.ending_sound),
        intr: o.ending_interrupt ?? null,
      },
      ended: {
        text: ovChan(o.ended_text),
        tts: ovChan(o.ended_tts),
        sound: ovChan(o.ended_sound),
        intr: o.ended_interrupt ?? null,
      },
    };
  }
  ...
  // { mode, value } → *string for SetTriggerOverride. An empty custom value
  // reads as "off" (the Go side folds redundant values back to nil anyway).
  const chanOut = (c) => {
    if (!c || c.mode === "pkg") return null;
    if (c.mode === "off") return "";
    return (c.value || "").trim();
  };
  function serializeOverride() {
    if (!ovForm) return {};
    // The warning-time override: a positive whole number of seconds, anything
    // else (empty, 0, garbage) = follow the package.
    const endSecs = parseInt(ovForm.endingSeconds, 10);
    return {
      overlay: ovForm.overlay,
      color: ovForm.colorOn ? ovForm.color : null,
      timer_name: (ovForm.timerName || "").trim() || null,
      start_behavior: ovForm.startBehavior,
      ending_seconds: Number.isFinite(endSecs) && endSecs > 0 ? endSecs : null,
      text: chanOut(ovForm.on_match.text),
      tts: chanOut(ovForm.on_match.tts),
      sound: chanOut(ovForm.on_match.sound),
      interrupt: ovForm.on_match.intr,
      ending_text: chanOut(ovForm.ending.text),
      ending_tts: chanOut(ovForm.ending.tts),
      ending_sound: chanOut(ovForm.ending.sound),
      ending_interrupt: ovForm.ending.intr,
      ended_text: chanOut(ovForm.ended.text),
      ended_tts: chanOut(ovForm.ended.tts),
      ended_sound: chanOut(ovForm.ended.sound),
      ended_interrupt: ovForm.ended.intr,
    };
  }
  // Overlay choices for the customization panel — keep an unknown current
  // pick selectable, same rule as catOptions.
  $: ovCatOptions =
    ovForm && ovForm.overlay && !catNames.includes(ovForm.overlay)
      ? [ovForm.overlay, ...catNames]
      : catNames;
  // Whether the trigger being edited would show alert text, live against the
  // form's package fields — drives the overlay help rows under the select.
  $: formShowsAlert = !!(
    form &&
    ((form.on_match?.use_text && (form.on_match.display_text || "").trim()) ||
      (form.timer_enabled &&
        form.ending_enabled &&
        form.ending?.use_text &&
        (form.ending.display_text || "").trim()) ||
      (form.timer_enabled &&
        form.ended_enabled &&
        form.ended?.use_text &&
        (form.ended.display_text || "").trim()))
  );

  // Custom Color row: the swatch shows the color the trigger's bars/alerts
  // will actually use — the customization when set, else the overlay's own
  // color, resolved against the EFFECTIVE overlay (a member's re-assignment
  // included). Timer-bar color when the trigger runs a timer, alert color
  // otherwise.
  $: ccOverlay =
    (ovForm && ovForm.overlay !== null && ovForm.overlay !== undefined
      ? ovForm.overlay
      : form && form.category) || "Default";
  $: ccDefault = form
    ? styleColor(form.timer_enabled ? "timers" : "alerts", ccOverlay)
    : "#c8a951";

  // Top-level pane of the edit form (General / On Match / End Early /
  // Customize) — the single-column form outgrew its scroll.
  let mainTab = "general";
  ...
  function menuEditTrigger() {
    ...
    // The editable Category select edits the PACKAGE value. form.category
    // arrives as the EFFECTIVE assignment (override applied) — binding the
    // select to that would silently bake a local override into the package
    // XML on save, so swap the package value in before the form opens.
    form.category = form.pkg_category;
    formIsNew = false;
    formReadOnly = false;
    formCustomize = !menu.target.personal;
    ovForm = buildOvForm(menu.target);
    formErr = "";
    mainTab = "general";
    timerTab = "timer";
    loadMediaFiles();
  ...
  // A Fuse trigger for non-officers: the package fields render disabled, and
  // the My Customization panel underneath is where members act — overlay,
  // color, and per-slot text/speech/sound overrides.
  function menuViewTrigger() {
    form = cloneForm(menu.target);
    form.category = form.pkg_category;
    formIsNew = false;
    formReadOnly = true;
  ...
  // Straight to the Customize tab (Fuse triggers only — Personal triggers
  // have no customization layer beyond the color row). Opens the same form
  // View/Edit would — read-only package fields for members — then lands on
  // Customize instead of General.
  function menuCustomizeTrigger() {
    if (menu.target.editable) menuEditTrigger();
    else menuViewTrigger();
    mainTab = "customize";
  }
  ...
  async function saveForm() {
    formErr = "";
    // Member on a Fuse trigger: the package fields are read-only, so the only
    // thing to save is the customization layer.
    if (formReadOnly) {
      try {
        await SetTriggerOverride(form.group_id, form.name, serializeOverride());
        highlightId = form.id;
        form = null;
        await loadTree();
      } catch (e) {
    ...
    try {
      if (formIsNew) {
        ...
        highlightId = id;
        // A color picked while creating rides along as the trigger's first
        // override (nothing else in the panel exists for a new trigger).
        if (ovForm && ovForm.colorOn) {
          await SetTriggerOverride(
            formGroupId,
            form.name.trim(),
            serializeOverride(),
          );
        ...
        // Save the package edit first — a rename migrates the override key —
        // then write the customization panel against the new name.
...
<div class="trig-tab">
  ...
  <div class="main">
          ...
          {#if !catClosed[c.name]}
            <div transition:slide|local={{ duration: 150 }}>
              {#each c.timers as t (t.id)}
                <div class="tbar" class:cleared={!!t.cleared_at_ms}>
                  <!-- t.color: a per-trigger tint from the customization
                       layer wins over the overlay's color. -->
                  <div
                    class="tbar-fill"
                    style="width:{barFrac(t) * 100}%; background:{t.color ||
                      c.color}"
                  ></div>
    ...
    {:else if view === "overlays"}
      <div class="ov-intro">
        Pop out any overlay and position it over the game. Position and size are
        saved per character, starting from your default layout. Colors and fonts
        belong to the overlay itself, so they're the same on every character.
        Click an overlay to see its triggers; right-click to edit, re-assign, or
        delete it.
      </div>
      ...
      {#if visibleSpecials.length}
        ...
        <div class="ov-list">
          {#each visibleSpecials as so (so.kind)}
            <!-- Left-click slides open a sample of what this overlay shows;
                 right-click opens its settings, matching the category cards
                 above. The Pop out / Remove buttons stop propagation so they
                 don't toggle the preview underneath the click. -->
            <div
              class="ov-card sp"
              class:open={spPreview === so.kind}
              role="button"
              tabindex="0"
              on:click={() =>
                (spPreview = spPreview === so.kind ? "" : so.kind)}
              on:keydown={(e) =>
                e.key === "Enter" &&
                (spPreview = spPreview === so.kind ? "" : so.kind)}
            ...
            >
              ...
              {#if isPopped(so.kind, "")}
                <button
                  ...
                  on:click|stopPropagation={() => ClosePopout(so.kind, "")}
                  >Remove</button
                >
              {:else}
                <button
                  ...
                  on:click|stopPropagation={() => OpenPopout(so.kind, "")}
                  >Pop out</button
                >
              {/if}
              {#if spPreview === so.kind}
                <div class="sp-prev" transition:slide|local={{ duration: 150 }}>
                  <SpecialPreview kind={so.kind} />
                </div>
              {/if}
            </div>
          {/each}
      ...
      {#each [{ kind: "timers", title: "Timer Bars", cats: timerCats, empty: "No triggers start countdown timers." }, { kind: "alerts", title: "Alerts", cats: alertCats, empty: "No triggers show text alerts." }] as sec (sec.kind)}
        <div class="ov-sec-head">
          <span class="ov-sec-title">{sec.title}</span>
          <button
            class="ov-add"
            title="New {sec.title} overlay"
            aria-label="New {sec.title} overlay"
        ...
        <div class="ov-list">
          {#each sec.cats as c (c.name)}
            ...
            >
              ...
              >
              <!-- Same actions the right-click menu offers, as visible buttons —
                   a context menu is not discoverable enough for these. -->
              <button
                class="btn ov-gear"
                title="Re-assign triggers from “{c.name}” to another overlay"
                aria-label="Re-assign triggers from {c.name}"
                on:click|stopPropagation={() => openCatReassign(c)}>⇄</button
              >
              <button
                class="btn ov-gear"
            ...
            {#if openCat === catKey(c)}
              <div class="ov-tree" transition:slide|local={{ duration: 150 }}>
                ...
                {:else}
                  <div class="hint">
                    No triggers are assigned to this overlay yet.
                  </div>
                {/each}
              </div>
            {/if}
    ...
    {:else}
      {#if meta.officer && fuseRootNode && fuseRootNode.dirty}
        ...
        <div class="pub-bar">
          ...
          <button class="btn" disabled={publishing} on:click={openFuseEdits}
            >View edits</button
          >
          <button
            class="btn save"
      ...
      {:else}
        <div class="char-row">
          ...
          >
          {#if editChar && editChar !== (state.character_key || "")}
            <span class="char-note"
              >Editing another character — changes apply to {editCharName()} only.</span
            >
          {/if}
        </div>
      {/if}
...
{#if menu}
  <div class="ctx" style="left:{menu.x}px; top:{menu.y}px">
    ...
    {:else if menu.kind === "trigger"}
      ...
      {:else}
        <button class="ctx-item" on:click={menuViewTrigger}>View</button>
      {/if}
      {#if !menu.target.personal}
        <button class="ctx-item" on:click={menuCustomizeTrigger}
          >Customize</button
        >
      {/if}
...
{#if shareTrig}
  <ShareDialog
    ...
    previewLines={[
      ...
      shareTrig.category ? `Overlay: ${shareTrig.category}` : "",
    ].filter(Boolean)}
...
{#if spForm}
  ...
  <div class="overlay" on:click|self={() => (spForm = null)}>
    <div class="modal">
      ...
      <!-- Ordinary options first, in ordinary styling — the red box below is
           reserved for Sticky Mode, the one switch people shouldn't flip by
           accident. -->
      {#if spForm.opts && spForm.opts.length}
        <div class="opt-box">
          <!-- showIf: an option that only exists while its parent boolean is
               on (Cue Sound under Audio Cue) is filtered out, not grayed.
               hidden: an option another row draws (Bold, on Message font). -->
          {#each spForm.opts.filter((x) => !x.hidden && (!x.showIf || spForm.opts.some((p) => p.key === x.showIf && p.on))) as o (o.key)}
            {#if o.type === "choice" && o.choices.length > 4}
              <!-- Long list (the font families): a row of segmented buttons
                   would spill out of a 320px modal, so it becomes the same
                   dropdown the sound picker uses. An option may bring a
                   boolean along on its row (boldKey): the two are one decision
                   about how the text is dressed, and the overlay's own gear
                   panel pairs them the same way. A div rather than a label
                   then, because a label would hand every click on its text to
                   the checkbox; the select names itself instead. -->
              {@const bold = o.boldKey
                ? spForm.opts.find((x) => x.key === o.boldKey)
                : null}
              <div class="sw-row" title={o.title}>
                <span class="opt-lbl">{o.label}</span>
                {#if bold}
                  <label class="opt-bold" title={bold.title}>
                    <input
                      type="checkbox"
                      checked={bold.on}
                      on:change={(e) =>
                        setSpecialOpt(o.boldKey, e.target.checked)}
                    />
                ...
                <select
                  class="opt-sel"
                  aria-label={o.label}
                  value={typeof o.val === "string" ? o.val : o.def}
                  on:change={(e) => setSpecialOpt(o.key, e.target.value)}
                >
                  {#each o.choices as c (c.v)}
                ...
                <span class="opt-seg">
                  {#each o.choices as c (c.v)}
                    <button
                      class="opt-seg-btn"
                      class:on={o.val === c.v}
                      on:click={() => setSpecialOpt(o.key, c.v)}>{c.l}</button
                    >
                  {/each}
                </span>
              </div>
            {:else if o.type === "num"}
              <label class="sw-row" title={o.title}>
                <span class="opt-lbl">{o.label}</span>
                <input
                  class="opt-range"
                  type="range"
                  ...
                  value={+o.val || o.def}
                  on:input={(e) =>
                    setSpecialOpt(o.key, snapNum(o, +e.target.value))}
                />
                <!-- An option whose range starts at zero means "the overlay's
                     own value" down there, and "0px" reads as zero-height
                     text rather than as that. -->
                <span class="opt-val"
                  >{(+o.val || o.def) > 0 || o.min > 0
                    ? `${+o.val || o.def}${o.unit || ""}`
                    : "default"}</span
                >
              </label>
            {:else if o.type === "color"}
              <!-- Color plus an explicit way back to "not set": the swatch
                   cannot show empty, so Default is what clears it. Saved on
                   change, not while the picker is being dragged. -->
              <div class="sw-row" title={o.title}>
                <span class="opt-lbl">{o.label}</span>
                <span class="opt-color">
                  <input
                    type="color"
                    value={typeof o.val === "string" && o.val
                      ? o.val
                      : OPT_COLOR_UNSET}
                    on:change={(e) => setSpecialOpt(o.key, e.target.value)}
                  />
                  <button
                    ...
                    class="opt-seg-btn"
                    disabled={!o.val}
                    on:click={() => setSpecialOpt(o.key, "")}>Default</button
                  >
                </span>
              </div>
            {:else if o.type === "dec"}
              <!-- two-decimal number entry; saved on change, not per keystroke -->
              <label class="sw-row" title={o.title}>
                <span class="opt-lbl">{o.label}</span>
                <input
                  class="opt-dec"
                  ...
                  max={o.max}
                  step="0.01"
                  value={isFinite(+o.val) ? +o.val : o.def}
                  on:change={(e) => {
                    let v = parseFloat(e.target.value);
                    if (!isFinite(v)) v = o.def;
                    v = Math.max(
                      o.min,
                      Math.min(o.max, Math.round(v * 100) / 100),
                    );
                    e.target.value = v;
                    setSpecialOpt(o.key, v);
                  }}
                />
                <span class="opt-val">{o.unit || ""}</span>
              </label>
            {:else if o.type === "media"}
              <!-- sound picker, same model as the trigger editor: library
                   dropdown + sample + add-file (TriggerActions.svelte) -->
              {@const cur = typeof o.val === "string" ? o.val : o.def}
              <div class="sw-row" title={o.title}>
                <span class="opt-lbl">{o.label}</span>
                <span class="opt-media">
                  ...
                  >
                    ...
                    {#each cur && !mediaFiles.includes(cur) ? [cur, ...mediaFiles] : mediaFiles as m (m)}
                  ...
                  <button
                    ...
                    class="opt-media-btn"
                    on:click={async () => {
                      const n = await addMediaFile();
                      if (n) setSpecialOpt(o.key, n);
                    }}>Add…</button
                  >
                </span>
              </div>
              ...
              <label class="sw-row" title={o.title}>
                ...
                <span class="sw gold">
                  <input
                    type="checkbox"
                    checked={o.on}
                    on:change={(e) => setSpecialOpt(o.key, e.target.checked)}
                  />
      ...
      {/if}
      <!-- Racing Mode gets no Sticky: it's auto-placed over the game viewport
           and click-through, so exempting it from the automatic hides would
           just leave lines painted over whatever covers the game. -->
      {#if spForm.kind !== "racing"}
  ...
  <div class="overlay" on:click|self={() => (catForm = null)}>
    <div class="modal form">
      <div class="modal-title">
        ...
        {catForm.style.kind === "alerts" ? "Alerts" : "Timer Bars"} Overlay
      </div>
      ...
      {#if !catForm.isNew}
        <div class="f-note">
          Renaming reassigns every trigger currently in this overlay.
        </div>
      {/if}

      <div class="f-sep" />
      <div class="f-sect">Appearance</div>
      ...
      <div class="f-sep" />
      <div class="f-sect">Behavior</div>
      {#if catForm.style.kind === "timers"}
        <label class="f-label" for="cat-timermode">When you leave or swap</label
        >
        <select
          id="cat-timermode"
          class="in"
          value={catForm.style.carry_timers
            ? "keep"
            : catForm.style.auto_pause
              ? "pause"
              : "end"}
          on:change={(e) => {
            const m = e.target.value;
            catForm.style.auto_pause = m === "pause";
            catForm.style.carry_timers = m === "keep";
          }}
        >
          <option value="end">End this overlay's timers</option>
          <option value="pause">Pause &amp; resume per character</option>
          <option value="keep">Keep running across characters</option>
        </select>
        <div class="f-note">
          {#if catForm.style.carry_timers}
            For mob respawn windows: these bars count down in real time on every
            character and even while the app is closed, and expire when their
            time is up. Each character still needs this overlay positioned to
            see the bars float; they always show under Current Timers.
          {:else if catForm.style.auto_pause}
            For buff-style timers: bars freeze when the character leaves the
            world and resume where they left off on that same character's next
            login.
          {:else}
            Default: this overlay's timers end when the character leaves the
            world or you swap toons.
          {/if}
        </div>
        <div class="f-grid">
          <span class="f-label">Flash if ended early</span>
          <label
            class="sw-row"
            title="When an End Early condition ends one of this overlay's timers, flash it three times and fade it out instead of removing it instantly"
          >
            <span class="sw gold">
              <input
                type="checkbox"
                bind:checked={catForm.style.flash_ended_early}
              />
              <span class="sw-track"><span class="sw-thumb"></span></span>
            </span>
          </label>
      ...
      {:else}
        <div class="f-grid">
          <label class="f-label" for="cat-alertsecs">Time shown</label>
          <div class="f-inline">
            <input
              id="cat-alertsecs"
              class="in num"
              ...
              min="1"
              max="3600"
              bind:value={catForm.style.alert_seconds}
            />
            <span class="f-val">seconds</span>
          </div>
          <span class="f-label">Show age (m:ss)</span>
          <label
            class="sw-row"
            title="Append a ticking m:ss age to each alert on this overlay"
          >
            <span class="sw gold">
              <input
                type="checkbox"
                bind:checked={catForm.style.alert_stopwatch}
              />
              <span class="sw-track"><span class="sw-thumb"></span></span>
            </span>
          </label>
          ...
          <label
            class="sw-row"
            title="When an End Early condition answers one of this overlay's alerts, flash it three times and fade it out instead of removing it instantly"
          >
            <span class="sw gold">
              <input
                type="checkbox"
      ...
      {:else}
        <div
          ...
          style="background:{rgba(
            ...
            .style.font_family || 'inherit'}"
        >
          <span class="cf-prev-alert-txt"
            >{catForm.style.name ||
              "Alert text"}</span
          >{#if catForm.style.alert_stopwatch}<span class="cf-prev-sw"
              >0:42</span
            >{/if}
        </div>
      {/if}
...
{#if catDel}
  <div class="overlay" on:click|self={() => (catDel = null)}>
    <div class="modal">
      ...
      {#if !meta.officer}
        <div class="f-note">
          You're not an officer, so Fuse triggers are moved via local
          customization on this PC — the guild package itself is unchanged.
        </div>
      {/if}
      <div class="modal-actions">
...
<!-- bulk re-assign: move a selection of an overlay's triggers elsewhere -->
{#if catReassign}
  <div class="overlay" on:click|self={() => (catReassign = null)}>
    <div class="modal gina-modal">
      <div class="modal-title">Re-assign Triggers — {catReassign.name}</div>
      {#if catReassign.rows.length}
        <label class="f-chk gina-all">
          <input
            type="checkbox"
            checked={raAllSelected}
            on:change={toggleAllReassign}
          /> Select all
        </label>
        <div class="gina-list">
          {#each catReassign.rows as r (r.id)}
            <label class="gina-item">
              <input type="checkbox" bind:checked={r.sel} />
              <span class="gina-name">{r.name}</span>
              <span class="gina-count">{r.path}</span>
            </label>
          {/each}
        </div>
        <label class="f-label" for="ra-to">Move selected to</label>
        <select id="ra-to" class="in" bind:value={catReassign.to}>
          <option value="">No overlay (flagged as needing setup)</option>
          {#each catReassign.options as n (n)}
            <option value={n}>{n}</option>
          {/each}
        </select>
        {#if !meta.officer}
        ...
        {/if}
      {:else}
        <div class="f-note">No triggers are assigned to this overlay.</div>
      {/if}
      {#if catReassign.err}<div class="f-err">{catReassign.err}</div>{/if}
      <div class="modal-actions">
        <button
          class="btn save"
          disabled={catReassign.busy || !raAnySelected}
          on:click={confirmReassign}
          >{catReassign.busy ? "Moving…" : "Re-assign"}</button
        >
        <button class="btn" on:click={() => (catReassign = null)}>Cancel</button
        >
      </div>
    </div>
  </div>
...
<!-- officer "View edits": unpublished changes vs the published Fuse set -->
{#if fuseEdits}
  <div class="overlay" on:click|self={() => (fuseEdits = null)}>
    <div class="modal gina-modal fed-modal">
      <div class="modal-title">Unpublished Changes</div>
      {#if fuseEdits.loading}
        <div class="f-note">Comparing with the published set…</div>
      {:else if fuseEdits.err}
        <div class="f-err">{fuseEdits.err}</div>
      {:else}
        <div class="f-note">
          What <strong>Publish to guild</strong> will change, compared with the
          published Fuse Triggers v{fuseEdits.data.server_version}.
          {#if fuseEdits.data.server_version > fuseEdits.data.base_version}
            <span class="pub-warn"
              >v{fuseEdits.data.server_version} was published by another officer
              after you started editing (from v{fuseEdits.data.base_version}),
              so the differences below include undoing their changes.</span
            >
          {/if}
        </div>
        {#if !fedTotal}
          <div class="f-note">
            No differences — your local set matches the published copy.
          </div>
        {:else}
          <div class="fed-list">
            {#each fedGroupSections as s (s.kind)}
              <div class="fed-sec">
                {s.label} groups ({s.items.length})
              </div>
              {#each s.items as it (it.kind + "•" + it.path + "•" + it.name)}
                <div class="fed-item">
                  <div class="fed-name">{it.path || it.name}</div>
                  {#each it.changes || [] as c}
                    <div class="fed-chg">
                      <span class="fed-field">{c.field}:</span>
                      <span class="fed-old">{c.old}</span>
                      <span class="fed-arrow">→</span>
                      <span class="fed-new">{c.new}</span>
                    </div>
                  {/each}
                </div>
              {/each}
            {/each}
            {#each fedTrigSections as s (s.kind)}
              <div class="fed-sec">
                {s.label} triggers ({s.items.length})
              </div>
              {#each s.items as it (it.kind + "•" + it.path + "•" + it.name)}
                <div class="fed-item">
                  <div class="fed-name">
                    {it.name}
                    {#if it.path}<span class="fed-path">{it.path}</span>{/if}
                  </div>
                  {#if it.kind === "moved"}
                    <div class="fed-chg">
                      <span class="fed-field">Group:</span>
                      <span class="fed-old">{it.old_path || "(top level)"}</span
                      >
                      <span class="fed-arrow">→</span>
                      <span class="fed-new">{it.path || "(top level)"}</span>
                    </div>
                  {/if}
                  {#each it.changes || [] as c}
                    <div class="fed-chg">
      ...
      {/if}
      <div class="modal-actions">
        <button class="btn" on:click={() => (fuseEdits = null)}>Close</button>
      </div>
    </div>
  </div>
{/if}
...
{#if form}
  <div class="overlay" on:click|self={() => (form = null)}>
    <div class="modal form trig-form">
      <div class="modal-title">
        {formIsNew
          ? "New Trigger"
          : (formReadOnly ? "View Trigger" : "Edit Trigger") +
            (form.name ? ` - ${form.name}` : "")}
      </div>
      {#if form.path}
        <div class="f-trigpath" title={form.path}>{form.path}</div>
      {/if}
      {#if formReadOnly}
        <div class="f-ro">
          Fuse Triggers are maintained by officers — the package fields are
          read-only. Use the <b>Customize</b> tab to change this trigger's overlay,
          color, sounds, speech, and alert text on this PC.
        </div>
      {/if}

      <!-- Top-level panes. The nav sits OUTSIDE the fieldsets below so the
           tabs stay clickable in the read-only (member) view. -->
      <div class="tabs main-tabs">
        <button
          type="button"
          class="tab"
          class:on={mainTab === "general"}
          on:click={() => (mainTab = "general")}>General</button
        >
        <button
          type="button"
          class="tab"
          class:on={mainTab === "match"}
          on:click={() => (mainTab = "match")}>On Match</button
        >
        <button
          type="button"
          class="tab"
          class:on={mainTab === "enders"}
          on:click={() => (mainTab = "enders")}>End Early</button
        >
        {#if ovForm && formCustomize}
          <button
            type="button"
            class="tab"
            class:on={mainTab === "customize"}
            on:click={() => (mainTab = "customize")}>Customize</button
          >
        {/if}
      </div>

      {#if mainTab === "general"}
        <!-- fieldset natively disables every control inside it in one place;
           styled to mirror .modal's flex column so layout is unchanged -->
        <fieldset class="f-body" disabled={formReadOnly}>
          ...
          <label class="f-label" for="tf-cat">Overlay</label>
          <div class="f-catrow">
          ...
          {/if}
          <!-- Where this trigger's output lands, derived live from the actions
           below. Inline SVGs tinted the accent gold (stroke="currentColor" +
           CSS color) — emoji glyphs carry their own colors and can't be
           tinted. -->
          {#if form.timer_enabled}
            <div class="f-ovhint">
              ...
              This trigger sends data to the <b>Timer Bars</b> overlay of the above
              name.
            </div>
          {/if}
          {#if formShowsAlert}
            <div class="f-ovhint">
              ...
              This trigger sends data to the <b>Alerts</b> overlay of the above name.
            </div>
          {/if}
        </fieldset>
        {#if ovForm}
          <!-- Per-trigger color, for EVERY trigger: a compact row whose swatch
               shows the effective color (the overlay's until customized).
               Clicking the swatch opens the picker; a changed pick flips the
               state text to (Custom) and shows the reset back to default.
               Outside the fieldset — it's the local override layer, not a
               package field, so it stays live in the read-only member view. -->
          <div
            class="f-inline ccolor-row"
            title="Tints this trigger's timer bar and alert text inside its overlays (instead of the overlay's own color). Saved on this PC."
          >
            <span class="f-label">Custom Color</span>
            <input
              type="color"
              value={ovForm.colorOn ? ovForm.color : ccDefault}
              on:input={(e) => {
                ovForm.color = e.target.value;
                ovForm.colorOn = true;
              }}
            />
            <span class="ccolor-state"
              >({ovForm.colorOn ? "Custom" : "Overlay default"})</span
            >
            {#if ovForm.colorOn}
              <button
                type="button"
                class="btn ccolor-reset"
                title="Back to the overlay's own color"
                on:click={() => (ovForm.colorOn = false)}
                >Use overlay default</button
              >
            {/if}
          </div>
        {/if}
      {:else if mainTab === "match"}
        <fieldset class="f-body" disabled={formReadOnly}>
          <TriggerActions
          ...
          />
          <label class="f-chk">
            <input type="checkbox" bind:checked={form.copy_clipboard} /> Copy to
            clipboard
          </label>
          ...
          {#if form.use_counter}
            <div class="f-grid">
              <label class="f-label" for="tf-creset"
                >Reset after (seconds)</label
              >
              <input
                id="tf-creset"
                class="in num"
          ...
          {#if form.timer_enabled}
            ...
            {#if timerTab === "timer"}
              <div class="f-grid">
                ...
                />
                <label class="f-label" for="tf-behavior"
                  >If already running</label
                >
                <select
                  id="tf-behavior"
                  class="in"
                ...
                >
                  ...
                  <option value="RestartTimer"
                    >Restart this trigger's timer</option
                  >
                  <option value="IgnoreIfRunning">Ignore</option>
                </select>
              </div>
              <div class="f-note">
                Show bar for last: 0 shows the bar the whole time; a smaller
                value keeps it hidden until only that many seconds remain.
              </div>
              <!-- Early end conditions moved to the End Early tab. -->
      ...
      {:else if mainTab === "enders"}
        <fieldset class="f-body" disabled={formReadOnly}>
          ...
          <div class="f-note">
            If any of these matches a later log line, this trigger's timer ends
            early (before its countdown runs out).
          </div>
          {#if !form.timer_enabled}
            <div class="f-note">
              This trigger has no countdown timer — turn on
              <b>Start countdown timer</b> under On Match for these to apply.
            </div>
          {/if}
      ...
      {:else if mainTab === "customize" && ovForm}
        <!-- The personal override layer (Fuse triggers, everyone): live even
             in the read-only member view — members because the package fields
             are locked, officers because package edits change it for the
             whole guild. -->
        <div class="ovr-box">
          <div class="ovr-head">
            <span class="ovr-title">My Customization</span>
            <span class="ovr-sub"
              >Saved globally for all characters, survives package updates.</span
            >
            <button type="button" class="btn" on:click={resetOvForm}
              >Reset all</button
            >
          </div>
          <label class="f-label" for="tf-ovcat">Overlay</label>
          <select id="tf-ovcat" class="in" bind:value={ovForm.overlay}>
            <option value={null}
              >Package overlay ({form.pkg_category || "none"})</option
            >
            <option value="">No overlay</option>
            {#each ovCatOptions as c (c)}
              <option value={c}>{c}</option>
            {/each}
          </select>
          {#if form.timer_enabled}
            <label class="f-label" for="tf-ovtname">Timer name</label>
            <input
              id="tf-ovtname"
              class="in"
              placeholder={form.timer_name || form.name}
              bind:value={ovForm.timerName}
            />
            <div class="f-note">
              What this trigger's countdown bar displays. Captures like
              <code>$&#123;1&#125;</code> still substitute. Leave empty to use the
              package name.
            </div>
            <label class="f-label" for="tf-ovbehavior">If already running</label
            >
            <select
              id="tf-ovbehavior"
              class="in"
              bind:value={ovForm.startBehavior}
            >
              <option value={null}
                >Package setting ({START_BEHAVIOR_LABELS[
                  form.timer_start_behavior
                ] || START_BEHAVIOR_LABELS.StartNewTimer})</option
              >
              <option value="StartNewTimer"
                >{START_BEHAVIOR_LABELS.StartNewTimer}</option
              >
              <option value="RestartTimer"
                >{START_BEHAVIOR_LABELS.RestartTimer}</option
              >
              <option value="IgnoreIfRunning"
                >{START_BEHAVIOR_LABELS.IgnoreIfRunning}</option
              >
            </select>
          {/if}
          <!-- Same Custom Color row as the General tab — one shared state
               (ovForm.colorOn/color), two doors: the customization panel must
               be complete on its own, and General keeps it for every trigger. -->
          <div
            class="f-inline ccolor-row"
            title="Tints this trigger's timer bar and alert text inside its overlays (instead of the overlay's own color). Saved on this PC."
          >
          ...
          <TriggerOverrideSlot
            bind:ov={ovForm.on_match}
            pkg={form.on_match}
            {mediaFiles}
            onAdd={addMediaFile}
            onSample={sampleMediaFile}
          />
          {#if form.timer_enabled && form.ending_enabled}
            <div class="f-section">Timer Ending</div>
            <div
              class="f-inline"
              title="How long before the timer runs out this warning fires. Leave empty to use the package's timing."
            >
              <label class="f-label" for="tf-ovendsecs"
                >Seconds before end</label
              >
              <input
                id="tf-ovendsecs"
                ...
                type="number"
                min="1"
                placeholder={form.ending_seconds}
                bind:value={ovForm.endingSeconds}
              />
              <span class="ccolor-state"
                >({ovForm.endingSeconds === "" ||
                ovForm.endingSeconds === null ||
                +ovForm.endingSeconds <= 0
                  ? "Package"
                  : "Custom"})</span
              >
            </div>
            <TriggerOverrideSlot
              bind:ov={ovForm.ending}
              pkg={form.ending}
              {mediaFiles}
              onAdd={addMediaFile}
              onSample={sampleMediaFile}
            />
          {/if}
          {#if form.timer_enabled && form.ended_enabled}
            <div class="f-section">Timer Ended</div>
            <TriggerOverrideSlot
              bind:ov={ovForm.ended}
              pkg={form.ended}
              {mediaFiles}
              onAdd={addMediaFile}
              onSample={sampleMediaFile}
...
{#if fuseImp}
  <div class="overlay" on:click|self={() => (fuseImp = null)}>
    <div class="modal fimp-modal">
      <div class="modal-title">
        <Icon name="globe" /> Import Fuse Triggers from XML
      </div>
...
<style>
  ...
  /* The edit form's top-level panes: same tab anatomy, a step larger than the
     timer sub-tabs so the two levels read as different depths. */
  .main-tabs {
    margin: 2px 0 6px;
  }
  .main-tabs .tab {
  ...
  /* "View edits" diff modal (fed = Fuse edit diff) */
  .fed-modal {
    min-width: 460px;
    max-width: 580px;
  }
  ...
  /* Neutral per-overlay option (DPS Breakdown): deliberately ordinary — the
     red .sticky-box treatment is reserved for the switch people shouldn't
     flip by accident. */
  .opt-box {
    padding: 7px 9px;
    border: 1px solid var(--border);
  ...
  /* Non-boolean option controls in the special-overlay editor. */
  .opt-seg {
    display: inline-flex;
    gap: 2px;
  }
  ...
  /* "Default" beside a color swatch, greyed once there is nothing to clear. */
  .opt-seg-btn:disabled {
    opacity: 0.4;
    cursor: default;
  }
  .opt-seg-btn:disabled:hover {
    color: var(--text-muted);
    border-color: var(--border);
  }
  .opt-color {
  ...
  .opt-media-sel,
  /* "Bold" beside the Message font dropdown: box and word as one target, small
     enough that the pair still fits on a single row of a 320px modal. */
  .opt-bold {
    display: inline-flex;
    align-items: center;
    gap: 4px;
  ...
  .opt-media-btn:hover:not(:disabled) {
    color: var(--accent);
    border-color: var(--accent-dim);
  }
  .opt-media-btn:disabled {
    opacity: 0.45;
    cursor: default;
  }
  .opt-dec {
  ...
  /* Absolutely positioned: a margin-based dot collapses its top margin
     through the track and rides high instead of centering. */
  .sw-thumb {
    position: absolute;
    top: 2px;
    left: 2px;
  ...
  /* Gold-when-on, matching the Manage Timers enable switches. The red
     checked state above stays the sticky switch's own. */
  .sw.gold input:checked + .sw-track {
    background: rgba(200, 169, 81, 0.45);
  }
  .sw.gold input:checked + .sw-track .sw-thumb {
  ...
  .ov-card.sp {
    cursor: pointer;
    align-items: center;
    /* The preview row wraps below the text + button, card-wide — which is
       exactly "only as wide as the overlay button". */
    flex-wrap: wrap;
  }
  .sp-prev {
    flex-basis: 100%;
  ...
  .tbar.cleared {
    animation:
      tclearflash 0.5s ease-in-out 3,
      tclearfade 2.5s linear 1.5s forwards;
  }
  @keyframes tclearfade {
    to {
      opacity: 0;
    }
  }
  @keyframes tclearflash {
    0%,
    100% {
      filter: brightness(1);
    }
  ...
  /* The trigger editor runs wider than the other form modals: tabbed panes
     plus tri-state customization rows need the room. */
  .modal.form.trig-form {
    width: 520px;
  }
  /* The trigger name rides in the title; keep a long one on one line. */
  .trig-form .modal-title {
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
  }
  /* Where the trigger lives — the full group chain under the form title. */
  .f-trigpath {
    color: var(--text-muted);
    font-size: 11px;
    margin: -4px 0 2px;
  ...
  /* The form body: a fieldset so disabled={formReadOnly} switches every
     control off natively; mirrors .modal's column layout exactly. */
  .f-body {
    display: flex;
    flex-direction: column;
    gap: 7px;
  ...
  /* The row's button ("+ New" / "Create") must never be squeezed into a
     two-line wrap taller than the dropdown beside it: no shrink, no wrap,
     and stretch to exactly the dropdown's height. */
  .f-catrow .btn {
    flex-shrink: 0;
    white-space: nowrap;
    align-self: stretch;
  ...
  /* Overlay help rows under the assignment select: an accent-gold SVG icon
     (tinted via currentColor — emoji can't be recolored) + muted help text. */
  .f-ovhint {
    display: flex;
    align-items: center;
    gap: 6px;
  ...
  /* "My Customization" panel: the personal override layer, visually set apart
     from the package fields above it. */
  .ovr-box {
    display: flex;
    flex-direction: column;
    gap: 8px;
  ...
  /* Section headers inside the overlay edit form (Appearance / Behavior /
     Preview). */
  .f-sect {
    font-size: 10px;
    font-weight: 700;
  ...
  /* Per-trigger Custom Color row: swatch + state text + (when custom) the
     way back to the overlay's own color. */
  .ccolor-row {
    cursor: help;
  }
  .ccolor-state {
  ...
  /* The alerts preview's stopwatch sample — mirrors the overlay's age
     readout (pinned right, near-full-size mono digits) so the setting is
