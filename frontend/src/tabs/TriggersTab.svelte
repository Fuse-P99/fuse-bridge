<script>
  ...
  const SPECIAL_OVERLAYS = [
    ...
    },
    {
      ...
      desc: "Raid and event timers specific to their given raid — Ring War timer, Sirran timer, mob AE timers, etc. Hidden when nothing is running.",
    },
    {
      kind: "voicespeakers",
      name: "Voice Speakers",
    ...
    },
    {
      kind: "randoms",
      name: "Randoms",
      desc: "Live /random results, grouped by number rolled. Lists item when detected. Hidden when nobody is rolling.",
    ...
    },
    {
      kind: "threat",
      name: "Threat Meter",
      desc: "Your DPS, aggro-tool success counts, and estimated threat vs the tank on your target. Officer-only.",
      officer: true,
    },
  ];
  // The raid overlays render server raid data an unlinked client never
  // receives; local ones stand on their own. Officer-only overlays hide from
  // everyone else (the server enforces the role regardless).
