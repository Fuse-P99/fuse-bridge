  ...
  const SP_OPTS = {
    ...
    raidclerics: [
      ...
      },
      {
        ...
        label: "Show Timing",
        title:
          "Show “+3.4s” after each cleric in the CH chain — how long after the previous caster they started their cast. Shown for every cleric, refreshed on each new cast.",
        def: true,
      },
      {
        key: "pulseaudio",
        label: "Turn Cue",
        title:
          "Play one audio cue the moment the cleric before you starts casting — the audible version of your gold pre-cast ring appearing.",
        def: false,
      },
      {
        key: "pulsesound",
        ...
        type: "media",
        title:
          "Sound played when the cleric before you starts casting — any of your trigger sounds.",
  ...
  <div class="overlay" on:click|self={() => (catForm = null)}>
    <div class="modal form">
      ...
      {#if catForm.style.kind === "timers"}
        ...
        <div class="f-grid">
          <span class="f-label">Ended early</span>
          <label
            class="sw-row"
          ...
          >
            <span class="opt-lbl">Flash if ended early</span>
            <span class="sw gold">
              <input
                type="checkbox"
      ...
      {:else}
        <div class="f-grid">
          ...
          <span class="f-label">Stopwatch</span>
          <label
            class="sw-row"
            title="Append a ticking m:ss age to each alert on this overlay"
          >
            <span class="opt-lbl">Show age (m:ss)</span>
            <span class="sw gold">
              <input
                type="checkbox"
        ...
        <div class="f-note">
          A long Time Shown turns this overlay into a to-do list: an alert stays
          until its time runs out or one of its trigger's End Early conditions
          matches.
        </div>
      {/if}
      ...
      {:else}
        ...
        >
          {catForm.style.name ||
            "Alert text"}{#if catForm.style.alert_stopwatch}<span
              class="cf-prev-sw">0:42</span
            >{/if}
        </div>
      {/if}
...
<style>
  ...
  /* The alerts preview's stopwatch sample — mirrors the overlay's age
     readout so the setting is visible before saving. */
  .cf-prev-sw {
    margin-left: 6px;
    font-size: 0.72em;
    font-weight: 600;
