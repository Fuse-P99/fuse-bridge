<script>
  ...
  const SPECIAL_OVERLAYS = [
    ...
    },
    {
      kind: "randoms",
      name: "Randoms",
      desc: "Live /random results ranked highest first, grouped by the number being rolled. Names the item when any recent chat — guild, raid, group, say, ooc, auction or a tell — called it out with the same number. Hidden when nobody is rolling.",
  ...
  // receives; local ones stand on their own.
  $: visibleSpecials = SPECIAL_OVERLAYS.filter((so) => meta.linked || so.local);
