<script>
  ...
  // The hour boundary the fill is approaching: its tick index (0 = the
  ...
  function nextHourInfo(c, ms) {
    ...
    let h12 = nh % 12;
    if (h12 === 0) h12 = 12;
    return { hour: nh, pos: nh === 0 ? 100 : (nh / 24) * 100, label: h12 };
  }
  ...
  // per unobserved respawn — mirroring Gynok's own drift counter.
  ...
  const SPECIAL_OVERLAYS = [
    ...
    },
    {
      kind: "voicespeakers",
      name: "Voice Speakers",
      desc: "Displays current speaker(s) and user count for the Fuse discord #raid-voice channel.",
    },
  ];
      ...
      {#if world && world.enabled}
        <div class="world">
          ...
          <div class="w-sec">
            ...
            {#if gameClock && gameClock.have_game}
              ...
              <div class="w-day">
                <div
                  class="w-day-fill"
                  style="width:{(gameHourFloat(gameClock, now) / 24) * 100}%"
                ></div>
                {#each hourTicks as h}
                  <div
                    class="w-tick"
                ...
                <div
                  ...
                  title={(e.from_quake
                    ? "Anchored on the earthquake " +
                      agoStr(now - e.updated_ms) +
                      " — spawns 30 minutes after a quake"
                    : "Last report " + agoStr(now - e.updated_ms)) +
                    (v.marks > 0
                      ? ` — ${v.marks} respawn${v.marks === 1 ? "" : "s"} since a confirmed ToD`
                      : " (exact)")}
                >
                  <div class="tbar-fill w-fill" style="width:{v.frac * 100}%" />
                  <span class="tbar-name"
                    >{e.name}{#if e.from_quake}<span class="w-src">quake</span
                      >{/if}</span
                  >
                  <span class="tbar-time"
                    >{fmtLong(v.at - now)}{#if v.marks > 0}<span class="w-marks"
          ...
          <div class="w-sec">
            ...
            {#each world.boats || [] as b (b.key)}
              {#if b.have}
                {@const v = boatView(b, now)}
                <div
                  class="w-boat"
                  title="{b.name} — last sighting {agoStr(now - b.seen_ms)}"
                >
                  <div class="w-route">
                  ...
                  <div class="w-eta">
                    <span class="w-eta-t">{v.a > 0 ? fmtLong(v.a - now) : "—"}</span>
                    <span class="w-boat-name">{b.name}</span>
                    <span class="w-eta-t">{v.b > 0 ? fmtLong(v.b - now) : "—"}</span>
                  </div>
                </div>
              {:else}
                <div class="w-boat w-boat-none" title="Waiting for a dock announcement from anyone running the relay in {b.end_a} or {b.end_b}">
                  <div class="w-route">
                    <span class="w-port">{b.end_a}</span>
                    <div class="w-track"><div class="w-track-line"></div></div>
                    <span class="w-port w-port-r">{b.end_b}</span>
                  </div>
                  <div class="w-eta">
                    <span class="w-eta-t">—</span>
                    <span class="w-boat-name">{b.name} · no sighting yet</span>
                    <span class="w-eta-t">—</span>
                  </div>
                </div>
              {/if}
            ...
            >
              ...
              {#if catClosed[c.name]}
                <span class="cat-count"
                  >{c.timers.length} timer{c.timers.length === 1 ? "" : "s"}</span
                >
              {/if}
            </button>
    ...
    {:else if view === "overlays"}
      ...
      {#if ovDef.editing}
        ...
        <div class="ov-def ov-def-editing">
          ...
          <div class="ovd-actions">
            <button class="btn ovd-go" disabled={ovBusy} on:click={toggleDefaultEditing}
              >Done editing defaults</button
            >
      ...
      {:else if !ovDef.configured}
        ...
        <div class="ov-def ov-def-cta">
          ...
          <div class="ovd-actions">
            <button class="btn ovd-go" disabled={ovBusy} on:click={toggleDefaultEditing}
              >Configure default overlays</button
            >
        ...
        <div class="ov-def">
          <div class="ovd-actions">
            <span class="ovd-title ovd-inline"
              ...
              ></span
            >
            <button class="btn" disabled={ovBusy} on:click={toggleDefaultEditing}
      ...
      {:else}
        ...
        {#if fuseImp.removed_count}
          <div class="fimp-sec">
            ...
            <div class="fimp-list">
              {#each fuseImp.removed as p}<div class="fimp-row del">{p}</div>{/each}
        ...
        {#if fuseImp.changed_count}
          <div class="fimp-sec">
            ...
            <div class="fimp-list">
              {#each fuseImp.changed as p}<div class="fimp-row chg">{p}</div>{/each}
        ...
        {#if fuseImp.added_count}
          <div class="fimp-sec">
            ...
            <div class="fimp-list">
              {#each fuseImp.added as p}<div class="fimp-row add">{p}</div>{/each}
...
<style>
  ...
  /* the hour the fill is approaching — watch the gap close */
  .w-tick-next {
    width: 2px;
    height: 100%;
    background: var(--accent);
  ...
  /* the approaching hour's number, in a circle riding its tick */
  .w-hour-badge {
    position: absolute;
    top: 50%;
  ...
  /* where a timer came from, when it wasn't a Gynok report */
  .w-src {
    margin-left: 6px;
    padding: 0 4px;
    border-radius: 3px;
