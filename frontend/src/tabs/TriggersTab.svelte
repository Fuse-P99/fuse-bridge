<script>
  ...
  // them run. The default view stays the live board — that's what you want
  // mid-raid; the setup pages are for configuration passes.
  const PAGES = [
  ...
  function evTitle(e, v, ms) {
    if (e.from_quake) {
      return (
        ...
        " — roll is 30 minutes after last quake time"
      );
    }
    return (
      "Last report " +
  ...
  const SPECIAL_OVERLAYS = [
    ...
    },
    {
      kind: "othertimers",
      name: "Other Timers",
      desc: "Event timers for the current raid — Ring War waves and the Narandi countdown. Hidden when nothing is running.",
    },
    {
      kind: "voicespeakers",
      name: "Voice Speakers",
  ...
  <div class="main">
          ...
          {#if worldOpen}
            <div class="w-body" transition:slide|local={{ duration: 150 }}>
                ...
                {#each world.events || [] as e (e.key)}
                  ...
                  <div class="w-alarmrow">
                    {#if e.have}
                      ...
                      <div class="tbar w-bar" title={evTitle(e, v, now)}>
                        <div
              ...
              <div class="w-sec">
                ...
                <!-- Calibration for the boat lines above: ride one and this
                     turns the repeats into a measured loop time. -->
                <BoatRecorder />
              </div>
            </div>
          {/if}
    ...
    {:else}
      ...
      <!-- Overlay setup call-to-action: timers are only useful once the
           overlays showing them are positioned. Default layouts are the
           once-per-install step, so point every visitor at it. -->
      <div class="ov-cta">
        <span class="ov-cta-msg">
          <strong>New to timers?</strong> Check that you have some timers enabled
          you want to use and then set up your default overlays — position each overlay
          once and every character starts from that layout.
        </span>
