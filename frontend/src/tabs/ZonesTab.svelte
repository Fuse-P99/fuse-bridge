...
<div class="zones">
  {#if !$linked}
    <div class="unlinked">
      ...
      <div class="hint">
        Zone data comes from the Fuse server and requires a linked Discord
        account.
      </div>
      ...
      <div class="tree">
        {#each model.guilds as g (g.name)}
          <div class="row guild-row" on:click={() => toggleGuild(g.name)}>
            ...
            <!-- Only <Fuse> — the attendance log is our own guild's roster.
                 The $linked check is belt-and-braces (the whole tab is already
                 behind one) so the button can't outlive that gate if this tree
                 is ever moved: the endpoint rejects unlinked clients, and a
