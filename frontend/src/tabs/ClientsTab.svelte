<script>
  ...
  import { GetClients, GetClientActivity, SetClientMuted } from '../../wailsjs/go/main/App'
  ...
  async function toggleMute(c) {
    try {
      ...
      clients = clients.map(x => x.id === c.id ? { ...x, muted: !c.muted } : x)
    } catch (e) { error = String(e) }
  }
        ...
        <table>
          ...
          <tbody>
            {#each clients as c (c.id)}
              <tr class:connected={c.status === 'active' || c.status === 'connected'} class:mutedRow={c.muted}>
                <td class="c-status">
                  <span
                    class="dot {c.status}"
                ...
                <td class="c-name">{c.name}{#if c.muted}<span class="muted-tag">MUTED</span>{/if}</td>
                <td class="c-toon">
                ...
                <td class="c-mute">
                  <button
                    class="mute-btn"
                    class:muted={c.muted}
                    title={c.muted ? 'Resume accepting data from this client' : 'Ignore all data from this client'}
...
<style>
  ...
  .c-mute { width:74px; text-align:right; }
  .mute-btn {
    background:none; border:1px solid var(--border); border-radius:4px;
    color:var(--text-secondary); cursor:pointer; font-size:11px; padding:2px 10px;
    transition:color 0.15s, border-color 0.15s;
  }
  .mute-btn:hover { color:var(--text-primary); border-color:var(--text-muted); }
  .mute-btn.muted { color:#e3a008; border-color:#e3a008; }
  .muted-tag {
    color:#ef4444; font-size:9px; font-weight:800; letter-spacing:0.08em;
    margin-left:6px; vertical-align:middle;
  }
  tr.mutedRow td { opacity:0.55; }
  tr.mutedRow td.c-mute { opacity:1; }
