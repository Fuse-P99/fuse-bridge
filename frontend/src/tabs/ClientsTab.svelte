<script context="module">
  // Settings-dashboard aggregate cache: the tab is destroyed on every main-tab
  // switch, so the last fetch lives here (5-minute TTL) and reopening the
...
<script>
  ...
  import SettingsDashboard from "../lib/dashboard/SettingsDashboard.svelte";
  ...
  // Table order: activity bands first (green → yellow → grey), alphabetical
  // within each band — so the installs that matter right now sit on top and a
  // given client is findable by name inside its band.
  const STATUS_RANK = { active: 0, connected: 1 };
  ...
  // ── Settings dashboard (click the one-line strip) ─────────────────────────
  // The strip stays on top in both views; the body below swaps between the
  // client list and the aggregate of every install's daily settings snapshot.
  let mainView = "list"; // "list" | "dashboard"
  let summary = mSummary;
  let summaryLoading = false;
  let summaryErr = "";
  async function loadSummary(force = false) {
    if (!force && mSummary && Date.now() - mSummaryAt < SUMMARY_TTL_MS) {
      summary = mSummary;
      return;
    }
    summaryLoading = true;
  ...
  function toggleView() {
    mainView = mainView === "list" ? "dashboard" : "list";
    if (mainView === "dashboard") loadSummary();
  }
  function dashKey(e) {
  ...
  $: coverage = (summary && summary.reporting) || {};
  $: coverageTip = `**Reporting installs**\n|Linked|${coverage.linked ?? 0}\n|Unlinked|${coverage.unlinked ?? 0}\n|Window|${coverage.window_days ?? 14} days\nInstalls whose settings snapshot updated inside the window. Each install reports once a day during off hours.`;
  $: summaryStamp =
    summary && summary.generated_ms
      ? new Date(summary.generated_ms).toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        })
      : "";
  ...
  // An unlinked install (no member behind it) renders its raw IP as the
  // group header — the least informative rows in the report. Fold all of
  // those into ONE "Unidentified" group pinned to the bottom, keeping each
  // row's IP visible inside it for follow-up. Keyed on the server's linked
  // flag, not on whether the name happens to look like an IP.
  const UNIDENT = "Unidentified";
  ...
  $: cgGroups = (() => {
    const rows = cgGuildSel
      ...
      : crossGuild;
    const out = [];
    const unident = { client: UNIDENT, unident: true, toons: [] };
    for (const cg of rows) {
      if (!cg.linked) {
        unident.toons.push(cg);
        continue;
      }
    ...
    if (unident.toons.length) {
      unident.toons.sort((a, b) => a.toon.localeCompare(b.toon));
      out.push(unident);
    }
    return out;
  })();
  ...
  async function load() {
    try {
      // Independent fetches, so run them TOGETHER: the tab's load time is the
      // slowest call, not the sum of four server round trips.
      const [cl, act, feeds, cg] = await Promise.all([
        GetClients().then((v) => v || {}),
        GetClientActivity().catch(() => []),
        GetOtherGuildChat().catch(() => []),
        GetCrossGuildToons().catch(() => []),
      ]);
      clients = cl.clients || [];
...
<div class="clients">
  ...
  {:else}
    <!-- One-line fleet dashboard. Clicking it opens the settings dashboard
         (the strip stays on top); clicking again returns to the list. -->
    {#if clients.length}
      <div
        class="dash"
        ...
        tabindex="0"
        title={mainView === "list"
          ? "Open the settings dashboard — how the fleet is configured"
          : "Back to the client list"}
        on:click={toggleView}
        on:keydown={dashKey}
      >
        {#if mainView === "dashboard"}
          <span class="back-pill">◂ Client list</span>
        ...
        {#if mainView === "dashboard"}
          <div class="dash-sep"></div>
          <div class="stat" title={coverageTip}>
            <span class="stat-val">{coverage.installs ?? 0}</span>
            <span class="stat-label">reported · 14d</span>
          </div>
          {#if summaryStamp}
            <span
              class="dash-updated"
              title="When the server last built this aggregate (it caches for five minutes)"
              >updated {summaryStamp}</span
            >
          {/if}
          <button
            class="mute-btn dash-refresh"
            title="Re-fetch the aggregate now"
            disabled={summaryLoading}
            on:click|stopPropagation={() => loadSummary(true)}
            >{summaryLoading ? "…" : "Refresh"}</button
          >
        {/if}
      </div>
    {/if}
            ...
            {#each sortedClients as c (c.id)}
              ...
              >
                ...
                <td class="c-name"
                  >{#if c.linked}<span
                      class="lock-wrap"
                      title="Linked to Discord — this install is attributed to a member"
                      ><svg
                        class="lock"
                        viewBox="0 0 24 24"
                        width="11"
                        ...
                        /></svg
                      ></span
                    >{/if}{c.name}{#if c.muted}<span class="muted-tag">MUTED</span
                    >{/if}</td
                >
                <td class="c-toon">
                ...
                {#each grp.toons as cg (cg.client + cg.toon)}
                  <div class="log-line cg-row">
                    ...
                    {#if grp.unident}
                      <span
                        class="ogc-time"
                        title="the install's IP — no linked member name to show"
                        >{cg.client}</span
                      >
                    {/if}
...
<style>
  ...
  /* The strip doubles as the settings-dashboard toggle. */
  .dash[role="button"] {
    cursor: pointer;
    transition: border-color 0.15s;
  }
  .dash[role="button"]:hover,
  .dash[role="button"]:focus-visible {
  ...
  /* Linked-install lock, before the name. */
  .lock-wrap {
