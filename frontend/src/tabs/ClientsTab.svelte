<script>
  ...
  // parked in non-Fuse guilds. Admin mode only, and the server enforces its
  // own admin list on top (non-admins get an empty list back).
  let adminMode = false;
  let guildFeeds = [];
  let selFeed = "";
  // Cross-guild report: characters on members' installs whose latest /who
  // sighting puts them in a non-Fuse guild. Same admin gating as the feeds.
  let crossGuild = [];
  $: curFeed = guildFeeds.find((f) => f.guild === selFeed) || guildFeeds[0] || null;
  ...
  onMount(async () => {
    adminMode = await IsAdminMode().catch(() => false);
    await load();
...
<div class="clients">
  ...
  {:else}
    ...
    <!-- Bottom panels pinned under the table: Activity, plus (admin only)
         the other-guild chat feeds -->
    <div class="bottom-panels">
      ...
      {/if}
      {#if adminMode && guildFeeds.length}
        <div class="activity">
          <div class="section-label">Other Guild Chat</div>
          <div class="ogc-tabs">
            {#each guildFeeds as f (f.guild)}
              <button
                class="ogc-tab"
                class:active={curFeed && curFeed.guild === f.guild}
                on:click={() => (selFeed = f.guild)}
