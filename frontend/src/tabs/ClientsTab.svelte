<script>
  ...
  // A client the server knows only by IP address (no linked display name)
  // renders its raw IP as the group header — the least informative rows in
  // the report. Fold all of those into ONE "Unidentified" group pinned to
  // the bottom, keeping each row's IP visible inside it for follow-up.
  const UNIDENT = "Unidentified";
  const looksLikeIP = (s) =>
    /^\d{1,3}(\.\d{1,3}){3}$/.test(s) || /^[0-9a-fA-F:]*:[0-9a-fA-F:]+$/.test(s);
