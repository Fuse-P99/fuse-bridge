...
func fetchCharInfos(names []string) map[string]CharInfo {
	...
	if len(names) == 0 || !serverForwardOK() {
		return out
	}
...
func getAllCharNames(eqDir string) []string {
	...
	for _, charName := range logFileCharNames(eqDir) {
...
// logFileCharNames returns the characters that have an eqlog file under eqDir —
// i.e. the toons actually played on this machine. Unlike getAllCharNames it
// doesn't fold in the zone cache, which is full of other players seen in /who.
func logFileCharNames(eqDir string) []string {
...
// logFileCharKeys returns the per-(character, world) storage keys of every
// character with an eqlog under eqDir, mapped to their display-cased bare
// names. Unlike logFileCharNames it does NOT fold same-named characters on
// different servers together — Franko(Blue) and Franko(Green) are two entries.
func logFileCharKeys(eqDir string) map[string]string {
	...
	for _, logsDir := range logsDirCandidates(eqDir) {
		...
		for _, e := range entries {
			...
			name := charNameFromLog(n)
			if name == "" {
				continue
			}
			key := storeKeyFor(name, serverTokenFromLog(n))
			if _, ok := out[key]; !ok {
				out[key] = name
			}
		}
