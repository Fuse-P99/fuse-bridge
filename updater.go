...
// startUpdaterHelper launches the swap-and-relaunch PowerShell detached from
// this process, so a Windows Job Object around an autostart (Run-key) launch
// can't tear the helper down when we exit — a cause of "updated at boot, never
// reopened". CREATE_BREAKAWAY_FROM_JOB is refused by jobs that don't allow
// breakaway (CreateProcess fails), so it degrades: breakaway+detached, then
// detached, then a plain hidden spawn. All keep CREATE_NO_WINDOW so no console
// flashes. Returns an error only if every attempt fails to start (the caller
// then aborts the update and stays on the current build — a safe no-op).
func startUpdaterHelper(script string) error {
	...
	if err := attempt(createNoWindow | detachedProcess | createBreakawayJob); err == nil {
		return nil
	}
	if err := attempt(createNoWindow | detachedProcess); err == nil {
		return nil
	}
