...
func exitIfAlreadyRunning() {
	...
	start := time.Now()
	deadline := start.Add(singleInstanceWait)
	signalled := false
	for {
		...
		if err != windows.ERROR_ALREADY_EXISTS {
			...
			if waited := time.Since(start); waited > 300*time.Millisecond {
				writeLog(fmt.Sprintf("single-instance: acquired after waiting %dms (relaunch overlap)", waited.Milliseconds()))
			}
		...
		if time.Now().After(deadline) {
			writeLog(fmt.Sprintf("single-instance: another FuseBridge held the lock for %ds — showing it and exiting", int(singleInstanceWait.Seconds())))
			os.Exit(0)
		}
