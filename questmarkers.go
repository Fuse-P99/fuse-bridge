...
type QuestMarker struct {
	...
	// QuestID and Char say what the marker is FOR, so clicking the flag on the
	// map can jump straight to that character's quest (Characters → Quests).
	// Zero/empty on markers placed without a character in scope (the officer
	// editor) or by older builds; the click then falls back to the current
	// character.
...
func (a *App) AddQuestMarker(zone string, y, x float64, label string, questID int, char string) {
	...
	m := QuestMarker{
		...
		QuestID: questID, Char: strings.TrimSpace(char),
	}
	qmMu.Lock()
	defer qmMu.Unlock()
	qmLoadLocked()
...
// has recorded.
func addQuestMarkerAuto(zone string, y, x float64, label string, center bool, questID int, char string) {
	zone = canonicalZone(strings.TrimSpace(zone))
	if zone == "" || strings.TrimSpace(label) == "" {
		return
	}
