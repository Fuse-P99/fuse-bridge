...
func (a *App) ShowHoverCard(fromWindow, kind, query string, cx, cy float64) {
	...
	w.Show()
	// The card and the overlay it serves are both always-on-top, and a window
	// shown without activation is NOT moved to the front of that band — so the
	// card came up BEHIND the very overlay the cursor was over. Re-asserting
	// HWND_TOPMOST is the documented way to raise an already-topmost window
	// above its peers, and with SWP_NOACTIVATE it does so without touching
	// focus (the same call the topmost self-heal makes). Every show, not just
	// the first: a click on the overlay since the last hover has put it back
	// on top.
	if hwnd := w32.HWND(uintptr(w.NativeWindow())); hwnd != 0 {
