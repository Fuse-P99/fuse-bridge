...
type CategoryStyle struct {
	...
	// FlashEndedEarly: when an End Early condition ends this overlay's timer
	// bar or alert, flash it three times and fade it out (a few readable
	// seconds) instead of removing it instantly. Default ON; per-overlay
	// opt-out. Both kinds.
...
func resolveCatStyle(kind, name string) CategoryStyle {
	...
	// Authoritative, default TRUE — records from before the setting existed
	// are seeded in loadCatStyles, so false here is a real opt-out.
	out.FlashEndedEarly = s.FlashEndedEarly
	return out
}
...
func loadCatStyles() {
	...
	for k, raw := range f.Cats {
		...
		if json.Unmarshal(raw, &probe) == nil {
			...
			// Same absence-means-unconfigured rule for the ended-early flash,
			// whose default is ON: a record saved before the setting existed
			// must not read as an opt-out.
			if _, ok := probe["flash_ended_early"]; !ok {
				s.FlashEndedEarly = true
			}
		}
		catStyles[k] = &s
	}
