...
func popoutDragBegin(w *application.WebviewWindow, name string) {
	...
	// Since 2.5.3369 the title bar starts manual drags (popouts_drag.go) and
	// never enters the OS move loop, so this hook firing at all is news worth
	// logging — as is Windows refusing the activation, which was the leading
	// suspect for why bracketing alone didn't cure the wedge in the field.
	if w32.GetForegroundWindow() != hwnd {
		writeLog("overlay drag: OS move loop on " + name + " — foreground request DENIED")
	} else {
		writeLog("overlay drag: OS move loop on " + name + " — activated")
	}
}
