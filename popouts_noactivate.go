...
//
// The function is kept because the module patch still honours the style bit
// (a window without it shows exactly as stock Wails), so any future
// experiment has the tested primitive ready.
//
// TAKE TWO shipped in v2.5.3569 as "no-steal shows": the module patch (V2)
// keeps a registry of overlay HWNDs — application.FuseBridgeSetNoStealShow,
// called at overlay creation in popouts.go — and gives registered windows
// SW_SHOWNOACTIVATE shows and SWP_NOACTIVATE re-ordering while leaving the
// window bone-stock for clicks and drags. Non-activating SHOWS were always
// the safe half of the old design; suppressing CLICK activation was the half
// that wedged. See eq-relay/patches/README.md.
