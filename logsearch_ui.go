...
// expanded view shows.
const (
	// Files are searched oldest-first, so overflow past this cap falls off the
	// OLD end: what the UI gets is the most recent logSearchMaxHits matches.
	logSearchMaxHits = 5000
	logContextRadius = 500
	// How much of a file's head is read to find the timestamp it starts at.
	// The first line of an EQ log is stamped; this is bounded so a file that
	// isn't one can't cost more than a single read.
	logStartProbeBytes = 8192
...
//
// Hits come back in log order — by the line's own timestamp, oldest first —
// however many files they came from, so an archived search reads exactly like
// scrolling one long log instead of the live file followed by shuffled blocks
// of archives.
func (a *App) SearchLogs(character, query string, startMs, endMs int64, includeArchived bool) LogSearchResult {
	...
	// Walk the files in the order their contents happened, not the order the
	// directory handed them over. Two things depend on it: the hit list comes
	// out all but sorted already, and the cap then drops the OLDEST overflow
	// rather than whichever file happened to be scanned last.
	spans := orderLogFiles(files)
	byFile := make(map[string]logFileSpan, len(spans))
	...
	for _, sp := range spans {
		...
		if info, err := os.Stat(sp.path); err == nil && info.ModTime().Before(start) {
			continue
		}
		searchLogFile(sp.path, q, start, end, &res)
	}
	sortLogHits(res.Hits, byFile)
	return res
}

// logFileSpan pairs a log file with the timestamp its content begins at, plus
// its position in the chronological walk — the tiebreaker for hits that share
// a timestamp.
type logFileSpan struct {
	path    string
	startMs int64
	idx     int
}

// orderLogFiles sorts the files to search by when their content starts.
func orderLogFiles(files []string) []logFileSpan {
	spans := make([]logFileSpan, 0, len(files))
	for _, f := range files {
		spans = append(spans, logFileSpan{path: f, startMs: logFileStartMs(f)})
	}
...
// logFileStartMs is when a log file's content begins: the first bracketed
// timestamp in its head. Falls back to the modification time, which still
// orders a stampless file sensibly instead of dropping it to the front of
// everything; 0 only if the file can't be read at all.
func logFileStartMs(path string) int64 {
	if fh, err := os.Open(path); err == nil {
		buf := make([]byte, logStartProbeBytes)
		n, _ := io.ReadFull(fh, buf)
		fh.Close()
		for _, line := range strings.Split(string(buf[:n]), "\n") {
			if ts, ok := parseLogLineTime(strings.TrimRight(line, "\r")); ok {
				return ts.UnixMilli()
			}
		}
	}
...
// sortLogHits puts the hit list in log order: by the line's own timestamp,
// oldest first. A line carrying no timestamp (the leading junk before a file's
// first stamped line) sorts with the file it came from rather than to the top
// of everything, and hits sharing a timestamp keep file-then-line order — so
// a single-file search comes out exactly as the file reads.
func sortLogHits(hits []LogSearchHit, spans map[string]logFileSpan) {
	key := func(h LogSearchHit) (int64, int) {
		sp := spans[h.File]
		if h.AtMs != 0 {
			return h.AtMs, sp.idx
		}
		return sp.startMs, sp.idx
	}
	...
	for {
		...
		if len(raw) > 0 {
			...
			if inRange && strings.Contains(strings.ToLower(line), qLower) {
				...
				// Over the cap, the oldest match falls off the front: files
				// arrive oldest-first and each is chronological, so what
				// survives is the most recent window of matches — the end of
				// the search a reader is nearly always after. Total still
				// counts every one.
