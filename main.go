...
func main() {
	...
	// FIRST line of every launch, before anything can exit — so a process that
	// bows out in exitIfAlreadyRunning (or anywhere before the later "FuseBridge
	// starting" line) still leaves a trace. Critical for diagnosing update
	// relaunches: it tells us the relaunched process actually started.
	writeLog(fmt.Sprintf("process launched pid=%d clientVersion=%s", os.Getpid(), clientVersion))
	...
	// Daily settings snapshot (off-hours; settings_snapshot.go).
	startSettingsSnapshotLoop()
	...
	go func() {
		...
		// Re-identify hourly: keeps the version, toon inventory, and each
		// log's last-played mtime fresh for clients that stay up for days
		// (the cross-guild report shows those as the member's own last play).
		go func() {
			for range time.Tick(time.Hour) {
				identifyClient(installDir)
			}
		}()
	...
	go func() {
		for {
			...
			case line := <-rawLines:
				...
				// Guild-join pair watcher: flags a character who joins a
				// non-Fuse guild so their guild chat is diverted server-side.
				RecordGuildJoinLine(line)
				...
				// Our own guild chat, straight into the Guild Chat overlay: the
				// server's copy of the same line is a second or three behind,
				// and merges onto this row when it lands (guildfeed.go).
				RecordGuildChatLocalLine(line)
				...
				if y, x, z, ok := ExtractLoc(line); ok {
					...
					// Sharing a position off the home world would paint the Blue
					// map with a marker for someone who isn't on Blue — never do
					// it (SendMapLoc double-guards this too).
					if onHomeServer() && GetSettings().ShareMapPosition {
						sender.SendMapLoc(currentCharName, GetPosition())
					}
				}
				// Off the home world nothing forwards: no guild chat, no TOD/slain
				// anchor, no /who block. Local tracking above still runs; only the
				// server-bound enqueues are gated. (sender.send drops the queue
				// too — this keeps the status log honest and saves the work.)
				if !onHomeServer() {
					continue
				}
				if ShouldForward(line) {
					line = rewriteSelfGuildSay(line)
					// An unlinked client can only feed the other-guild intel
					// endpoint, which takes guild chat alone — skip everything
					// else so nothing extra queues for an unauthenticated URL
					// (the sender routes the queue there while unlinked).
					if IsLinked() || guildChatPattern.MatchString(line) {
						...
						// Engage alerts are time-critical — flush immediately rather
						// than waiting for the 2-second batch window.
