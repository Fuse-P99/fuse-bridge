...
//
//	handin    mob, faction, plat, items in and out
//	combine   tradeskill, skill, items in (with consumed flags) and out
//	loot      mob, items out
//	ground    zone, items out
//	dialogue  mob, say, faction, items out
type QuestStep struct {
	Kind       string `json:"kind"`
	Tradeskill string `json:"tradeskill"`
	SkillReq   int    `json:"skill_req"`
	// MobName and ZoneName come back resolved for display and are ignored on
	// save — mob names aren't unique in eqmobs, so the id is the only safe
	// reference.
...
// needs from outside, priced. Everything the quest's own steps produce has
