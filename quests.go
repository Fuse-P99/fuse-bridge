...
type QuestReward struct {
	Kind string `json:"kind"`
	Name string `json:"name"`
	// Slot is the reward item's equipment slot, resolved server-side for
	// display and filtering. Belongs to the item rather than the quest, so it
	// comes back on a read and is ignored on save.
...
// (kept in step with the server's Quest struct in quests.go)
...
type Quest struct {
	...
	// DirectRewards: rewards price by their own sale history only — the
	// server's component walk skips this quest (Ring War loot).
	DirectRewards bool          `json:"direct_rewards"`
