...
// fetchSettingsSummary pulls the officer-level settings dashboard aggregate
// (clientSettings.go on the server). Passed through as a generic map — the
// dashboard is the only reader and its shape is the server's to evolve.
func fetchSettingsSummary() (map[string]any, error) {
	base := strings.TrimSuffix(serverURL, "/submit")
	req, err := http.NewRequest(http.MethodGet, base+"/clients/settings/summary", nil)
	if err != nil {
		return nil, err
	}
	req.Header.Set("Authorization", authHeader())
	...
	case http.StatusForbidden, http.StatusUnauthorized:
		return nil, fmt.Errorf("officers only")
	default:
		return nil, fmt.Errorf("server returned %d", resp.StatusCode)
	}
...
// sighting places it in a non-Fuse guild. LastPlayedMs is the member's own
// last play (their install's log mtime), unlike GuildSeenMs which times the
// character's last /who sighting whoever was on it; 0 until their client
// reports it.
type CrossGuildToon struct {
	...
	Linked       bool   `json:"linked"` // the install is attributed to a member
}
