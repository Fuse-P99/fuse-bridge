...
// fetchOtherGuildChat pulls the admin-only other-guild chat feeds. A 403
// (caller isn't on the server's admin list) comes back as an empty list, not
// an error — the section simply doesn't render.
...
// fetchCrossGuildToons pulls the admin-only cross-guild character report.
// 403 (not on the server's admin list) is an empty report, not an error.
