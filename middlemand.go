...
func (c *mmConn) handlePacket(data []byte, from *net.UDPAddr) {
	...
	if from.IP.Equal(c.remoteAddr.IP) && from.Port == c.remoteAddr.Port {
		...
		// New client, or the previous session went quiet: start serving this one.
		if !c.inSession || now.Sub(c.lastRecv) > mmSessionTimeout {
			c.localAddr = from
			c.sequenceFree()
		}
...
func SetMiddlemandEnabled(enabled bool) {
	...
	if enabled {
		if err := StartMiddlemand(); err != nil {
			addStatus("Middlemand failed to start: %v", err)
			return
		}
