...
// overlay compares the viewer's hate to the main tank's. Reading the table is
// open to every linked member — it was officer-only during calibration, and
// was opened up once the model had been tested. Raid identification only gates
// the threat-gauge section, never the parse.
...
var (
	...
	// Sourceless damage on a mob: our own spell landing, our damage shield, or
	// a proc. Which one it is depends on whether we have a cast in flight, and
	// failing that on the DS flavor line below.
	threatNonMeleeAnonRE = regexp.MustCompile(`^(.+?) was hit by non-melee for (\d+) points? of damage\.$`)
	// A damage shield announces itself the instant before its damage line:
	// "Sabretooth tiger was pierced by thorns." / "was burned." / "was
	// tormented." — the discriminator between DS and a proc when no cast is in
	// flight. Users can filter these lines, so ABSENCE proves nothing.
	threatDSFlavorRE = regexp.MustCompile(`^(.+?) was (?:pierced by thorns|burned|tormented)\.$`)
...
type mobThreat struct {
	...
	spellHate int
	// catDmg splits this fight's outgoing damage by type for the optional
	// DPS-breakdown bar (melee verbs, bow/throw, spell, damage shield). Every
	// tally here is also in mt.damage — the headline DPS counts ALL damage,
	// matching the raid-DPS board's row for the same character.
	catDmg map[string]int
}

// threatCatFor buckets a first-person melee verb for the DPS breakdown. The
// bare "hit" verb is how bow and thrown damage prints (a wielded weapon
// always names its own verb), so it wears that label; verbs beyond the
// common seven fold into "Other" — the breakdown palette has eight validated
// hues, and a rare verb doesn't earn a ninth.
func threatCatFor(verb string) string {
	switch verb {
	case "slash":
		return "Slash"
	case "crush":
...
// threatCatAdd tallies breakdown damage on a fight ledger entry.
func threatCatAdd(mt *mobThreat, cat string, dmg int) {
	if mt.catDmg == nil {
		mt.catDmg = map[string]int{}
	}
	mt.catDmg[cat] += dmg
}

// ThreatCatUI is one damage-type slice of the current fight's breakdown.
type ThreatCatUI struct {
...
var (
	...
	// The last damage-shield flavor line seen ("X was pierced by thorns."),
	// normalized mob + log time. The non-melee damage line that follows it
	// within the window is DS; without it (line filtered, or genuinely a
	// proc) the damage counts as proc instead.
	threatDSFlavorMob string
	threatDSFlavorAt  time.Time
...
func threatWeaponFor(name string) (threatWeaponInfo, bool) {
	...
	weapMu.Unlock()
	if (!ok || !info.Found) && tryDue && serverForwardOK() {
...
func fetchThreatSpells(names []string) {
	if !serverForwardOK() || len(names) == 0 {
		return
	}
	body, _ := json.Marshal(struct {
		Names []string `json:"names"`
...
func RecordThreatLine(line string) {
	...
	if !procEffect && !castArmed &&
		...
		// Damage-shield flavor lines — the DS/proc discriminator.
		!strings.HasSuffix(content, " was pierced by thorns.") &&
		!strings.HasSuffix(content, " was burned.") &&
		!strings.HasSuffix(content, " was tormented.") &&
	...
	// MUST precede the " of damage." case below — this line ends with that
	// suffix too, and sitting after it this case was unreachable: the melee
	// case swallowed every anonymous non-melee line and matched nothing,
	// which silently disabled both the DS/proc split and the cast-in-flight
	// spell attribution. The "You were hit by non-melee" INCOMING form is
	// excluded so it still reaches the melee case's engagement handling.
	//
	// This line names no source. A character only ever sees it for damage
	// THEY caused — their damage shield, a proc, or their own spell
	// landing — so the discriminators are, in order:
	//
	//   cast in flight → our nuke landed. Hate is the spell's BASE value
	//                    from the ratings table, never the damage rolled.
	//   DS flavor line → a damage shield ("X was pierced by thorns.",
	//                    same instant). Real damage, but zero hate, so
	//                    the ledger must not move.
	//   neither        → a proc, or a DS whose flavor line the user has
	//                    filtered — counted as proc damage by choice.
	case strings.Contains(content, " was hit by non-melee for ") &&
		!strings.HasPrefix(content, "You were hit by non-melee"):
		threatProc3p++
		threatProc3pLast = content
		if m := threatNonMeleeAnonRE.FindStringSubmatch(content); m != nil {
			// A cast in flight claims the line ONLY when it resolves and its
			// window is still open. It must never swallow the line otherwise:
			// a buff cast produces no damage landing, so its pend entry
			// dangles armed — and with an early return here, one buff cast
			// made every later DS tick and proc vanish from the parse until
			...
			if !claimed {
				// No cast in flight: damage shield or proc, split by the DS
				// flavor line ("X was pierced by thorns.") that precedes a DS
				// hit in the same instant. The flavor line is filterable, so
				// no-flavor means proc BY CHOICE, not by proof. Either way
				// zero hate — but the damage is real and counts toward
				// mt.damage, so the headline DPS agrees with the raid-DPS
				// board's row (which has always counted self non-melee).
				dmg, _ := strconv.Atoi(m[2])
				norm := normThreatMob(m[1])
				// The flavor line and its damage line share one log instant;
				// 1s covers the whole-second stamp boundary. Consumed on use —
				// one flavor line vouches for exactly one damage line, so a
				// proc landing right after a DS tick isn't swept up with it.
				if threatDSFlavorMob == norm &&
					!threatDSFlavorAt.IsZero() &&
					at.Sub(threatDSFlavorAt) <= time.Second {
					// Flavor-confirmed DS may OPEN the fight: on a DS pull
					// this pair is the first thing in the log — the mob's own
					// swing line arrives after it, so requiring an existing
					// ledger entry dropped the opening tick. lastAct is
					// stamped for the same reason: a freshly ensured entry
					// with a zero lastAct read as long-idle, and the FIRST
					// swing after the pull restarted the window and wiped
					// this very tally.
					mt, ch := threatEnsureLocked(norm, m[1], at, tun)
					changed = ch
					mt.damage += dmg
					mt.lastAct = at
					threatCatAdd(mt, "Dmg Shield", dmg)
					threatDSFlavorMob = ""
				} else if mt := threatMobs[norm]; mt != nil {
					// Unconfirmed = proc. A proc only comes off our own swing,
					// so its mob is already in the ledger — the existing-entry
					// requirement stays, keeping a stray unpaired tick from
					// opening or extending a fight (lastAct untouched).
					mt.damage += dmg
					threatCatAdd(mt, "Spell", dmg)
				}
			}
	...
	// non-melee lines are handled above (named form by suffix, anonymous form
	// by the case just before this one) and can't reach this case.
	//
	...
	case strings.HasSuffix(content, " of damage."):
		if m := threatMeleeHitRE.FindStringSubmatch(content); m != nil {
			...
			changed = ch
			mt.damage += dmg
			threatCatAdd(mt, threatCatFor(m[1]), dmg)
	...
	case strings.HasSuffix(content, " was pierced by thorns.") ||
		strings.HasSuffix(content, " was burned.") ||
		strings.HasSuffix(content, " was tormented."):
		// The DS announcement — remembered so the non-melee damage line that
		// follows (same log instant) is classified as damage shield.
		if m := threatDSFlavorRE.FindStringSubmatch(content); m != nil {
			threatDSFlavorMob = normThreatMob(m[1])
			threatDSFlavorAt = at
		}
...
// ── server table fetch + bound getter ────────────────────────────────────────
...
type ThreatMeterUI struct {
	// Officer is vestigial: reads were officer-gated during calibration and
	// are open to every member now, so this is always true. Kept so the
	// overlay's stored default shape doesn't change; delete both together.
	...
	// Breakdown is the fight's outgoing damage split by type, biggest first.
	// Its slices sum to OwnDamage — the headline DPS counts all damage,
	// damage shield included.
	Breakdown []ThreatCatUI `json:"breakdown,omitempty"`
	// The live two-hand swing model — threat per swing, backed-out weapon
	// DMG, and estimated base delay per hand, plus the haste the delay
	// estimate divided out. Offhand fields are meaningful when Dual. No longer
	// rendered on the overlay (that readout was for calibration); these still
	// feed GetThreatDebug, which is how a bad reading gets diagnosed.
...
func (a *App) GetThreatMeter() ThreatMeterUI {
	...
	out.RaidMode = serverForwardOK() && threatInLiveRaidZone()
	...
	if mt != nil && now.Sub(mt.lastAct) <= threatEngagedIdle(tun) {
		...
		// Damage-type composition, biggest slice first — the overlay renders
		// the segments in this order (order by size; color by identity).
		if len(mt.catDmg) > 0 {
			cats := make([]ThreatCatUI, 0, len(mt.catDmg))
			for k, v := range mt.catDmg {
				cats = append(cats, ThreatCatUI{Label: k, Dmg: v})
			}
			sort.Slice(cats, func(i, j int) bool {
				if cats[i].Dmg != cats[j].Dmg {
					return cats[i].Dmg > cats[j].Dmg
				}
				return cats[i].Label < cats[j].Label
			})
			out.Breakdown = cats
		}
	...
	if !out.RaidMode || !serverForwardOK() || out.Mob == "" {
		return out
	}
