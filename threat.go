...
func threatInventory() invScan {
	...
	if note != invNote {
		invNote = note
		if !invCache.found {
			addStatus("Threat Meter: no inventory file for %s — run /outputfile inventory in game to enable gear-based threat.", name)
		} else {
			...
			addStatus("Threat Meter: %s gear — primary %s · offhand %s · worn haste %d%%.", name, p, s, invCache.wornPct)
		}
	}
	return invCache
}
...
// real log line, capturing the target's name. The wiki writes the target as
// "Soandso" ("Soandso's movements slow."), so that word becomes the capture
...
func threatLandRE(template string) *regexp.Regexp {
	...
	i := strings.Index(strings.ToLower(t), "soandso")
	if i < 0 {
		return nil // no target placeholder — can't tell whose it was
	}
	pat := "^" + regexp.QuoteMeta(t[:i]) + "(.+?)" + regexp.QuoteMeta(t[i+len("soandso"):]) + "$"
...
func threatGearTripwireLocked(verb string, tun ThreatTuning) {
	...
	addStatus("Threat Meter: gear data for %s looks stale (%s) — using inference. Run /outputfile inventory to refresh.", currentCharName, reason)
}
...
func ThreatZoneReset() {
	...
	threatMu.Unlock()
	if had > 0 {
		addStatus("Threat Meter: zoned — hate reset to zero on %d mob(s).", had)
		emitThreatChanged()
	}
}
...
func (a *App) GetThreatDebug() string {
	...
	fmt.Fprintf(&b, "=== Threat Meter debug — %s ===\n", now.Format("2006-01-02 15:04:05"))
...
// GetThreatMeter returns the overlay's full state. Open to any linked member:
// the parse and the gauge describe the viewer's own fight, which is the least
// officer-ish thing in the app. Unlinked clients get an empty state — the
