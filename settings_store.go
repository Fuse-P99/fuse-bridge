...
// Settings is the client's persisted configuration (settings.json).
//
// SETTINGS TELEMETRY (CLAUDE.md): every BOOL here is shipped automatically in
// the daily settings snapshot (settings_snapshot.go settingsBoolMap walks the
// json tags) and aggregated on the officer dashboard. A new bool still needs
// a label + tip in frontend/src/lib/settingsMeta.js to show up named. Strings
// and ints are NOT automatic — add them to snapshotPrefs by hand, and never
// ship a path, token, or secret (EQDirectory, Token, ShareSecret,
// ArchiveLogDir are deliberately absent from the snapshot).
type Settings struct {
	...
	// InstallID is a random id generated once per install (settings_snapshot.go
	// newInstallID) — the key the daily settings snapshot is stored under. It
	// names the install, never the person, and survives linking/unlinking.
...
// enabled. Fields not listed here (StartupConfigured, EQDirectory)
...
func LoadSettings() Settings {
	...
	if err != nil {
		return ensureInstallID(s)
	}
	if err := json.Unmarshal(data, &s); err != nil {
		return ensureInstallID(defaultSettings())
	}
	return ensureInstallID(s)
}

// ensureInstallID mints the install id on first load and persists it at once,
// so every later read (and the first settings snapshot) sees the same value.
func ensureInstallID(s Settings) Settings {
	if s.InstallID == "" {
