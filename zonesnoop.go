...
func fetchZoneSnoop() ([]zoneData, error) {
	// No IsLinked gate: /whozones is served without auth (zone rosters are
	// /who output — public in game anyway), so the Zones tab works unlinked.
	// The auth header still goes along for linked clients; the server ignores
	// it here.
