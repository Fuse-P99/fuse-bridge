...
// TrackGuideMedia is one attachment on a guide post.
type TrackGuideMedia struct {
	Kind string `json:"kind"` // "image" | "video" | "file"
	URL  string `json:"url"`
	Name string `json:"name"`
}

// TrackGuideContent is a guide post's content, fetched by the server's bot
// and rendered in-app (formatting, pictures, video) instead of linking out.
type TrackGuideContent struct {
	OK       bool              `json:"ok"`
	Error    string            `json:"error"`
	Author   string            `json:"author"`
	PostedAt string            `json:"posted_at"`
	Text     string            `json:"text"`
	Media    []TrackGuideMedia `json:"media"`
	URL      string            `json:"url"`
}

// GetTrackGuideContent fetches the saved guide post for a (mob, role) pairing.
// Content-level problems (deleted post, stale link) come back as OK:false with
// a friendly Error rather than a Go error.
func (a *App) GetTrackGuideContent(mob, role string) (TrackGuideContent, error) {
	var out TrackGuideContent
	req, err := http.NewRequest(http.MethodGet,
		registerBase()+"/trackguides/content?mob="+url.QueryEscape(mob)+"&role="+url.QueryEscape(role), nil)
	if err != nil {
		return out, err
	}
	req.Header.Set("Authorization", authHeader())
