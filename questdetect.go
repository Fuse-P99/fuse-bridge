...
func RecordQuestLootLine(line string) {
	...
	// Keyed by the per-(character, world) storage key: a Green namesake's loot
	// must land in THEIR quest state, not the Blue character's.
	toon := currentCharKey
	if toon == "" {
		return
	}
...
func questNoteItem(toon, itemName string, itemID int, location, source string, atSec int64) {
	...
	for qid, step := range perQuest {
		...
		if added := tqMarkSteps(toon, qid, set, nil); len(added) > 0 {
			...
			for _, op := range set {
				questMarkersRetireNeed(questStepNeed(&q, op.Order))
			}
		}
	}
...
func questScanInventory(name string) {
	// name may be a per-(character, world) storage key; EQ's dump file carries
	// only the bare character name (it is NOT server-qualified — same-named
	// toons on two servers share one dump, an ambiguity we can't remove).
	bare, _ := splitStoreKey(strings.ToLower(strings.TrimSpace(name)))
	eqDir := GetSettings().EQDirectory
	items := readInventoryItems(bare, eqDir)
	if items == nil {
		return // no dump for this character
	}
	var modMs int64
	if p := eqRootFilePath(eqDir, bare+"-Inventory.txt"); p != "" {
	...
	// Prune flags whose step is already done. The tick-time retirement above
	// only sees the TRANSITION — a marker for a step ticked before this build
	// (or while the app was closed) would otherwise stand until the
	// visited-and-left lifecycle got around to it.
	questMarkersSweep(name)
...
// questMarkersSweep retires the nudge markers of every already-done step of a
// character's assigned quests. Run after each inventory scan and on each zone
// change — the moments a stale flag would otherwise be looked at.
func questMarkersSweep(toon string) {
	for _, qid := range tqAssignedQuestIDs(toon) {
		q, ok := questByID(qid)
		if !ok {
			continue
		}
		for si := range q.Steps {
			if tqStepDone(toon, qid, si) {
				questMarkersRetireNeed(questStepNeed(&q, si))
			}
		}
	}
}
...
func questInvPoller(done <-chan struct{}) {
	...
	for {
		...
		case <-done:
			return
		case <-tick.C:
			toon := currentCharKey // state key; the dump file uses the bare name
			if toon == "" {
				continue
			}
			bare, _ := splitStoreKey(toon)
			p := eqRootFilePath(GetSettings().EQDirectory, bare+"-Inventory.txt")
			if p == "" {
				continue
			}
			info, err := os.Stat(p)
