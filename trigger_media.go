...
// uploadMissingTriggerMedia pushes local media files the server lacks (or whose
// bytes differ). Officer-gated by the caller.
func uploadMissingTriggerMedia(manifest map[string]triggerMediaMeta) {
	...
	for _, e := range entries {
		...
		data, err := os.ReadFile(filepath.Join(triggerMediaDir(), e.Name()))
		if err != nil || len(data) == 0 {
			continue
		}
		sha := sha256Hex(data)
		if m, ok := manifest[strings.ToLower(e.Name())]; ok && strings.EqualFold(m.SHA256, sha) {
			continue // already up to date
		}
		if uploadTriggerMediaFile(e.Name(), sha, data) {
			sent++
		}
	}
	if sent > 0 {
		addStatus("Triggers: published %d media file(s) to the server", sent)
	}
}
