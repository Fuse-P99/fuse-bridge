...
func openSettingsWindow() {
					...
					Children: []Widget{
						...
						PushButton{
							...
							OnClicked: func() {
								...
								if !looksLikeEQDir(path) {
									walk.MsgBox(settingsDlg, "Invalid folder",
										"The selected folder does not look like an EverQuest installation (no eqgame.exe found).\nPlease select your EverQuest installation folder.",
										walk.MsgBoxIconError|walk.MsgBoxOK)
									return
								}
								os.MkdirAll(filepath.Join(path, "Logs"), 0755)
	...
	save := func() {
		...
		UpdateSettings(Settings{
			...
			UseMiddlemand:      current.UseMiddlemand,
