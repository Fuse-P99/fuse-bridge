...
// serverForwardOK is the gate for every server round-trip that carries gameplay
// data or reads Blue-guild state: the client must be linked AND on the home
// world. The Fuse trigger package is the deliberate exception and gates on
// IsLinked() alone, never this — a linked client keeps the package on any world.
