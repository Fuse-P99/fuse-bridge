...
type TriggerCategoryUI struct {
	...
	Count int    `json:"count"` // enabled triggers feeding it
}

// GetTriggerCategories inventories the categories the active trigger set can
// produce, for the Manage Overlays page. Derived from the compiled set, so it
// reflects the current character's enablement.
func (a *App) GetTriggerCategories() []TriggerCategoryUI {
	type acc struct{ timers, alerts int }
	byCat := map[string]*acc{}

	trigStateMu.Lock()
	for _, c := range trigActive {
		...
		if c.useText || c.endedAlert != "" {
			a.alerts++
		}
	}
	trigStateMu.Unlock()

	out := make([]TriggerCategoryUI, 0, len(byCat))
	for name, a := range byCat {
		if a.timers > 0 {
			out = append(out, TriggerCategoryUI{Name: name, Kind: "timers", Count: a.timers})
		}
		if a.alerts > 0 {
			out = append(out, TriggerCategoryUI{Name: name, Kind: "alerts", Count: a.alerts})
		}
	}
...
func (a *App) GetTriggerTreeFor(charName string) []TriggerGroupUI {
	...
	conv = func(g *GinaGroup) TriggerGroupUI {
		...
		for _, t := range g.Triggers {
			ug.Triggers = append(ug.Triggers, triggerToUI(t, g, editable, ctx))
		}
