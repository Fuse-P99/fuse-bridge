...
func (a *App) GetTriggerTreeFor(charName string) []TriggerGroupUI {
	...
	var conv func(g *GinaGroup, mutedInherit, classAuto bool) (TriggerGroupUI, catAgg)
	conv = func(g *GinaGroup, mutedInherit, classAuto bool) (TriggerGroupUI, catAgg) {
		...
		for _, t := range g.Triggers {
			...
			tu.Muted = trigMuteTriggers[trigToggleKey(g, t)]
		...
		for _, c := range g.Groups {
			cu, ca := conv(c, mutedEff, childAuto)
...
// BeginTriggerDefaults opens a fresh staging set for the defaults editor. The
