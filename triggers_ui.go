...
type TriggerAlertUI struct {
	...
	// Color is the firing trigger's personal tint (trigger_overrides.go);
	// empty = the overlay's own color.
	Color string `json:"color,omitempty"`
	AtMs  int64  `json:"at_ms"`
	// ClearedAtMs is set when one of the alert's early-end conditions matched
	// a later log line — the overlay triple-flashes the alert and drops it.
	ClearedAtMs int64 `json:"cleared_at_ms,omitempty"`
}
...
type TriggerTimerUI struct {
	...
	Category string `json:"category"`
	// Color is the owning trigger's personal tint (trigger_overrides.go);
	// empty = the overlay's own color.
	Color       string `json:"color,omitempty"`
	...
	// Keep marks a "keep running across characters" (carry) timer so the UI can
	// badge it — a bar visible on a character who can't fire it needs a why.
	// Derived from the overlay's CarryTimers setting.
	Keep bool `json:"keep,omitempty"`
	// ClearedAtMs is set when an early-end condition ended this bar — the
	// overlay triple-flashes it (frozen where it was) and drops it.
	ClearedAtMs int64 `json:"cleared_at_ms,omitempty"`
}
...
type TriggerStateUI struct {
	...
	// Character is display text ("Franko", or "Franko (P99 Green)" off-home);
	// CharacterKey is the per-(character, world) storage key the config
	// bindings take.
...
func (a *App) GetTriggerState() TriggerStateUI {
	...
	out := TriggerStateUI{
		Imported:     imported,
		Character:    charDisplayName(trigActiveChar, trigActiveKey),
		CharacterKey: trigActiveKey,
	...
	for _, a := range trigAlerts {
		ui := TriggerAlertUI{
			ID: a.id, Text: a.text, Category: a.category, Color: a.color,
			AtMs: a.at.UnixMilli(),
		}
		if !a.clearedAt.IsZero() {
			ui.ClearedAtMs = a.clearedAt.UnixMilli()
		}
		out.Alerts = append(out.Alerts, ui)
	}
	// The in-tab banner shows the latest LIVE alert — one mid-clear-flash is
	// already answered, not news.
	for i := len(out.Alerts) - 1; i >= 0; i-- {
		if out.Alerts[i].ClearedAtMs == 0 {
			latest := out.Alerts[i]
			out.Alert = &latest
			break
		}
	...
	for _, lt := range liveTimers {
		...
		ui := TriggerTimerUI{
			ID: lt.id, Name: lt.name, Category: lt.category, Color: lt.color,
			StartedAtMs: startedMs, EndsAtMs: lt.endsAt.UnixMilli(),
			TriggerID: lt.triggerID, Path: lt.path,
			Keep: categoryCarryTimers(lt.category),
		}
		if !lt.clearedAt.IsZero() {
			ui.ClearedAtMs = lt.clearedAt.UnixMilli()
		}
		out.Timers = append(out.Timers, ui)
	}
...
type TriggerCharacterUI struct {
	Name       string `json:"name"` // display: "Franko", or "Franko (P99 Green)"
	Key        string `json:"key"`  // storage key the config bindings take
...
// every (character, world) with an eqlog file on this machine, plus any key
// that already has saved overrides (so a toon whose log was archived doesn't
// vanish along with its settings). Same-named characters on different worlds
// are separate entries. Sorted with the current character first.
func (a *App) GetTriggerCharacters() []TriggerCharacterUI {
	names := map[string]string{} // storage key → display-cased bare name
	...
	add(currentCharKey, currentCharName)
	for key, name := range logFileCharKeys(GetSettings().EQDirectory) {
		add(key, name)
	}
	trigStoreMu.Lock()
	...
	for k := range trigTogglesAll {
		configured[k] = true
		add(k, "") // display falls back to the key's own name segment below
	}
	trigStoreMu.Unlock()
	...
	out := make([]TriggerCharacterUI, 0, len(keys))
	for _, k := range keys {
		display := names[k]
		if display == "" {
			bare, _ := splitStoreKey(k)
			display = bare
		}
		out = append(out, TriggerCharacterUI{
			Name:       charDisplayName(display, k),
			Key:        k,
...
func (a *App) GetTriggerCategories() []TriggerCategoryUI {
	...
	ctx := trigCtxForLocked(currentCharKey, false)
	for id, t := range trigByID {
		g := trigGroupOf[id]
		// The EFFECTIVE overlay assignment — a personal re-assignment moves
		// the trigger's counts to the overlay it actually feeds.
		cat := effCategoryLocked(g, t)
		if cat == "" {
			// Unassigned triggers are flagged for configuration (see
			// triggerGaps) rather than silently filed under an overlay.
			continue
		}
		...
		// A timer-ending warning or timer-ended alert is a text alert too (the
		// engine pushes both into the same overlay), so any of the three slots
		// makes this an alerts trigger. effAlertTexts applies the override
		// layer: a member hiding the text removes the trigger from this
		// overlay, a reworded one stays.
		var ov *TriggerOverride
		if g != nil {
			ov = trigOverrides[trigToggleKey(g, t)]
		}
		if m, ing, ed := effAlertTexts(t, ov); m != "" || ing != "" || ed != "" {
			e.alerts++
			if live {
				e.alertsEnabled++
			}
...
type TriggerEditUI struct {
	...
	// Category is the EFFECTIVE overlay assignment (package value with any
	// personal Overlay override applied) — what the tree, popout buttons, and
	// overlay filters key on. PkgCategory is what the package XML says; the
	// officer edit form binds its editable select to THAT, never to the
	// effective value, or saving would bake a local override into the package.
	Category    string `json:"category"`
	...
	// Personal marks a trigger in the user-owned subtree (vs the Fuse set) —
	// the edit form uses it to decide which customization panel to offer.
	Personal bool `json:"personal,omitempty"`
	// Path is the " › "-joined group chain from the root down to the
	// containing group — shown under the edit form's title so a deep trigger
	// says where it lives. Display only; saves ignore it.
	Path string `json:"path,omitempty"`
	// Override is this trigger's personal customization (nil = none);
	// HasOverride backs the tree's "customized" badge. ShowsAlert is
	// alert-overlay membership with the override layer applied — the popout
	// buttons and overlay trees use it instead of re-deriving from the
	// package on_match/ended fields.
	Override    *TriggerOverride `json:"override,omitempty"`
	HasOverride bool             `json:"has_override,omitempty"`
	ShowsAlert  bool             `json:"shows_alert"`
...
// groupPathLocked renders a group's full chain from the root down, joined
// with " › " (the same separator the re-assign and View-edits lists use).
// Caller holds trigStoreMu (walks groupParentOf).
func groupPathLocked(g *GinaGroup) string {
	var names []string
...
func triggerToUI(t *GinaTrigger, g *GinaGroup, editable bool, ctx trigEnableCtx) TriggerEditUI {
	...
	// Personal customization: the effective overlay assignment and the
	// alert-membership answer both fold it in; the raw package values ride
	// along for the (officer) edit form.
	ov := trigOverrides[trigToggleKey(g, t)]
	m, ing, ed := effAlertTexts(t, ov)
	return TriggerEditUI{
		...
		Category:            effCategoryLocked(g, t),
		PkgCategory:         t.Category,
		...
		Incomplete:          triggerGaps(t, g),
		Editable:            editable,
		Personal:            isPersonalGroupLocked(g.GroupID),
		Path:                groupPathLocked(g),
		Override:            ov,
		HasOverride:         !ov.empty(),
		ShowsAlert:          m != "" || ing != "" || ed != "",
	}
}

// (triggerShowsAlert moved into effAlertTexts in trigger_overrides.go, which
// answers the same question with the personal override layer applied.)
...
// needs an overlay assignment to route to; a sound/speech-only trigger doesn't.
// The no-op clause mirrors the compile gate (package values, pre-override); the
// assignment check uses the EFFECTIVE overlay, since a personal re-assignment
// is exactly as routable as a package one. Caller holds trigStoreMu.
func triggerGaps(t *GinaTrigger, g *GinaGroup) string {
	...
	if (timer || alert || ended) && effCategoryLocked(g, t) == "" {
		why = append(why, "isn't assigned to an overlay, so it has nowhere to be shown")
	}
	if len(why) == 0 {
		return ""
	}
...
func (a *App) GetTriggerTree() []TriggerGroupUI {
	return a.GetTriggerTreeFor(currentCharKey)
}

// GetTriggerTreeFor returns the hierarchy with enablement evaluated for a
// specific character (by their storage key, from GetTriggerCharacters), so
// Manage Timers can show and edit any toon's set. Passing "" means the current
// character. Read-only: no bucket is created for a character just because it
// was viewed.
func (a *App) GetTriggerTreeFor(charName string) []TriggerGroupUI {
	if strings.TrimSpace(charName) == "" {
		charName = currentCharKey
	}
	trigStoreMu.Lock()
	...
	conv = func(g *GinaGroup, mutedInherit, clipInherit, classAuto bool) (TriggerGroupUI, catAgg) {
		...
		classNote := defaultsView && !personal && groupIsClassSpecificSectionLocked(g)
		...
		for _, t := range g.Triggers {
			...
			// Overlay spread (EFFECTIVE assignments, so group popout shortcuts
			// follow personal re-assignments): unassigned triggers don't break
			// uniformity (they have no overlay to open either way).
			if c := strings.TrimSpace(tu.Category); c != "" {
...
func (a *App) SaveTrigger(in TriggerEditUI) error {
	...
	// A rename changes the trigger's stable key — carry every per-key store
	// over (toggles, mutes, clipboard blocks, overrides). This used to move
	// only the toggles, silently orphaning members' mutes and blocks.
	oldKey := trigToggleKey(trigGroupOf[t.ID], t)
	applyTriggerEdit(t, in)
	migrateTrigKeyLocked(oldKey, trigToggleKey(trigGroupOf[t.ID], t))
	...
	if err == nil {
		auditEventHourly("app_updatedtimer_edited", "")
	}
	return err
}
...
func (a *App) CreateTrigger(groupID int, in TriggerEditUI) (int, error) {
	...
	RebuildTriggerActivation()
	if err == nil {
		auditEvent("app_createdtimer_"+tname, "group: "+gname)
	}
	return id, err
}
...
func (a *App) DeleteTrigger(id int) error {
	...
	for i, t := range g.Triggers {
		if t.ID == id {
			// An explicit delete also retires the trigger's per-key records
			// (toggles, mute, clipboard block, override), so a future trigger
			// reusing the name doesn't inherit stale state.
			removeTrigKeyLocked(trigToggleKey(g, t))
			saveTrigKeyStoresLocked()
...
func (a *App) SetTriggerGroupEnabledFor(charName string, id int, enabled bool) error {
	...
	walk = func(g *GinaGroup, inClass bool) {
		...
		in := inClass || groupIsClassSpecificSectionLocked(g)
		for _, c := range g.Groups {
			walk(c, in)
		}
	}
	...
	RebuildTriggerActivation()
	if err == nil {
		auditTimerHourly(enabled, "app_updatedtimer_enabled", "app_updatedtimer_disabled")
	}
	return err
}
...
// resolveToggleChar maps an empty character argument to the tailed character's
// STORAGE key, so the no-argument bindings keep editing whoever is logged in
// (under their own world's bucket).
...
func (a *App) DeleteTriggerGroup(id int) error {
	...
	// Drop the subtree from the indexes and clean up its per-key records —
	// slider overrides, mutes, clipboard blocks, customization overrides, and
	// the group-level mute/block flags — so a future group reusing an id/name
	// doesn't inherit stale state (nextGroupIDLocked reuses freed ids).
	...
	scrub = func(g *GinaGroup) {
		...
		delete(trigMuteGroups, g.GroupID)
		delete(trigClipGroups, g.GroupID)
		for _, t := range g.Triggers {
