...
func (a *App) GetTriggerCategories() []TriggerCategoryUI {
	...
	for id, t := range trigByID {
		...
		g := trigGroupOf[id]
		live := g != nil && effectiveGroupEnabledLocked(g, ctx) &&
			effectiveTriggerEnabledLocked(g, t, ctx)
		e := at(cat)
...
type TriggerEditUI struct {
	...
	// EarlyEnders are search conditions that end this trigger's running timer
	// ahead of schedule (GINA's TimerEarlyEnders). Only meaningful when
	// TimerEnabled; empty when the trigger has none.
	EarlyEnders []TriggerEnderUI `json:"early_enders"`
	Category    string           `json:"category"`
	Unsupported bool             `json:"unsupported"`
	Enabled     bool             `json:"enabled"`
	// Incomplete explains why a trigger can never show anything ("" when it's
	// fine) — see triggerGaps. Rendered as a grey flag wherever triggers list.
...
// triggerGaps reports why a trigger would produce nothing visible, or "" when
// it's fully configured. A trigger that matches but has no timer and no alert
// text is a no-op; one with no category can't be routed to an overlay, since
// overlays are opened per category.
func triggerGaps(t *GinaTrigger) string {
	...
	var why []string
	if !timer && !alert && !ended {
		why = append(why, "shows no alert text and starts no timer")
	}
...
func (a *App) GetTriggerTreeFor(charName string) []TriggerGroupUI {
	...
	conv = func(g *GinaGroup) TriggerGroupUI {
		...
		for _, t := range g.Triggers {
			...
			// A trigger in a disabled group can't fire even with its own slider
			// on, so it doesn't count toward the enabled tally.
			if ug.Enabled && tu.Enabled {
				ug.EnabledTriggers++
			}
