...
func fetchAttendance(query string) (AttendanceSet, error) {
	...
	// A read of server-side Blue tracking state — fine from any world.
	if !IsLinked() {
...
type RaidChannelOption struct {
	...
	// Raiders is how many characters that channel already credits — shown as
	// "#channel (N)" so an already-entered log is obvious before posting a
	// second copy.
...
func (a *App) SendAttendance(channelID string, raidID int, zone string) (int, error) {
	// Posts server-side snapshot data to Discord — nothing from the local log,
	// so an officer can send it while playing on any world.
	if !IsLinked() {
