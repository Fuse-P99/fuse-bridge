...
// RaidClassCount mirrors a server class head count.
type RaidClassCount struct {
	Class string `json:"class"`
	Count int    `json:"count"`
}

// RaidGuildGroup mirrors one other guild in the raid zone (the competition).
type RaidGuildGroup struct {
	Name    string           `json:"name"`
	Total   int              `json:"total"`
	Classes []RaidClassCount `json:"classes"`
}
...
type RaidRaiders struct {
	...
	Others []RaidGuildGroup `json:"others"`
}
...
type BatphoneBanner struct {
	...
	// Mob names the raid mob when the server resolved the batphone to one; the
	// bar's swords link keys off it. Empty for freeform banners (and on older
	// servers, which never send it).
	Mob string `json:"mob"`
}
...
// fetchBatphones retrieves current batphone banners. Linked members only, but
// on ANY world — a boxer on Green still wants to see a Blue batphone go up.
// Reads of Blue-guild state gate on IsLinked alone; serverForwardOK guards
// only pushes of gameplay data.
...
// character so the server can verify Fuse membership. Linked members see the
// board from any world (it's a read of Blue state — nothing gameplay-derived
// goes up); the toon param is only skipped off-home since an off-world name
// can't verify as a Blue Fuse toon.
