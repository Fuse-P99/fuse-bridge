...
// VoiceSpeaker mirrors the server's current raid-voice speaker.
type VoiceSpeaker struct {
	Name     string `json:"name"`
	Speaking bool   `json:"speaking"`
}

// fetchVoiceSpeaker retrieves who is currently speaking in the raid-voice channel.
func fetchVoiceSpeaker() VoiceSpeaker {
	var out VoiceSpeaker
	base := strings.TrimSuffix(serverURL, "/submit")
	req, err := http.NewRequest(http.MethodGet, base+"/voicespeaker", nil)
	if err != nil {
		return out
	}
